...
- **Use `match`/`case` over `if/elif ==` chains** for dispatch on a discriminator
  ...
  more `if/elif <var> == …` branches on the same variable in a `python/extras/**`,
  `python/oracles/**`, or `tests/**` `.py` file are blocked (exit 2) at Edit/Write
  time. A single `if x == y:` is fine — only chains must become `match`/`case`.
...
- **`glab` CLI** — the GitLab-side equivalent of the `github` MCP above; there is no GitLab MCP configured in this repo, so `glab` (authenticated to `gitlab.mpcdf.mpg.de` as `anhahn`) is the working path for pipeline/MR/job status on the **primary** remote (see [[remote-primary-gitlab]]). Prefer `glab ci status -b <branch>`, `glab ci view`, `glab mr list/view` over `WebFetch`-scraping the GitLab web UI or REST API — WebFetch has no auth context and either 403s or silently returns only what an anonymous request can see. Verified 2026-07-11: `glab ci status --repo anhahn/spectrafit-core -b main` returns full per-job pipeline state in one call.
- **web** (WebSearch / WebFetch) — anything outside the repo and not covered by a library's context7 docs (algorithms, error messages, recent changes). For **GitHub** Actions/PR status specifically, prefer the `github` MCP; only fall back to WebFetch against `api.github.com` for read-only public-repo lookups the MCP doesn't cover (e.g. raw workflow-run/job listings — there is currently no MCP tool for GitHub Actions run status), and note that job **log downloads** 403 for WebFetch (unauthenticated) even on public repos — that needs `gh run view --log` or the github MCP.
...
> Project-scope `.mcp.json` carries `rrt`, `analyzer`, `spectrafit-reports`, `zen-of-languages`, and `ai-agent-guidelines` (project-local tooling). `serena`, `context7`, and `github` are configured **user-scope** in `~/.claude.json` (`github` is HTTP+Bearer to `https://api.githubcopilot.com/mcp`; tokens MUST NOT be committed). Run `claude mcp list` to verify all eight resolve. **If any MCP shows `✗ Failed to connect` at session start, surface it to the user in your first message instead of degrading silently** — restarting with `--resume` after a one-line fix is cheaper than completing a session on the fallback path.

## Skill catalog (consolidated)

The skill catalog started as **7 consolidated skills** (down from 28) and has
since grown to **9** with two additional process skills (`semantic-debugging`,
`big-picture-driven-development` — new disciplines, not consolidations of
retired specialists), anchored to a single declarative registry. The registry
is [`.claude/skills/INDEX.yaml`](.claude/skills/INDEX.yaml) (validated by
...
| `semantic-debugging` | meta, crosscut | *(none — new)* holds the trunk ledger, classifies finds (failure-kind + branch-verdict), dispatches instance bugs to `systematic-debugging` and class bugs to `big-picture-driven-development` |
| `big-picture-driven-development` | meta | *(none — new)* MAP → CLASSIFY → ENFORCE → SWEEP: fix the class, not the instance |
...
- **Use `uv run poe run_bg <task>` for anything slow** (the full suite, the
  ...
  **On a local (non-cloud-sandbox) checkout, verify the log actually has pytest
  ...
  (`scripts/run_pytest_bg.sh:41`) — on a machine without that path, `source`
  ...
  near-empty log — indistinguishable from a fast, trivial success unless you
  ...
  (`… > /path/to/log.txt 2>&1`, `run_in_background: true`) instead of `run_bg`.
...
## "Local pre-commit is green" ≠ "GitHub's Pre-Commit Check will pass"

Two independent gaps, both discovered only via a real GitHub Actions run, each cost a
full fix → push → publish → fail → re-fix round trip (2026-07-13/14):
...
- **GitHub's `Pre-Commit Check` runs against the SQUASHED, exclusion-filtered snapshot**
  ...
  `scripts/publish_exclusions.py` (`docs/superpowers/plans/*`, `docs/superpowers/specs/*`,
  `docs/superpowers/ledgers/*`, `.claude/audit/*.jsonl`) are fully removed from the public
  snapshot. `DECISIONS.md` is handled differently: `scripts/publish_remove_excluded.py`
  stubs it with placeholder content (`_STUB_CONTENT`) rather than removing it, so a hook
  requirement like `rrt-folder-check`'s "`DECISIONS.md` must exist" stays green on the
  snapshot too — this two-tier behavior (remove vs. stub) was itself a fix for an earlier
  version of this gap, where full removal made that hook requirement pass locally forever
  (the file always exists in the real repo) while permanently failing on the actual GitHub
  run. Local `pre-commit run --all-files` cannot catch either failure mode; only a real
  publish (or manually applying the exclusions to a scratch checkout first) can.
- **`.gitlab/20-lint.yml`'s `lint:python`/`lint:rust` jobs have historically been
  narrower than the full `.pre-commit-config.yaml` hook set** — a `ruff format --check`
  ...
  either gap, and only on the PRs/pushes that trigger it — push to main, Dependabot
  PRs, or a PR opened by an existing owner/member/collaborator (which is what
  `CONTRIBUTING.md`'s "Fast iteration on GitHub" lane relies on; not as rare as
  "Dependabot's PRs" alone would suggest). Before assuming a `lint:*` GitLab job is a
  superset of `.pre-commit-config.yaml`, check the two are actually kept in sync —
  the mismatch is invisible in a clean local run.
...
Use one of them as a worked example when adding the next. (`gaussian_nd` — the
N-D fitting kernel, not the older `gaussian2d` — is a different case: it's
wired through `python/oracles/_engine_multidim.py`, a standalone showcase path
outside the standard `MODEL_REGISTRY`/`CaseSpec` pipeline this section
describes, so it is not a worked example of *this* 6-step sequence.)
...
2. `cd web && npm run contract` → runs `openapi-typescript http://localhost:8000/openapi.json`,
   writing `web/src/openapi.gen.ts`. `web/src/contract/index.ts` re-exports the named view
   types (`BenchReport`, `Featured`, `BackendProfile`, `MultiDim`, `Projection`, `NdPeak`,
   `GlobalFit`, `GlobalFitSlice`, `PeakTrace`, `TrustBlock`, `WireResult`, `SuiteCase`,
   `SuiteMetric`, `CaseInference`, `Point2D`, `SpreadPt`, `PeakACS`) from it, so the views
   never change. `Featured` also carries
...
The web UI (`web/`, Vite + React) has **2 destinations** — **Standing**
(`#standing`, default; the verdict: gate PASS/FAIL, geomean speedup, win rate, and the
render-truth credibility-rung money figure) and **Evidence** (`#evidence`; the data — all
backends side by side; `#audit` now redirects here — the former standalone Audit
destination was folded in). Evidence has two sub-views: `overview` (all cases — suite
table, Δr²/speedup CI, winner stability) and `case` (single-case drill-down); a
`#case=<id>` permalink routes to Evidence and opens the case sub-view at mount. Every
panel is a declarative `PanelRecord` in `web/src/panels/registry.tsx` (**the single
source of truth**); each destination renders `renderPanels(dest, report, ctx)`
(`web/src/shell/renderPanels.tsx`) over a scope-filtered registry, and `Shell.tsx` is a
thin nav + destination switch. SVG charts mount through `web/src/plots/PlotMount.tsx`
(ResizeObserver + `replaceChildren`, responsive width, isolated from React
reconciliation — never imperative `appendChild` into a React-managed div). The binding
has **no silent `?? PRIMARY` fallback** and enumerates backends via `solversOf(report)`
(no hardcoded backend ids — a source-scan vitest test enforces this). The Python engine
fits a genuine **N-D (3-D)** problem with the parametric `gaussian_nd` kernel (`_multidim`,
SP-2 — D inferred from the node's `center_<i>` params) and a shared-model multi-spectrum
global fit via a `GlobalFitGraph` joint fit (`_global_fit`), and both showcases **are now
rendered** — a dedicated "Native showcases" section inside Evidence's `overview` sub-view
(`web/src/shell/EvidenceOverview.tsx`; `EvidencePanel.tsx` is routing glue that delegates
to it).
...
- **Run:** `uv run poe benchmark` (or `uv run python -m oracles.cli run`) → writes
  ...
  packaging, 2026-06-23); run the bench via `uv run poe benchmark` or
...
- **`BenchReport.baseline_solver_id`** (`bench_contract.py`) — names which solver
...
- **`oracles.cli`** — Typer CLI module; invoke as `uv run python -m oracles.cli run
  ...
  console-script entry was removed (Option A packaging, 2026-06-23 — it ImportErrors
  on a clean install because `typer` lives in the `[benchmark]` extra).
  `python -m oracles.cli` still works for IDE debugging.
- **`SHAPE_BOUNDS`** (`oracles.models`) — shape-param bound table (pearson7
  `m`, moffat `beta`, students-t `nu`, fano `q`, asym_ir `k`) added after the
  CX-033 NaN cascade, registered once and read by both the lmfit and
  scipy-ls oracle backends. Adding a model with a new long-tail shape param
  requires one entry here so an LM-family search can't drive it into the
  formula's overflow region.
- **Number formatting** — `fmtP()` in `web/src/series/format.ts` plus the collapse-proof tick
  formatter `tickLabels` in `web/src/series`; most panel bodies (`web/src/panels/bodies/*.tsx`)
  call `.toFixed()` directly inline rather than through `registry.tsx`, which has none. There is
