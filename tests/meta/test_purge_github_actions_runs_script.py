...
_SCRIPT = Path(__file__).resolve().parents[2] / "scripts" / "purge_github_actions_runs.py"
...
def test_dry_run_never_calls_delete() -> None:
    with (
        mock.patch("sys.argv", ["purge_github_actions_runs.py", "--dry-run", "--token", "x"]),
...
def test_no_runs_found_is_a_clean_noop() -> None:
    with (
        mock.patch("sys.argv", ["purge_github_actions_runs.py", "--token", "x"]),
...
def test_delete_run_returns_false_and_prints_on_non_204() -> None:
    with mock.patch.object(purge, "_request", return_value=(403, {"message": "Forbidden"})):
