...
def _make_repo_with_feature_branch(tmp_path: Path) -> tuple[Path, str]:
    """Build a throwaway repo: base commit on main, two commits on a named
    feature branch (standing in for a GitHub-first iteration-lane branch).
    Returns (repo_path, feature_branch_name).
    """
    repo = tmp_path / "repo"
    repo.mkdir()
    _git(repo, "init", "-q", "-b", "main")
    ...
    (repo / "f.txt").write_text("two\n")
    _git(repo, "add", "f.txt")
    _git(repo, "commit", "-q", "-m", "widget: first pass")
    (repo / "f.txt").write_text("three\n")
    _git(repo, "add", "f.txt")
    _git(repo, "commit", "-q", "-m", "widget: fix typo")
    return repo, "feature/widget"


def test_main_backports_a_named_non_main_branch(tmp_path: Path) -> None:
    repo, feature_branch = _make_repo_with_feature_branch(tmp_path)
