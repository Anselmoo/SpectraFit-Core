...
def test_legacy_alias_has_deprecation_headers(
    ...
    """``Deprecation: true`` + ``Sunset`` + ``Link`` ride every ``/api/*`` response;
    absent on ``/api/v1/*``.

    RFC 8594 (Sunset + successor-version Link) + draft-ietf-httpapi-deprecation-header.
    The headers must appear on EVERY legacy endpoint (not just one) and must NEVER leak
    ...
    for path in legacy_paths:
        ...
        assert "sunset" in {k.lower() for k in resp.headers}, f"legacy {path} missing Sunset header"
        successor = "/api/v1" + path[len("/api") :]
        assert resp.headers.get("link") == f'<{successor}>; rel="successor-version"', (
            f"legacy {path} missing/incorrect successor-version Link header"
        )
    for path in canonical_paths:
        ...
        assert "link" not in {k.lower() for k in resp.headers}, (
            f"canonical {path} leaked Link header"
        )
