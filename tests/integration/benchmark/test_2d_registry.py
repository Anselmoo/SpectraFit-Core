...
def test_casespec_dimensions_2_materializes_via_registry() -> None:
    ...
    spec = CaseSpec(
        ...
        n_points=64,
        noise=0.02,
        dimensions=2,  # ty: ignore[unknown-argument]  # field absent until Cycle 17
    )
    assert spec.dimensions == 2  # ty: ignore[unresolved-attribute]  # field absent until Cycle 17
    ...
    assert multidim.nx > 0
    assert multidim.ny > 0
