...
def test_index_dedupe_matches_run_id_and_category_not_run_id_alone(tmp_path) -> None:
    ...
    def _tiny_report(case_id: str) -> BenchReport:
        return BenchReport(
            solvers=[
                SolverMeta(
                    id="spectrafit", label="spectrafit", color="#fff", soft="#eee"
                )
            ],
    ...
    benchmark_run_dir = write_run(
        _tiny_report("BM-001"), category="benchmark", root=tmp_path
    )
    solo_run_dir = write_run(
        _tiny_report("SOLO-001"), category="spectrafit_solo", root=tmp_path
    )
    ...
    assert (
        latest_results("benchmark", root=tmp_path) == benchmark_run_dir / "results.json"
    )
    assert (
        latest_results("spectrafit_solo", root=tmp_path)
        == solo_run_dir / "results.json"
    )
