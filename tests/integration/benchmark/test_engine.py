...
def test_only_spectrafit_subject_claims_real_history() -> None:
    ...
    report = build_report(n_reps=1, n_mc=2, catalog=catalog, ngrid=[128, 256])
    f = report.analyzed[0]
    real_backends = {sid for sid, prof in f.profiles.items() if prof.history_source == "real"}
...
def test_index_dedupe_matches_run_id_and_category_not_run_id_alone(tmp_path) -> None:
    """Two different categories can share a ``run_id`` (per-category NNN counters in
    :func:`oracles.reports.allocate_run_dir`); the index must keep both entries.

    Regression: ``_update_index`` used to dedupe on ``run_id`` alone, so a same-day
    first run in ``spectrafit_solo`` (run_id ``<today>_run_001``) would silently evict
    the same-day first run in ``benchmark`` (also ``<today>_run_001``) from
    ``index.json`` — even though they are unrelated runs in different categories.
    """
    from oracles.bench_contract import BenchReport, CategoryMeta, SolverMeta, SuiteCase

    def _tiny_report(case_id: str) -> BenchReport:
        return BenchReport(
            solvers=[
                SolverMeta(
                    id="spectrafit",
    ...
    benchmark_run_dir = write_run(
        _tiny_report("BM-001"),
        category="benchmark",
        root=tmp_path,
    )
    solo_run_dir = write_run(
        _tiny_report("SOLO-001"),
        category="spectrafit_solo",
        root=tmp_path,
    )
    # Both first-ever runs in their category get run NNN 001 for the same day —
    # same run_id, different category. This is the exact collision shape.
    assert benchmark_run_dir.name == solo_run_dir.name

    index = json.loads((tmp_path / "index.json").read_text())
    assert len(index) == 2
    categories = {r["category"] for r in index}
    assert categories == {"benchmark", "spectrafit_solo"}

    assert latest_results("benchmark", root=tmp_path) == benchmark_run_dir / "results.json"
    assert latest_results("spectrafit_solo", root=tmp_path) == solo_run_dir / "results.json"
