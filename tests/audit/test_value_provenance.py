...
def test_value_provenance_importable():
    ...
    from oracles.audit.provenance import (  # noqa: F401
        VALUE_PROVENANCE,
        OracleRef,
        ValueProvenance,
    )
...
def test_every_claim_has_a_provenance_record():
    ...
    assert not missing, "claim ↔ provenance parity failed:\n" + "\n".join(missing + mismatched)
    assert not mismatched, "claim ↔ provenance parity failed:\n" + "\n".join(missing + mismatched)
...
def test_proxy_records_are_machine_declared():
    ...
    offenders = [
        rec.id for rec in VALUE_PROVENANCE.values() if rec.status == "proxy" and not rec.proxy_task
    ]
...
def test_convergence_metric_is_now_real():
    ...
    assert rec.status == "real", "Phase 4: the χ²-floor proxy was replaced by the real metric"
