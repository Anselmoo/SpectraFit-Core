            ...
            for key, value in schema.items():
                match (key, value):
                    ...
                    case ("schemas", dict(schemas)):
                        out[key] = {
                            _collapse_dunder_fqn(k): normalise_schema(v) for k, v in schemas.items()
                        }
                    case _:
...
def test_normalise_collapses_dunder_fqn_in_ref() -> None:
    ...
    ref = out["paths"]["/x"]["get"]["responses"]["200"]["content"]["application/json"]["schema"][
        "$ref"
    ]
...
def test_normalise_passes_through_non_fqn_schema_name() -> None:
    ...
    assert (
        out["paths"]["/r"]["get"]["responses"]["200"]["content"]["application/json"]["schema"][
            "$ref"
        ]
