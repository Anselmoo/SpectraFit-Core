...
def test_gap_is_a_valid_wire_status():
    ...
    wr = WireResult(wire_id="W2c", name="jacobian_kappa", status="gap", evidence="κ(J) not exposed")
...
def test_w2c_fail_when_a_computed_kappa_is_non_finite():
    ...
    out = wire_w2c_jacobian_kappa(audit_records=[_rec(10.0), _rec(float("inf"), case="T-2")])
    assert out[0].status == "fail"
...
def _wires(statuses: dict[str, WireStatus]) -> list[WireResult]:
    return [WireResult(wire_id=k, name=k, status=v, evidence="stub") for k, v in statuses.items()]
...
def test_real_fail_still_caps_the_rung():
    statuses: dict[str, WireStatus] = {
        "W1": "pass", "W2a": "fail", "W2b": "pass", "W2c": "gap",
        "W3": "pass", "W4": "pass", "W5": "skipped", "W6": "pass", "W7": "pass",
    }
...
def test_runner_gap_lifts_rung_and_keeps_claim_unaudited(
    ...
    n_unaudited = sum(
        1 for c in CLAIM_REGISTRY.values() if c.wire_id in {"W2c", "W8"}
    )
