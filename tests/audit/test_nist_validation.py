...
``run_nist_validation()`` re-runs 22 of NIST's 27 StRD nonlinear-regression
datasets (Gauss1/2/3 + Lanczos1/2/3 + BoxBOD + Misra1a + Misra1b + MGH17 +
Bennett5 + MGH09 + Eckerle4 + Roszman1 + DanWood + Kirby2 + Hahn1 + Thurber +
Rat42 + Rat43 + Chwirut1 + Chwirut2), and returns a structured
``NistValidation`` whose per-dataset / per-parameter significant-figure
agreement mirrors what the scenario tests assert. This is the data that backs
wire W8 and the honest RUNG_5 unlock.
...
def test_run_nist_validation_returns_twenty_two_datasets() -> None:
    v = run_nist_validation()
    assert isinstance(v, NistValidation)
    assert len(v.datasets) == 22
...
def test_every_dataset_passes_at_threshold() -> None:
    ...
    for d in v.datasets:
        ...
        assert d.min_sig_figs >= NIST_SIGFIG_THRESHOLD, (
            f"{d.name} only agreed to {d.min_sig_figs} sig figs (threshold {NIST_SIGFIG_THRESHOLD})"
        )
