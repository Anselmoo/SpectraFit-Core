...
def _faithful_records(
    payload: dict,
) -> Iterator[tuple[str, str, np.ndarray, np.ndarray, dict]]:
...
def _assert_metric_identity(
    field: str,
    oracle: Callable[[np.ndarray, np.ndarray], float],
) -> None:
...
@pytest.mark.skipif(
    _latest_results_path() is None,
    reason="no benchmark run on disk yet",
)
