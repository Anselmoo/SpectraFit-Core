...
def test_wire_result_status_is_literal():
    with pytest.raises(ValidationError):
        WireResult(wire_id="W1", name="x", status="green", evidence="x")  # type: ignore[arg-type]
...
def test_trust_block_rejects_extra_fields():
    with pytest.raises(ValidationError):
        TrustBlock(rung=CredibilityRung.RUNG_2, wires=[], n_claims_audited=0, n_claims_total=0, rogue="x")  # type: ignore[call-arg]
