...
def test_trust_block_aggregates_wires():
    block = TrustBlock(
        ...
        wires=[
            WireResult(
                wire_id="W1", name="synth_invariants", status="pass", evidence=""
            ),
            WireResult(
                wire_id="W2a", name="metric_identity", status="pass", evidence=""
            ),
        ],
        n_claims_audited=2,
        n_claims_total=2,
...
def test_trust_block_rejects_extra_fields():
    with pytest.raises(ValidationError):
        TrustBlock(
            rung=CredibilityRung.RUNG_2,
            wires=[],
            n_claims_audited=0,
            n_claims_total=0,
            rogue="x",  # type: ignore[call-arg]  # ty: ignore[unknown-argument]  # deliberately unknown field to test extra='forbid'
        )
