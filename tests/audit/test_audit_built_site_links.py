...
def test_section_cards_resolve(pages: list[Path]) -> None:
    ...
    for page in pages:
        ...
        for target in card_href.findall(html):
            ...
            if _is_ci_assembled(page, path):
                continue
            total += 1
            if not _resolves(page, path):
                broken.append(f"{page.relative_to(SITE)} -> {target}")

    assert total > 0, "no .sf-card links found — has the template or class changed?"
    assert not broken, f"{len(broken)} of {total} section cards are dead:\n  " + "\n  ".join(broken)


def test_rust_api_links_resolve_to_the_rustdoc_root(pages: list[Path]) -> None:
    """A link *meant* for `/rust-api/` must actually land there.

    `_CI_ASSEMBLED` exempts `rust-api/` from the existence check above, and it
    has to: rustdoc output is copied in by CI after the docs build, so at test
    time `site/rust-api/` does not exist and every such link would read as
    broken.

    But that exemption was doing more than it intended. It is applied AFTER
    resolving the href against the page, so a link with the wrong number of
    `../` segments simply resolved somewhere else, failed to match the
    `rust-api/` prefix, and then fell through to the ordinary existence check
    that reported it as an unrelated dead link — or, when it happened to
    resolve under another exempt prefix, was skipped entirely.

    That is exactly what happened: `docs/reference/rust/overview.md` linked to
    `../../rust-api/`. Zensical publishes directory URLs, so that page is
    served at `/reference/rust/overview/` and the link resolved to
    `/reference/rust-api/` — two segments short, dead on both mirrors, and
    invisible to every gate the project had.

    This test closes that hole without needing the directory to exist: any href
    whose text mentions `rust-api` must, once resolved against its page, have
    `rust-api` as its FIRST path segment. That is a pure shape check, so it
    works at docs-build time, and it fails loudly on precisely the off-by-N-
    segments mistake that the existence exemption cannot see.
    """
    wrong = []
    checked = 0
    for page in pages:
        ...
        for target in _internal_targets(html):
            path = urlsplit(target).path
            if "rust-api" not in path:
                continue
            candidate = _resolve_candidate(page, path)
            if candidate is None:
                wrong.append(f"{page.relative_to(SITE)} -> {target} (escapes the site root)")
                continue
            checked += 1
            site_relative = candidate.relative_to(SITE.resolve()).as_posix()
            if site_relative.split("/")[0] != "rust-api":
                wrong.append(
                    f"{page.relative_to(SITE)} -> {target} "
                    f"(resolves to /{site_relative}, not /rust-api/...)",
                )

    assert checked > 0, (
        "no rust-api links found in the built site — the crate card grid on "
        "reference/rust/overview.md should produce one per crate"
    )
    assert not wrong, (
        f"{len(wrong)} rust-api link(s) resolve outside /rust-api/:\n  " + "\n  ".join(wrong)
    )
