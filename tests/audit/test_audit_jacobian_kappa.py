    ...
    for case in payload.get("analyzed") or []:
        for backend_id, profile in (case.get("profiles") or {}).items():
            if not profile.get("r2"):
                continue
            if (
                profile.get("jacobian_condition_number") is None
                and profile.get("jacobianConditionNumber") is None
            ):
                missing.append(f"{case.get('id')}/{backend_id}")
    assert not missing, (
        f"κ(J) missing from {len(missing)} (case, backend) pairs: {missing[:5]}"
    )
...
def test_overlapping_peaks_have_high_kappa():
    ...
    for case in payload.get("analyzed") or []:
        ...
        for profile in (case.get("profiles") or {}).values():
            kappa = profile.get("jacobian_condition_number") or profile.get(
