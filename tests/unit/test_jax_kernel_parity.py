...
_X_RANGE: dict[str, tuple[float, float]] = {
    ...
    # exp_over_linear denominator (lin_const+lin_slope·x) is monotone increasing
    # for x>=0 with both coefficients positive — never zero on this grid.
...
_PARAM_RANGE: dict[str, tuple[float, float]] = {
    ...
    "den_lin": (0.05, 0.5),
    "den_const": (0.05, 0.5),
    # rational_cubic numerator: unconstrained.
    "a0": (1.0, 1.4),
    "a1": (0.2, 0.4),
    "a2": (0.1, 0.2),
    "a3": (0.0, 0.04),
    # rational_cubic denominator D = 1 + b1·x + b2·x² + b3·x³ on x ∈ [-4, 8]. The
    # only interior stationary point sits near x ≈ -1.4; across this whole box the
    # worst-case D there is ≈0.92, well clear of zero (mirrors the single-point
    # values in tests/parity/test_kernel_parity.py, where D ∈ [0.97, 3.19]).
    "b1": (0.03, 0.07),
    "b2": (0.01, 0.03),
    "b3": (0.0005, 0.0015),
    # generalised_logistic: shift shares the amplitude/rate/shape ranges above;
    # 1 + exp(shift - rate·x) stays positive (no domain guard needed) for any shift.
    "shift": (0.8, 1.6),
    # exp_over_linear denominator lin_const + lin_slope·x on x ∈ [0, 8] (see
    # _X_RANGE override above): both terms positive keeps it >= lin_const.
    "lin_const": (3.0, 5.0),
    "lin_slope": (0.3, 0.7),
