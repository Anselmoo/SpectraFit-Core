...
def test_fit_accepts_3d_data_with_gaussian_nd() -> None:
    ...
    truth_y = 3.0 * np.exp(
        -(coords[:, 0] ** 2) / 2.0 - (coords[:, 1] ** 2) / 2.0 - (coords[:, 2] ** 2) / 2.0,
    )
    graph = FitGraph(
        nodes=[
            ModelNodeSpec(
...
def test_fit_accepts_2d_data() -> None:
    ...
    values = [
        3.0 * np.exp(-((x - 0.5) ** 2) / 2.0 - ((y + 1.0) ** 2) / (2.0 * 1.5**2)) for x, y in coords
    ]
...
def test_fit_unknown_solver_is_rejected() -> None:
    """Unknown solver strings are rejected, never silently defaulted to LM.

    Matches the confirmed business rule (Phase 5 Behavior Contract,
    `crates/spectrafit-solver/src/dispatch.rs:98-122`): an unrecognised
    `FitOptions.solver` string raises rather than silently falling back —
    closing a foot-gun where a typo like ``lmm`` or wrong-case ``Trf`` would
    otherwise run LM with a misleading result. This test previously asserted
    the opposite (silent fallback); that was stale relative to the rule.
    """
    graph = FitGraph(
        nodes=[
            ModelNodeSpec(
    ...
    with pytest.raises(ValueError, match="unrecognised solver"):
        fit(graph, data, opts)
