...
_FACTORY_FIXTURES: Final[dict[ModelType, tuple[Callable[..., ModelNodeSpec], dict[str, float]]]] = {
    ModelType.GAUSSIAN: (
        gaussian,
    ...
    ModelType.RATIONAL_CUBIC: (
        rational_cubic,
        {"a0": 1.2, "a1": 0.3, "a2": 0.15, "a3": 0.02, "b1": 0.05, "b2": 0.02, "b3": 0.001},
    ),
    ModelType.GENERALISED_LOGISTIC: (
        generalised_logistic,
        {"amplitude": 1.0, "shift": 1.2, "rate": 0.5, "shape": 1.0},
    ),
    ModelType.EXP_OVER_LINEAR: (
        exp_over_linear,
        {"rate": 0.5, "lin_const": 4.0, "lin_slope": 0.5},
    ),
}
...
def test_every_model_type_has_a_canonical_param_table() -> None:
    ...
    assert not missing, f"compose.CANONICAL_PARAMS missing: {sorted(m.value for m in missing)}"
    ...
    handrolled = FitGraph(
        nodes=[
            ModelNodeSpec(
                ...
                parameters={name: Parameter(value=value) for name, value in params.items()},
            ),
        ],
    )
...
def test_default_bounds_are_infinite() -> None:
    ...
    assert math.isinf(p.min)
    assert p.min < 0
    assert math.isinf(p.max)
    assert p.max > 0
...
def test_fitgraph_accepts_builder_directly_via_iter() -> None:
    ...
    via_compose = FitGraph(nodes=compose(nodes))  # ty: ignore[invalid-argument-type]  # Pydantic coerces the iterable ComposeBuilder
...
def test_bind_is_chainable_and_preserves_order() -> None:
    ...
    assert [(e.target_node, e.target_param, e.expression) for e in graph.expr_edges] == [
        ("g1", "sigma", "g0.sigma"),
        ("g2", "sigma", "g0.sigma"),
    ]
...
def test_bind_rejects_malformed_target() -> None:
    ...
    builder = compose([gaussian(id="g", a=1.0, c=0.0, s=1.0)])
    with pytest.raises(ValueError, match=re.escape("'node_id.param'")):
...
def test_compose_param_names_match_bench_registry() -> None:
    ...
    for model_type, expected in CANONICAL_PARAMS.items():
        ...
        assert peak.param_names == expected, (
            f"{model_type.value}: compose says {expected}, bench registry says {peak.param_names}"
        )
