...
@pytest.mark.parametrize(
    ...
    [
        # Values are the stored Gram κ(JᵀJ); the verdict is taken on the
        # κ(J) = sqrt(...) axis, so the bands are 1e6 / 1e12 here.
        (23.0, "well-conditioned"),  # κ(J) ≈ 4.8
        (5e8, "acceptably conditioned"),  # κ(J) ≈ 2.2e4
        (4e12, "ill-conditioned"),  # κ(J) ≈ 2.0e6
    ],
)
...
def test_explain_kappa_value_matches_source_field() -> None:
    """The printed κ is κ(J), i.e. sqrt of the stored Gram condition number."""
    gram_kappa = 1234.5
    prose = _make_result(condition_number=gram_kappa).explain()
    ...
    assert_allclose(floats[-1], math.sqrt(gram_kappa), rtol=0.01)
