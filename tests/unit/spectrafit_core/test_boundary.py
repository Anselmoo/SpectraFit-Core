...
# ---------------------------------------------------------------------------
# Cross-dataset x dimensionality: enforced, not zero-padded.
#
# `_phase_prepare_arrays` used to pad every short row up to the widest row seen
# across all datasets, so mixing a 1-D dataset with a 2-D one silently invented
# `x[1] = 0.0` for every point of the 1-D one and returned a wrong fit with no
# error.  The JSON entrypoint already rejected ragged x; these pin the array
# entrypoint to the same contract, which is what `fit`'s docstring promises.
# ---------------------------------------------------------------------------


def test_prepare_arrays_rejects_mixed_dimensionality_across_datasets() -> None:
    """A 1-D dataset alongside a 2-D one raises ValueError instead of padding."""
    from spectrafit_core.fit import _phase_prepare_arrays

    datasets = [
        MeasurementData(x=[0.0, 1.0], y=[1.0, 2.0]),
        MeasurementData(x=[[0.0, 0.0], [1.0, 1.0]], y=[1.0, 2.0]),
    ]
    with pytest.raises(ValueError, match=r"inconsistent x dimensionality"):
        _phase_prepare_arrays(datasets)


def test_prepare_arrays_error_names_both_datasets_and_coordinate_counts() -> None:
    """The message identifies the mismatching datasets and their dimensionalities."""
    from spectrafit_core.fit import _phase_prepare_arrays
    ...
    assert "dataset 1" in message
    assert "dataset 0" in message
    assert "1 coordinate(s)" in message


def test_prepare_arrays_keeps_uniform_dimensionality_working() -> None:
    """Uniform 2-D datasets still flatten point-major with stride n_dims."""
    from spectrafit_core.fit import _phase_prepare_arrays

    datasets = [
        MeasurementData(x=[[0.0, 10.0], [1.0, 11.0]], y=[1.0, 2.0]),
        MeasurementData(x=[[2.0, 12.0]], y=[3.0]),
    ]
    x_arr, y_arr, sigma_arr, sizes, n_dims = _phase_prepare_arrays(datasets)
    assert n_dims == 2
    assert sizes == [2, 1]
    assert sigma_arr is None
    assert list(x_arr) == [0.0, 10.0, 1.0, 11.0, 2.0, 12.0]
    assert list(y_arr) == [1.0, 2.0, 3.0]


def test_prepare_arrays_empty_dataset_defaults_to_one_dimension() -> None:
    """An empty dataset carries no points, so n_dims falls back to 1."""
    from spectrafit_core.fit import _phase_prepare_arrays

    _x, _y, _sigma, sizes, n_dims = _phase_prepare_arrays([MeasurementData(x=[], y=[])])
    assert n_dims == 1
    assert sizes == [0]
