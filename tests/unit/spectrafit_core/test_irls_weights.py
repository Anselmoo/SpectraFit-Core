...
``WeightFn`` was unreachable from Python (`docs/reference/rust/binding-audit.md` §2).
...
def test_irls_default_matches_explicit_huber() -> None:
    ...
    assert a_default == pytest.approx(a_huber, abs=1e-9), (
        f"`irls` default-arm parity broken: irls→{a_default:.6f} vs irls:huber→{a_huber:.6f}"
    )
