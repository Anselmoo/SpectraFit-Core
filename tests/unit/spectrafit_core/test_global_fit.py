...
def test_global_fit_graph_simultaneous_shared_peak_local_offsets() -> None:
    ...
    d0 = MeasurementData(
        x=x.tolist(),
        y=(peak + off0 + rng.normal(0, 0.01, x.size)).tolist(),
    )
    d1 = MeasurementData(
        x=x.tolist(),
        y=(peak + off1 + rng.normal(0, 0.01, x.size)).tolist(),
    )
    ...
    assert r.dataset_slices is not None
    assert len(r.dataset_slices) == 2
...
def test_global_fit_graph_shared_local_param_across_slices() -> None:
    ...
    ds = [
        MeasurementData(
            x=x.tolist(),
            y=(_gaussian_y(x, a, c, shared_sigma) + rng.normal(0, 0.01, x.size)).tolist(),
        )
        for a, c in truth
    ]
