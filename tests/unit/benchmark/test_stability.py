...
def test_headline_matches_hand_computation() -> None:
    report = build_report()
    speedups = [case.m["spectrafit"].speedup for case in report.suite if "spectrafit" in case.m]
    med_ms = [case.m["spectrafit"].med_ms for case in report.suite if "spectrafit" in case.m]
...
def test_cli_stability_missing_dir_exits_2(tmp_path: Path) -> None:
    ...
    result = runner.invoke(
        app,
        ["stability", str(tmp_path / "nope"), "--out", str(tmp_path / "out")],
    )
