...
class TestBackendOutcomeFitDof:
    ...
    def test_lmfit_fit_dof_equals_n_minus_nvarys_on_fx_case(self) -> None:
        ...
        n_total = sum(len(comp.to_params()) for comp in case.comp_true)
        assert outcome.fit_dof is not None, "LmfitBackend.extract() must set fit_dof"
    ...
    def test_spectrafit_fit_dof_equals_result_dof_on_fx_case(self) -> None:
        ...
        assert outcome.fit_dof is not None, "SpectraFitBackend.extract() must set fit_dof"
        # Consistency: chi2 / fit_dof ≈ stored reduced_chi2
        recomputed = outcome.chi2 / outcome.fit_dof
        assert abs(recomputed - outcome.reduced_chi2) < 1e-6, (
            f"chi2/fit_dof ({recomputed:.8f}) != stored reduced_chi2 ({outcome.reduced_chi2:.8f})"
        )
    ...
    def test_scipy_fit_dof_is_set(self) -> None:
        ...
        assert outcome.fit_dof is not None, "ScipyLeastSquaresBackend.extract() must set fit_dof"
