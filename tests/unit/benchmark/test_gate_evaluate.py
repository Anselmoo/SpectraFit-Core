...
def test_threshold_validation_rejects_non_numeric_min_geomean() -> None:
    ...
    with pytest.raises(ValidationError):
        GateThresholds(min_geomean="bad", max_dr2=1e-3, max_regressions=0)  # ty: ignore[invalid-argument-type]  # deliberately wrong type to test ValidationError
...
def test_threshold_validation_rejects_extra_fields() -> None:
    ...
    with pytest.raises(ValidationError):
        GateThresholds(
            ...
            max_dr2=1e-3,
            max_regressions=0,
            min_geoman=1.0,  # ty: ignore[unknown-argument]  # deliberate typo of min_geomean to test extra='forbid'
        )
...
def test_gate_report_rejects_extra_fields() -> None:
    ...
    with pytest.raises(ValidationError):
        GateReport(
            ...
            extra_field="boom",  # ty: ignore[unknown-argument]  # deliberately unknown field to test extra='forbid'
        )
