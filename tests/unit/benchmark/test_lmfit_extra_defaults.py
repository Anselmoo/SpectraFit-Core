...
def _pv_spec() -> CaseSpec:
    ...
    return CaseSpec(
        ...
        difficulty=0.3,
        components=[
            PseudoVoigtSpec(amplitude=3.0, center=0.0, sigma=0.8, fraction=0.4)
        ],
        x_min=-5.0,
        x_max=5.0,
        n_points=100,
...
def test_extra_defaults_applied_when_fraction_absent_from_guess(monkeypatch) -> None:
    ...
    def _to_params_without_fraction(self) -> dict[str, float]:
    ...
    assert "p0_fraction" in params, (
        "fraction parameter missing from built lmfit Parameters"
    )
