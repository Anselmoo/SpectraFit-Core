...
class TestScipyLsFixedParams:
    ...
    def test_fixed_center_stays_at_truth_trf_dogbox(self, method: str) -> None:
        ...
        assert outcome is not None, f"{method}: fit must succeed"
        assert outcome.success, f"{method}: fit must succeed"
        ...
        assert abs(fitted_center - truth_center) < 1e-4, (
            f"{method}: fixed center drifted (truth={truth_center:.6f}, fitted={fitted_center:.6f})"
        )
    ...
    def test_free_amplitude_recovers_trf_dogbox(self, method: str) -> None:
        ...
        assert rel < 0.10, (
            f"{method}: amplitude off by {rel:.1%} (truth={truth_amp:.4f}, fitted={fitted_amp:.4f})"
        )
...
class TestFailedFitSentinel:
    ...
    def _run_with_bad_outcome(self, r2: float, reduced_chi2: float):
        ...
        with patch("oracles.engine._safe_fit", return_value=bad_outcome):
            return run_suite(catalog, [backend], n_reps=1)
...
class TestApiRootLanding:
    ...
    def test_root_is_not_bare_detail_not_found(self) -> None:
        ...
        try:
            body = resp.json()
        except Exception:  # noqa: BLE001 - HTML or other non-JSON body is fine here
            return
        assert body != {"detail": "Not Found"}, "GET / returned the bare 404 detail payload"
