...
def _report_with_dr2(sf_r2: float, base_r2: float = 0.99):
    ...
    case = SuiteCase.model_construct(
        ...
        difficulty=0.1,
        m={
            "spectrafit": _suite_metric(r2=sf_r2),
            "lmfit": _suite_metric(r2=base_r2, speedup=1.0),
        },
        winner="spectrafit",
        regression=False,
        winner_reason=None,
...
def test_headline_collects_nonfinite_dr2_case_ids() -> None:
    ...
    _geomean, max_dr2, _win, _reg, _harm, nonfinite_ids = _compute_headline_numbers(
        report
    )
    assert nonfinite_ids == ["GH-001"], "the NaN-r² case must be flagged, not dropped"
    assert math.isfinite(max_dr2), (
        "max_abs_delta_r2 must stay finite (max over finite deltas)"
    )
