...
def test_unknown_pair_raises_with_actionable_message() -> None:
    ...
    with pytest.raises(
        ValueError,
        match=re.escape("No migration path from 2.0 to 3.0"),
    ):
...
def test_migrate_payload_to_current_raises_on_unknown_version() -> None:
    ...
    with pytest.raises(ValueError, match=re.escape("No migration path from 0.9")):
...
def test_migrate_1_5_to_1_6_renames_time_resolved_to_global_fit() -> None:
    ...
    assert "times" not in gf
    assert gf["datasetAxis"] == [0.0, 0.5]
    assert "tLabel" not in gf
    assert gf["axisLabel"] == "time"
    # per-slice rename
