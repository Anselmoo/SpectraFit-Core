...
def test_harmonic_mean_leq_geomean(monkeypatch, tmp_path: Path) -> None:
    ...
    assert signals.harmonic_mean_speedup_vs_baseline <= signals.geomean_speedup_vs_baseline
...
def test_old_payload_without_harmonic_field_validates_with_none() -> None:
    ...
    # Stable round-trip
    again = ManifestSignals.model_validate_json(
        signals.model_dump_json(by_alias=True)
    )
    assert again.harmonic_mean_speedup_vs_baseline is None


def test_bench_report_manifest_harmonic_roundtrip(
    monkeypatch, tmp_path: Path
) -> None:
    ...
    report = report.model_copy(
        update={"manifest": compute_manifest_signals(report)}
    )
