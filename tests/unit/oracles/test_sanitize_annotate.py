...
def test_inf_in_dict_is_nulled_and_annotated() -> None:
    ...
    assert result["max_abs_delta"] is None, "inf should be nulled"
    assert "max_abs_delta_suppressed" in result, (
        "missing sibling annotation for nulled inf"
    )
    assert "inf" in result["max_abs_delta_suppressed"].lower(), (
        "annotation should mention inf"
    )
...
def test_neg_inf_in_dict_is_nulled_and_annotated() -> None:
    ...
    assert "score_suppressed" in result
    assert "inf" in result["score_suppressed"].lower()
...
def test_nan_in_dict_is_nulled_and_annotated() -> None:
    ...
    assert "residual_suppressed" in result
    assert "nan" in result["residual_suppressed"].lower()
...
def test_finite_dict_gets_no_annotations() -> None:
    ...
    assert result == original, "finite dict must be unchanged"
    suppressed_keys = [k for k in result if k.endswith("_suppressed")]
...
def test_none_value_gets_no_annotation() -> None:
    ...
    assert "value_suppressed" not in result
...
def test_mixed_dict_annotates_only_nonfinite() -> None:
    ...
    assert result["good"] == 1.23
    assert result["label"] == "mixed"
    assert result["bad"] is None
    assert "bad_suppressed" in result
    assert "good_suppressed" not in result
    assert "label_suppressed" not in result
...
def test_nested_dict_annotates_at_correct_level() -> None:
    ...
    assert inner_dict["inner"] is None
    assert "inner_suppressed" in inner_dict
    assert inner_dict["ok"] == 1.0
    assert "ok_suppressed" not in inner_dict
...
def test_list_of_dicts_each_annotated() -> None:
    ...
    assert "v_suppressed" in result[0]
    assert result[1]["v"] == 2.0
    assert "v_suppressed" not in result[1]
...
def test_annotation_value_is_a_string_not_float() -> None:
    ...
    suppressed_val = result.get("x_suppressed")
    ...
    assert result2.get("x_suppressed") == suppressed_val
