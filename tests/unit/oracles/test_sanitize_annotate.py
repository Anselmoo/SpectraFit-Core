"""G5 — _nullify_non_finite() must annotate suppressed non-finite values.
...
from oracles.audit.runner import _nullify_non_finite
...
def test_inf_in_dict_is_nulled_and_annotated() -> None:
    ...
    result = _nullify_non_finite({"max_abs_delta": float("inf")})
    assert isinstance(result, dict)
    sanitized: dict = result
    assert sanitized["max_abs_delta"] is None, "inf should be nulled"
    assert "max_abs_delta_suppressed" in sanitized, "missing sibling annotation for nulled inf"
...
def test_neg_inf_in_dict_is_nulled_and_annotated() -> None:
    ...
    result = _nullify_non_finite({"score": float("-inf")})
    assert isinstance(result, dict)
    sanitized: dict = result
...
def test_nan_in_dict_is_nulled_and_annotated() -> None:
    ...
    result = _nullify_non_finite({"residual": float("nan")})
    assert isinstance(result, dict)
    sanitized: dict = result
...
def test_finite_dict_gets_no_annotations() -> None:
    ...
    result = _nullify_non_finite(original)
    assert isinstance(result, dict)
    sanitized: dict = result
...
def test_mixed_dict_annotates_only_nonfinite() -> None:
    ...
    result = _nullify_non_finite(data)
    assert isinstance(result, dict)
    sanitized: dict = result
...
def test_output_is_json_valid_after_sanitize() -> None:
    """After _nullify_non_finite, json.dumps(allow_nan=False) must succeed (no Inf/NaN leak)."""
    data = {
...
def test_scalar_inf_outside_dict_is_nulled_no_annotation() -> None:
    ...
    assert _nullify_non_finite(float("inf")) is None
    assert _nullify_non_finite(float("-inf")) is None
    assert _nullify_non_finite(float("nan")) is None


def test_finite_scalar_pass_through() -> None:
    """Finite scalars, booleans, strings, None, int — pass through unchanged."""
    assert _nullify_non_finite(3.14) == 3.14
    assert _nullify_non_finite(0) == 0
    assert _nullify_non_finite("hello") == "hello"
    assert _nullify_non_finite(True) is True
    assert _nullify_non_finite(None) is None
...
def test_annotation_value_is_a_string_not_float() -> None:
    ...
    result = _nullify_non_finite({"x": float("inf")})
    assert isinstance(result, dict)
    sanitized: dict = result
    ...
    result2 = _nullify_non_finite(result)
