...
import pytest
from oracles import bench_contract
...
def _minimal_value(annotation: object) -> object:
    ...
    if origin is dict:
        return {}
    if origin is tuple:
        return tuple(_minimal_value(arg) for arg in get_args(annotation) if arg is not Ellipsis)
...
def test_alias_exemptions_still_exist() -> None:
    ...
    for (model_name, field_name), alias in _ALIAS_EXEMPT.items():
        ...
        assert info is not None, f"exemption for unknown field {model_name}.{field_name}"
