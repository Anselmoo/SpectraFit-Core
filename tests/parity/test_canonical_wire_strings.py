...
#
# Heuristic: any *.py file under python/oracles/ that contains the
...
# fail on drift, it asserts that the canonical declarations live in
# ``oracles.bench_contract`` and are importable.
# ---------------------------------------------------------------------------
...
def test_contract_module_is_the_single_source_of_truth() -> None:
    """No module under python/oracles/ other than the contract module itself should
    re-declare a frozenset of solver ids or a tuple of gate-state literals — that
    would be the start of the drift bug Top-10 #7 was raised to prevent.

    Heuristic match: an inline `frozenset({"spectrafit", "lmfit", ...})` literal,
    or `("pass", "warn", "fail")` literal, outside the contract module is flagged.

    The canonical module is ``python/oracles/bench_contract.py`` — the direct
    (git-traced) descendant of the ``python/extras/bench/contract.py`` this scan was
    written against, renamed via ``python/benchmark/contract.py``. It is NOT
    ``python/oracles/contract.py``, a later Plan-H module holding only the shared
    ``_Contract`` base and ``SolverMeta``; that module has never declared
    ``GATE_STATES``/``KNOWN_SOLVER_IDS`` and every consumer (``cli.py``,
    ``reports.py``, this test) imports them from ``oracles.bench_contract``.
    Exempting the wrong filename here would make this guard flag its own source of
    truth.
    """
    oracles_root = Path(__file__).resolve().parent.parent.parent / "python" / "oracles"
    canonical = "bench_contract.py"
    forbidden_patterns = [
        re.compile(r'frozenset\(\{[^}]*"spectrafit"'),
        re.compile(r'\("pass",\s*"warn",\s*"fail"\)'),
    ]
    offenders: list[str] = []
    for py in oracles_root.rglob("*.py"):
        if py.name == canonical:
            continue
        ...
        for pat in forbidden_patterns:
            if pat.search(text):
                offenders.append(f"{py.relative_to(oracles_root)} matches {pat.pattern}")
    assert not offenders, (
        f"Inline wire-string declarations found outside {canonical} — "
