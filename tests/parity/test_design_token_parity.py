...
def test_type_families_match(
    ...
    def first(stack: str) -> str:
        ...
        return stack.split(",", maxsplit=1)[0].strip().strip("\"'").lower()
...
def test_no_solver_is_missing_from_docs(
    ...
    web_solvers = {
        m.group(1) for k in web if (m := re.fullmatch(r"--c-([\w-]+?)(?:-soft)?", k)) is not None
    }
    docs_solvers = {
        m.group(1)
        for k in docs_slate
