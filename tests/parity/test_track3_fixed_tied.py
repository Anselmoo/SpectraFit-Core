...
def _safe_fit(backend, case: BenchCase):
    ...
    except Exception:  # noqa: BLE001 - defensive: any backend failure just skips this case
        return None
...
def test_fixed_param_center_recovered_exactly(case_id: str) -> None:
    ...
    assert outcome is not None, "fit must succeed"
    assert outcome.success, "fit must succeed"
...
def test_fixed_case_free_params_recover(case_id: str) -> None:
    ...
    case = _case_by_id(case_id)
    fixed_keys = {f"{nid}.{p}" for nid, ps in case.spec.fixed_params.items() for p in ps}
...
def test_tied_param_group_consistent_spectrafit(case_id: str) -> None:
    ...
    outcome = _safe_fit(spectrafit, case)
    assert outcome is not None, "spectrafit fit must succeed"
    assert outcome.success, "spectrafit fit must succeed"
...
def test_tied_param_group_consistent_lmfit(case_id: str) -> None:
    ...
    assert outcome is not None, "lmfit fit must succeed"
    assert outcome.success, "lmfit fit must succeed"
...
def test_tied_case_truth_recovery_spectrafit(case_id: str) -> None:
    ...
    tied_targets = {f"{e['target_node']}.{e['target_param']}" for e in case.spec.expr_edges}
    truth = case.true_params
    for key, tval in truth.items():
        ...
        rel = abs(fval - tval) / max(abs(tval), 1e-3)
        assert rel < 0.12, (
            f"{case_id}: param {key} off by {rel:.1%} (truth={tval:.4f}, fitted={fval:.4f})"
        )
