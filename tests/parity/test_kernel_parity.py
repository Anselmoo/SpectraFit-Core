...
_PARAM_VALUES: dict[str, float] = {
    ...
    # rational_cubic numerator: unconstrained, any finite values are valid.
    "a0": 1.2,
    "a1": 0.3,
    "a2": 0.15,
    "a3": 0.02,
    # rational_cubic denominator: D = 1 + b1·x + b2·x² + b3·x³ must not vanish on
    # the parity grid x ∈ [-4, 8]. These keep D ∈ [0.97, 3.19] there — its only
    # interior stationary point is x ≈ -1.4, where D ≈ 0.966.
    "b1": 0.05,
    "b2": 0.02,
    "b3": 0.001,
    # generalised_logistic. Only `shift` is new: `amplitude`, `rate` and `shape`
    # are shared with other models above, and at those values 1+exp(shift−rate·x)
    # runs 1.14 to 17.4 on x in [-4, 8] — no overflow, no underflow.
    "shift": 1.2,
    # exp_over_linear denominator. These are DELIBERATELY not named den_const /
    # den_lin: this table is keyed by parameter name across every model, so
    # reusing mgh09_rational's names would silently overwrite its values, and
    # its 0.14/0.12 put this kernel's denominator through zero at x = -1.167,
    # inside the grid. Here D runs 2.0 to 8.0.
    "lin_const": 4.0,
    "lin_slope": 0.5,
}
...
def test_emg_extreme_tail_numpy_rust_parity() -> None:
    ...
    graph = FitGraph(nodes=[node])
    data = MeasurementData(x=_EMG_TAIL_X.tolist(), y=[0.0] * len(_EMG_TAIL_X))
