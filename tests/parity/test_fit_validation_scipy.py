...
def _lorentz(x: np.ndarray, amplitude: float, center: float, sigma: float) -> np.ndarray:
...
def test_differential_validation_single_gaussian() -> None:
    ...
    sf_vals = [sf.params["peak.amplitude"].value, sf.params["peak.center"].value, sf.params["peak.sigma"].value]
    sc_vals = list(sc.x)
    ...
    sf_stderrs = [sf.params["peak.amplitude"].stderr, sf.params["peak.center"].stderr, sf.params["peak.sigma"].stderr]
...
def test_differential_validation_two_overlapping_gaussians() -> None:
    ...
    # ---- scipy ----
    def _resid(theta: np.ndarray) -> np.ndarray:
        return _gauss(x, theta[0], theta[1], theta[2]) + _gauss(x, theta[3], theta[4], theta[5]) - y_noisy

    x0 = np.array([3.5, 0.3, 0.5, 2.5, 1.8, 0.7])
    sc = least_squares(_resid, x0, method="lm")
    assert sc.success, "scipy fit did not converge on Case 2"

    param_keys = ["g1.amplitude", "g1.center", "g1.sigma", "g2.amplitude", "g2.center", "g2.sigma"]
