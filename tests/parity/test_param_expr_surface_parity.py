...
def _node_g2_expr_arith() -> ModelNodeSpec:
    ...
    return ModelNodeSpec(
        ...
        model_type=ModelType.GAUSSIAN,
        parameters={
            "amplitude": Parameter(value=1.5, expr="0.5 * g1.amplitude", vary=False),
            "center": Parameter(value=0.8),
            "sigma": Parameter(value=0.4, min=1e-3),
        },
    )
...
for _solver in _SOLVERS:
    for (
        ...
        _data_label = (
            "noisy" if (_data is _DATA_NOISY or _data is _DATA_ARITH_NOISY) else "clean"
        )
        _CELLS.append(
            (_solver, _ga_fn, _gb_fn, _data, _tied, _src, _factor, _truth, _free_keys)
        )
