...
def _gaussian_node(node_id: str = "g1", center: float = 0.0) -> ModelNodeSpec:
    """Build a canonical Gaussian node (amplitude/center/sigma per the model reference)."""
...
def test_model_type_enum_parity() -> None:
    ...
    rust_wire = set(json.loads(core.model_type_wire_strings()))
...
def test_dataset_slice_parity_for_multi_dataset_fit() -> None:
    ...
    data = _gaussian_data()
    x_flat: list[float] = [
        float(row[0])  # ty: ignore[not-subscriptable]  # data.x is (N,D) post-validation (before-validator coerces to list[list[float]])
        for row in data.x
    ]
