...
def test_regression_smoke_gaussian() -> None:
    ...
    assert sf_amp == pytest.approx(A_true, rel=0.01), (
        f"spectrafit amplitude off: {sf_amp!r}"
    )
    assert sf_cen == pytest.approx(c_true, rel=0.01), (
        f"spectrafit center off: {sf_cen!r}"
    )
    assert sf_sig == pytest.approx(s_true, rel=0.01), (
        f"spectrafit sigma off: {sf_sig!r}"
    )
