"""NIST StRD Gauss2 — multi-component FitGraph V&V (Cycle 16.C).

Extends the Gauss1 multi-component V&V (Cycle 16.A) to the Gauss2 dataset.
Both share the same 3-component model (one exponential decay + two Gaussians);
only the data and certified values differ. NIST classifies Gauss2 as
**"Lower"** difficulty — recovery is robust from both published starting
guesses — which makes it a direct structural parallel to Gauss1.

* Gauss1 (Cycle 16.A): peaks near x≈67 and x≈179.
* **Gauss2 (this file)**: peaks near x≈107 and x≈153. Different x-range for
  the Gaussian centroids stresses the multi-node Jacobian assembly and
  covariance block from a different geometric configuration.
...
import numpy as np
import pytest
from oracles.nist_strd.gauss2 import (
    CERTIFIED,
    DOF,
    N_OBS,
    RSS,
...
def _build_graph(start: dict[str, float]) -> FitGraph:
    """Build the 3-component FitGraph matching the NIST Gauss2 model.
...
def _project_to_nist(result) -> dict[str, float]:
...
def test_gauss2_recovers_nist_certified_parameters(
    start_name: str,
    start: dict[str, float],
) -> None:
    """From either NIST starting guess, spectrafit recovers all 8 certified
    Gauss2 parameters within 1e-3 relative tolerance.
    ...
    assert result.success is True, f"Gauss2 ({start_name}) did not converge: {result.message}"
...
def test_gauss2_rss_matches_nist_certified() -> None:
    """The fitted residual sum of squares matches NIST's certified RSS.

    Certified: 1.2475282092e+03 (10 sig figs).
    ...
    assert rel < 1e-4, f"RSS: recovered {rss_recovered:.10e}, certified {RSS:.10e}, rel {rel:.2e}"


def test_gauss2_reduced_chi2_matches_certified() -> None:
...
def test_gauss2_fixture_is_well_formed() -> None:
    ...
    # Two visible peaks at b4 ≈ 107 and b7 ≈ 153 in the certified values.
    # The data should show local maxima nearby (within ±10 grid points).
    # 75 % envelope is used because the peaks are broad (σ_1 ≈ 23/√2 ≈ 17,
    # σ_2 ≈ 19/√2 ≈ 14 grid points) and the y at the *exact* certified center
    # may sit somewhat below the windowed local max.
    for name in ("b4", "b7"):
        cx, _ = CERTIFIED[name]
        i = round(cx) - 1  # x is 1-indexed; arrays 0-indexed
