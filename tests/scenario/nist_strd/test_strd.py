...
def _build_eckerle4_graph(start: str) -> FitGraph:
    ...
    match start:
        ...
        case _:  # pragma: no cover - dev guard
...
def test_eckerle4_recovers_nist_certified_parameters(start: str) -> None:
    ...
    assert result.success is True, f"Eckerle4 ({start}) did not converge: {result.message}"
    ...
    assert rel_b1 < 1e-3, (
        f"b1: recovered {b1_recovered:.6e}, certified {NIST_B1:.6e}, rel {rel_b1:.2e}"
    )
    assert rel_b2 < 1e-3, (
        f"b2: recovered {b2_recovered:.6e}, certified {NIST_B2:.6e}, rel {rel_b2:.2e}"
    )
    assert rel_b3 < 1e-6, (
        f"b3: recovered {b3_recovered:.6e}, certified {NIST_B3:.6e}, rel {rel_b3:.2e}"
    )
...
def test_eckerle4_stderr_matches_nist_certified() -> None:
    ...
    cov_amp_amp = cov[idx_amp][idx_amp]
    cov_sig_sig = cov[idx_sigma][idx_sigma]
    cov_amp_sig_cell = cov[idx_amp][idx_sigma]
    assert cov_amp_amp is not None
    assert cov_sig_sig is not None
    assert cov_amp_sig_cell is not None
    var_amp = float(cov_amp_amp)
    var_sig = float(cov_sig_sig)
    cov_amp_sig = float(cov_amp_sig_cell)
    var_b1 = sigma * sigma * var_amp + amp * amp * var_sig + 2.0 * amp * sigma * cov_amp_sig
