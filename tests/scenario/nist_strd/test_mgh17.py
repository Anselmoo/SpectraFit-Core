...
def test_mgh17_recovers_nist_certified_parameters_start1() -> None:
    ...
    assert result.success is True, f"MGH17 (start1) did not converge: {result.message}"
...
def test_mgh17_recovers_nist_certified_parameters_start2() -> None:
    ...
    assert result.success is True, f"MGH17 (start2) did not converge: {result.message}"
...
def test_mgh17_rss_matches_nist_certified() -> None:
    ...
    assert rel < 1e-4, f"RSS: recovered {rss_recovered:.6e}, certified {RSS:.6e}, rel {rel:.2e}"
...
def test_mgh17_reduced_chi2_and_dof() -> None:
    ...
    assert rel < 1e-4, (
        f"reduced χ²: recovered {recovered:.6e}, certified {certified_red_chi2:.6e}, rel {rel:.2e}"
    )
...
def test_mgh17_certified_stderr_within_tolerance() -> None:
    ...
    for name, (_, certified_sigma) in CERTIFIED.items():
        ...
        assert fitted_sigma is not None, f"{name}: stderr is None (covariance not computed?)"
...
def test_mgh17_fixture_is_well_formed() -> None:
    ...
    assert math.isclose(math.sqrt(RSS / DOF), RESIDUAL_STD_DEV, rel_tol=1e-9), (
