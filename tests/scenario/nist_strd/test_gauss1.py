"""NIST StRD Gauss1 — multi-component FitGraph V&V (Cycle 16.A).
...
Gauss1 as **"Lower"** difficulty — recovery is robust from both
published starting guesses — which makes it the right second test:

* Eckerle4 (Cycle 16): single-Gaussian, "Higher" difficulty, sensitive
  to start. Stresses the trust-region step machinery.
* **Gauss1 (this file)**: 3-component composite, "Lower" difficulty,
...
import numpy as np
import pytest
from oracles.nist_strd.gauss1 import (
    CERTIFIED,
    DOF,
    N_OBS,
    RSS,
...
def _build_graph(start: dict[str, float]) -> FitGraph:
    """Build the 3-component FitGraph matching the NIST Gauss1 model.
...
def test_gauss1_recovers_nist_certified_parameters(
    start_name: str,
    start: dict[str, float],
) -> None:
    """From either NIST starting guess, spectrafit recovers all 8 certified
    Gauss1 parameters within 1e-3 relative tolerance.
    ...
    assert result.success is True, f"Gauss1 ({start_name}) did not converge: {result.message}"
...
def test_gauss1_rss_matches_nist_certified() -> None:
    """The fitted residual sum of squares matches NIST's certified RSS.

    Certified: 1.3158222432e+03 (10 sig figs).
...
def test_gauss1_reduced_chi2_matches_certified() -> None:
...
def test_gauss1_fixture_is_well_formed() -> None:
    ...
    # Two visible peaks at b4 ≈ 67 and b7 ≈ 179 in the certified values.
    # The data should show local maxima nearby (within ±10 grid points).
    # 80 % envelope is generous because the peaks are broad (σ_1 ≈ 23/√2 ≈ 16,
    # σ_2 ≈ 18/√2 ≈ 13 grid points) and the noise on Gauss1 is non-trivial —
    # the y at the *exact* certified center can sit well below the local max
    # (e.g., Y[178]=84.27 vs neighborhood max 90.66 = 93 %).
    for name in ("b4", "b7"):
        cx, _ = CERTIFIED[name]
