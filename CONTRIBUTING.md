...
GitLab remains the source of truth throughout; this lane only makes the iteration loop faster. See [`docs/contributor-guide/github-mirror-workflow.md`](docs/contributor-guide/github-mirror-workflow.md) for the full mechanism.

**Don't let a branch sit forgotten on the mirror.** `uv run poe check_stale_github_branches` lists every GitHub-only branch with un-backported commits older than 14 days (`--min-age-days` to tune, `--strict` to exit non-zero) — nothing runs this automatically yet, so run it yourself occasionally, or if you're picking up someone else's stalled fast-iteration branch.

## External contributions (no GitLab access)

Everything above — "every feature lands there as a GitLab MR, no exceptions" —
assumes push access to GitLab MPCDF. That's an institutional GitLab instance;
an outside contributor almost certainly cannot get an account on it.

What's verifiable: GitHub `main` here is a disposable, periodically
force-pushed mirror (see "Git remotes" above); nothing merged directly into it
survives the next publish; GitLab MPCDF is the real merge target, and access
to it is institutional, not self-service.

What isn't documented anywhere: how a contribution from someone without GitLab
access is actually supposed to reach `main`.

> TODO(maintainer): decide and document the outside-contribution intake path.
> Concretely, answer: does a GitHub PR from an external contributor get
> triaged and manually replayed onto GitLab by a maintainer? Are patches
> accepted by some other channel (email, an attached diff on an issue)? Or are
> outside contributions not accepted at all right now? Until one of these is
> chosen and written down here, treat opening a GitHub PR or issue as starting
> a conversation a maintainer would need to manually carry to GitLab — not as
> a merge request in the GitLab sense.
...
- Include tests (pytest / cargo / vitest as appropriate); the benchmark **gate** (`uv run poe benchmark_gate`, or `uv run python -m oracles.cli gate` — the `spc-bench` console script was removed 2026-06-23) must stay green.
- The repository structure is enforced by `rrt folder check` (pre-commit + CI) — do not remove required root files or directories.

## Review process

There is currently no documented statement of who reviews a contribution, on
what timeline, or against what criteria — [`CODEOWNERS`](CODEOWNERS) names an
owner per area, but that's routing, not a review policy.

> TODO(maintainer): document the review process. At minimum, answer:
> who is expected to review a PR/MR (just the `CODEOWNERS` owner, or others)?
> Is there any expected response-time window, even an approximate one? What
> must be true for a change to be accepted — passing CI only, or specific
> criteria beyond the automated gates (e.g. benchmark-gate deltas, doc
> updates, changelog entries)? Do different areas (Rust kernels vs. Python
> oracles vs. docs) have different bars? This section deliberately does not
> commit to an SLA — add one here only if a real, meetable commitment is
> decided.
