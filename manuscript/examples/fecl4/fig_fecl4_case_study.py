...
def build(spec: dict, peaks: list) -> Any:
    ...
    # expression edge instead of a free parameter. Sourced from
    # fecl4_constraint_scenarios.tie(), the one place this expression is
    # written, rather than a second hand-typed copy of the same string — the
    # import is deferred to call time because fecl4_constraint_scenarios
    # imports *this* module at its own top level for CS.load/CS.SPECTRA, and a
    # top-level import here would close that into a real cycle.
    from fecl4_constraint_scenarios import tie

    expr, target = tie("step", "")  # key is unused for "step"; "spin_orbit" needs it
    return compose(nodes).bind(expr, to=target).build()
...
def main() -> None:
    ...
    # sort_keys because the parameter mapping's insertion order is not stable
    # across runs: two consecutive runs of this script produced byte-different
    # files that were semantically identical, showing up as a ~258-line diff of
    # pure reordering. data_availability.md states that the figures and tables
    # rebuild from a clean checkout, and a reader cannot check that by diffing an
    # artifact that will not reproduce itself.
    (HERE / "fecl4_fit_results.json").write_text(
