...
| `true_voigt` | $A \cdot \mathrm{Re}[w(z)]/\mathrm{Re}[w(z_0)]$, $z=((x-c)+i\gamma)/(\sigma\sqrt{2})$, $z_0=i\gamma/(\sigma\sqrt{2})$ — true Gaussian $\otimes$ Lorentzian via the Faddeeva function (Rust: Hui–Armstrong–Wray, ~1e-6; numpy: `scipy.special.wofz` → wheel-vs-numpy parity ~1e-4) | amplitude, center, sigma, gamma | `TRUE_VOIGT` |
