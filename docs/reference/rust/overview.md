...
- **Add a new solver family** — [Extending SpectraFit-Core](../../contributor-guide/extending.md#adding-a-solver-family)
  lists the crate-DAG rules a new crate must respect, and every touchpoint
  from the `Solver` enum to the binding audit.
- **See how these crates fit into one request** — [Architecture](../../contributor-guide/architecture.md)
  traces a fit from the Python API call through the DAG compiler to these
  Rust kernels and back.
