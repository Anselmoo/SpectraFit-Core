...
> **Known partial drift (2026-07-24):** the `python/benchmark/` → `python/oracles/` path rename
> (F13, 2026-06-27) and the `spc-bench` console-script removal date have been corrected
> throughout this doc. The deeper architectural claims below — e.g. "adding a model touches five
> places" citing `_MODEL_MAP`/`backends/_shared.py`'s `CaseStructure.model` `Literal` — were
> **not** re-verified against the current registry-driven `MODEL_REGISTRY` design (see CLAUDE.md's
> "Adding a New Benchmark Model") and may also be stale; treat CLAUDE.md as authoritative for that
> specific procedure until this document gets a full pass.
...
| Benchmark harness (cases, catalog, backends, runners, export) | `python/oracles/` |
...
- **spectrafit** — the Rust core (`python/oracles/backends/_spectrafit.py`).
- **lmfit** — scipy/lmfit reference (`python/oracles/backends/_lmfit.py`).
- **jax** — a JAX VarPro/least-squares path (`python/oracles/backends/_jax.py`).
...
`python/oracles/models.py` (`gaussian`, `lorentzian`, `pseudo_voigt`,
...
```
python/oracles/models.py   →  synthetic y = f_python(x; θ_true) + noise
                                          │
                                          ▼
...
Keep `python/oracles/models.py` as the data generator, and **add a parity
...
3. **Adding a model touches five places.** Per `CLAUDE.md`: a Python formula in
   `python/oracles/models.py`; a `model_hint` value in the
...
All commands are `poe` tasks (`pyproject.toml`) or `python -m oracles.cli`
invocations (the `spc-bench` console script was removed in Option A packaging,
2026-06-23 — run the bench via `uv run poe benchmark` or
...
`python/oracles/cli.py`.

| Run type | Command | Reps / sweep |
|---|---|---|
| Default benchmark | `poe benchmark` / `uv run python -m oracles.cli run` | 5 reps, full catalog (`--reps 5 --mc 20`) |
| Quick (local iteration) | `poe benchmark_quick` / `uv run python -m oracles.cli run --reps 3 --mc 6` | 3 reps |
| Gate | `poe benchmark_gate` / `uv run python -m oracles.cli gate` | reads latest manifest |
| Reps sweep | `uv run python -m oracles.cli sweep --tiers 1,3,5,10,20` | arbitrary comma-separated rep ladder |
...
`BenchReport` contract (see `python/oracles/contract.py`).
...
`python/oracles/reports.py` (run id pattern `^(\d{4}-\d{2}-\d{2})_run_(\d{3,})$`).
