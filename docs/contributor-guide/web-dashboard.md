...
description: Running the Vite + React benchmark dashboard locally against the FastAPI /api/report service, and its Standing/Audit/Evidence panel structure.
tags:
  - Benchmarking
  - Python
---
...
- **Look up the endpoint this dashboard fetches** — [Python: benchmark
  API](../reference/python/benchmark-api.md) covers the `/api/report`
  FastAPI app, its `/api/v1` vs. legacy dual mount, and the CORS constraint
  tied to the report's git provenance.
- **See the full tooling configuration** — [`pyproject.toml`,
  explained](project-configuration.md) covers why the `poe` tasks referenced
  above (`report_html`, `contract_regen`, `web_smoke`) are configured the
  way they are.
