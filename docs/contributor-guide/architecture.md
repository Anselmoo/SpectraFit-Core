...
!!! danger "Generated block — do not hand-edit"
...
reports, are covered in [Solver](../explanation/solver-selection.md) — not
...
├── engine.py      # build_report: run_suite (all 154) + run_featured (deep-dive every case);
...
- **Learn the two extension points** — [Extending SpectraFit-Core](extending.md)
  lists every touchpoint adding a model or a new solver family requires,
  building on the crate layout and DAG IR covered above.
- **See how this whole workspace is gated** — [CI pipeline & cache
  architecture](ci-pipeline.md) covers why the GitLab/GitHub pipelines are
