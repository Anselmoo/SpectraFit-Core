...
!!! note "This page is a teaser — the full report is one click away"
    ...
    this same run — open `report.html`, published at the site root on both
    GitLab Pages and GitHub Pages (see
    `tests/audit/test_audit_built_site_links.py`'s `_CI_ASSEMBLED` allowlist
    for why this root-absolute link is correct even though `report.html`
    isn't part of the docs source tree itself).
