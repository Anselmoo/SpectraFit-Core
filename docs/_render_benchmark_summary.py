...
def _render_speedup_figure(manifest: dict) -> str | None:
    ...
    rows = [
        (
            "Geometric mean",
            manifest["geomean_speedup_vs_baseline"],
            _FIG_ACCENT_PRIMARY,
        ),
    ]
...
def _render_speedup_distribution_figure(manifest: dict) -> str | None:
    ...
    if not solver_ids:
        return None
    data = [[p["speedup"] for p in points[solver_id] if "speedup" in p] for solver_id in solver_ids]
...
def _format_hero_proof(manifest: dict) -> str:
    ...
    Carries the run scope (case count, run id, date) alongside the headline
    numbers. Before 2026-08-22 it emitted gate + speedup + regressions and
    dropped all three, even though the sibling ``_format_summary`` reads them
    from the same dict — so the first number a visitor met was the only one on
    the site with no denominator, and it silently disagreed with the 16.4x that
    ``LIMITATIONS.md`` quotes from an older, smaller pinned run.
    """
    gate = manifest["gate_state"]
    passed = gate == "pass"
    badge_class = "sf-hero__proof-badge--pass" if passed else "sf-hero__proof-badge--fail"
...
<div class="sf-hero__proof">
  ...
  <span class="sf-hero__proof-scope">{n_cases} cases &middot; run <code>{run_id}</code> &middot; {date}</span>
</div>
"""
