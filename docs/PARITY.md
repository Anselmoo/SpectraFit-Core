...
the same 34 wire values (the model catalog has grown since this table was last
enumerated by hand). `ModelTypeStr::as_str()` in `crates/spectrafit-types/src/types.rs`
is the single canonical source for the current list — see also `MODELS.md`'s formula
table — rather than duplicating it here where it will drift again.
...
| delta0 | `float \| None` | `Option<f64>` `#[serde(default)]` | OK (Cycle 8.2 trust-region knob) |
| max_delta | `float \| None` | `Option<f64>` `#[serde(default)]` | OK (Cycle 8.2 trust-region knob) |
| eta | `float \| None` | `Option<f64>` `#[serde(default)]` | OK (Cycle 8.2 trust-region knob) |

### FitResult <-> FitResultSpec / DatasetSlice <-> DatasetSliceSpec

All 25 `FitResult` fields and all 5 `DatasetSlice` fields match the Rust structs
...
`evaluate_components`, `model_type_wire_strings`.
...
`model_type_wire_strings` is not wrapped by a high-level Python name either —
it is called directly as `_core.model_type_wire_strings()` by the Rust↔Python
`ModelType` parity test, `tests/parity/test_schema_parity.py`, which is its only
consumer.)
