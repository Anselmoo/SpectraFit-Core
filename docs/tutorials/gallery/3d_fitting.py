...
def _plot_projections(
    ...
    Parameters
    ----------
    grid : ndarray, shape (8,)
        The shared per-axis coordinate vector (identical along all 3 axes).
    data_cube, fit_cube : ndarray, shape (8, 8, 8)
        Observed data / fitted model, reshaped back from the flat (512,)
        arrays in the same (x, y, z) index order used to build the coords.
    fitted_center : tuple of float
        The fitted ``(center_0, center_1, center_2)`` values, used to pick
        which slice index to display along each fixed axis.
...
def main() -> None:
    """Synthesize, fit, report, and plot the native 3-D Gaussian example."""
