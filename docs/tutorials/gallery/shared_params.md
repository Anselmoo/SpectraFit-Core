...
**When to use this pattern.** You have multiple peaks in a single spectrum and want to enforce that some parameters are identical across peaks. For example, two Lorentzian peaks that must share the same line width ($\sigma$), or three multiplet lines with identical broadening.
...
numerically interchangeable: same chi², same tied `sigma`, to within
