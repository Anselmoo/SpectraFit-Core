...
│   ├── spectrafit-models/     # Model trait + the full model catalog (see docs/reference/models/index.md), analytical/FD Jacobians
...
│   │   ├── compose.py         # factory functions + fluent ComposeBuilder (not operator overloading)
...
this writing. See the [Model Reference](docs/reference/models/index.md) for
the full, authoritative formula table (canonical parameter names, real LaTeX
formulas, one row per model, kept in sync with the Rust kernels and the
Python parity oracles by convention) — **not duplicated here**: an earlier
hand-copied subset of that table lived on this page, in plain-text notation,
and had already drifted (still `A * exp(...)` after the model reference
itself moved to real LaTeX). One table, one place, linked from here instead.
...
(see the [Model Reference](docs/reference/models/index.md)'s naming-history
section).
