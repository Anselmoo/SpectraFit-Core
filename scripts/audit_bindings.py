...
Usage:
    ...
    python scripts/audit_bindings.py --check  # exit 1 if the generated page is stale

`--check` guards a *different* invariant than the default (no-flag) mode: the
default mode enforces `binding_audit_notes.toml` against the Rust source;
`--check` enforces the generated `docs/reference/rust/binding-audit.md` page
against that TOML. It is not currently wired into any CI job or `lint_ci` —
a contributor who updates the TOML (forced by the default gate) and skips
`poe audit_bindings_regen` can still ship a stale page silently.
"""
...
def _render_markdown(
    ...
    # Front matter must match the page byte-for-byte or `--check` reports drift.
    # `description` and `tags` were hand-added to the generated page by the
    # 2026-08-21 docs-tagging pass (commit 0c5447a0) without teaching this
    # renderer to emit them, so `--write` would have silently stripped a page's
    # metadata and `--check` flagged the page as stale. Emitting them here keeps
    # regeneration lossless — every other docs page carries icon/description/tags,
    # and a generated one should not be the exception.
    lines = [
        "---",
        "icon: lucide/list-checks",
        (
            "description: The generated audit of every PyO3 entrypoint and "
            "Solver::Variant, regenerated from scripts/binding_audit_notes.toml "
            "— do not hand-edit this page."
        ),
        "tags:",
        "  - PyO3",
        "  - Solvers",
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            f"Exit non-zero if {AUDIT_MD.relative_to(ROOT)} is stale relative to "
            f"{NOTES_TOML.relative_to(ROOT)} + source (does not write)."
        ),
    )
    ...
    if args.check:
        if drift:
            print(
                f"Cannot check {AUDIT_MD.relative_to(ROOT)}: "
                f"{NOTES_TOML.relative_to(ROOT)} is out of sync with source — "
                "fix that drift first (run this script with no flags).",
                file=sys.stderr,
            )
            for line in drift:
                print(f"  • {line}", file=sys.stderr)
            return 1
        expected = _render_markdown(pyfunctions, solvers, notes)
        actual = AUDIT_MD.read_text() if AUDIT_MD.exists() else None
        if actual != expected:
            print(
                f"{AUDIT_MD.relative_to(ROOT)} is stale relative to "
                f"{NOTES_TOML.relative_to(ROOT)}. Run: uv run poe audit_bindings_regen",
                file=sys.stderr,
            )
            return 1
        print(f"{AUDIT_MD.relative_to(ROOT)} is up to date.")
        return 0
