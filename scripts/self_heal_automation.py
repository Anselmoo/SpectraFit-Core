...
_APPLIES_TO_RE = re.compile(r"^>\s*Applies to:\s*(.+?)\s*$")


def _expand_brace_group(pattern: str) -> list[str]:
    """Expand a single `{a,b,c}` brace group in a glob pattern, if present."""
    if "{" not in pattern or "}" not in pattern:
        return [pattern]
    pre, rest = pattern.split("{", 1)
    opts, post = rest.split("}", 1)
    return [pre + opt + post for opt in opts.split(",")]


def _applies_to_glob_is_alive(repo_root: Path, glob_expr: str) -> bool:
    """Return True if `glob_expr` matches at least one real path under repo_root."""
    for pattern in _expand_brace_group(glob_expr):
        try:
...
def _validate_instruction_file(
    ...
    """Validate a `.claude/instructions/*.md` file.

    These files use a `> Applies to: <glob>|<glob>...` blockquote as their
    first content line (not YAML frontmatter). This checks that the line is
    present and that every `|`-separated glob segment still resolves to at
    least one real path — the exact "wired to a path that moved" failure
    mode this validator previously could not see (it targeted `.github/`).
    """
    path = repo_root / rel_path
    text = path.read_text(encoding="utf-8")
    lines = text.splitlines()
    first_nonblank = next((line for line in lines if line.strip()), "")
    match = _APPLIES_TO_RE.match(first_nonblank)
    if not match:
        findings.append(
            Finding(
                surface="instructions",
                path=rel_path,
                code="instruction_missing_applies_to",
                severity="warning",
                message="Instruction file has no '> Applies to: <glob>' line as its first content line.",
            ),
        )
        return
    ...
    if not segments:
        findings.append(
            Finding(
                surface="instructions",
                path=rel_path,
                code="instruction_empty_applies_to",
                severity="warning",
                message="Instruction file 'Applies to:' line has no glob segments.",
            ),
        )
        return
    dead_segments = [seg for seg in segments if not _applies_to_glob_is_alive(repo_root, seg)]
    if dead_segments:
        findings.append(
            Finding(
                surface="instructions",
                path=rel_path,
                code="instruction_dead_applies_to_glob",
                severity="warning",
                message=(
                    "Instruction file 'Applies to:' glob segment(s) match no files in "
                    f"the repo (path likely moved or renamed): {', '.join(dead_segments)}"
                ),
                details={"dead_segments": dead_segments, "all_segments": segments},
            ),
        )
...
def _validate_agent_file(
    ...
    if "## termination criteria" not in lowered and "## completion criteria" not in lowered:
        findings.append(
            Finding(
                surface="agents",
                path=rel_path,
...
def run(mode: str, report_root: Path, paths: list[str], fail_on_fixes: bool) -> int:
    ...
    should_check_hooks_contracts = not paths or any(
        path.startswith(".claude/hooks/") or path == ".claude/settings.json" for path in selected
    )
    if should_check_hooks_contracts:
        _validate_settings_and_hook_contracts(repo_root, findings)

    for rel_path in selected:
        if rel_path.startswith(".claude/instructions/") and rel_path.endswith(".md"):
        ...
        elif rel_path.startswith(".claude/skills/") and rel_path.endswith("/SKILL.md"):
    ...
    remaining_errors = [item for item in findings if item.severity == "error" and not item.fixed]
    applied_fixes = [item for item in findings if item.fixed]
    exit_code = 0
    if remaining_errors or (mode == "fix-safe" and fail_on_fixes and applied_fixes):
        exit_code = 1
