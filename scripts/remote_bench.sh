...
#
# The host comes from .env (REMOTE=<ssh-host>); override per-run with
# REMOTE=<ssh-host>, the depth with REPS=<n>.
set -euo pipefail

# The host name lives in .env, which is gitignored, so the machine this project
# benchmarks on never reaches the public mirror. Override per-invocation with
# REMOTE=<ssh-host> in the environment; that wins over the file.
# Read only this one key, rather than sourcing the file. `.env` here holds keys
# that are not valid shell identifiers (`GITLAB-CI...`), so `. ./.env` aborts on
# them — and sourcing a dotenv executes whatever is in it, which a host name
# does not need.
if [ -z "${REMOTE:-}" ] && [ -f .env ]; then
    REMOTE="$(sed -n 's/^[[:space:]]*REMOTE[[:space:]]*=[[:space:]]*//p' .env | tail -1 | tr -d "\"'" )"
fi
REMOTE="${REMOTE:?set REMOTE=<ssh-host> in .env (gitignored) or in the environment}"
REPS="${REPS:-25}"
MC="${MC:-4}"
...
# The payload is fed over stdin, NOT interpolated into `bash -lc '...'`. Every
# command block below contains single quotes of its own (`sed 's/^/  /'`,
# `tr -d ' '`), and each one closed the wrapper's quoting early — the remote
# shell then died on the first unquoted `(` it met. Piping to `bash -s` has no
# quoting layer to break.
rsh() { ssh "${SSH_OPTS[@]}" "$REMOTE" "bash -l -s" <<<"$1"; }
