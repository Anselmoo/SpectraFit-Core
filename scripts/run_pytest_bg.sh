...
# Build the job-metadata JSON blob once from the named fields below, so
# write_metadata() and the jobs.json updater (both consumers) share one
# structure instead of each independently unpacking the same 16 positional
# arguments in the same order — a single insertion/reorder used to silently
# shift every argument after it, with no static check, at both call sites.
build_job_metadata_json() {
  python3 - <<'PY' "$@"
import json
import sys

(status, job_id, pid, started_at, started_at_bern, label, mode, log_path,
 command, target_repo, target_repo_root, target_reports_root,
 target_tests_root, export_root, expected_exports) = sys.argv[1:]
print(json.dumps({
    "job_id": job_id,
    "status": status,
    "pid": int(pid),
...
JOB_METADATA_JSON=$(build_job_metadata_json "running" "$JOB_ID" "$PID" "$STARTED_AT" "$STARTED_AT_BERN" "$LABEL" "$MODE" "$LOG_PATH" "$COMMAND" "$TARGET_REPO" "$TARGET_REPO_ROOT" "$TARGET_REPORTS_ROOT" "$TARGET_TESTS_ROOT" "$EXPORT_ROOT" "$EXPECTED_EXPORTS_JSON")

write_metadata "$META_PATH" "$JOB_METADATA_JSON"

python3 - <<'PY' "$JOBS_JSON" "$JOBS_LOG" "$JOB_METADATA_JSON"
import json
import pathlib
import sys
from datetime import datetime, timezone
...
jobs[job_id] = {**entry, "updated_at_bern": now_bern}
...
with jobs_log_path.open("a", encoding="utf-8") as handle:
  handle.write(f"{entry['started_at_bern']} | {job_id} | {entry['mode']} | running | {entry['command']}\n")
PY
