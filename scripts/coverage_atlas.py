...
def parse_cobertura(path: Path, language: str, root: Path) -> list[FileCov]:
    ...
    for cls in tree.iter("class"):
        ...
        uncov = _collapse_to_ranges(
            [int(ln.get("number", "0")) for ln in lines if int(ln.get("hits", "0")) == 0],
        )
        files.append(FileCov(rel, language, total, hit, uncov))
    return files
...
<link href="https://fonts.googleapis.com/css2?family=IBM+Plex+Sans:wght@400;600;700&family=IBM+Plex+Mono:wght@400;600;700&family=JetBrains+Mono:wght@400;600;700&display=swap" rel="stylesheet">
<style>
__ATLAS_STYLE_TOKENS__
html, body {
...
.total {
    ...
    font-family: 'JetBrains Mono', "IBM Plex Mono", ui-monospace, monospace; letter-spacing: -0.02em;
}
.continent-legend {
...
.legend-chip[data-lang="python"]::before { background: var(--lang-python); }
.legend-chip[data-lang="rust"]::before { background: var(--lang-rust); }
.legend-chip[data-lang="web"]::before { background: var(--lang-web); }
...
.filelist-h {
    ...
    letter-spacing: 0.07em; font-weight: 700; padding: 0 20px 10px;
}
.filelist-row {
    display: flex; align-items: center; gap: 8px; width: 100%;
    padding: 6px 20px; border: none; background: none; cursor: pointer;
    color: var(--ink); font-family: inherit; font-size: 11.5px; text-align: left;
}
.filelist-row:hover, .filelist-row.active { background: var(--surface-2); }
.filelist-row .fl-dot {
    width: 8px; height: 8px; border-radius: 2px; flex: none;
}
.filelist-row .fl-path {
    flex: 1; min-width: 0; overflow: hidden; text-overflow: ellipsis;
    white-space: nowrap; font-family: 'JetBrains Mono', "IBM Plex Mono", ui-monospace, monospace;
}
.filelist-row .fl-pct {
    flex: none; color: var(--ink-2); font-variant-numeric: tabular-nums;
}
.copy-btn {
    flex: none; border: 1px solid var(--line); background: var(--surface-2);
    color: var(--ink-2); border-radius: 4px; font-size: 10px; cursor: pointer;
    padding: 2px 6px; font-family: inherit; line-height: 1.6;
}
.copy-btn:hover { color: var(--ink); border-color: var(--ink-3); }
.copy-btn.copied { color: var(--lang-web); border-color: var(--lang-web); }
rect.cell {
    cursor: pointer;
...
rect.cell.active-cell {
    stroke: var(--ink); stroke-width: 2.5;
}
text.cell-label {
    font-size: 10.5px; fill: var(--ink); pointer-events: none;
    font-family: 'JetBrains Mono', "IBM Plex Mono", ui-monospace, monospace; letter-spacing: 0;
    paint-order: stroke; stroke: rgba(0, 0, 0, 0.6); stroke-width: 2;
}
.detail-empty {
...
.lang-badge[data-lang="python"] { background: color-mix(in srgb, var(--lang-python) 22%, black); color: var(--lang-python); }
.lang-badge[data-lang="rust"]   { background: color-mix(in srgb, var(--lang-rust) 22%, black);   color: var(--lang-rust); }
.lang-badge[data-lang="web"]    { background: color-mix(in srgb, var(--lang-web) 22%, black);    color: var(--lang-web); }
</style>
</head>
<body>
<header>
...
<main>
    <aside id="filelist">
        <div class="filelist-h">Least-covered first</div>
...
const LANG_COLORS = { python: "#0A84FF", rust: "#5E5CE6", web: "#30D158" };
...
function renderFileList() {
    const rows = document.getElementById("filelist-rows");
    const sorted = ATLAS.leaves.slice().sort((a, b) => {
        const covA = a.total ? a.hit / a.total : 1;
        const covB = b.total ? b.hit / b.total : 1;
        return covA - covB;
    });
    rows.innerHTML = "";
    for (const file of sorted) {
        const cov = file.total ? (file.hit / file.total) * 100 : 100;
        const row = document.createElement("button");
        row.type = "button";
        row.className = "filelist-row";
        row.dataset.path = file.path;
        row.innerHTML = `
            <span class="fl-dot" style="background: ${colorFor(file)}"></span>
            <span class="fl-path" title="${file.path}">${file.path}</span>
            <span class="fl-pct">${cov.toFixed(0)}%</span>
            <button type="button" class="copy-btn" title="Copy file path">⧉</button>
        `;
        row.addEventListener("click", () => showDetail(file));
        row.querySelector(".copy-btn").addEventListener("click", (event) => {
            event.stopPropagation();
            copyText(event.currentTarget, file.path);
        });
...
function colorFor(file) {
    const base = LANG_COLORS[file.lang] || "#8E8E93";
    const cov = file.total ? file.hit / file.total : 1;
    // 0% → deep, almost black; 100% → bright peak
    const mixPct = 18 + cov * 78;
    return `color-mix(in srgb, ${base} ${mixPct.toFixed(0)}%, black)`;
}
...
function render() {
    ...
    svg.innerHTML = "";
    for (const key of Object.keys(cellRects)) delete cellRects[key];
...
function showDetail(cell) {
    ...
    const copyPayload = [cell.path, ...ranges.map(fmtRange)].join("\n");

    document.querySelectorAll(".filelist-row").forEach((row) => {
        row.classList.toggle("active", row.dataset.path === cell.path);
    });
    document.querySelectorAll("rect.cell.active-cell").forEach((r) => r.classList.remove("active-cell"));
    const rectEl = cellRects[cell.path];
    if (rectEl) rectEl.classList.add("active-cell");

    aside.innerHTML = `
        <div class="detail-title">
            <span class="lang-badge" data-lang="${cell.lang}">${cell.lang}</span>${cell.path}
            <button type="button" class="copy-btn" id="copy-gaps" title="Copy path + uncovered ranges">⧉ copy gaps</button>
        </div>
    ...
    `;
    const copyBtn = document.getElementById("copy-gaps");
    if (copyBtn) copyBtn.addEventListener("click", () => copyText(copyBtn, copyPayload));
}
...
def main() -> int:
    ...
    html = HTML_TEMPLATE.replace("__ATLAS_STYLE_TOKENS__", _ATLAS_STYLE_TOKENS)
    html = html.replace("__ATLAS_JSON__", json.dumps(atlas))
