...
def main() -> int:
    ...
    parser.add_argument(
        "--token", required=True, help="GitHub token (Actions: Read and write)"
    )
    parser.add_argument(
        "--dry-run",
        action="store_true",
    ...
    try:
        run_ids = list_all_run_ids(args.repo, args.token)
    ...
    if args.dry_run:
        print(f"purge_github_actions_runs: DRY RUN — would delete {len(run_ids)} run(s):")
    ...
    failures = [
        run_id for run_id in run_ids if not delete_run(args.repo, args.token, run_id)
    ]
    if failures:
