...
/// * Otherwise default to [`StepKind::NormalEqLlt`]; a cheap conditioning probe
///   (column-norm ratio) for escalating borderline cases is a known follow-up.
///
...
pub fn select_regime(n_residuals: usize, n_params: usize) -> StepKind {
    ...
    // (`p ≤ 40` but `m < 8·p`) also default to NE today; a cheap conditioning
    // probe (column-norm ratio) to escalate them to SVD when `J` is
    // ill-conditioned is a known follow-up. `n_residuals` is named for that future use.
    let _ = n_residuals;
    if n_params > 40 {
        StepKind::SvdSecular
    } else {
...
pub enum StepFactor {
    ...
    Ne {
        /// The Gauss–Newton Hessian `H = JᵀJ` (`p×p`).
        h: Mat<f64>,
    },
    ...
    Svd {
        ...
        u: Mat<f64>,
        /// Singular values `s` (length `k`), `k = min(m, p)`.
        s: Vec<f64>,
        /// Right singular vectors `V` (`p×k`).
        v: Mat<f64>,
    },
}
...
///
/// # Errors
/// Returns [`StepError::Factorization`] when the [`StepKind::SvdSecular`]
/// thin SVD of the column-scaled Jacobian fails to converge. The
/// [`StepKind::NormalEqLlt`] path forms `H = JᵀJ` directly and cannot fail
/// here (its own Cholesky factorization happens later, per-`λ`, in
/// [`StepFactor::solve`]/[`StepFactor::solve_rhs`]).
pub fn factorize(
    kind: StepKind,
    j: MatRef<'_, f64>,
    diag: &[f64],
    ...
    match kind {
        ...
        StepKind::SvdSecular => {
            ...
            let svd = j_scaled
                .as_ref()
                .thin_svd()
                // faer::SvdError does not implement Display; use Debug format.
