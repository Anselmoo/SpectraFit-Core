...
/// stalls to a `NoImprovement` "failure" (G25; TI-003 `fraction` pinned at
/// 1.0). Use a Coleman–Li-style scaled measure `‖v·g‖_∞` instead — `v_i → 0`
...
/// optimality gate use the unnormalized distance is tracked as G28.
///
...
pub fn minimize<P: TrustRegionProblem>(problem: &mut P, cfg: &StrategyConfig) -> Report {
    ...
    loop {
        ...
        // rationale and the G25/G28 notes behind the scaled measure.
