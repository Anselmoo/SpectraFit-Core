...
//!
//! # Known divergence from scipy: the `ftol` stop
//!
//! The `ftol` test here is `actual ≤ ftol · cost` evaluated on an **accepted**
//! step, and it deliberately omits the `ratio > 0.25` guard that
//! `scipy.optimize._lsq.common.check_termination` applies alongside the same
//! inequality. Consequence: an accepted but low-gain step — `ρ ∈ (1e-4, 0.25]`,
//! i.e. accepted because it did reduce the cost, but by much less than the
//! model predicted — with a tiny reduction stops this driver where scipy would
//! take at least one more step. It is a stop-one-step-early difference, not a
//! wrong answer: the reported point is still an accepted, cost-reducing
//! iterate. Documented rather than changed so the existing accuracy gates keep
//! measuring the solver as shipped.
//!
//! # Global state
//!
//! This driver does not touch faer's global parallelism. Callers are expected
//! to set it (`spectrafit-solver::fit` sets `Par::Seq` for the whole fit); the
//! driver itself does not touch global state.

use faer::{Mat, MatRef};

use crate::error::StepError;
use crate::step::{factorize, StepFactor, StepKind};
...
// Allowed: this is the per-outer-iteration hot path — the mutable problem,
// its cached factorization/Jacobian/residual/gradient, the current point and
// LM velocity, the damping/probe-step/mixing scalars (diag/lambda/h/alpha),
// and the shared eval counter are each consumed directly below; wrapping
// them in a struct here would just rename this same parameter list without
...
/// Update the monotone Moré column scaling `D` in place from the current
/// Jacobian's column norms: `D_j ← max(D_j, ‖J_:,j‖)`, floored at 1 for an
/// all-zero column, seeded (not maxed) on the first call. Per MINPACK, `diag`
/// never shrinks, so the trust region stays measured in units that equalise
/// the Jacobian columns regardless of how many outer iterations have run.
fn update_more_scaling(diag: &mut [f64], diag_initialized: &mut bool, j: MatRef<'_, f64>) {
    // faer stores columns contiguously, so `col(jc).norm_l2()` is a fast SIMD
    // reduction.
    for (jc, d) in diag.iter_mut().enumerate() {
        let cn = j.col(jc).norm_l2();
...
/// Gradient `g = Jᵀr`, its raw infinity norm, and the first-order optimality
/// measure used for the `gtol` stop.
///
/// For a bounded problem the raw `‖g‖_∞` is the wrong measure at an active
/// bound: the gradient component pointing into the wall can stay large at a
/// legitimate constrained optimum, so the raw test never fires and the loop
/// stalls to a `NoImprovement` "failure" (repro: TI-003, `fraction` pinned
/// at 1.0). Use a Coleman–Li-style scaled measure `‖v·g‖_∞` instead — `v_i → 0`
/// as parameter `i` approaches the bound its descent direction points into —
/// so an active-bound gradient no longer blocks convergence. Unbounded
/// problems (`trust_scaling` = `None`) keep the raw norm, unchanged. The raw
/// `gnorm` (not `opt_norm`) is what callers should record for observability.
///
/// NOTE: `trust_scaling` returns `v` normalized by box width (`v_i =
/// dist_i/range_i`, in `spectrafit-solver::lm_problem`), NOT scipy TRF's
/// unnormalized distance. So this gate is laxer than scipy's by ~1/range for
/// a wide-box parameter sitting mid-box facing a wall; the accuracy gate
/// (max |Δr²| vs the oracle) bounds the practical impact. Making the
/// optimality gate use the unnormalized distance is a known follow-up, not
/// yet done.
///
/// Returns `(g, gnorm, opt_norm, trust_v)` — `trust_v` is handed back (rather
/// than recomputed) so the caller can reuse it for the Coleman–Li step-diag
/// fold in [`compute_step_diag`] without a second `trust_scaling` call.
fn compute_gradient_and_optimality<P: TrustRegionProblem>(
    problem: &P,
    j: MatRef<'_, f64>,
    r: MatRef<'_, f64>,
) -> (Mat<f64>, f64, f64, Option<Vec<f64>>) {
    let g = j.transpose() * r;
...
/// near active bounds. `diag` itself is never modified — this returns a
/// derived copy for the current factorization only.
fn compute_step_diag(diag: &[f64], bound_scaling: bool, trust_v: Option<Vec<f64>>) -> Vec<f64> {
    if bound_scaling {
        match trust_v {
            Some(v) => diag
                .iter()
...
///
/// On return the problem holds the best parameters found. Deterministic: no
/// RNG, no clock — identical inputs give identical iterates — given a fixed
/// faer parallelism policy; `spectrafit-solver::fit` sets `Par::Seq` for the
/// whole fit, and a caller that leaves faer on its default (potentially
/// multi-threaded) policy can see floating-point summation-order differences
/// across runs.
pub fn minimize<P: TrustRegionProblem>(problem: &mut P, cfg: &StrategyConfig) -> Report {
    ...
    loop {
        ...
        update_more_scaling(&mut diag, &mut diag_initialized, j.as_ref());

        // Gradient g = Jᵀr and first-order optimality test — see
        // `compute_gradient_and_optimality`'s doc comment for the Coleman–Li
        // rationale behind the scaled measure.
        let (g, gnorm, opt_norm, trust_v) =
            compute_gradient_and_optimality(problem, j.as_ref(), r.as_ref());
        ...
        // Per-iteration step scaling — see `compute_step_diag`'s doc comment.
        let step_diag = compute_step_diag(&diag, cfg.bound_scaling, trust_v);
        ...
        // Bump λ (shrink the trust region) after a rejected/failed trial, advance
        // the Nielsen ν schedule, and bail out with `NoImprovement` once the
        // per-outer-iteration λ-search budget is exhausted (200 bumps, or λ
        // overflowing to non-finite). Identical 4-line sequence at every
        // "this trial doesn't work" exit from the inner loop below — a failed
        // factorization, a non-positive predicted reduction (pre- or
        // post-geodesic), and an outright rejected step — so it is named once
        // here instead of repeated verbatim at each call site. Local macro (same
        // technique as `report!` above) rather than a function: it mutates
        // `lambda`/`nu`/`bumps` and reads `cost`/`gnorm` from the enclosing
        // scope, so a helper would need all five threaded through its signature
        // for no benefit. Defined here (not near `report!` above) because macro
        // hygiene resolves its free identifiers against what's in scope at the
        // macro's *definition* point, not its call site — `bumps` (declared just
        // above) must already exist for that to work.
        macro_rules! bump_lambda {
            () => {{
                lambda *= nu;
                nu *= 2.0;
                bumps += 1;
        ...
        loop {
            ...
            // Recompute the model reduction for the
            ...
            let pred_eff = if clipped {
                ...
                -col_dot(g.as_ref(), delta_a.as_ref()) - 0.5 * jda.as_ref().squared_norm_l2()
            } else {
                pred
            };
        ...
        if n_residual_evals >= cfg.max_nfev {
            // `gnorm` — ‖Jᵀr‖∞ at the start of this outer iteration, the most
            // recent point where the Jacobian was evaluated. This used to
            // recompute `(Jᵀ r).norm_max()` pairing the OLD `j` with the
            // post-step `r`, a quantity that is the gradient at neither point.
            // See `Report::gradient_norm`.
            report!(Termination::MaxEval, cost, gnorm);
        }
    }
}
