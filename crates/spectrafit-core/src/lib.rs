//! spectrafit-core — PyO3 cdylib bindings.
//!
...
//!   - `fit_arrays_numpy(graph_json, x, y, sigma, dataset_sizes, n_dims, options_json)
//!     -> (str, ndarray)` — like `fit_arrays`, but bypasses JSON serialisation of
//!     per-point arrays and returns the fitted curve as a NumPy array
//!   - `evaluate(graph_json, params_json, data_json) -> str`
//!   - `evaluate_components(graph_json, params_json, data_json) -> str`
//!   - `model_type_wire_strings() -> str` — returns the canonical wire-format
//!     string of every supported model type, in `ModelTypeStr` declaration order
//!
...
//
// The JSON entrypoints (`fit`, `evaluate`, `evaluate_components`) capture only
// `&str`, so their closures are `UnwindSafe` and call `guard` directly. The
// array entrypoints (`fit_arrays`, `fit_arrays_numpy`) capture PyO3 handles
// (`PyReadonlyArray1`, `Python<'py>`) that are not `UnwindSafe`, so they wrap
// the closure in `std::panic::AssertUnwindSafe`. That assertion is sound here
// because the guarded closure only *reads* its inputs (the numpy buffers are
// read-only views, never written) and builds fresh owned outputs; it mutates no
// shared or long-lived state, so an unwind cannot leave a caller-visible value
// half-modified. The captured handles are borrows the interpreter keeps alive
// for the whole call, and PyO3 itself re-raises whatever this returns.
// ---------------------------------------------------------------------------
...
/// Run a full fit and return the result as JSON.
///
/// The solver is selected by `options_json`'s `solver` field (default `lm`);
/// see `spectrafit_solver::fit` for the accepted solver strings.
///
...
/// Run a fit using raw numpy arrays for measurement data.
///
/// The solver is selected by `options_json`'s `solver` field (default `lm`);
/// see `spectrafit_solver::fit` for the accepted solver strings.
///
...
/// Returns a JSON string matching the `FitResult` Python schema.
///
/// Every failure — including a Rust panic raised inside the solver — reaches
/// Python as a catchable `ValueError` (see the `guard` panic boundary above).
#[pyfunction]
...
fn fit_arrays(
    ...
    // Same panic boundary as the JSON `fit` path — see the `guard` block above
    // for why `AssertUnwindSafe` is sound for this closure.
    guard(std::panic::AssertUnwindSafe(|| {
...
///
/// This is the single deliberate exemption from the "every `#[pyfunction]`
/// returns a JSON string" boundary rule: the ndarray is the whole point of the
/// zero-copy path. `.claude/hooks/pre-merge-pyO3.sh` allowlists this one name
/// and fails on any other non-JSON return type.
///
/// Every failure — including a Rust panic raised inside the solver — reaches
/// Python as a catchable `ValueError` (see the `guard` panic boundary above).
...
fn fit_arrays_numpy<'py>(
    ...
    // Same panic boundary as the JSON `fit` path — see the `guard` block above
    // for why `AssertUnwindSafe` is sound for this closure. `guard` is generic
    // over the success type, so the `(String, Bound<'py, PyArray1<f64>>)` return
    // needs no separate helper; only the `UnwindSafe` bound has to be asserted,
    // because the closure captures the `Python<'py>` token and the read-only
    // numpy views.
    guard(std::panic::AssertUnwindSafe(|| {
        let graph: FitGraphSpec = serde_json::from_str(graph_json).map_err(json_err)?;
        let options: FitOptionsSpec = serde_json::from_str(options_json).map_err(json_err)?;
...
///
/// Returns a JSON-encoded array of strings (not `Vec<String>`) — every
/// `#[pyfunction]` in this module returns a JSON string so the PyO3 boundary
/// has one uniform contract; callers `json.loads()` the result.
#[pyfunction]
fn model_type_wire_strings() -> PyResult<String> {
    let wire: Vec<&str> = ModelTypeStr::ALL.iter().map(|m| m.as_str()).collect();
    serde_json::to_string(&wire).map_err(json_err)
}
