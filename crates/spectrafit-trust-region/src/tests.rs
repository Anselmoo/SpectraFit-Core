...
impl SubproblemStep for CauchyStep {
    fn solve(&self, sub: &Subproblem<'_>, radius: f64) -> StepResult {
        ...
        let tau_unconstrained = if ghg > 0.0 { gnorm2 / ghg } else { f64::INFINITY };
...
fn gauss_newton_converges_in_one_step_on_consistent_system() {
    ...
    assert!(report.cost < 1e-18, "cost not driven to zero: {}", report.cost);
    let p = prob.params();
...
fn radius_shrinks_by_quarter_on_rejected_steps() {
    ...
    assert!(radii.len() >= 3, "expected several solve calls, got {radii:?}");
...
fn report_history_invariants_hold() {
    ...
    let report = minimize_tr(&mut prob, &GaussNewtonStep, &cfg);
    assert_eq!(report.cost_history.len(), report.gradient_norm_history.len());
    ...
    for w in report.cost_history.windows(2) {
        assert!(w[1] <= w[0] + 1e-12, "cost increased: {:?}", report.cost_history);
    }
}
