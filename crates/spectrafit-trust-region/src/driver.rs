...
//! inequality. Consequence: an accepted but low-gain step — `ρ ∈ (eta, 0.25]`,
...
pub struct TrustRegionConfig {
    ...
    ///
    /// Must be `< 0.25` (debug-asserted in [`minimize_tr`]). The Δ update
    /// shrinks the radius only when `ρ < 0.25`, so an `eta ≥ 0.25` opens a band
    /// `ρ ∈ [0.25, eta]` in which a step is rejected while Δ is left unchanged
    /// — the accept/reject subloop then re-solves the identical subproblem at
    /// the identical radius until the evaluation budget runs out.
    ///
    /// The `debug_assert!` above does not run in release builds, so this
    /// crate does not itself enforce the bound outside `cfg(debug_assertions)`
    /// — callers must validate `eta` before constructing this config.
    /// `spectrafit-solver::dispatch` does, rejecting `eta >= 0.25` (or
    /// non-finite / negative `eta`) with `SolverError::InvalidEta` before it
    /// reaches this driver.
    pub eta: f64,
}
...
pub fn minimize_tr<P: TrustRegionProblem, S: SubproblemStep>(
    ...
    debug_assert!(
        cfg.eta < 0.25,
        "eta must be < 0.25: a rejected step only shrinks Δ when ρ < 0.25"
    );
    ...
    // This driver records cost and gradient-norm history only, not the
    // per-iteration θ trajectory (that is the faer LM driver's job). Left
    // empty (honest) rather than partially populated.
    ...
    loop {
        ...
        // to a NoImprovement "failure" (same fix as the LM driver).
        ...
        // parameter — same caveat as the LM driver, and the same known
        // follow-up to use the unnormalized distance instead.
        ...
        match run_delta_subloop(
            ...
            DeltaLoopOutcome::Continue => {} // re-evaluate the Jacobian at the accepted point
            DeltaLoopOutcome::Terminate(term, final_cost, final_gnorm) => {
                report!(term, final_cost, final_gnorm);
            }
        }
...
/// Outcome of one call to [`run_delta_subloop`].
enum DeltaLoopOutcome {
    /// A step was accepted; the caller should re-evaluate the Jacobian at the
    /// new point and proceed to the next outer iteration.
    Continue,
    /// The driver should terminate immediately with this termination reason,
    /// reporting the given `(cost, gradient_norm)` pair — identical to the
    /// arguments the inlined loop used to pass to the `report!` macro at each
    /// of its call sites.
    Terminate(Termination, f64, f64),
}

/// The inner Δ-radius accept/reject subloop, run at a fixed Jacobian `j`/`g`
/// (evaluated once per outer iteration of [`minimize_tr`]). Repeatedly
/// proposes a step via `step.solve`, shrinking `delta` on a degenerate or
/// rejected step, until either a step is accepted
/// ([`DeltaLoopOutcome::Continue`] — the caller re-evaluates the Jacobian) or
/// a termination condition fires ([`DeltaLoopOutcome::Terminate`]).
///
/// This is a verbatim extraction of the loop body that used to live inline in
/// `minimize_tr`: it mutates `problem`'s parameters, `*delta`, `*r`/`*r_trial`
/// (via `mem::swap`), `*cost`, `*n_residual_evals`, and `*n_iter` in place,
/// exactly as the inlined loop mutated the corresponding outer-scope
/// variables — nothing here is "purely functional." `p_cur`, `j`, `g` are the
/// fixed parameter snapshot / unscaled Jacobian / unscaled gradient taken at
/// the start of this outer iteration (read-only across the accept/reject
/// attempts below); `gnorm` is the unscaled gradient norm from that same
/// snapshot, reported verbatim on termination exactly as the inlined loop
/// did (it is never recomputed inside this subloop).
// Allowed: this is a verbatim extraction of `minimize_tr`'s inner loop body
// (see the doc comment above) — the config/state it mutates in place
// (problem, delta, r/r_trial, cost, the eval/iter counters) and the
// read-only outer-iteration snapshot (p_cur/j/g/gnorm) are exactly the
// outer-scope variables the original inline loop closed over; bundling them
// into a struct would just rename this same parameter list.
#[allow(clippy::too_many_arguments)]
fn run_delta_subloop<P: TrustRegionProblem, S: SubproblemStep>(
    problem: &mut P,
    step: &S,
    sub: &Subproblem<'_>,
    cfg: &TrustRegionConfig,
    ...
    loop {
        ...
        if !(pred.is_finite() && pred > 0.0) {
            ...
            *delta *= 0.25;
            if *delta <= f64::MIN_POSITIVE * 4.0 {
                return DeltaLoopOutcome::Terminate(Termination::NoImprovement, *cost, gnorm);
            }
            continue;
        }
        ...
        if problem.residuals_into(r_trial.as_mut()).is_err() {
            return DeltaLoopOutcome::Terminate(Termination::NumericalError, *cost, gnorm);
        }
        *n_residual_evals += 1;
        ...
        // (same fix as the LM driver). Interior
        ...
        if rho > cfg.eta {
            ...
            std::mem::swap(r, r_trial);
            *cost = cost_trial;
            *n_iter += 1;

            if *cost == 0.0 {
                return DeltaLoopOutcome::Terminate(Termination::ResidualsZero, *cost, gnorm);
            }
            if actual <= cfg.ftol * (*cost).max(f64::MIN_POSITIVE) {
                return DeltaLoopOutcome::Terminate(Termination::Ftol, *cost, gnorm);
            }
            if step_norm <= cfg.xtol * (cfg.xtol + param_norm) {
                return DeltaLoopOutcome::Terminate(Termination::Xtol, *cost, gnorm);
            }
            return DeltaLoopOutcome::Continue; // re-evaluate the Jacobian at the accepted point
        } else {
        ...
        if *n_residual_evals >= cfg.max_nfev {
            return DeltaLoopOutcome::Terminate(Termination::MaxEval, *cost, gnorm);
        }
    }
}
