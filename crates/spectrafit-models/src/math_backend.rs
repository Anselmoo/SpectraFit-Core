...
mod platform {
    ...
    // The workspace denies `unsafe_code` (see `[workspace.lints.rust]` in the
    // root Cargo.toml). This is the single sanctioned exception: calling into
    // Accelerate's `vvexp` is an FFI boundary and cannot be expressed in safe
    // Rust. The `unsafe` block below carries the SAFETY argument; the aliasing
    // contract it relies on is documented in this module's header.
    //
    // `allow(unsafe_code)` is needed at BOTH sites: the attribute is
    // item-scoped, so the one on this `extern` block covers only the
    // declaration, not the call in `batch_exp_impl` below. The allow there sits
    // on the function rather than the `unsafe` block itself because attributes
    // on a trailing expression need the unstable `stmt_expr_attributes`.
    #[allow(unsafe_code)]
    extern "C" {
