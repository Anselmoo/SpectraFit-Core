...
/// value as in the peak-model convention documented in `docs/reference/models/index.md`.
...
mod tests {
    ...
    // (h=1e-6/1e-8, epsilon=1e-5/1e-4). Each test keeps its name
    // (traceability) but now checks only its own parameter index across the
    // full regime/x-point grid.
    const POWER_SAT_REGIMES: [[f64; 2]; 3] = [
        [10.0, 0.001], // nominal (slow rate, mirrors Misra1b scale)
        [1e-3, 1e-4],  // small amplitude/rate
        [50.0, 2.0],   // large amplitude, fast rate
    ];
    const POWER_SAT_XS: [f64; 5] = [0.5, 3.0, 20.0, 100.0, 500.0];

    fn check_power_sat_param_central_diff(idx: usize) {
        for p in POWER_SAT_REGIMES {
            for &x in &POWER_SAT_XS {
    ...
    fn jacobian_numerical_amplitude() {
        check_power_sat_param_central_diff(0);
    }

    // FD check for ∂y/∂rate
    #[test]
    fn jacobian_numerical_rate() {
        check_power_sat_param_central_diff(1);
    }
