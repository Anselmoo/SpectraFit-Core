...
/// the spectral-peak convention documented in `docs/reference/models/index.md`. For Bennett5 the
...
mod tests {
    ...
    // several x-points, replacing the old single-point forward differences
    // (h=1e-4..1e-7, max_relative=1e-4). Each test keeps its name
    // (traceability) but now checks only its own parameter index across the
    // full regime/x-point grid. Regimes and x-points keep u = offset + x
    // strictly positive (the domain-guard boundary).
    const POWER_LAW_OFFSET_REGIMES: [[f64; 3]; 3] = [
        [-2500.0, 46.7, 0.93], // nominal (near-certified Bennett5 fit)
        [1e-3, 1.0, 0.5],      // small amplitude, unit offset
        [50.0, 5.0, -2.0],     // positive amplitude, negative shape
    ];
    const POWER_LAW_OFFSET_XS: [f64; 5] = [1.0, 3.0, 8.0, 15.0, 40.0];

    fn check_power_law_offset_param_central_diff(idx: usize) {
        for p in POWER_LAW_OFFSET_REGIMES {
            for &x in &POWER_LAW_OFFSET_XS {
    ...
    fn jacobian_fd_amplitude() {
        check_power_law_offset_param_central_diff(0);
    }

    // FD check for ∂y/∂offset
    #[test]
    fn jacobian_fd_offset() {
        check_power_law_offset_param_central_diff(1);
    }

    // FD check for ∂y/∂shape
    #[test]
    fn jacobian_fd_shape() {
        check_power_law_offset_param_central_diff(2);
    }
