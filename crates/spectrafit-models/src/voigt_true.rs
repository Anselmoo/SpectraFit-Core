...
mod tests {
    ...
    fn jacobian_matches_central_difference_across_regimes() {
        ...
        // against.
        //
        // Exception (sanctioned in the V&V hardening plan): the Hui-Armstrong-
        // Wray rational approximation to the Faddeeva function is itself only
        // accurate to ~1e-6 (see the module doc comment on `faddeeva_complex`
        // and the "Accuracy note" on `jacobian`), and that accuracy degrades
        // further from the documented "|z| not too large" spectroscopy domain
        // outward. A standalone sweep over |x-centre|/sigma up to 3 found the
        // analytic/numeric gap for d/d(sigma) and d/d(gamma) crossing 1e-5 by
        // 2026-08-21T-era measurement past that point (e.g. ~1.1e-5 at
        // |x-c|=3*sigma), so this test is deliberately bounded to
        // |x-c| <= 2*sigma, where the worst measured agreement is ~9e-6 —
        // consistent with, not looser than, the ~1e-5 floor the plan
        // sanctions for this one kernel. This loosened tolerance is NOT
        // extended to cauchy_dispersion, doniach, or skewed_gaussian, which
        // are exact closed-form derivatives verified to epsilon = 1e-7.
        let m = TrueVoigt;
        let param_sets = [
            [4.0, 0.5, 1.0, 0.7],    // nominal
            [1e-3, 0.0, 0.05, 0.05], // small amplitude, width and Lorentzian HWHM
            [5.0, -2.0, 3.0, -1.2],  // offset centre, wide, negative gamma
        ];
        for p in param_sets {
            let (center, sigma) = (p[1], p[2]);
            // Offsets expressed in units of sigma so every regime samples the
            // same shape of the profile (near the peak, on the shoulder, and
            // toward but not past the ~2*sigma HAW-accuracy boundary above).
            for &mult in &[-2.0_f64, -1.0, -0.1, 0.0, 0.1, 1.0, 2.0] {
                let x = center + mult * sigma;
                ...
                for i in 0..p.len() {
                    ...
                    assert_relative_eq!(j[i], fd, epsilon = 1e-5, max_relative = 1e-5);
                }
            }
        }
    }
