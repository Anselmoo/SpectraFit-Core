...
impl Model for DoniachSunjic {
    ...
    fn jacobian(&self, x: &[f64], params: &[f64]) -> Vec<f64> {
        ...
        // ∂f/∂γ
        let dg =
            a / d * (-sin_phi * (std::f64::consts::FRAC_PI_2 - atan_u) + 0.5 * cos_phi * u2_1.ln());
...
mod tests {
    ...
    fn jacobian_matches_central_difference_across_regimes() {
        ...
        let m = DoniachSunjic;
        let param_sets = [
            [2.0, 0.0, 1.0, 0.3],   // nominal, moderate asymmetry
            [1e-3, 0.0, 0.05, 0.1], // small amplitude and width
            [5.0, -2.0, 3.0, 0.7],  // offset centre, wide, strong asymmetry
        ];
        for p in param_sets {
