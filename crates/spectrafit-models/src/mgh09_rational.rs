...
mod tests {
    ...
    fn eval_first_mgh09_point_approx() {
        let p = [
            1.9280693458e-01_f64,
            1.9128232873e-01,
            1.2305650693e-01,
            1.3606233068e-01,
        ];
        let y = model().eval(&[4.0], &p);
        assert!((y - 0.1957).abs() < 5e-3, "Expected ~0.1957, got {y}");
    }
    ...
    // (h=1e-5..1e-7, max_relative=1e-4..1e-5). Each test keeps its name
    // (traceability) but now checks only its own parameter index across the
    // full regime/x-point grid. Regimes and x-points keep
    // D = x² + den_lin·x + den_const away from 0 (the domain-guard boundary).
    const MGH09_REGIMES: [[f64; 4]; 3] = [
        [0.19, 0.19, 0.12, 0.14], // nominal (near-certified MGH09 fit)
        [1e-3, 1e-3, 1e-3, 1e-2], // small amplitude/coefficients
        [5.0, -2.0, 3.0, 8.0],    // offset/negative, wide
    ];
    const MGH09_XS: [f64; 5] = [0.1, 0.5, 1.5, 2.0, 4.0];

    fn check_mgh09_param_central_diff(idx: usize) {
        for p in MGH09_REGIMES {
            for &x in &MGH09_XS {
