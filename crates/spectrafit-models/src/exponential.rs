...
mod tests {
    ...
    // (h=1e-6, epsilon=1e-5). Each test keeps its name (traceability) but
    // now checks only its own parameter index across the full grid.
    const DOUBLE_EXP_REGIMES: [[f64; 4]; 3] = [
        [2.0, 0.5, 1.0, 2.0],     // nominal
        [1e-3, 1e-2, 1e-3, 1e-2], // small amplitudes/rates
        [5.0, -1.0, -3.0, 4.0],   // negative rates (growth), large amplitudes
    ];
    const DOUBLE_EXP_XS: [f64; 5] = [-2.0, -0.5, 0.0, 1.0, 3.0];

    fn check_double_exp_param_central_diff(idx: usize) {
        for p in DOUBLE_EXP_REGIMES {
            for &x in &DOUBLE_EXP_XS {
    ...
    fn jacobian_numerical_a1() {
        check_double_exp_param_central_diff(0);
    }

    #[test]
    fn jacobian_numerical_lam1() {
        check_double_exp_param_central_diff(1);
    }

    #[test]
    fn jacobian_numerical_a2() {
        check_double_exp_param_central_diff(2);
    }

    #[test]
    fn jacobian_numerical_lam2() {
        check_double_exp_param_central_diff(3);
    }
