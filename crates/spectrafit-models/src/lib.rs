...
pub mod emg;
/// Exponential over a line (NIST StRD Chwirut1, Chwirut2):
/// `exp(−rate·x) / (den_const + den_lin·x)`.
pub mod exp_over_linear;
...
/// Rational cubic over cubic, denominator constant pinned at 1 (NIST StRD
/// Kirby2, Hahn1, Thurber): `(a0+a1x+a2x²+a3x³) / (1+b1x+b2x²+b3x³)`.
...
/// Dimension-aware model construction.
///
...
pub fn model_from_str(model_type: &str) -> Option<Box<dyn Model>> {
    match model_type {
        ...
        // from the node's explicit `n_dims`.
        ...
        "rational_cubic" => Some(Box::new(crate::rational_cubic::RationalCubic)),
        "generalised_logistic" => Some(Box::new(crate::generalised_logistic::GeneralisedLogistic)),
        "exp_over_linear" => Some(Box::new(crate::exp_over_linear::ExpOverLinear)),
        _ => None,
    }
}
