...
pub fn model_from_str_with_dims(
    model_type: &str,
    n_dims: Option<usize>,
) -> Option<Box<dyn Model>> {
    match model_type {
        "gaussian_nd" => n_dims.map(|d| {
            Box::new(crate::gaussian_nd::GaussianND::new(d)) as Box<dyn Model>
        }),
...
pub fn model_from_str(model_type: &str) -> Option<Box<dyn Model>> {
    match model_type {
        ...
        "saturating_exponential" => Some(Box::new(crate::saturating_exponential::SaturatingExponential)),
