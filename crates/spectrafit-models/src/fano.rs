...
mod tests {
    ...
    fn jacobian_finite_diff_check() {
        // Upgraded from a single-point forward difference (h=1e-5,
        // epsilon=1e-4, O(h) truncation error the size of its own tolerance)
        // to a central difference (O(h^2)) swept over three parameter
        // regimes and several x-points relative to the resonance centre.
        let param_sets = [
            [2.0, 0.2, 0.8, 1.5],   // nominal
            [1e-3, 0.0, 0.05, 0.5], // small amplitude/width
            [5.0, -2.0, 3.0, 10.0], // offset centre, wide, large asymmetry
        ];
        for p in param_sets {
            let (center, gamma) = (p[1], p[2]);
            for &mult in &[-3.0_f64, -1.0, -0.1, 0.0, 0.1, 1.0, 3.0] {
                let x = [center + mult * gamma];
                ...
                for i in 0..p.len() {
                    ...
                    a[i] += h;
                    b[i] -= h;
                    let fd = (Fano.eval(&x, &a) - Fano.eval(&x, &b)) / (2.0 * h);
                    assert_relative_eq!(j_anal[i], fd, epsilon = 1e-7, max_relative = 1e-6);
                }
            }
        }
    }
    ...
    // ----- Limiting-case asymptotic (ground-truth verification) -----
    //
