...
mod tests {
    ...
    fn jacobian_into_matches_finite_difference() {
        ...
        // parameter regimes and several (x, y) offsets from each regime's
        // own centre.
        let m = Gaussian2D;
        let param_sets = [
            [2.0, 0.5, -1.0, 1.0, 1.5],   // nominal
            [1e-3, 0.0, 0.0, 0.05, 0.05], // small amplitude/width
            [5.0, -2.0, 3.0, 2.5, 1.5],   // offset centre, wide
        ];
        for params in param_sets {
            let (cx, cy) = (params[1], params[2]);
            for &(ox, oy) in &[
                (0.0_f64, 0.0),
                (0.7, -0.3),
                (-2.5, 2.0),
