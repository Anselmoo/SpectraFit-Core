...
mod tests {
    ...
    fn arctan_jacobian_finite_diff_check() {
        ...
        // parameter regimes and several x-points relative to the centre.
        let param_sets = [
            [2.0, 0.5, 0.8],   // nominal
        ...
        for p in param_sets {
            ...
            for &mult in &[-3.0_f64, -1.0, -0.1, 0.0, 0.1, 1.0, 3.0] {
                ...
                for i in 0..p.len() {
                    ...
                    a[i] += h;
                    b[i] -= h;
                    let fd = (ArctanStep.eval(&x, &a) - ArctanStep.eval(&x, &b)) / (2.0 * h);
                    assert_relative_eq!(j_analytical[i], fd, epsilon = 1e-7, max_relative = 1e-6);
                }
            }
        }
    }
        ...
        for p in param_sets {
            ...
            for &mult in &[-3.0_f64, -1.0, -0.1, 0.0, 0.1, 1.0, 3.0] {
                ...
                for i in 0..p.len() {
                    ...
                    a[i] += h;
                    b[i] -= h;
                    let fd = (TanhStep.eval(&x, &a) - TanhStep.eval(&x, &b)) / (2.0 * h);
                    assert_relative_eq!(j_analytical[i], fd, epsilon = 1e-7, max_relative = 1e-6);
                }
            }
        }
    ...
    fn erfc_jacobian_finite_diff_check() {
        // Upgraded from a single-point forward difference (h=1e-5,
        // epsilon=1e-5) to a central difference (O(h^2)) swept over three
        // parameter regimes and several x-points relative to the centre.
        let param_sets = [
            [1.0, 0.0, 1.0],   // nominal
            [1e-3, 0.0, 0.05], // small amplitude/width
        ...
        for p in param_sets {
            ...
            for &mult in &[-3.0_f64, -1.0, -0.1, 0.0, 0.1, 1.0, 3.0] {
                ...
                for i in 0..p.len() {
                    ...
                    a[i] += h;
                    b[i] -= h;
                    let fd = (ErfcStep.eval(&x, &a) - ErfcStep.eval(&x, &b)) / (2.0 * h);
                    assert_relative_eq!(j_analytical[i], fd, epsilon = 1e-7, max_relative = 1e-6);
                }
            }
        }
