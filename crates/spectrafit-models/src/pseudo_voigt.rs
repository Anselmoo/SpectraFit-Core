...
mod tests {
    ...
    fn jacobian_finite_diff_check() {
        ...
        // keeping the mixing fraction inside [0,1] (the clamp boundary is
        // covered separately by `fraction_clamped_to_unit_interval`).
        let param_sets = [
            [2.0, 0.3, 0.9, 0.4],   // nominal
        ...
        for p in param_sets {
            ...
            for &mult in &[-3.0_f64, -1.0, -0.1, 0.0, 0.1, 1.0, 3.0] {
                ...
                for i in 0..p.len() {
                    ...
                    a[i] += h;
                    b[i] -= h;
                    let fd = (PseudoVoigt.eval(&x, &a) - PseudoVoigt.eval(&x, &b)) / (2.0 * h);
