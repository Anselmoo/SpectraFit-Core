...
/// peak-model convention documented in `docs/reference/models/index.md`.
...
mod tests {
    ...
    // several x-points, replacing the old single-point forward difference
    ...
    const SAT_EXP_REGIMES: [[f64; 2]; 3] = [
        [3.0, 0.5],   // nominal
        [1e-3, 1e-3], // small amplitude/rate
        [50.0, 2.0],  // large amplitude, fast rate
    ];
    const SAT_EXP_XS: [f64; 5] = [0.1, 0.5, 1.5, 4.0, 10.0];

    fn check_sat_exp_param_central_diff(idx: usize) {
        for p in SAT_EXP_REGIMES {
            for &x in &SAT_EXP_XS {
    ...
    fn jacobian_numerical_amplitude() {
        check_sat_exp_param_central_diff(0);
    }

    #[test]
    fn jacobian_numerical_rate() {
        check_sat_exp_param_central_diff(1);
    }
