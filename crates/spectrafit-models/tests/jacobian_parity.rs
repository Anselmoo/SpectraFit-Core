...
//!
//! Wires individual models to `assert_jacobian_parity`.
//!
...
//! ## Coverage scope
//!
//! Wires all 29 model kernels registered in `model_from_str`. An earlier pass
...
//! This follow-up adds the 13 STEP and SPECIAL-SHAPE kernels:
...
//! those four remains a follow-up, not yet done.
...
fn jacobian_parity_gaussian() {
    let model = spectrafit_models::model_from_str("gaussian").expect("gaussian model exists");
...
fn jacobian_parity_lorentzian() {
    let model = spectrafit_models::model_from_str("lorentzian").expect("lorentzian model exists");
    // Parameters: [amplitude, center, sigma]
    let params = vec![1.5, 0.0, 0.8];
    // Sweep x from -2.0 to 2.0 in 0.2 steps
    let xs: Vec<f64> = (0..21).map(|i| -2.0 + (i as f64) * 0.2).collect();
...
fn jacobian_parity_voigt() {
    let model = spectrafit_models::model_from_str("voigt").expect("voigt model exists");
...
fn jacobian_parity_pseudo_voigt() {
    let model =
        spectrafit_models::model_from_str("pseudo_voigt").expect("pseudo_voigt model exists");
    // Parameters: [amplitude, center, sigma, fraction]
    let params = vec![1.5, 0.0, 0.8, 0.5];
    // Sweep x from -2.0 to 2.0 in 0.2 steps
    let xs: Vec<f64> = (0..21).map(|i| -2.0 + (i as f64) * 0.2).collect();
...
fn jacobian_parity_fano() {
    let model = spectrafit_models::model_from_str("fano").expect("fano model exists");
...
fn jacobian_parity_moffat() {
    let model = spectrafit_models::model_from_str("moffat").expect("moffat model exists");
...
fn jacobian_parity_students_t() {
    let model = spectrafit_models::model_from_str("students_t").expect("students_t model exists");
...
fn jacobian_parity_breit_wigner() {
    let model =
        spectrafit_models::model_from_str("breit_wigner").expect("breit_wigner model exists");
...
fn jacobian_parity_asym_ir() {
    let model = spectrafit_models::model_from_str("asym_ir").expect("asym_ir model exists");
...
fn jacobian_parity_kww() {
    let model = spectrafit_models::model_from_str("kww").expect("kww model exists");
...
fn jacobian_parity_log_normal() {
    let model = spectrafit_models::model_from_str("log_normal").expect("log_normal model exists");
    // Parameters: [amplitude, center, sigma]
...
fn jacobian_parity_tauc() {
    let model = spectrafit_models::model_from_str("tauc").expect("tauc model exists");
...
fn jacobian_parity_polynomial_quadratic() {
    let model = spectrafit_models::model_from_str("quadratic").expect("quadratic model exists");
...
fn jacobian_parity_gaussian2d() {
    let model = spectrafit_models::model_from_str("gaussian2d").expect("gaussian2d model exists");
...
fn jacobian_parity_constant() {
    let model = spectrafit_models::model_from_str("constant").expect("constant model exists");
    // Parameters: [c]
    let params = vec![3.41];
...
fn jacobian_parity_linear() {
    let model = spectrafit_models::model_from_str("linear").expect("linear model exists");
...
fn jacobian_parity_arctan_step() {
    let model = spectrafit_models::model_from_str("arctan_step").expect("arctan_step model exists");
    // Parameters: [amplitude, center, sigma]
...
fn jacobian_parity_tanh_step() {
    let model = spectrafit_models::model_from_str("tanh_step").expect("tanh_step model exists");
    // Parameters: [amplitude, center, sigma]
...
fn jacobian_parity_erfc_step() {
    let model = spectrafit_models::model_from_str("erfc_step").expect("erfc_step model exists");
    // Parameters: [amplitude, center, sigma]
...
fn jacobian_parity_true_voigt() {
    let model = spectrafit_models::model_from_str("true_voigt").expect("true_voigt model exists");
    ...
    // analytical Jacobian remains a follow-up, not yet done. Tolerance reflects FD scheme mismatch.
...
fn jacobian_parity_skewed_gaussian() {
    let model =
        spectrafit_models::model_from_str("skewed_gaussian").expect("skewed_gaussian model exists");
    // Parameters: [amplitude, center, sigma, gamma]
...
fn jacobian_parity_exp_gaussian() {
    let model =
        spectrafit_models::model_from_str("exp_gaussian").expect("exp_gaussian model exists");
    // Parameters: [amplitude, center, sigma, gamma]
...
fn jacobian_parity_doniach_sunjic() {
    let model =
        spectrafit_models::model_from_str("doniach_sunjic").expect("doniach_sunjic model exists");
    // Parameters: [amplitude, center, sigma, gamma]
...
fn jacobian_parity_pearson7() {
    let model = spectrafit_models::model_from_str("pearson7").expect("pearson7 model exists");
...
fn jacobian_parity_split_gaussian() {
    let model =
        spectrafit_models::model_from_str("split_gaussian").expect("split_gaussian model exists");
...
fn jacobian_parity_split_pearson7() {
    let model =
        spectrafit_models::model_from_str("split_pearson7").expect("split_pearson7 model exists");
...
fn jacobian_parity_harmonic_ir() {
    let model = spectrafit_models::model_from_str("harmonic_ir").expect("harmonic_ir model exists");
    // Parameters: [amplitude, center, sigma]
