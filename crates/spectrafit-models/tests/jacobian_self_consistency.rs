...
fn recipe(model_type: &str) -> Option<Recipe> {
    let r = match model_type {
        ...
        "mgh09_rational" => Recipe {
            ...
            params: vec![1.9280693458e-01, 1.9128232873e-01, 1.2305650693e-01, 1.3606233068e-01],
...
fn analytic_jacobian_matches_central_difference_for_every_model() {
    ...
    for &model_type in all_model_types() {
        let model = model_from_str(model_type)
            .unwrap_or_else(|| panic!("all_model_types() lists {model_type:?} but model_from_str rejects it"));
...
fn registry_is_non_empty() {
    assert!(all_model_types().len() >= 30, "registry shrank unexpectedly");
}
