...
fn recipe(model_type: &str) -> Option<Recipe> {
    let r = match model_type {
        ...
        "rational_cubic" => Recipe {
            // [a0, a1, a2, a3, b1, b2, b3] on x in [-4, 8], where D stays in
            // [0.97, 3.19].
            //
            // Deliberately NOT Kirby2's certified values on Kirby2's x range. That
            // pins a3 = b3 = 0 (Kirby2 is quadratic/quadratic) over x up to 371,
            // where x^3 ~ 5e7 — so d(y)/d(b3) is ~1e9 and no finite step resolves
            // it: the central difference measures curvature instead of slope and
            // the check fails on a correct Jacobian. This test's job is the
            // derivative algebra; the NIST conditioning is covered by the
            // certified-value validation in oracles.audit.nist.
            params: vec![1.2, 0.3, 0.15, 0.02, 0.05, 0.02, 0.001],
            points: sweep(-4.0, 0.6, 21),
            kind: JacKind::Analytic,
        },

        "generalised_logistic" => Recipe {
            // [amplitude, shift, rate, shape] on x in [-4, 8] (sweep takes a
            // STEP, so 0.6 x 20), where 1 + exp(shift − rate·x) runs 1.14 to 17.4.
            //
            // Deliberately NOT Rat43's certified values on Rat43's x range. A
            // logistic saturates, so out on its plateau d(y)/d(rate) falls to
            // ~3e-9 while the central difference underflows to exactly 0 — a
            // relative error of 1.0 on a correct derivative. This test checks the
            // derivative algebra; Rat43's own conditioning is covered by the
            // certified-value validation in oracles.audit.nist.
            params: vec![2.5, 1.2, 0.4, 1.4],
            points: sweep(-4.0, 0.6, 21),
            kind: JacKind::Analytic,
        },

        "exp_over_linear" => Recipe {
            // [rate, lin_const, lin_slope]; Chwirut2's certified values on its own
            // x range 0.5 to 6 (sweep takes a STEP, so 0.275 x 20), where the
            // denominator runs 0.011 to 0.078 — small but
            // strictly positive, and never near the root at x = -0.425.
            params: vec![1.6657666537e-01, 5.1653291286e-03, 1.2150007096e-02],
            points: sweep(0.5, 0.275, 21),
            kind: JacKind::Analytic,
        },
