...
pub fn assemble_result(
    ...
    let (mut r_squared, mut reduced_chi2, mut aic, mut bic) =
    ...
    let (cov_opt, condition_number): (Option<faer::Mat<f64>>, Option<f64>) = if n_free == 0 {
        (None, None)
    } else if result_problem.has_tied() {
        ...
        (cov, faer_cond_from_gram(gram_scaled.as_ref()))
    } else {
        ...
        if sigma_provided {
            ...
            (cov, faer_condition_number(j_scaled.as_ref()))
        }
    };
    ...
    let (mut success_val, message_val) = apply_postfit_guards(
        graph,
        free_keys,
        &final_flat,
        x_all,
...
mod tests {
    ...
    fn derived_diagnostics_are_finite_or_sentinel_not_nan() {
        ...
        let mut aicg = aic;
        let mut bicg = bic;
        if !r2g.is_finite() { r2g = 0.0; success = false; }
        if !rchi2g.is_finite() { rchi2g = 0.0; success = false; }
        if !aicg.is_finite() { aicg = 0.0; success = false; }
        if !bicg.is_finite() { bicg = 0.0; success = false; }
        assert!(r2g.is_finite(),   "guarded r_squared must be finite (n_total=0), got {r2g}");
        assert!(rchi2g.is_finite(),"guarded reduced_chi2 must be finite (n_total=0), got {rchi2g}");
        assert!(aicg.is_finite(),  "guarded aic must be finite (n_total=0), got {aicg}");
        assert!(bicg.is_finite(),  "guarded bic must be finite (n_total=0), got {bicg}");
        ...
        if !aicg.is_finite() { aicg = 0.0; success = false; }
        if !bicg.is_finite() { bicg = 0.0; success = false; }
        assert!(r2g.is_finite(),    "guarded r_squared must be finite (chi2=inf), got {r2g}");
        assert!(rchi2g.is_finite(), "guarded reduced_chi2 must be finite (chi2=inf), got {rchi2g}");
        assert!(aicg.is_finite(),   "guarded aic must be finite (chi2=inf), got {aicg}");
        assert!(bicg.is_finite(),   "guarded bic must be finite (chi2=inf), got {bicg}");
        assert!(!success,           "degenerate chi2=inf must flip success to false");

        // ── case 3: chi2 = inf AND ss_tot = inf → r_squared = NaN ───────────
        let (r2, rchi2, aic, bic) = compute_scalar_diagnostics(f64::INFINITY, 10, 2, 8, f64::INFINITY);
        assert!(
            [r2, rchi2, aic, bic].iter().any(|v| !v.is_finite()),
        ...
        for (name, &v) in ["r_squared", "reduced_chi2", "aic", "bic"].iter().zip(guarded.iter()) {
            assert!(v.is_finite(), "guarded {name} must be finite (case 3), got {v}");
        }
        ...
        assert!(rchi2.is_finite(), "normal reduced_chi2 must be finite, got {rchi2}");
    ...
    fn chi2_finite_guard_closes_ef_rust_02_gap() {
        ...
        assert!(!raw_chi2.is_finite(), "test precondition: chi2 must start non-finite");
        ...
        assert!(
            !success_val,
            "non-finite chi2 must flip success to false"
        );
        ...
        assert!(chi2_nan.is_finite(), "guarded NaN chi2 must be finite, got {chi2_nan}");
        ...
        assert!((chi2_ok - 2.5).abs() < 1e-12, "finite chi2 must be unchanged by guard");
