...
//!
//! [`SolverError`] is defined as the typed error shape for boundary-crossing
//! solver failures. An audit found that all severity-9 `expect()` /
//! `unwrap()` sites in spectrafit-solver were in test code; the single
//! production site (the `expect("INVARIANT: both values are finite")` in
//! `global.rs`'s `select_best_individual`) is an INVARIANT-guarded call.
//! SolverError variants are available for new boundary-facing code;
//! widespread conversion of existing `CoreError` returns remains a
...
pub enum SolverError {
    ...
    #[error(
        "unrecognised solver '{0}' — expected one of: \"lm\", \"lm-legacy\", \"trf\", \
         \"geodesic\", \"dogleg\", \"newton-cg\", \"varpro\", \"auto\", \"global\", \
         \"irls\" (optionally \"irls:huber\"/\"irls:bisquare\"/\"irls:cauchy\")"
    )]
    ...
    #[error("unrecognised irls weight function '{0}' — expected one of: \"huber\", \"bisquare\" (or \"biweight\"), \"cauchy\"")]
    ...
    #[error("eta must be finite and in [0.0, 0.25); got {0}")]
    InvalidEta(f64),
}
...
impl From<SolverError> for CoreError {
    fn from(value: SolverError) -> Self {
        match value {
            ...
            | SolverError::TiedTargetParamMissing(_)
            | SolverError::UnrecognisedSolver(_)
            | SolverError::UnrecognisedWeightFn(_)
            | SolverError::InvalidEta(_) => CoreError::Eval(value.to_string()),
        }
    }
}
...
mod tests {
    ...
    #[test]
    fn unrecognised_solver_maps_to_eval() {
        let e: CoreError = SolverError::UnrecognisedSolver("lmm".into()).into();
        match e {
            CoreError::Eval(msg) => {
    ...
    #[test]
    fn unrecognised_weight_fn_maps_to_eval() {
        let e: CoreError = SolverError::UnrecognisedWeightFn("buisquare".into()).into();
        match e {
            CoreError::Eval(msg) => {
                assert!(msg.contains("buisquare"));
                assert!(msg.contains("expected one of"));
            }
            other => panic!("expected CoreError::Eval, got {other:?}"),
        }
    ...
    #[test]
    fn invalid_eta_maps_to_eval() {
        let e: CoreError = SolverError::InvalidEta(0.5).into();
        match e {
            CoreError::Eval(msg) => {
