...
/// - `CompiledGraph::compile(graph)` fails (malformed graph).
/// - [`compute_de_bounds`] fails to resolve a free key to its node/parameter
///   (a free key that does not match `"node_id.param_name"`, or names a node
///   that is not in the graph).
/// - The post-DE LM refinement fit ([`crate::dispatch::fit`], forced to
///   `solver = "lm"`) fails.
///
/// Note: a candidate whose model evaluation errors during the DE loop itself
/// is not an error path — its cost is scored as `f64::INFINITY` so the
/// population selection routes around it.
pub fn solve_global(
    graph: &FitGraphSpec,
    ...
    // ── 2. Build combined x/y for cost evaluation + data-aware bounds ──────
    let (x_all, y_all, bounds) = compute_de_bounds(graph, &free_keys, &datasets)?;
    ...
    let mut population = init_population(&mut rng, &bounds, pop_size);
    let mut costs: Vec<f64> = population.iter().map(|p| cost_fn(p)).collect();

    // ── 4. DE main loop ──────────────────────────────────────────────────────
    let gens_run = run_de_evolution(
        &mut rng,
        &mut population,
        &mut costs,
        &bounds,
    ...
    let best_idx = select_best_individual(&costs)?;
    let best_params = &population[best_idx];

    // ── 6. Seed LM from best DE solution ────────────────────────────────────
    let refined_graph = seed_refined_graph(graph, &free_keys, best_params, &bounds);
...
/// `(x_all, y_all, bounds)` — the combined cost-evaluation arrays plus the
/// per-free-parameter DE search bounds, as returned by [`compute_de_bounds`].
type DeBoundsData = (Vec<f64>, Vec<f64>, Vec<(f64, f64)>);

/// Build the combined `x`/`y` arrays used for cost evaluation, and the
/// per-free-parameter DE search bounds.
///
/// Computing `x_all`/`y_all` here (rather than in `solve_global` directly) is
/// what lets unbounded parameters fall back to *data-aware* ranges rather
/// than a fixed ±10 window around the seed. Point-major (stride = n_dims) so
/// `evaluate_compiled` reads each point's full coordinate; 1-D reduces to the
/// old per-dataset `ds.x[0]` concatenation. Using only `ds.x[0]` broke any
/// n-D model (e.g. gaussian2d) under the global/DE path.
fn compute_de_bounds(
    graph: &FitGraphSpec,
    free_keys: &[String],
    datasets: &[MeasurementSpec],
) -> Result<DeBoundsData, SolverError> {
    ...
    Ok((x_all, y_all, bounds))
}

/// Uniformly sample `pop_size` candidate vectors within `bounds`.
///
/// RNG-order-sensitive: draws exactly `pop_size * bounds.len()` values from
/// `rng`, in row-major (candidate, then dimension) order — the same order the
/// DE main loop and its tests assume when reproducing a trajectory from a
/// fixed seed.
fn init_population(rng: &mut Lcg64, bounds: &[(f64, f64)], pop_size: usize) -> Vec<Vec<f64>> {
    (0..pop_size)
        .map(|_| {
            bounds
                .iter()
...
/// Run the DE/rand/1/bin main loop in place over `population`/`costs`.
///
/// RNG-order-sensitive: this is the verbatim body of the former inline main
/// loop, moved as-is. Each generation draws from `rng` via `pick3` (mutation)
/// then the crossover draws (`j_rand` plus one conditional draw per
/// dimension, short-circuited exactly as before) — reordering any of this
/// relative to `init_population`'s draws would change the entire population
/// trajectory even though the per-step logic is unchanged.
///
...
fn run_de_evolution(
    ...
    for gen in 0..config.max_gen {
        ...
        for i in 0..pop_size {
            ...
            let (a, b, c) = pick3(rng, pop_size, i);
...
/// Index of the best (lowest-cost) finite candidate in `costs`.
///
/// INVARIANT: the iterator is pre-filtered by `.filter(|(_, c)| c.is_finite())`, so
...
fn select_best_individual(costs: &[f64]) -> Result<usize, SolverError> {
    costs
        .iter()
        .enumerate()
...
/// Clone `graph`, seeding each free parameter from `best_params` and widening
/// any still-unbounded min/max to the data-aware `bounds` DE searched.
///
/// Constrains LM refinement to the same data-aware range DE explored.
/// Otherwise an originally-unbounded parameter can diverge off-domain during
/// refinement (e.g. a Gaussian centre shooting to 1e216), undoing the global
/// search. User-supplied finite bounds are left untouched.
fn seed_refined_graph(
    graph: &FitGraphSpec,
    free_keys: &[String],
    best_params: &[f64],
    bounds: &[(f64, f64)],
