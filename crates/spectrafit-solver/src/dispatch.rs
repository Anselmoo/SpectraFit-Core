...
fn dispatch_solver(
    ...
    match solver {
        ...
        Solver::Varpro => {
            ...
            let result = match solve_varpro_path(graph, &datasets, options) {
                Ok(r) => r,
                Err(e) => return ControlFlow::Break(Err(e)),
            };
            ControlFlow::Break(Ok(finalize_varpro_result(graph, &datasets, result)))
        }
        Solver::Auto if graph_prefers_varpro(graph) => {
            ControlFlow::Break(solve_varpro_path(graph, &datasets, options))
        }
