...
use spectrafit_graph::{evaluate_compiled, CompiledGraph};
...
use crate::irls::{solve_irls, WeightFn, DEFAULT_MAX_OUTER_ITER, DEFAULT_TOL_WEIGHTS};
use crate::lm_problem::LmProblem;
use crate::postfit::{self, LmSolveOutcome};
...
impl Solver {
    /// Parse the `FitOptionsSpec.solver` string.
    ///
    /// Unrecognised strings are rejected rather than silently defaulting to
    /// LM — previously a typo (e.g. `"lmm"`, wrong-case `"Trf"`) silently ran
    /// the LM solver instead of erroring, which could mask a misconfigured
    /// fit with a plausible-looking but unintended result.
    fn parse(s: &str) -> Result<Self, SolverError> {
        match s {
            "lm" => Ok(Solver::Lm),
            "lm-legacy" => Ok(Solver::LmLegacy),
            "trf" => Ok(Solver::Trf),
            "geodesic" | "lm-geodesic" => Ok(Solver::Geodesic),
            "dogleg" => Ok(Solver::Dogleg),
            "newton-cg" | "newton_cg" | "newtoncg" | "steihaug" => Ok(Solver::NewtonCg),
            "global" => Ok(Solver::Global),
            "varpro" => Ok(Solver::Varpro),
            "auto" => Ok(Solver::Auto),
            _ if s.starts_with("irls") => {
                let name = s.split_once(':').map(|(_, name)| name).unwrap_or("huber");
                Ok(Solver::Irls(WeightFn::from_str(name)?))
            }
            other => Err(SolverError::UnrecognisedSolver(other.to_string())),
        }
    }
}
...
/// Run a fit of `graph` against `datasets` using the solver named by
/// `options.solver` (see `Solver::parse` for the accepted strings: `lm`,
/// `lm-legacy`, `trf`, `geodesic`, `dogleg`, `newton-cg`, `irls[:weight]`,
/// `global`, `varpro`, `auto`).
///
/// The LM family (`lm`, `lm-legacy`, `trf`, `geodesic`) plus the
/// trust-region methods (`dogleg`, `newton-cg`) and `auto`'s non-VarPro
/// fallback share one pre-solve/solve/post-fit path through this function.
/// `irls`, `global`, and `varpro` are routed to their own modules/crates in
/// `dispatch_solver` and return before that shared path runs.
///
...
/// - `options.solver` does not parse (unrecognised solver name, or an
///   unrecognised `irls:<weight>` weight-function name) — `Solver::parse`.
/// - The routed non-LM-family solver fails: `irls` (`crate::irls::solve_irls`),
///   `global` (differential evolution), or `varpro` (not separable, tied
///   parameters present, or per-dataset scoping unsupported).
/// - Graph compilation or the initial model evaluation fails (malformed
///   graph, a free-key/parameter lookup miss, or a model evaluation error) —
///   `prepare_lm_pre_solve`.
/// - The LM-family solve itself fails or produces a numerically unusable
///   result (e.g. ill-conditioned Jacobian) that maps to `CoreError::Solver`.
/// - `options.eta` is set alongside `solver="dogleg"` / `"newton-cg"` but is
///   not finite and in `[0.0, 0.25)` — `SolverError::InvalidEta`.
/// - Post-fit assembly fails while re-evaluating the model or its Jacobian
///   at the solution — `postfit::assemble_result`.
pub fn fit(
    graph: &FitGraphSpec,
    ...
    // so faer's default Rayon dispatch costs more in thread-pool wake-up
    // (~30–50 µs per matmul) than the arithmetic itself — measured ~4–6×
    // slowdown overall, ~4.7× on a 50k×3 fit, including the n×p post-fit
    // Gram/SVD. The data-parallelism that pays off lives one level down in the
    // graph executor's own Rayon path (over points), not in these
    // per-iteration p×p factorizations.
    //
    // THIS IS THE ONLY SITE that sets faer's global parallelism. The LM and
    // trust-region drivers each used to set it again on entry to their own
    // `minimize`/`minimize_tr`; setting one process-global three times from
    // three crates made the effective policy depend on call order and left the
    // drivers quietly mutating global state their doc comments never mentioned.
    // Ownership now sits here, at the one place that already owns the whole
    // fit — faer is solver-internal in this workspace, so a single global
    // policy set at the fit boundary is safe.
    ...
    let datasets = match dispatch_solver(solver, graph, datasets, options) {
        ControlFlow::Break(result) => return result,
        ControlFlow::Continue(datasets) => datasets,
    };

    // ── 1/2/3/6/7. Compile the graph, collect free-param specs, evaluate the
    // initial model, and build the index-based node-param buffers ───────────
    let LmPreSolve {
        cg,
        free_keys,
        all_params,
        ...
        free_to_node_param,
        tied_to_node_param,
    } = prepare_lm_pre_solve(graph, &datasets)?;
    ...
    // ── 4/8. Initialise tied parameters and run the LM-family solve ──────────
    let outcome = run_lm_solve(problem, solver, options, free_keys.len(), y_all.len())?;

    // ── 5/6/7. Post-fit statistics, covariance, and result assembly ──────────
    postfit::assemble_result(
        outcome,
        postfit::PostfitInputs {
            cg: &cg,
            graph,
            datasets: &datasets,
            x_all: &x_all,
...
/// Route `solver` to its own module/crate when it is not part of the LM
/// family, otherwise hand `datasets` back unchanged. Step 0 of [`fit`]'s
/// sequence, extracted verbatim from the original inline `match`.
///
/// Returns [`ControlFlow::Break`] carrying the already-final
/// `Result<FitResultSpec, CoreError>` for `Irls` / `Global` / `Varpro` / an
/// `Auto` that prefers VarPro — `fit` returns this immediately. Returns
/// [`ControlFlow::Continue`] carrying `datasets` handed back unchanged for the
/// LM family (`Lm` / `LmLegacy` / `Trf` / `Geodesic` / `Dogleg` / `NewtonCg`,
/// and an `Auto` that does not prefer VarPro) — `fit` falls through to the
/// shared pre-solve + solve path for these.
fn dispatch_solver(
    solver: Solver,
    graph: &FitGraphSpec,
    datasets: Vec<MeasurementSpec>,
    options: &FitOptionsSpec,
) -> ControlFlow<Result<FitResultSpec, CoreError>, Vec<MeasurementSpec>> {
    match solver {
        Solver::Irls(weight) => ControlFlow::Break(solve_irls(
            graph,
            datasets,
            options,
            weight,
            DEFAULT_MAX_OUTER_ITER,
            // The IRLS weight-convergence tolerance is its own constant, not
            // `options.tolerance` (the LM cost/step/gradient tolerance) — see
            // `DEFAULT_TOL_WEIGHTS`.
            DEFAULT_TOL_WEIGHTS,
        )),
        ...
        Solver::Global => {
            ...
            ControlFlow::Break(solve_global(graph, datasets, options, config))
        }
        Solver::Varpro => {
            ...
            if !spectrafit_varpro::is_separable(graph) {
                return ControlFlow::Break(Err(SolverError::VarproNotSeparable.into()));
            }
            if graph_has_tied_params(graph) {
                return ControlFlow::Break(Err(SolverError::VarproExprEdgesUnsupported.into()));
            }
            ...
            if graph.nodes.iter().any(|n| n.dataset_index.is_some()) {
                return ControlFlow::Break(
                    Err(SolverError::VarproDatasetScopingUnsupported.into()),
                );
            }
            // Falls through to the shared VarPro tail below, which applies the
            // post-fit success guards. Do not call `solve_varpro_path` directly
            // here — see the note on that tail.
            ControlFlow::Break(run_varpro_guarded(graph, &datasets, options))
        }
        Solver::Auto if graph_prefers_varpro(graph) => {
            // MUST use the same guarded tail as the explicit `Solver::Varpro`
            // arm above. This arm previously called `solve_varpro_path` raw,
            // skipping `finalize_varpro_result` — so on a graph where automatic
            // routing chooses VarPro, `solver="auto"` reported success on
            // exactly the degenerate fits `solver="varpro"` reported as failed,
            // on identical data. `auto` is the documented default, which made
            // it a silent wrong answer on the recommended path.
            ControlFlow::Break(run_varpro_guarded(graph, &datasets, options))
        }
        ...
        | Solver::Dogleg
        | Solver::NewtonCg
        | Solver::Auto => ControlFlow::Continue(datasets),
    }
}

/// Bundle of everything the shared LM-family pre-solve step produces (steps
/// 1/2/3/6/7 of [`fit`]'s sequence: compile the graph, collect free-parameter
/// specs, evaluate the initial model, and build the index-based node-param
/// buffers). [`fit`] destructures this into the same flat local bindings the
/// original inline code used, so field names mirror the original variable
/// names exactly.
struct LmPreSolve {
    /// Compiled DAG (step 1), including `dataset_offsets` for simultaneous
    ...
    bounds: Vec<(f64, f64)>,
    /// Per-free-parameter `Parameter.scale` factors (identity when unset).
    scales: Vec<f64>,
    /// Per-point σ (default 1.0 where `MeasurementSpec.sigma` is `None`).
    sigma: Vec<f64>,
    /// Point-major concatenated x across all datasets.
    x_all: Vec<f64>,
    /// Concatenated y across all datasets.
    y_all: Vec<f64>,
    /// The model evaluated at the initial parameters, before optimisation.
    init_fit: Vec<f64>,
    /// Initial free-parameter vector in the optimiser's scaled working units.
    init_params: DVector<f64>,
    /// Per-node parameter buffers (fixed pre-filled, free updated in-place).
    node_param_bufs: Vec<Vec<f64>>,
    /// free-param index → `(node_idx, param_pos)`.
    free_to_node_param: Vec<(usize, usize)>,
    /// tied-target index → `(node_idx, param_pos)`.
    tied_to_node_param: Vec<(usize, usize)>,
}

/// Compile `graph`, collect free-parameter specs, evaluate the initial model,
/// and build the index-based node-param buffers — steps 1/2/3/6/7 of [`fit`]'s
/// sequence, extracted verbatim from the original inline code (including the
/// `Parameter.scale` wiring doc comment and its `debug_assert_eq!`).
fn prepare_lm_pre_solve(
    graph: &FitGraphSpec,
    datasets: &[MeasurementSpec],
) -> Result<LmPreSolve, CoreError> {
    // ── 1. Compile graph ────────────────────────────────────────────────────
    ...
    // ── 2/3. All-param flat map + initial values/bounds/scales for free params ─
    let (all_params, init_vals, bounds, scales) = collect_free_param_specs(graph, &free_keys)?;
    ...
    let (node_param_bufs, free_to_node_param, tied_to_node_param) =
        build_node_param_buffers(&cg, &all_params, &bounds, &init_vals_clone)?;
...
/// Initialise tied parameters from the starting free values, then dispatch
/// `solver` to the matching LM-family engine — extracted verbatim from the
/// original inline code: `lm-legacy` on the retained nalgebra
/// `levenberg-marquardt` crate (the parity oracle); `dogleg` / `newton-cg` on
/// the Δ-radius trust-region drivers; everything else (`lm` / `trf` /
/// `geodesic` / `auto`) on the faer-native LM core.
fn run_lm_solve<'a>(
    mut problem: LmProblem<'a>,
    solver: Solver,
    options: &FitOptionsSpec,
    free_keys_len: usize,
    y_all_len: usize,
) -> Result<LmSolveOutcome<'a>, CoreError> {
    ...
    // for every LM-family solver string (`"lm"`, `"auto"`, `"trf"`, `"geodesic"`);
    // the proven `levenberg-marquardt` (nalgebra) crate is retained as the
    // `"lm-legacy"` oracle for parity testing until it is removed.
    let tol = if options.tolerance > 0.0 {
        options.tolerance
    } else {
        1e-8
    };
    let max_nfev = patience.saturating_mul(free_keys_len + 1);
    ...
    let (
        ...
        Solver::Dogleg | Solver::NewtonCg => {
            ...
            // Power-user knobs for the TR core: each `Option<f64>` on
            ...
            if let Some(e) = options.eta {
                // `TrustRegionConfig::eta` is only `debug_assert!`-checked by
                // the driver, which compiles out in release builds — this is
                // the check that actually enforces the bound for callers that
                // reach the Rust core directly (the JSON `fit` entrypoint),
                // not just the Pydantic `FitOptions` validator.
                if !(e.is_finite() && (0.0..0.25).contains(&e)) {
                    return Err(SolverError::InvalidEta(e).into());
                }
                cfg.eta = e;
            }
        ...
        _ => {
            let cfg = spectrafit_levenberg_marquardt::StrategyConfig {
                kind: spectrafit_levenberg_marquardt::select_regime(y_all_len, free_keys_len),
                ftol: tol,
                xtol: tol,
                gtol: tol,
                max_nfev,
                ...
                // route to VarPro also lands here as TRF, per the 2026-06-03 ADR in
                // `DECISIONS-archive.md`: a one-off comparison of all nine strategies
                // over one representative case per scenario family (each the smallest
                // by point count) found TRF the fastest LM-family strategy at top
                // accuracy in ~7 of 9 problem classes. That ADR is the only surviving
                // record — the comparison's script/report are not in this repo — and
                // it carries its own caveat: the TRF-over-VarPro speed result may
                // invert for very large separable problems, which is why VarPro
                // routing is retained for the fully-unconstrained separable case.
...
/// `(all_params, init_vals, bounds, scales)` — see [`collect_free_param_specs`].
type FreeParamSpecs = (HashMap<String, f64>, Vec<f64>, Vec<(f64, f64)>, Vec<f64>);

/// `(node_param_bufs, free_to_node_param, tied_to_node_param)` — see
/// [`build_node_param_buffers`].
type NodeParamBuffers = (Vec<Vec<f64>>, Vec<(usize, usize)>, Vec<(usize, usize)>);

/// Build the flat all-parameters map (free + fixed) plus per-free-parameter
/// initial values, bounds, and scale factors.
///
/// Returns `(all_params, init_vals, bounds, scales)`, all indexed in `free_keys`
/// order for the latter three. `scales[i]` defaults to 1.0 (identity — a
/// non-finite or non-positive `Parameter.scale` is meaningless for
/// normalization and falls back to the identity too).
fn collect_free_param_specs(
    graph: &FitGraphSpec,
    free_keys: &[String],
) -> Result<FreeParamSpecs, CoreError> {
    ...
    Ok((all_params, init_vals, bounds, scales))
}

/// Build index-based node param buffers, the free→node/param index mapping,
/// and the tied-target→node/param index mapping.
///
/// `node_param_bufs[i]` holds `cg.nodes[i]`'s current param values in model
/// `param_names()` order — fixed params are pre-filled and never changed;
/// free params are updated in-place by `LmProblem::set_params()`.
/// `free_to_node_param` inverts `cg.node_free_cols` (built during `compile()`)
/// so free-key index → `(node_idx, param_pos_in_node)`, and is used here to
/// apply the initial bounds-clamp to `node_param_bufs`. `tied_to_node_param`
/// maps each tied target (`tied_plan.order`) to its `(node_idx, param_pos)`
/// slot so the solver can write each recomputed tied value back — empty when
/// the graph declares no ties (`expr_edges` or `Parameter.expr`).
fn build_node_param_buffers(
    cg: &CompiledGraph,
    all_params: &HashMap<String, f64>,
    bounds: &[(f64, f64)],
    init_vals_clone: &[f64],
) -> Result<NodeParamBuffers, CoreError> {
    ...
    let tied_to_node_param: Vec<(usize, usize)> = cg
        .tied_plan
        .order
        .iter()
        .map(|tp| {
            let (nid, pname) = tp
                .target
                .split_once('.')
                .ok_or_else(|| SolverError::MalformedTiedTarget(tp.target.clone()))?;
            let ni = cg
                .nodes
                .iter()
                .position(|n| n.id == nid)
                .ok_or_else(|| SolverError::TiedTargetNodeMissing(nid.to_string()))?;
            let pos = cg.nodes[ni]
                .param_names
                .iter()
                .position(|p| p == pname)
    ...
    Ok((node_param_bufs, free_to_node_param, tied_to_node_param))
}

/// Run the VarPro path and apply the shared post-fit success guards.
///
/// Both VarPro entry points — the explicit `Solver::Varpro` arm and the
/// `Solver::Auto` arm when `graph_prefers_varpro` — must route through here.
/// They are separate match arms because they have different *preconditions*
/// (explicit VarPro rejects non-separable/tied/dataset-scoped graphs with a
/// specific error; automatic routing only reaches VarPro when the graph already
/// qualifies), but they must share the same *postcondition*. Keeping the guard
/// in one function is what makes "the two solver strings agree on `success`"
/// structural rather than a thing to remember.
fn run_varpro_guarded(
    graph: &FitGraphSpec,
    datasets: &[MeasurementSpec],
    options: &FitOptionsSpec,
) -> Result<FitResultSpec, CoreError> {
    let result = solve_varpro_path(graph, datasets, options)?;
    Ok(finalize_varpro_result(graph, datasets, result))
}

/// Apply the shared post-fit success guards (off-domain runaway + degenerate
/// peak-collapse) to a VarPro result.
///
/// VarPro builds its own [`FitResultSpec`] outside `postfit::assemble_result`
/// (it has no `LmProblem`/covariance path to hang off), so this re-derives the
/// inputs `apply_postfit_guards` needs — free-key list, flat parameter map,
/// concatenated x/y — from the already-assembled `result` and applies the same
/// guard the LM-family path gets for free. Without this a degenerate VarPro
/// fit would report false success.
fn finalize_varpro_result(
    graph: &FitGraphSpec,
    datasets: &[MeasurementSpec],
    mut result: FitResultSpec,
) -> FitResultSpec {
    let x_all: Vec<f64> = datasets.iter().flat_map(point_major_x).collect();
    let y_all: Vec<f64> = datasets
        .iter()
    ...
    let vp_free_keys: Vec<String> = result
        ...
        .map(|(k, _)| k.clone())
        .collect();
    let (success, message) = postfit::apply_postfit_guards(
        graph,
        &vp_free_keys,
        &final_flat,
        &x_all,
...
mod tests {
    ...
    // ── Test: unrecognised `solver`/weight-fn strings are rejected, not
    //    silently defaulted ────────────────────────────────────────────────
    //
    // Before this fix, `Solver::parse` silently mapped ANY unrecognised
    // string to `Solver::Lm` (and `WeightFn::from_str` silently mapped any
    // unrecognised name to Huber) — a typo like `solver="lmm"` ran a fit
    // with the LM solver instead of erroring, which could hide a
    // misconfigured request behind a plausible-looking successful result.

    #[test]
    fn fit_rejects_unrecognised_solver_string_instead_of_defaulting_to_lm() {
        let mut params: HashMap<String, ParameterSpec> = HashMap::new();
        params.insert("amplitude".into(), make_param(4.0, true));
        params.insert("center".into(), make_param(1.8, true));
        params.insert("sigma".into(), make_param(0.6, true));
        ...
        let dataset = MeasurementSpec {
            schema_version: None,
            x: vec![vec![0.0, 1.0, 2.0]],
            y: vec![1.0, 2.0, 1.0],
            sigma: None,
            label: None,
        };
        let opts = FitOptionsSpec {
            solver: "lmm".to_string(), // typo for "lm"
            ..default_options()
        };

        let err = fit(&graph, vec![dataset], &opts)
            .expect_err("a typo'd solver name must error, not silently run LM");
        let msg = err.to_string();
        assert!(
            msg.contains("lmm"),
            "error should name the offending string, got: {msg}"
        );
        assert!(
            msg.contains("expected one of"),
            "error should list valid solver names, got: {msg}"
        );
    }

    // ── Test: `eta >= 0.25` is rejected on the JSON/Rust path, not just by
    //    the Pydantic `Field(ge=0.0, lt=0.25)` validator ─────────────────────
    //
    // Before this fix, `FitOptionsSpec.eta` was only bounds-checked in
    // `python/spectrafit_core/options.py`; a caller reaching the Rust core
    // directly (the JSON `fit` entrypoint, or any non-Python consumer) could
    // set `eta >= 0.25`, which the trust-region driver only caught with a
    // `debug_assert!` that compiles out of release builds — so a release
    // build silently ran with the invalid ratio.

    #[test]
    fn fit_rejects_eta_at_or_above_a_quarter_on_dogleg() {
        let mut params: HashMap<String, ParameterSpec> = HashMap::new();
        params.insert("amplitude".into(), make_param(4.0, true));
        params.insert("center".into(), make_param(1.8, true));
        params.insert("sigma".into(), make_param(0.6, true));
        ...
        let opts = FitOptionsSpec {
            solver: "dogleg".to_string(),
            eta: Some(0.5),
            ..default_options()
        };

        let err = fit(&graph, vec![dataset], &opts)
            .expect_err("eta >= 0.25 must error, not silently run with an invalid ratio");
        let msg = err.to_string();
        assert!(
            msg.contains("eta"),
            "error should name the offending option, got: {msg}"
        );
    }

    #[test]
    fn fit_rejects_unrecognised_irls_weight_fn_instead_of_defaulting_to_huber() {
        let mut params: HashMap<String, ParameterSpec> = HashMap::new();
        params.insert("amplitude".into(), make_param(4.0, true));
        params.insert("center".into(), make_param(1.8, true));
        params.insert("sigma".into(), make_param(0.6, true));
        ...
        let opts = FitOptionsSpec {
            solver: "irls:buisquare".to_string(), // typo for "bisquare"
            ..default_options()
        };

        let err = fit(&graph, vec![dataset], &opts)
            .expect_err("a typo'd irls weight-fn name must error, not silently run Huber");
        let msg = err.to_string();
        assert!(
            msg.contains("buisquare"),
            "error should name the offending string, got: {msg}"
        );
    }
    ...
    #[test]
    fn auto_and_explicit_varpro_agree_on_success() {
        // REGRESSION. The `Solver::Auto if graph_prefers_varpro` arm used to call
        // `solve_varpro_path` raw while `Solver::Varpro` wrapped it in
        // `finalize_varpro_result`. The guards live in that wrapper, so on a
        // VarPro-eligible graph the two solver strings could disagree about
        // `success` for identical data — with `auto`, the documented default,
        // being the one that reported the false success.
        //
        // The assertion is deliberately on AGREEMENT rather than on a specific
        // verdict: whether this particular fit converges is not the point, and
        // pinning it would make the test brittle to solver tuning. What must
        // hold is that routing does not change the answer.
        let graph = two_gaussian_param_expr_graph(false);
        assert!(
            graph_prefers_varpro(&graph),
            "fixture must actually route auto -> varpro, or this proves nothing"
        );
        ...
        let x: Vec<f64> = (0..n)
            .map(|i| -3.0 + 6.0 * i as f64 / (n - 1) as f64)
            .collect();
        // Data two Gaussians cannot represent: a high-frequency oscillation.
        // The fit converges but to a poor optimum, which is what puts r² under
        // OFF_DOMAIN_R2_FLOOR (0.5) so `apply_postfit_guards` actually engages.
        // A clean, well-fit dataset makes this test VACUOUS — the guard's own
        // r²-quality escape short-circuits it and both paths trivially agree.
        ...
        let r_auto = fit(&graph, vec![data.clone()], &opts("auto")).expect("auto fit");
        let r_varpro = fit(&graph, vec![data], &opts("varpro")).expect("varpro fit");

        assert_eq!(
            r_auto.success, r_varpro.success,
            "auto and explicit varpro must reach the same success verdict on the \
             same graph and data; a difference means one path skipped the \
             post-fit guards in `finalize_varpro_result`"
        );
        assert_relative_eq!(r_auto.chi2, r_varpro.chi2, max_relative = 1e-12);
    }
