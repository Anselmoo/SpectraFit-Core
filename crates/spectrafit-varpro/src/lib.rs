//! spectrafit-varpro — Variable Projection (VarPro) solver for separable models.
//!
...
//!
//! References:
//! - Golub, G., Pereyra, V. (1973). "The Differentiation of Pseudo-Inverses
//!   and Nonlinear Least Squares Problems Whose Variables Separate." *SIAM
//!   J. Numer. Anal.* 10(2), 413-432.
//! - Kaufman, L. (1975). "A variable projection method for solving
//!   separable nonlinear least squares problems." *BIT* 15, 49-57.
#![warn(missing_docs)]

// Modules are private: the crate's public surface is the curated `pub use`
// list below (house rule 26). Nothing outside this crate reached
// `model::`/`solver::` by module path.
mod model;
mod solver;

pub use model::GraphSeparableModel;
...
mod tests {
    ...
    fn model_type_str_varpro_parity_guard() {
        ...
        let non_varpro: &[&str] = &[
            ...
            "rational_cubic", // 4 independent linear numerator coeffs (a0..a3), no single amplitude
            "exp_over_linear", // lin_const/lin_slope both reshape the curve, not just scale it
            ...
            "generalised_logistic", // Rat43: amplitude/(1+exp(shift-rate·x))^(1/shape) — eligible, deferred
        ];
