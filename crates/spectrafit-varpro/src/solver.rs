...
pub fn solve_varpro(
    ...
    let covariance_param_order = build_covariance_param_order(&alpha_keys, &compiled);
...
/// The free-parameter name order matching the VarPro Jacobian's column order
/// used in [`fill_parameter_stderr`]: alpha_keys first, then amplitude keys in
/// node order.
fn build_covariance_param_order(alpha_keys: &[String], compiled: &CompiledGraph) -> Vec<String> {
    alpha_keys
        .iter()
        .cloned()
