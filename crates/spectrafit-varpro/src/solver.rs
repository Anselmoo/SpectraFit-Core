...
use spectrafit_graph::{
    evaluate_compiled, evaluate_components_compiled, jacobian_compiled, CompiledGraph,
};
...
pub fn solve_varpro(
    ...
    let (stacked, x_concat, y_concat) = stack_datasets(datasets);

    // ── 1. Build the separable model ─────────────────────────────────────────
    let model = build_model(graph, &stacked, all_params)?;
    let alpha_keys = model.alpha_keys.clone();

    // ── 2+3. Build y/weights, build the fitting problem, and solve ────────────
    let (alpha, coefficients) =
        solve_separable_problem(model, &y_concat, stacked.sigma.as_deref())?;

    // ── 4. Reconstruct parameters ─────────────────────────────────────────────
    let compiled = CompiledGraph::compile(graph).map_err(|e| CoreError::Eval(format!("{e}")))?;
    let mut parameters =
        build_parameter_results(&alpha_keys, &alpha, &coefficients, &compiled, all_params);

    // ── 5. Compute best-fit, residuals, statistics ────────────────────────────
    let stats = compute_fit_statistics(
        &compiled,
        &parameters,
        all_params,
        &x_concat,
    ...
    let dataset_slices = build_dataset_slices(datasets, &stats.best_fit);

    // The covariance/Jacobian column order is `compiled.free_keys` — the same
    // order `jacobian_compiled` emits and the same convention the LM/TRF path
    // reports (see `spectrafit-solver::postfit`). VarPro's own α-then-amplitude
    // ordering is an *internal* solve basis and must never leak here: reporting
    // it as the column order is exactly the permutation bug this replaced.
    let covariance_param_order = compiled.free_keys.clone();
...
// ---------------------------------------------------------------------------
// solve_varpro helpers (pure structural extraction — see git log for the
// pre-extraction monolithic body if a behavior question ever arises)
// ---------------------------------------------------------------------------

/// Stack all datasets into a single synthetic `MeasurementSpec`. Shared
/// nonlinear params (α) apply to the concatenated problem; amplitudes (linear
/// coefficients) are solved jointly across the stacked data. Returns the
/// stacked spec together with the concatenated x/y vectors, both of which are
/// used directly (not re-derived) by callers.
fn stack_datasets(datasets: &[MeasurementSpec]) -> (MeasurementSpec, Vec<f64>, Vec<f64>) {
    ...
    (stacked, x_concat, y_concat)
}

/// Build the `GraphSeparableModel` for the stacked measurement.
fn build_model(
    graph: &FitGraphSpec,
    stacked: &MeasurementSpec,
    all_params: &HashMap<String, ParameterSpec>,
) -> Result<GraphSeparableModel, CoreError> {
    GraphSeparableModel::new(graph, stacked, all_params)
        .map_err(|e| CoreError::Eval(format!("VarPro model build error: {e}")))
}

/// Build the observation vector + optional weights, build the VarPro
/// `SeparableProblem`, and run `LevMarSolver`. Returns the solved nonlinear
/// parameters (α) and linear coefficients as owned `Vec<f64>` — the VarPro fit
/// result's `linear_coefficients()` is a view borrowed from the (function-
/// local) fit result, so it cannot outlive this function; copying to owned
/// vectors here is required, not a style choice.
fn solve_separable_problem(
    model: GraphSeparableModel,
    y_concat: &[f64],
    sigma: Option<&[f64]>,
) -> Result<(Vec<f64>, Vec<f64>), CoreError> {
    // ── Build y and optional weights ──────────────────────────────────────
    let y = DVector::from_vec(y_concat.to_vec());
    let weights_opt = sigma.map(|s| {
        DVector::from_vec(
            s.iter()
    ...
    // ── Build the fitting problem and solve ────────────────────────────────
    let fit_result = if let Some(weights) = weights_opt {
        ...
        LevMarSolver::default()
            .solve(problem)
            // varpro::solvers::levmar::FitResult does not implement Display; use Debug format.
    ...
    let alpha: Vec<f64> = fit_result.nonlinear_parameters().iter().copied().collect();
    let coefficients: Vec<f64> = fit_result
        .linear_coefficients()
    ...
    Ok((alpha, coefficients))
}

/// Reconstruct the `ParameterResultSpec` map from the solved α (nonlinear) and
/// amplitude (linear) values, filling in any parameter VarPro did not touch
/// (e.g. fixed/expression-bound parameters) from `all_params`.
...
/// Statistics + evaluated curves produced from the reconstructed parameters,
/// bundled for return from [`compute_fit_statistics`]. `flat_params` is kept
/// here (rather than recomputed) because [`fill_parameter_stderr`] needs the
/// exact same map to evaluate the Jacobian at the solution.
struct FitStatistics {
    flat_params: HashMap<String, f64>,
    best_fit: Vec<f64>,
    init_fit: Vec<f64>,
...
/// Compute best-fit, residuals, and fit statistics (χ², R², AIC/BIC, …) from
/// the reconstructed parameters.
// Allowed: the compiled graph, the reconstructed (post-VarPro) parameters
// alongside the original spec map, the concatenated data, and the VarPro
// linear/nonlinear split (alpha_keys/coefficients) are each read
// independently below to assemble best-fit, init-fit, and the scalar
// diagnostics; bundling them into a struct would just rename this same
// parameter list.
#[allow(clippy::too_many_arguments)]
fn compute_fit_statistics(
    compiled: &CompiledGraph,
    parameters: &HashMap<String, ParameterResultSpec>,
    all_params: &HashMap<String, ParameterSpec>,
    x_concat: &[f64],
    y_concat: &[f64],
    alpha_keys: &[String],
    coefficients: &[f64],
) -> Result<FitStatistics, CoreError> {
...
/// Fill in `stderr` on `parameters` from Σ = (JᵀJ)⁻¹ · (χ²/DOF), using the
/// analytical Jacobian at the solution (sigma-weighted when sigma was
/// supplied). Leaves `stderr: None` wherever the Jacobian or the inversion is
/// unavailable, mirroring the original best-effort (non-erroring) behavior.
///
/// Column indexing is keyed off `compiled.free_keys`, which is the order
/// `jacobian_compiled` actually emits (node-id sorted, then model-param order
/// within a node). It is *not* VarPro's internal α-then-linear-coefficient
/// basis; conflating the two silently permutes the reported uncertainties.
fn fill_parameter_stderr(
    parameters: &mut HashMap<String, ParameterResultSpec>,
    compiled: &CompiledGraph,
    flat_params: &HashMap<String, f64>,
    x_concat: &[f64],
    datasets: &[MeasurementSpec],
    ...
    if let Ok(j_final) = jacobian_compiled(compiled, flat_params, x_concat) {
        ...
        if let Some(cov) = cov_opt {
            // Column `col` of J (and hence row/col `col` of Σ) belongs to
            // `compiled.free_keys[col]` — assign by name, never positionally
            // against some other ordering.
            for (col, key) in compiled.free_keys.iter().enumerate() {
                if col >= cov.nrows() {
                    break;
                }
                let v = cov[(col, col)];
...
/// fits (`None` for a single dataset).
...
mod tests {
    ...
    // ── Regression: stderr must land on the parameter it belongs to ─────────
    //
    // VarPro optimises in an internal basis (nonlinear α first, then the linear
    // amplitude coefficients). The reported covariance, however, comes from
    // `jacobian_compiled`, whose columns follow `CompiledGraph::free_keys`
    // (node-id sorted, then model-param order → amplitude, center, sigma).
    // Writing the covariance diagonal back in α-then-amplitude order permuted
    // every reported uncertainty; this test pins the correct mapping against a
    // finite-difference covariance built independently, keyed by name.
    #[test]
    fn solve_varpro_stderr_is_not_permuted() {
        use nalgebra::DMatrix;

        let true_amp = 5.0_f64;
        let true_center = 2.0_f64;
        let true_sigma = 0.7_f64;
        let n = 400usize;
        let x: Vec<f64> = (0..n).map(|i| 4.0 * i as f64 / (n - 1) as f64).collect();

        // Deterministic pseudo-noise (LCG) — real residuals are required or
        // χ²/DOF collapses to zero and every stderr degenerates to ~0.
        let mut seed = 12_345_u64;
        let y: Vec<f64> = gaussian_data(&x, true_amp, true_center, true_sigma)
            .into_iter()
            .map(|yi| {
                seed = seed.wrapping_mul(6_364_136_223_846_793_005).wrapping_add(1);
                let u = (seed >> 11) as f64 / (1_u64 << 53) as f64; // [0,1)
                yi + 0.08 * (u - 0.5)
            })
            .collect();
        ...
        let node_id = "g0";
        let mut node_parameters = HashMap::new();
        node_parameters.insert("amplitude".to_string(), free_param(1.0));
        node_parameters.insert("center".to_string(), free_param(1.8));
        node_parameters.insert("sigma".to_string(), free_param(0.5));

        let params_map: HashMap<String, ParameterSpec> = node_parameters
            .iter()
            .map(|(k, v)| (format!("{node_id}.{k}"), v.clone()))
            .collect();
        ...
        let result = solve_varpro(&graph, &[dataset], &params_map, &FitOptionsSpec::default())
            .expect("VarPro must succeed on a clean 1-Gaussian dataset");

        let compiled = CompiledGraph::compile(&graph).expect("graph must compile");
        assert_eq!(
            result.covariance_param_order, compiled.free_keys,
            "reported column order must be the Jacobian's own free_keys order"
        );

        // Independent reference: central-difference Jacobian, columns keyed by
        // name in `free_keys` order, then Σ = (JᵀJ)⁻¹ · (χ²/DOF).
        let flat: HashMap<String, f64> = result
            .parameters
            .iter()
            .map(|(k, v)| (k.clone(), v.value))
            .collect();
        let keys = &compiled.free_keys;
        let mut j = DMatrix::<f64>::zeros(n, keys.len());
        for (col, key) in keys.iter().enumerate() {
            let v0 = flat[key];
            let h = 1e-6 * v0.abs().max(1.0);
            let mut up = flat.clone();
            up.insert(key.clone(), v0 + h);
            let mut dn = flat.clone();
            dn.insert(key.clone(), v0 - h);
            let fu = evaluate_compiled(&compiled, &up, &x).expect("eval up");
            let fd = evaluate_compiled(&compiled, &dn, &x).expect("eval down");
            for row in 0..n {
                j[(row, col)] = (fu[row] - fd[row]) / (2.0 * h);
            }
        }
        let cov = (j.transpose() * &j)
            .try_inverse()
            .expect("JᵀJ must be invertible")
            * (result.chi2 / result.dof as f64);

        for (col, key) in keys.iter().enumerate() {
            let expected = cov[(col, col)].sqrt();
            let got = result.parameters[key]
                .stderr
                .unwrap_or_else(|| panic!("{key} must carry a stderr"));
            assert_relative_eq!(got, expected, max_relative = 1e-4, epsilon = 1e-12);
        }

        // Guard the specific symptom: amplitude and center had swapped errors,
        // and on this fixture their magnitudes differ by ~2.6x.
        let amp_err = result.parameters["g0.amplitude"]
            .stderr
            .expect("amp stderr");
        let ctr_err = result.parameters["g0.center"]
            .stderr
            .expect("center stderr");
        assert!(
            amp_err > ctr_err,
            "amplitude stderr ({amp_err}) must exceed center stderr ({ctr_err}) here"
        );
    }
}
