...
pub struct ParameterSpec {
    ...
    pub vary: bool,
    #[serde(default)]
    /// Optional symbolic expression tying this parameter to others;
    /// re-evaluated on every solver iteration (dependency-ordered), so the
    /// model always sees the current tied value.
    pub expr: Option<String>,
    #[serde(default)]
    /// Positive scale factor for the solver's working variable: the
    /// optimiser iterates on `value / scale` so parameters of very different
    /// magnitude are conditioned alike. Does not change the reported
    /// physical value.
...
model_manifest! {
    ...
    /// Axis-aligned N-D Gaussian peak (parametric dimensionality). `D` is
    /// inferred from the node's indexed `center_<i>` parameters (there is no
    /// `n_dims` field); params are indexed `center_0..center_{D-1}` /
    /// `sigma_0..sigma_{D-1}`.
    ...
    /// `(a0 + a1·x + a2·x² + a3·x³) / (1 + b1·x + b2·x² + b3·x³)`.
    RationalCubic => "rational_cubic",
    /// Generalised logistic / Richards curve (Rat42, Rat43):
    /// `amplitude / (1 + exp(shift − rate·x))^(1/shape)`.
    GeneralisedLogistic => "generalised_logistic",
    /// Exponential over a line (Chwirut1, Chwirut2):
    /// `exp(−rate·x) / (den_const + den_lin·x)`.
    ExpOverLinear => "exp_over_linear",
}
...
pub struct MeasurementSpec {
    ...
    /// Independent coordinates on the wire as an `(N, D)` array: one row per
    /// data point, `D` columns for a `D`-dimensional fit. The Python
    /// `MeasurementData` model promotes a flat length-`N` list to `(N, 1)`
    /// before serialising; this type requires the nested form. The PyO3
    /// boundary (`transpose_x_for_solver` in `spectrafit-core`) transposes
    /// this into the solver's dims×points layout before fitting.
    pub x: Vec<Vec<f64>>,
...
pub struct FitOptionsSpec {
    ...
    pub schema_version: Option<String>,
    /// Solver name, one of: `"lm"` (default), `"lm-legacy"`, `"trf"`,
    /// `"geodesic"`, `"dogleg"`, `"newton-cg"`,
    /// `"irls"`/`"irls:huber"`/`"irls:bisquare"`/`"irls:cauchy"`, `"global"`,
    /// `"varpro"`, or `"auto"`.
    pub solver: String,
    /// Solver patience. The function-evaluation budget is
    /// `max_iterations × (n_free + 1)`; the LM family stops with
    /// `max_iterations` termination when it is exhausted.
    ...
    /// `serde(default)` so payloads predating this field still deserialize.
    #[serde(default)]
    pub delta0: Option<f64>,
    /// Hard upper bound on the trust-region radius `Δ` for Dogleg / Newton-CG.
    /// `None` keeps the library default (1e3).
    #[serde(default)]
    pub max_delta: Option<f64>,
    /// Step-acceptance threshold for Dogleg / Newton-CG (accept when `ρ > eta`).
    /// `None` keeps the library default (1e-4). A probability-like ratio, so
    /// it is bounded to `[0, 0.25)`: the trust-region driver
    /// (`spectrafit-trust-region::TrustRegionConfig::eta`) only shrinks `Δ`
    /// when `ρ < 0.25`, so an `eta >= 0.25` would open a band in which a step
    /// is rejected without shrinking the radius, and the solver could spin
    /// to `max_nfev` instead of failing cleanly. `spectrafit-solver::dispatch`
    /// rejects `eta >= 0.25` (or non-finite / negative `eta`) with
    /// `SolverError::InvalidEta` before it reaches the driver — the driver's
    /// own `debug_assert!(eta < 0.25)` compiles out of release builds, so it
...
pub struct ParameterResultSpec {
    ...
    pub expr: Option<String>,
    #[serde(default)]
    /// Echo of the input [`ParameterSpec::scale`]: the solver's
    /// working-variable scaling, which does not change the reported
    /// physical value. `None` when no scaling was applied.
    pub scale: Option<f64>,
    #[serde(default)]
...
pub struct FitResultSpec {
    ...
    pub r_squared: f64,
    /// Degrees of freedom `n_points − n_free`, clamped to a minimum of 1 so
    /// the reduced statistics stay finite.
    pub dof: i64,
    /// Akaike Information Criterion, from a Gaussian-deviance term (not raw
    /// χ²): `n_points · ln(χ² / n_points) + 2 · n_free`. See
    /// `compute_scalar_diagnostics` in `spectrafit-solver::postfit`.
    pub aic: f64,
    /// Bayesian Information Criterion, from a Gaussian-deviance term (not raw
    /// χ²): `n_points · ln(χ² / n_points) + n_free · ln(n_points)`. See
    /// `compute_scalar_diagnostics` in `spectrafit-solver::postfit`.
    pub bic: f64,
    /// Accepted solver iterations for the faer-native solvers; for
    /// `lm-legacy` the number of function evaluations (that engine does not
    /// expose iterations). `varpro` reports 0 (its engine does not expose
    /// iterations either).
    pub n_iter: u64,
    ...
    pub success: bool,
    /// Stable termination-reason key, not free text — one of the strings
    /// produced by `TerminationReason::as_str` / `faer_termination_str`
    /// (e.g. `converged_ftol`, `converged_xtol`, `converged_gtol`,
    /// `converged` for lm-legacy, `max_iterations`,
    /// `no_improvement_possible`, `residuals_zero`, `numerical_error`).
    ///
    /// Three post-fit forms replace or extend the key vocabulary, all
    /// applied by `apply_postfit_guards` in `spectrafit-solver::postfit`:
    /// - An originally-unbounded free parameter that escaped the
    ///   data-derived domain replaces the key entirely with
    ///   `diverged_off_domain (<key>=<val> escaped data domain [...])`, e.g.
    ///   `diverged_off_domain (p1.amplitude=1.234e5 escaped data domain
    ///   [-3.900e0, 7.800e0])`, and downgrades the result to
    ///   `success=false`.
    /// - A post-fit guard that detects a collapsed fit (r² < 0 with a
    ///   near-zero amplitude/height) replaces the key with a
    ///   `degenerate_fit (…)` string carrying the guard's reason, e.g.
    ///   `degenerate_fit (r2=-1.234e0 < 0, peak amplitude collapsed)`.
    /// - A soft-stop termination (`no_improvement_possible` or
    ///   `max_iterations`) upgraded to success because r² ≥ 0.9 instead keeps
    ///   the original key and appends an `_accepted_at_r2_<value>` suffix,
    ///   e.g. `no_improvement_possible_accepted_at_r2_0.9921`.
    ///
    /// Match on the `diverged_off_domain` / `degenerate_fit` prefix or the
    /// original key prefix respectively, rather than the whole string — the
    /// bracketed/appended part is diagnostic text, not a stable key.
    pub message: String,
    ...
    ///
    /// Each entry is the gradient **at the most recent point where the Jacobian
    /// was evaluated** — the start of that outer iteration — not at the point
    /// whose cost sits at the same index. When the loop stopped after an
    /// accepted step the terminal entry therefore repeats the previous
    /// iteration's gradient; the final entry always equals the driver's
    /// reported final gradient norm
    /// (`spectrafit_trust_region::Report::gradient_norm`), which
    /// `FitResultSpec` does not mirror as a scalar.
    #[serde(default)]
