//! spectrafit-graph — DAG compilation and evaluation engine.
//!
...
// `pub use` list below (house rule 26). Previously these were `pub mod`, so a
// consumer could reach any item in `compiler`/`executor`/`expr` directly,
// bypassing this list.
mod compiler;
mod error;
mod executor;
...
pub use executor::{
    ...
    jacobian_compiled_indexed, jacobian_compiled_indexed_into,
    jacobian_compiled_indexed_weighted_into, residuals_compiled_indexed_into,
};
...
///
/// # Errors
/// Returns [`CoreError::Eval`] if: the graph fails to compile (unknown model
/// type, missing parameter, duplicate node id, or a bad `expr_edges` cycle —
/// see [`CompiledGraph::compile`]); `params_flat` is missing a
/// `"node_id.param_name"` entry a node needs; `x.len()` is not an exact
/// multiple of the graph's coordinate dimensionality; or a node's
/// `dataset_index` is out of range for the compiled `dataset_offsets`.
pub fn evaluate(
    graph: &FitGraphSpec,
...
///
/// # Errors
/// Returns [`CoreError::Eval`] if: the graph fails to compile (see
/// [`CompiledGraph::compile`]); `params_flat` is missing a required
/// `"node_id.param_name"` entry; `x.len()` is not an exact multiple of the
/// graph's coordinate dimensionality; or a node's `dataset_index` is out of
/// range for the compiled `dataset_offsets`.
