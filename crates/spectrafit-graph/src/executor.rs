...
///
/// # Errors
/// Returns [`CoreError::Eval`] if: `flat` is missing a required
/// `"node_id.param_name"` entry for some node ([`GraphError::MissingParamKey`]);
/// `x.len()` is not an exact multiple of the graph's coordinate
/// dimensionality ([`GraphError::XBufferStrideMismatch`]); the graph's nodes
/// disagree on `n_dims` ([`GraphError::DimensionalityMismatch`]); or, when
/// multi-dataset scoping is active, a node's `dataset_index` is out of range
/// ([`GraphError::DatasetIndexOutOfRange`]).
...
///
/// # Errors
/// Returns [`CoreError::Eval`] if: `x.len()` is not an exact multiple of the
/// graph's coordinate dimensionality ([`GraphError::XBufferStrideMismatch`]);
/// the graph's nodes disagree on `n_dims`
/// ([`GraphError::DimensionalityMismatch`]); a node's `dataset_index` is out
/// of range for the recorded `dataset_offsets`
/// ([`GraphError::DatasetIndexOutOfRange`]); or `flat` is missing a required
/// `"node_id.param_name"` entry ([`GraphError::MissingParamKey`]).
...
/// `param_names()` order) and updates only the free-param slots on each
/// iteration, using a free-param-index → `(node_idx, param_pos)` mapping it
/// builds and owns itself (`spectrafit-solver`'s `LmProblem::free_to_node_param`).
///
...
/// Returns [`CoreError::Eval`] if `x.len()` is not an exact multiple of the
/// graph's coordinate dimensionality ([`GraphError::XBufferStrideMismatch`])
/// or the graph's nodes disagree on `n_dims`
/// ([`GraphError::DimensionalityMismatch`]).
...
/// graph's coordinate dimensionality ([`GraphError::XBufferStrideMismatch`]);
/// the graph's nodes disagree on `n_dims`
/// ([`GraphError::DimensionalityMismatch`]); or `out.len()` does not match the
/// computed point count ([`GraphError::OutputBufferLength`]).
...
/// Returns [`CoreError::Eval`] if: `x.len()` is not an exact multiple of the
/// graph's coordinate dimensionality ([`GraphError::XBufferStrideMismatch`]);
/// the graph's nodes disagree on `n_dims` ([`GraphError::DimensionalityMismatch`]);
/// `out`, `y`, or `sigma` does not have exactly one entry per point
/// ([`GraphError::OutputBufferShape`]); or, when multi-dataset scoping is
/// active, a node's `dataset_index` is out of range
/// ([`GraphError::DatasetIndexOutOfRange`]).
...
/// graph's coordinate dimensionality ([`GraphError::XBufferStrideMismatch`]);
/// the graph's nodes disagree on `n_dims`
/// ([`GraphError::DimensionalityMismatch`]); or, when multi-dataset scoping is
/// active, a node's `dataset_index` is out of range
/// ([`GraphError::DatasetIndexOutOfRange`]).
...
// NOTE: `jacobian_compiled_indexed_weighted` (the allocating counterpart of
// `..._weighted_into`) was removed when the crate's modules became private.
// It had no caller anywhere in the workspace — invisible while `pub mod
// executor;` made every item reachable, so rustc could not prove it unused.
// Restore from git history if an allocating variant is ever needed.
...
/// graph's coordinate dimensionality ([`GraphError::XBufferStrideMismatch`]);
/// the graph's nodes disagree on `n_dims`
/// ([`GraphError::DimensionalityMismatch`]); or a node's `dataset_index` is
/// out of range for the recorded `dataset_offsets`
/// ([`GraphError::DatasetIndexOutOfRange`]).
...
/// graph's coordinate dimensionality ([`GraphError::XBufferStrideMismatch`]);
/// the graph's nodes disagree on `n_dims`
/// ([`GraphError::DimensionalityMismatch`]); `sigma.len()` does not match the
/// computed point count ([`GraphError::OutputBufferLength`]); or a node's
/// `dataset_index` is out of range for the recorded `dataset_offsets`
/// ([`GraphError::DatasetIndexOutOfRange`]).
