...
// ---------------------------------------------------------------------------
// compile() sub-steps, extracted as named helpers so each can be read (and
// unit-tested) independently of the outer orchestration in `compile()`.
// ---------------------------------------------------------------------------

/// Step 0: reject duplicate node IDs.
///
/// Node IDs key the free-column layout (`node_idx_by_id`) and the per-node
/// component map (`evaluate_components_compiled`).  A duplicate ID silently
/// keeps only the last index (dropping/doubling free columns) and overwrites
/// one node's component with another — wrong fits with no error.  Reject up
/// front instead.
fn reject_duplicate_ids(graph: &FitGraphSpec) -> Result<(), CoreError> {
...
/// Step 1b: collect every tied parameter target, from both `expr_edges` and
/// any node parameter with its own `ParameterSpec.expr` set. Both sources are
/// harvested into `tied_plan` by `build_tied_plan`; both must also exclude the
/// parameter from the free set in `compile_nodes`.
fn collect_tied_targets(graph: &FitGraphSpec) -> std::collections::HashSet<(String, String)> {
...
/// Step 2: instantiate each node's model kernel and resolve its parameter
/// metadata (required-parameter presence, free/tied mask).
fn compile_nodes(
    graph: &FitGraphSpec,
    tied_targets: &std::collections::HashSet<(String, String)>,
    ...
    for node_spec in &graph.nodes {
        ...
        let param_names: Vec<String> = model.param_names().iter().map(|s| s.to_string()).collect();
...
/// Step 3: build `free_keys`, alpha-sorted by node_id, then by model param
/// order within a node.
fn build_free_keys(nodes: &[NodeEntry]) -> Vec<String> {
...
/// Step 4: pre-compute per-node Jacobian column layout.
///
/// Maps each free_key to (node_idx_in_nodes, local_param_pos) once at compile
/// time.  The solver reads `node_free_cols` directly instead of re-parsing
/// `free_keys` strings on every iteration.
fn build_node_free_cols(nodes: &[NodeEntry], free_keys: &[String]) -> Vec<Vec<(usize, usize)>> {
    let node_idx_by_id: HashMap<&str, usize> = nodes
        .iter()
        .enumerate()
        .map(|(i, n)| (n.id.as_str(), i))
    ...
    for (col, key) in free_keys.iter().enumerate() {
        ...
        // `format!("{}.{}", nid, pname)` in `build_free_keys`, so it always
        // contains exactly one '.' separator — `find` is infallible here.
        let dot = key.find('.').unwrap();
        let node_id = &key[..dot];
        let param_name = &key[dot + 1..];
        // INVARIANT: `node_id` came from `nodes`, so `node_idx_by_id` is
        // guaranteed to contain it.
        let node_idx = node_idx_by_id[node_id];
        // INVARIANT: `param_name` came from `node.param_names` during
        // `build_free_keys`, so the same slice is guaranteed to contain it.
...
impl CompiledGraph {
    ...
    pub fn compile(graph: &FitGraphSpec) -> Result<Self, CoreError> {
        ...
        // crate: `spectrafit-solver::lm_problem::set_free_and_tied` applies it on
        ...
        let tied_targets = collect_tied_targets(graph);
        let nodes = compile_nodes(graph, &tied_targets)?;
        let free_keys = build_free_keys(&nodes);
        let node_free_cols = build_node_free_cols(&nodes, &free_keys);
