...
### Added
- **A gate on the manuscript claim ledger (2026-09-12).**
  `tests/meta/test_manuscript_claim_evidence.py` makes a dangling evidence
  pointer, a line range past end of file, a line range on a directory, a
  malformed `lines` spec, an unknown evidence `type` and a duplicate claim id
  build failures. Nothing read the ledger's 391 pointers before this:
  `manuscript/render_manuscript.py` ignores the claims entirely and only
  `scripts/audit_latex.py` touches the state file, so stale pointers had
  accumulated unseen. Citing a directory stays legitimate evidence — the NIST
  fixtures are cited as `python/oracles/nist_strd` — so a pointer must resolve,
  and must be a file only when it carries line numbers. Each of the six checks
  was mutation-verified to fail on a planted defect.
- **Tier B review panel run against the current draft (2026-09-12).**
  `manuscript/review/PANEL-REVIEW-2026-09-12.md` and
  `MANUSCRIPT-panel-annotated-2026-09-12.docx` record an eight-persona
  pre-submission review: separate verdicts for the manuscript and for the
  codebase as a citable artifact, an adjudication of which NIST framing the
  repository can actually support, and a narrative-reconstruction brief.
  Twelve concerns are anchored as Word comments attributed per persona.
- **`robust` benchmark category — IRLS on outlier-contaminated spectra
  (2026-09-09).** Six cases (`RB-001` … `RB-006`): one Gaussian with 3 % noise
  plus 5 % or 15 % gross outliers (spikes of 4 × the peak height at
  id-seeded positions), fitted by spectrafit with `irls:huber`,
  `irls:bisquare` and `irls:cauchy` while the baselines run least squares on
  the same data. Closes the gap that the IRLS solver had no case in the suite
  at all. The category is scored on recovery-vs-truth and is exempt from the
  baseline-parity axes ($|\Delta r^2|$, regression, win-rate) exactly like
  `optfn`, now expressed as `CategoryDef.baseline_comparable = False` in the
  category registry instead of a hard-coded `"optfn"` string in `engine.py`
  and `reports.py`. `CaseSpec` gains an optional `outliers: OutlierSpec`
  recipe that `materialize` applies after the Gaussian noise.


### Changed
- **IRLS now warm-starts each outer pass and converges on its own weight
  tolerance (2026-09-08).** Each outer reweighting pass after the first
  begins from the most recent *successful* pass's fitted values instead of
  re-running LM from the caller's original initial guess every time, and the
  outer loop's own break condition — the max per-weight change dropping below
  `DEFAULT_TOL_WEIGHTS = 1e-4` — is now independent of the inner LM solve's
  cost tolerance (`options.tolerance`), which previously doubled as the outer
  loop's (informal) stopping signal. On the outlier-robustness fixture the
  cold-started loop never converged at all — it ran all 15 permitted outer
  passes and hit the cap; warm-started, it converges in 3.
- **BREAKING: `FitOptions.eta` must be `< 0.25` (was `< 1.0`), and the engine
  now rejects `eta >= 0.25` on the JSON/Rust path too (2026-09-08).** `eta` is
  the Dogleg / Newton-CG step-acceptance ratio threshold; the trust-region
  driver only shrinks its radius when `ρ < 0.25`, so an `eta >= 0.25` opens a
  band in which a step is rejected without the radius shrinking, and the
  solver could spin to `max_nfev` instead of failing cleanly. Previously only
  `python/spectrafit_core/options.py`'s Pydantic `Field` enforced a bound
  (`< 1.0`, looser than the driver actually tolerates), so a caller reaching
  the Rust core directly — the JSON `fit` entrypoint, or any non-Python
  consumer — could set an out-of-range `eta` that only a release-mode-inert
  `debug_assert!` in the trust-region driver ever caught. `spectrafit-solver`
  now rejects `eta >= 0.25` (or non-finite / negative `eta`) with a
  `SolverError::InvalidEta` before it reaches the driver, on every entry
  point. See the `eta` sections of `docs/how-to/choosing-a-solver.md` and the
  `FitOptions`/`FitOptionsSpec` docstrings for the enforced bound.
- **`fit`/`fit_fast` raise `ValueError` when datasets have different `n_dims`
  instead of silently zero-padding (2026-09-08).** Short coordinate rows were
  previously zero-padded up to the widest row seen across all datasets in a
  request, which fabricated coordinates — a 1-D dataset fitted alongside a
  2-D one was quietly given `x[1] = 0.0` for every point and produced a wrong
  fit with no error. The JSON entrypoint already rejected ragged `x`; the
  array entrypoint (`python/spectrafit_core/fit.py::_require_uniform_n_dims`)
  now agrees.
- **`spectrafit-solver::fit` is now the only site that sets faer's global
  parallelism policy (2026-09-08).** The LM and trust-region drivers each
  used to set `faer::set_global_parallelism` again on entry to their own
  `minimize`/`minimize_tr`, so the effective policy depended on call order;
  ownership now sits once at the top of `fit`, the one place that already
  owns the whole fit. No solver iterate changes as a result (verified: a
  single call site, so this cannot alter arithmetic order within one fit) —
  see the "Deterministic" caveat added to both drivers' doc comments, which
  now names the fixed parallelism policy this guarantee actually depends on.

### Fixed
- **The abstract claimed a NIST accuracy no comparator could match, and the
  committed artifact said otherwise (2026-09-12).** `abstract.md` read that the
  library recovers the NIST StRD certified values to at least four significant
  figures on all 22 implemented datasets, "which no comparator manages". lmfit
  manages it: its worst case in `manuscript/examples/figures/nist_table2.json`
  is 4.630 significant figures (Rat43), with no cell below the 4.0 threshold,
  and `docs/explanation/nist-validation.md` already said so — naming that exact
  wording as something the committed artifact contradicts. The sentence now
  states the parity and keeps the real result: spectrafit-core is the most
  accurate of the four implementations compared on 16 of the 22. `MANUSCRIPT.md`
  and `MANUSCRIPT.docx` were re-rendered. The claim was also the only assertion
  in the abstract carrying no entry in the claim ledger, which is how a
  261-claim audit passed while it was false; it is now registered as `abs-9`.
- **18 claim-evidence pointers in the manuscript ledger named the wrong place
  (2026-09-12).** Six pointed at files that no longer exist — the dissolved
  `sections/validation.md` (now under `_superseded/`), `manuscript/ADR-INDEX.md`
  (the file is at the repository root) and `scripts/manuscript_checksums.py`
  (moved under `manuscript/`) — and seven cited line numbers past the end of the
  file they named, including a regenerated `sweep.json` cited as lines 1-4400
  when it has 2612. Five more drifted when this branch fast-forwarded onto
  `main`, because the `robust` category shifted `python/oracles/reports.py`,
  `engine.py` and `cases.py`. The four claims re-homed out of the dissolved
  section now cite the live prose that carries them, not the archived copy.
- **`fit_arrays`/`fit_arrays_numpy` mapped a Rust panic to an uncatchable
  `pyo3_runtime.PanicException` instead of `ValueError` (2026-09-08).** The
  `guard` panic boundary (`std::panic::catch_unwind` → `PyValueError`) in
  `crates/spectrafit-core/src/lib.rs` previously wrapped only the three JSON
  entrypoints; the two array entrypoints — the ones `spectrafit_core.fit` and
  `spectrafit_core.fit_fast` actually call — were unguarded, so a panic on
  those paths slipped past every `except Exception` handler in the Python
  layer instead of honouring the `Raises: ValueError` contract advertised by
  `_core.pyi`.
- **`gradient_norm` / the last `gradient_norm_history` entry no longer mix a
  stale Jacobian with a post-step residual on `max_iterations` termination
  (2026-09-08).** Both are now the gradient norm at the most recent point
  where the Jacobian was actually evaluated, on every termination path — the
  `MaxEval` exit that fires immediately after an accepted step used to
  recompute `‖Jᵀr‖∞` with the OLD Jacobian against the NEW (post-step)
  residual, a value that is neither the gradient at the old point nor at the
  new one. This shifts the terminal `gradient_norm`/`gradient_norm_history`
  value reported in benchmark artefacts (`python/oracles/_engine_profile.py`)
  for any case that terminates via `max_iterations`; no change to the fitted
  parameters, cost, or any other statistic.

### Deprecated
- **Both deprecated public surfaces still ship in 0.1.0 — neither is removed by
  this release (2026-09-08).** The decision is to keep them for the whole of
  0.1.0 so nothing downstream breaks on the first tagged version, and to remove
  them in a later 0.1.x. Recorded here because the code comments marking them
  said "one release cycle" without naming which one.
  - **Report wire key `geomean_speedup_vs_lmfit`** (`python/oracles/reports.py`,
    `_headline`) — superseded by `geomean_speedup_vs_baseline`, which is the
    canonical key and the one the contract's `baseline_solver_id` describes. The
    legacy alias is still emitted alongside it, carrying the same value, so
    pre-1.1-schema manifests on disk and any dashboard or gate still reading the
    old name keep parsing. Read `geomean_speedup_vs_baseline`; the alias is
    scheduled for removal after 0.1.x.
  - **Legacy `/api/*` route prefix** (`python/oracles/api.py`) — superseded by
    `/api/v1/*`. The alias stays mounted and already answers with RFC 8594
    `Deprecation: true`, `Sunset: Sat, 06 Dec 2026 00:00:00 GMT` and a `Link`
    to the canonical successor, so callers see the migration window in HTTP
    itself. Move to `/api/v1/*`; the alias is removed no earlier than that
    sunset date.
