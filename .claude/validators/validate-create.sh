...
if [[ "$FILE_PATH" =~ python/spectrafit_core/schemas.*\.py$ ]]; then
  ...
  # Validate Python AST (syntax check). Content is passed via stdin, never
  # interpolated into the Python source string — file content containing
  # ''' or other Python-syntax-breaking sequences must not be able to
  # execute as code inside this validator.
  if ! printf '%s' "$FILE_CONTENT" | python3 -c "import ast, sys; ast.parse(sys.stdin.read())" 2>/dev/null; then
    if [[ -n "$violations" ]]; then
