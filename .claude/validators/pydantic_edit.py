...
class EditValidation(BaseModel):
    ...
    def _validate_rust_pyfunction(self) -> tuple[bool, str]:
    ...
    def _validate_cargo_toml(self) -> tuple[bool, str]:
        """Cargo.toml: check DAG dependencies (simplified - full version would parse TOML)."""
        return True, "Cargo.toml DAG check passed"

    def _validate_python_schema(self) -> tuple[bool, str]:
    ...
    def _validate_results_index(self) -> tuple[bool, str]:
        """Benchmark evidence index (results_index.json) shape check."""
        try:
        ...
        has_cold_speedup_for_cold_hot = all(
            not (isinstance(item, dict) and item.get("timing_mode") == "cold_and_hot")
    ...
    def _validate_results_feedback(self) -> tuple[bool, str]:
        """Benchmark feedback gates (results_feedback.json) shape check."""
        try:
            payload = json.loads(self.content)
        except json.JSONDecodeError as exc:
    ...
    def validate_context_specific(self) -> tuple[bool, str]:
        ...
        try:
            if self.path.endswith(".rs") and "spectrafit" in self.path:
                return self._validate_rust_pyfunction()
            elif self.path.endswith("Cargo.toml"):
                return self._validate_cargo_toml()
            elif self.path.endswith(".py") and "schemas" in self.path:
                return self._validate_python_schema()
            elif self.path.endswith("results_index.json"):
                return self._validate_results_index()
            elif self.path.endswith("results_feedback.json"):
                return self._validate_results_feedback()
