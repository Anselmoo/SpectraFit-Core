...
- `Grep`/`Glob`: inspect model symbol structure for mapping.
- `Grep`: detects repeated alias/type patterns across files.
...
| `Grep`, `Glob` | Inspect Pydantic model and serde struct symbols for field mapping |
...
- ✓ Tools: Grep, Glob, Read, mcp__github__get_file_contents, mcp__context7__resolve-library-id / mcp__context7__query-docs
