...
if [ -f "$VIOL_FILE" ] && [ -s "$VIOL_FILE" ]; then
    ...
    tail -n +$((HEAD_LINES + 1)) "$VIOL_FILE" | grep -v "^$CUTOFF_DATE" > "$ARCHIVE_VIOL" 2>/dev/null || true

    # Keep header + recent records
    head -n "$HEAD_LINES" "$VIOL_FILE" > "${VIOL_FILE}.tmp"
    echo "" >> "${VIOL_FILE}.tmp"
    tail -n +$((HEAD_LINES + 1)) "$VIOL_FILE" | grep "$CUTOFF_DATE" >> "${VIOL_FILE}.tmp" 2>/dev/null || true

    OLD_COUNT=$(wc -l < "$ARCHIVE_VIOL" 2>/dev/null || echo 0)
    NEW_COUNT=$(($(wc -l < "${VIOL_FILE}.tmp") - HEAD_LINES - 1))
