...
#
# Scans .superpowers/ledgers/ for OPEN ledgers whose trunk branch is already
...
cd "$root" || exit 0
# .superpowers/, not docs/superpowers/ — superpowers artifacts moved out of the
# published Zensical tree (2026-08-02). This default MUST track the path the
# semantic-debugging skill writes to; if they drift, this reaper scans an empty
# directory and reports "no stale ledgers" forever — correct-looking, zero coverage.
LEDGER_DIR="${LEDGER_DIR:-.superpowers/ledgers}"
[ -d "$LEDGER_DIR" ] || exit 0
