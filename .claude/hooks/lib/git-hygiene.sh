...
# 4. If a `gitlab` remote exists (the actual primary/source-of-truth remote in
#    this repo — see CLAUDE.md's remote-primary-gitlab policy), fetch it
#    explicitly and compare merge-base(HEAD, gitlab/main) against gitlab/main's
#    tip DIRECTLY — not via local `main`. Check (3) trusts that local `main`
#    already mirrors `gitlab/main`; that trust silently broke on 2026-07-14
#    (local `main` sat 8 commits behind `gitlab/main`, including a concurrent
#    session's fix for the exact bug being investigated — check (3) reported
#    "no drift" because it only ever compared against the stale local ref,
#    never the remote, and the bare `git fetch` in step 1 fetches `origin`
#    (the stale GitHub mirror in this repo), not `gitlab`). This check queries
#    the remote directly so a stale local `main` can no longer mask real
#    divergence.
...
check_branch_freshness() {
    ...
    # ---- check (c): merge-base(HEAD, gitlab/main) behind gitlab/main -----
    # Only when a `gitlab` remote is configured (this repo's canonical/primary
    # remote — CLAUDE.md remote-primary-gitlab). Explicitly fetch it: the
    # generic `git fetch` in step 1 does not necessarily touch `gitlab`, and
    # local `main` can silently lag it (see the doc comment above).
    if git remote get-url gitlab >/dev/null 2>&1; then
        if command -v timeout >/dev/null 2>&1; then
            timeout 10 git fetch --quiet gitlab main 2>/dev/null || true
        else
            git fetch --quiet gitlab main 2>/dev/null || true
        fi

        if git rev-parse --verify --quiet "gitlab/main" >/dev/null 2>&1; then
            local gitlab_merge_base gitlab_head
            gitlab_merge_base="$(git merge-base HEAD "gitlab/main" 2>/dev/null)" || gitlab_merge_base=""
            gitlab_head="$(git rev-parse "gitlab/main" 2>/dev/null)" || gitlab_head=""

            if [[ -n "$gitlab_merge_base" && -n "$gitlab_head" && "$gitlab_merge_base" != "$gitlab_head" ]]; then
                local behind_gitlab
                behind_gitlab="$(git rev-list --count "$gitlab_merge_base..gitlab/main" 2>/dev/null)" || behind_gitlab="?"
                echo "git-hygiene: WARNING — this branch's base is $behind_gitlab commit(s) behind gitlab/main (the canonical remote), independent of local main's freshness. A concurrent session may have already landed work overlapping yours — run \`git log HEAD..gitlab/main --oneline\` before continuing (GIT_HYGIENE_OFF=1 to silence)." >&2
            fi
        fi
    fi
...
prune_worktrees() {
    ...
    local prune_note=""
    [[ "$pruned_count" -eq 1 ]] && prune_note=" (stale registrations were pruned)"
    echo "git-hygiene: worktree sweep — pruned stale registrations, removed $removed merged, flagged $((flagged)) for review${prune_note}" >&2
    return 0
}
