...
# Report one VIOLATION line and count it — the repeated
# `echo "VIOLATION: ..."; ((VIOLATIONS_FOUND++))` idiom factored into one call.
violation() {
    echo "VIOLATION: $1"
    ((VIOLATIONS_FOUND++))
}
...
if [ ! -d "$REPORTS_DIR" ] || [ -z "$(ls -A "$REPORTS_DIR" 2>/dev/null)" ]; then
    violation ".spectrafit_reports/ is empty — run 'pytest tests/speedboat/' first."
else
    LATEST_RUN=$(ls -d "$REPORTS_DIR"/[0-9]* 2>/dev/null | sort -n | tail -1)
    if [ -z "$LATEST_RUN" ]; then
        violation "No numbered speedboat report folder found under .spectrafit_reports/."
    else
        FEEDBACK_JSON="$LATEST_RUN/feedback.json"
        if [ ! -f "$FEEDBACK_JSON" ]; then
            violation "Missing feedback.json in latest speedboat run folder: $LATEST_RUN"
        else
            OVERALL=$(python3 -c "import json; d=json.load(open('$FEEDBACK_JSON')); print(d['gates']['overall'])" 2>/dev/null || echo "False")
            if [ "$OVERALL" != "True" ]; then
                violation "Gate failed (feedback.gates.overall=false) — see $FEEDBACK_JSON"
            fi
        fi
    fi
fi
...
if [ ! -f "$BASELINE_FILE" ]; then
    violation "Baseline file not found at $BASELINE_FILE"
    ...
    if [ "$BASELINE_HAS_DATA" -lt 1 ]; then
        violation "Baseline file exists but lacks required metrics (median, IQR, N)."
    else
...
if [ ! -f "$INDEX_FILE" ]; then
    violation "Missing results_index.json evidence file (expected benchmark/results_index.json or latest .spectrafit_reports/*/results_index.json)."
else
    ...
    if [ "$HAS_SCENARIOS" != "true" ]; then
        violation "results_index.json does not contain a non-empty scenarios array."
    fi
    if [ "$HAS_SHORT" != "true" ]; then
        violation "results_index.json missing short dataset evidence."
    fi
    if [ "$HAS_LARGE" != "true" ]; then
        violation "results_index.json missing large dataset evidence."
    fi
    if [ "$HAS_COLD_HOT" != "true" ]; then
        violation "results_index.json missing cold_and_hot timing mode evidence."
    fi
    if [ "$HAS_HOT_SPEEDUP" != "true" ]; then
        violation "results_index.json missing speedup_lmfit_over_spectrafit_hot values."
    fi
    if [ "$HAS_COLD_SPEEDUP" != "true" ]; then
        violation "results_index.json missing speedup_lmfit_over_spectrafit_cold values for cold_and_hot scenarios."
    fi

    if [ "$DIAGNOSTIC_BYPASS" -eq 1 ] && [ "$SPEEDBOAT_COMPLETE" -ne 1 ]; then
        violation "diagnostic bypass requested but speedboat evidence is incomplete (need only regression_smoke + scaling_10k with required speedup fields)."
        audit_bypass_event "rejected" "incomplete_speedboat_evidence"
    fi
fi

if [ ! -f "$FEEDBACK_FILE" ]; then
    violation "Missing results_feedback.json evidence file (expected benchmark/results_feedback.json or matching .spectrafit_reports run artifact)."
else
    ...
    if [ "$HAS_GATES_OBJECT" != "true" ]; then
        violation "results_feedback.json missing valid gates object."
    fi
    if [ "$HAS_SHORT_GATE" != "true" ]; then
        violation "results_feedback.json missing short_hot_speedup_gt_1 boolean gate."
    fi
    if [ "$HAS_LARGE_GATE" != "true" ]; then
        violation "results_feedback.json missing large_hot_speedup_gt_1 boolean gate."
    fi
    if [ "$HAS_COLD_COVERAGE_GATE" != "true" ]; then
        violation "results_feedback.json missing cold_speedup_coverage_for_cold_and_hot boolean gate."
    fi
    if [ "$HAS_OVERALL_GATE" != "true" ]; then
        violation "results_feedback.json missing overall boolean gate."
    fi
    if [ "$HAS_RECOMMENDATIONS" != "true" ]; then
        violation "results_feedback.json missing non-empty recommendations array."
    fi
    if [ "$OVERALL_PASS" != "true" ]; then
        if [ "$DIAGNOSTIC_BYPASS" -eq 1 ] && [ "$SPEEDBOAT_COMPLETE" -eq 1 ]; then
            ...
            violation "results_feedback.json overall gate is false."
        fi
