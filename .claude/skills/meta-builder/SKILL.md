...
meta-builder mostly writes new files rather than editing existing
symbols — most tasks scaffold a new skill/agent/hook from scratch.
When a task does extend an existing SKILL.md or hook, locate the
insertion point with Grep/Glob first, same as any other stream.
