...
|----------|------|------|--------------|
| 1 | **Curiosity sub-cycle** | low | Map neighbors of the stuck wire via Grep/Glob; surface surprises |
...
> its neighborhood:
> 1. `Grep` for the symbol(s) the contract names.
> 2. `Grep` for each symbol's usages — list every caller
