...
| **Stuck-mode escape** | When a wire reopens (1st → curiosity sub-cycle via Grep/Glob neighbor map, 2nd → reframe+spike, 3rd → quality-council convene). See `references/stuck-mode.md`. |
...
3. **Record `composes_with`** — the process skills (TDD,
   ...
   domain skill.
4. **Honor the Grep-first mandate** — the sub-loop's first executable step
   on any code-touching action must be a `Grep`/`Glob` call to locate the
   relevant symbol before editing it.
