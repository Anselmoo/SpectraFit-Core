...
## Shape-bound table (CLAUDE.md)

When adding a model with a long-tail shape parameter, add an entry to
`SHAPE_BOUNDS` in `python/oracles/models.py` (registered once, read by both
the lmfit and scipy-ls oracle backends):
...
usually a starting-value bias or a missing `SHAPE_BOUNDS` entry.
