...
`fmtP()` in `web/src/series/format.ts` plus the collapse-proof tick
formatter `tickLabels` in `web/src/series`; most panel bodies
(`web/src/panels/bodies/*.tsx`) call `.toFixed()` directly inline
rather than through `registry.tsx`, which has none. There
...
`Grep "PanelRecord" web/src` and `Grep "PlotMount" web/src` to find
every reference.
