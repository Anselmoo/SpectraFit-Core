...
description: |
  ...
  superpowers:test-driven-development and cupertino-council. Locate before
  editing.
license: MIT
---
...
The first action on any code-touching task **MUST** be to locate and map
the relevant symbols with Grep/Glob before editing:

```
Glob web/src/**/*.tsx                       → orient across a TSX tree
Grep "interface PanelRecord|type PanelRecord" → locate a panel definition
Grep "PanelRecord" -- web/src                → who consumes this prop / type
```

Read the matched files fully before editing so the change lands in the
...
2. **While**: this skill's reference for the subject, Grep/Glob-driven.
