...
## Locate-first recipes

- Locating the existing model trait: `Grep "Model"` in
  `crates/spectrafit-models/src/lib.rs`.
- Locating the ModelTypeStr enum: `Grep "ModelTypeStr"` in
  `crates/spectrafit-types/`.
- Finding all callers of `as_str()` before renaming:
  `Grep "ModelTypeStr::as_str"`.

## Stuck-mode entry

A reopened `ModelTypeStr` wire usually means serde rename vs `as_str()`
got out of sync — curiosity sub-cycle on `Grep "ModelTypeStr"` across
`crates/spectrafit-types/` finds the gap fast.
