...
the next; see [[andon-loop]]). Use Grep/Glob to find the symbols and their
...
- *Tooling: use MCP servers for discovery* — MAP is a symbol-discovery task first.
...
MAP begins with `Grep`/`Glob` to reconstruct who computes and who consumes the
thing under defect — never a single-file read of the failing line.

## Discovery enrichment

After Grep/Glob maps *who computes and who consumes*, extend the MAP and ENFORCE phases with
...
These are enrichment steps, not substitutions. Grep/Glob always run first; these extend
