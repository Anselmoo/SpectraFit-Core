...
1. **Locate first** — `Grep "<FieldName>"` across `python/`, `web/src/`,
   and `tests/` before renaming or removing any contract field. The
...
known caller list. Curiosity sub-cycle: `Grep "<field>"` across
`python/`, `web/src/`, `tests/`. The hidden caller is almost always a
