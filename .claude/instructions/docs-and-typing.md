...
- JSON boundary types:
  ...
  - Public FFI functions that cross to Python MUST accept/return JSON strings, with exactly one allowlisted exemption (`fit_arrays_numpy`, which returns `(json, ndarray)` — see CLAUDE.md §4). Signature pattern: `fn fit(json: &str) -> Result<String, Error>` (or `PyResult<String>` for `#[pyfunction]`). Document the expected JSON schema in the doc comment.
...
- The Python↔Rust boundary must always use JSON strings, with exactly one allowlisted exemption (`fit_arrays_numpy` — see CLAUDE.md §4). Do not pass typed Python objects across FFI directly.
