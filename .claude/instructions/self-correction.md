...
- When a new pattern, workaround, or convention is discovered mid-session, capture
  it in `.claude/instructions/<domain>.md` before the session ends.
- When editing an instruction file, check for contradictions with other rules in
  the same file and with the root `CLAUDE.md` before saving.
- Prefer adding to an existing scoped instruction file over adding to
  `CLAUDE.md` — the scoped file loads only when relevant.
...
- Duplicate a rule that already exists in `CLAUDE.md` or another
...
When adding or modifying skills in `.claude/skills/`, verify template and validator quality.
