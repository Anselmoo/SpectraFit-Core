...
3. **Use JSON strings only across the FFI boundary**
   ...
   - **Action**: Every Rust function exported via PyO3 must accept `&str` (JSON string) and return `Result<String>` (JSON string), with exactly one allowlisted exemption (`fit_arrays_numpy`, which returns `(json, ndarray)` — see CLAUDE.md §4)
