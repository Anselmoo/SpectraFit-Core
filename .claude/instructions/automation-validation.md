> Applies to: .claude/settings.json|.claude/agents/**|.claude/skills/**|.claude/instructions/**
...
7. **Routing coverage**: For `.claude/settings.json`, ensure at least one narrowed hook reinforces MCP evidence-source selection (`Serena`, `GitHub MCP`, `Context7`, `fetch_webpage`) before broad exploration.
...
**Before committing an agent to `.claude/agents/`:**
...
**Before committing a skill to `.claude/skills/`:**
