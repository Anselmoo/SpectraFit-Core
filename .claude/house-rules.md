...
7. The Pseudo-Voigt Lorentzian mixing weight is always named **`fraction`**
   — never `eta`, never `frac`.
8. **Amplitude** = peak value at the center (not the area under the curve).
9. **Width** = σ (standard deviation), not FWHM. If a model's natural
   parameterization is FWHM, convert at the boundary
   (FWHM = 2√(2 ln 2)·σ ≈ 2.355·σ) — don't expose FWHM as a bare "width"
   parameter name.
10. See `docs/reference/models/index.md` for the authoritative formula table;
   a new/changed model's docstring or param names disagreeing with it is doc
   drift, not just a style nit.

## Rust crate conventions (`crates/**`)

11. **A crate that owns an error type declares it as a `thiserror` enum in its
...
12. **Declare `thiserror` iff you use it.** A crate with no error type must not
...
13. **`lib.rs` re-exports explicit named items, never a glob.** `pub use m::*;`
    ...
    the serde wire string (rule 19) with no compile error.

14. **`unsafe` is denied workspace-wide; the FFI site is the one sanctioned
    ...
    `#![forbid(unsafe_code)]`.
15. **`problem.rs` means "the problem contract", nothing else.** In the four
...
16. **A solver-shaped crate keeps its modules private; a library-shaped crate
    ...
    *Why this matters more than it looks:* `pub mod` silently defeats rule 13.
...
17. **A package that ships declares `__all__`; an internal harness need not.**
    ...
    aligned away.
18. **Every module begins `from __future__ import annotations`** — including
...
19. **One canonical wire-format string per model.** `ModelTypeStr::as_str()`
...
20. **Python `ModelType` must mirror Rust `ModelTypeStr`.** Same member
    name, same wire value (the serde rename from rule 19). Drift between
    ...
    residual drift).
21. **A new `ModelTypeStr` variant requires the `spectrafit-builder`
...
22. After any change to `python/oracles/bench_contract.py` (or the shared
...
23. spectrafit is the subject under test; lmfit and jax/optimistix are
...
24. **`lint:python`/`lint:rust` in `.gitlab/20-lint.yml` must stay a
...
25. **GitHub's `Pre-Commit Check` runs against the squashed,
    ...
    `.superpowers/*` — superpowers artifacts live under the dot-prefixed
    `.superpowers/`, not `docs/superpowers/`) will pass locally forever and fail
...
26. **Long/slow test runs must be backgrounded and logged, never streamed
...
27. **Never load a full `results.json` from `.spectrafit_reports/**` into
...
28. **Functionality before presentation (Invariant 0).** Before any
