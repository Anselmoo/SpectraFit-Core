...
from oracles._engine_nested import (
    _order_bench_case,  # noqa: F401  — re-exported for tests
    _run_nested_adequacy,
)
...
def _capture_git_provenance() -> tuple[str | None, str | None]:
    ...
    def _run(args: list[str]) -> str | None:
        ...
        except Exception:  # noqa: BLE001 - fallback on command failure
            return None
...
def _red_chi2_weighted(
    ...
    r"""Compute $\sigma$-weighted reduced-$\chi^2$: $\sum((y-\hat{y})/\sigma)^2$ / dof.

    Returns ``(value, None)`` when $\sigma$ > 0 and dof > 0;
    ``(None, reason)`` when undefined ($\sigma$ == 0, dof $\le$ 0, or non-finite result).
    The plain stored ``reduced_chi2`` stays UNWEIGHTED — this is the separate
    $\sigma$-weighted goodness-of-fit.
    """
    import numpy as np
    ...
    chi2 = float(np.sum(((y_arr - fit_arr) / sigma) ** 2))
    val = chi2 / dof
...
def _phase_suite_regression(
    ...
    r"""Return True if any supported backend failed on *case*.

    Regression policy mirrors the accuracy gate's exclusion of the
    non-comparable categories (``CategoryDef.baseline_comparable = False``:
    ``optfn`` and ``robust``; ``reports.py:_headline`` applies the same set to
    $|\Delta r^2|$).
    ...
    for b in backends:
        ...
        if o is None or not o.success:
            if case.category in NON_COMPARABLE_CATEGORIES and b.name != "spectrafit":
                continue
            return True
    return False
...
def run_suite(
    ...
    r"""Fit every case with every supported backend → the 139-row suite.

    ``baseline_solver_id`` names the solver whose median runtime is the speedup
    denominator ($\times 1.0$ by construction); was hardcoded ``"lmfit"`` and is now
    threaded through the contract so adding a fourth backend or retiring lmfit
    doesn't ripple across the codebase.

    Note:
        Non-finite ``r2``/``reduced_chi2`` are guarded at the producer: a model
        overflow (``ss_res=inf`` → ``r2=-inf``) or a ``chi2``/``dof`` combination
        with no finite guard (jax/scipy) must NOT raise a ``ValidationError``
        from the contract and sink the run. Such a fit is instead treated as
        failed (``success=False``) with safe finite sentinels substituted for
    ...
    for case in catalog:
        ...
        for name, o in outcomes.items():
            ...
            # $\sigma$-weighted reduced-$\chi^2$: defined when $\sigma > 0$ (noiseless optfn cases get None).
            ...
            if o.history_source == "real" and len(ch) >= 2 and o.n_iter > 0:
                ...
                ce = None
            # ill_conditioned: $\kappa(J) \ge 10^6$ signals a rank-deficient Jacobian.
        ...
        winner = max(
            metricmap,
            key=lambda s: metricmap[s].r2 * metricmap[s].speedup,
            default="",
        )
...
def _decim_indices(n: int, cap: int = _DECIM_CAP) -> list[int]:
    r"""Strided indices for curve plotting at $\le$ ``cap`` points.
...
def _phase_model_provenance(case: BenchCase) -> tuple[str | None, str | None]:
    """Source file + formula from the oracle model registry for one case.

    Takes the model of the first component (the primary shape of the case).
    Returns (None, None) for landscape / optfn cases without a registered
    peak model, or when the spec carries no components at all.
    """
    ...
    return model_source_file, model_formula
...
def run_featured(
    ...
    *,
    ...
    baseline_solver_id: str = "lmfit",
    raw_sink: dict[tuple[str, str], dict[str, Any]] | None = None,
    audit: bool = False,
) -> Featured:
    ...
    Note:
        The nested-model adequacy V&V fits orders m*-1/m*/m*+1 on the subject
        and verifies that model-selection criteria (LRT/F/AIC/BIC) recover the
        known generative order. It is only computed for cases with a known
        order (synthetic, at least 2 peaks).
    """
    ...
    model_source_file, model_formula = _phase_model_provenance(case)
...
def build_report(
    ...
    baseline_solver_id: str = "lmfit",
    audit_sink: dict[tuple[str, str], dict[str, Any]] | None = None,
) -> BenchReport:
    r"""Run the benchmark and assemble the report payload.
    ...
    Note:
        The returned report's ``manifest`` field is populated so the web
        GateBadge can render the four gate numbers (geomean / max $|\Delta r^2|$ /
        win-rate / pinned ratio) from the typed contract instead of telling
        the user to run ``spc-bench show-baseline``. ``compute_manifest_signals``
        shares math with ``reports._headline`` so the dict and the contract
    ...
    raw_sink: dict[tuple[str, str], dict[str, Any]] = audit_sink if audit_sink is not None else {}
    ...
    return report.model_copy(update={"inference": compute_inference(report, raw_sink)})
