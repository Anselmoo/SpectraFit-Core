...
class SelectionStats(BaseModel):
    r"""Selection statistics for nested model comparison.
...
def selection_stats(
    ...
    r"""Compute nested-model selection statistics.

    Args:
        rss_reduced (float): Residual sum of squares for the reduced model.
        rss_full (float): Residual sum of squares for the full model.
        k_reduced (int): Number of parameters in reduced model.
        k_full (int): Number of parameters in full model.
    ...
    # LRT: $n \ln(\mathrm{RSS}_{red}/\mathrm{RSS}_{full}) \sim \chi^2(dk)$ under $H_0$ (reduced adequate)
...
class NestedAdequacy(BaseModel):
    r"""Result of a nested-order V&V against a known generative order.
...
def nested_adequacy(
    ...
    r"""Oracle: verify that nested-model selection criteria recover the known generative order.
    ...
    tri-Gaussian, $\Delta$AIC = -1.12 (AIC prefers the 4-peak model) while $\Delta$BIC = +8.46
    (BIC correctly recovers the true 3-peak order).

    Args:
        true_order (int): Known generative model order (m*).
        fit_order (Callable[[int], tuple[float, int]]): Callable ``fit_order(order) -> (rss, k)`` supplying
            residual sum of squares and parameter count for that order.
        n (int): Number of observations.
        alpha (float): Significance threshold for the LRT reduced-model rejection test.
    ...
    return NestedAdequacy(
        ...
        recovered_true_order_aic=(
            reduced_rejected and over_not_preferred_aic and (selected_order_aic == true_order)
        ),
        recovered_true_order_bic=(
            reduced_rejected and over_not_preferred_bic and (selected_order_bic == true_order)
        ),
