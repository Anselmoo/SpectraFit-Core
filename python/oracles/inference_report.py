...
def _compute_equivalence(
    ...
    r"""Per-category equivalence (TOST): EVERY non-baseline backend vs baseline.

    Note:
        Uses PAIRED per-case $\Delta r^2$ (subject - baseline) on cases where both ran.
        Paired is correct here: an unpaired test would inflate the standard
        error with case-to-case $r^2$ spread and wrongly reject equivalence (the
        saturation claim is a per-case backend comparison, not a comparison
        of category means).
    """
    baseline = report.baseline_solver_id
    equivalence: list[EquivalenceResult] = []
    cats: dict[str, list[SuiteCase]] = {}
    ...
    for cat, rows in cats.items():
        ...
        for bn in sorted(backends):
            deltas = [r.m[bn].r2 - r.m[baseline].r2 for r in rows if bn in r.m and baseline in r.m]
            if len(deltas) < 2:
                continue
            tested_any = True
...
def _compute_winner_stability(
    ...
    return winner_stability(scores, b=cfg.bootstrap_b, seed=cfg.seed) if scores else {}
...
def _compute_calibration(
    ...
    """Aggregate subject pulls from all deep-dive analyzed cases.
    ...
    return CalibrationResult.model_validate(cal_stat.model_dump())


def _compute_speed_inference(
    cases: list[CaseInference],
    cfg: InferenceConfig,
) -> SpeedInferenceResult:
    """Bootstrap CI over per-case subject speedups.
    ...
    *best non-baseline backend* selected by composite score (r2 * speedup), NOT
    strictly the history_source=="real" subject used by W10 calibration.
    ...
    # Engine→contract bridge: snake_case model_dump() keys are accepted by the
    # contract views because _Base sets populate_by_name=True (do not remove that).
    return SpeedInferenceResult.model_validate(spd_stat.model_dump())
...
def compute_inference(
    ...
    cases = _compute_case_inferences(report, raw_sink, cfg)
    equivalence = _compute_equivalence(report, cfg)
    stab = _compute_winner_stability(report, cfg)
    calibration = _compute_calibration(report, cfg)
    speed_inference = _compute_speed_inference(cases, cfg)
