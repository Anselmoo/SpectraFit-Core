...
Note:
    The ``gate`` command used to mix run resolution, evidence loading, axis-summary
    formatting, the self-vs-self perf check, and final JSON/human emission in one
    function. ``_gate_resolve_run``, ``_gate_load_inference_evidence``,
    ``_gate_axis_summary``, ``_gate_self_perf_check``, and ``_gate_emit_verdict``
    split those cohesive sub-steps out; ``gate`` itself is now a thin sequencing
    wrapper (a pure structural move, no behavior change).
"""
...
from oracles.backends import Backend, get_backends
...
def _resolve_run_backends(
...
) -> tuple[list[Backend], str]:
    """Resolve --backends/--baseline into a concrete backend list + baseline id.

    ``backends_arg`` is ``None`` -> every available backend, baseline defaults to
    ``"lmfit"`` (unchanged full-suite behavior). When given, it's a comma-separated
    list of ``Backend.name`` values (e.g. ``"spectrafit"``); baseline then defaults
    to ``"lmfit"`` if present in the filtered list, else the first name in
    ``backends_arg`` -- so a single-backend run (``--backends spectrafit``) gets a
    well-defined, self-consistent baseline without needing ``--baseline`` too.
    """
    available = {b.name: b for b in get_backends()}
    if backends_arg is None:
        resolved = list(available.values())
    else:
        requested = [n.strip() for n in backends_arg.split(",") if n.strip()]
        if not requested:
            msg = "no backend names given (empty --backends value)"
        ...
        resolved = [available[n] for n in requested]
    if baseline_arg is not None:
        resolved_names = {b.name for b in resolved}
        if baseline_arg not in resolved_names:
            msg = (
                f"baseline '{baseline_arg}' did not run "
                f"(available: {', '.join(sorted(resolved_names))}) -- "
                "an unresolved baseline silently falls back to a vacuous "
                "speedup=1.0/Δr²=0.0 self-comparison"
            )
            raise typer.BadParameter(
                msg,
                param_hint="--baseline",
    ...
    elif backends_arg is None or "lmfit" in {b.name for b in resolved}:
        baseline_id = "lmfit"
    else:
        baseline_id = resolved[0].name
...
def run(
    ...
    mc: int = typer.Option(
        20,
        help="Monte-Carlo noise realizations for the featured case.",
    ),
    ...
    backends: str | None = typer.Option(
        ...
        "--backends",
        help=(
            "Comma-separated backend names to run (default: all available -- "
            "spectrafit, lmfit, jax, scipy-ls-lm/trf/dogbox). e.g. "
            "--backends spectrafit for a fast, single-backend smoke run."
        ),
    ),
    baseline: str | None = typer.Option(
        ...
        "--baseline",
        help=(
            "Baseline solver id for speedup/Δr² (default: 'lmfit', or the first "
            "--backends entry if lmfit isn't in the selected set)."
        ),
    ),
) -> None:
    ...
    Note:
        Backend regressions are summarized on one echoed line rather than left as
        per-case warning spam — that spam is what tempted an operator to ^C the
        ``report_html`` sequence before it could finish.
    """
    resolved_backends, baseline_solver_id = _resolve_run_backends(backends, baseline)
    audit_sink: dict[tuple[str, str], dict[str, Any]] = {}
    report = build_report(
        n_reps=reps,
        n_mc=mc,
...
def _gate_geomean(manifest: dict[str, Any]) -> float:
...
def _gate_axis_level(
    ...
    *,
    ...
    r"""Resolve a single gate-axis value to a canonical ``GateState``.
    ...
    - **accuracy** (max $|\Delta r^2|$) and **regressions** (count):
    ...
    if higher_is_better:
        ...
        if warn_threshold is not None and value < warn_threshold:
            return "warn"
        return "pass"
    # e.g. max $|\Delta r^2|$, regression count: fail if too high, warn if creeping up
    if value > fail_threshold:
        return "fail"
    if warn_threshold is not None and value > warn_threshold:
        return "warn"
...
class GateThresholds(BaseModel):
    ...
    Typer flags plus the optional ``--warn-*`` floors. Defaults match
...
def _append_bool_axis(
    axes: list[GateAxisResult],
    axis: GateAxisName,
    *,
    passed: bool,
) -> None:
    """Append one evidence-conditional boolean-verdict gate axis.

    Shared by the ``model_selection`` / ``sigma_calibration`` / ``speed_inference``
    axes in :func:`_gate_evaluate` — each maps a boolean pass/fail record to a
    ``GateAxisResult`` through the same fixed convention: 1.0 = pass, 0.0 = fail,
    ``higher_is_better=True``, ``fail_threshold=1.0``, no warn tier. Callers remain
    responsible for the evidence-conditional gating itself (``is not None`` /
    ``not skipped``) — this helper only performs the append once that condition has
    already been satisfied, so absent evidence never reaches here (no
    pass-by-absence).
    """
    value = 1.0 if passed else 0.0
    state = _gate_axis_level(
        value,
        fail_threshold=1.0,
        warn_threshold=None,
        higher_is_better=True,
    )
    axes.append(GateAxisResult(axis=axis, state=state, value=value, threshold=1.0))


def _gate_evaluate(
    manifest: dict[str, Any] | ManifestSignals,
    ...
    *,
    ...
    speed_state = _gate_axis_level(
        geomean,
        thresholds.min_geomean,
        thresholds.warn_geomean,
        higher_is_better=True,
    )
    ...
    # A non-finite $|\Delta r^2|$ is a hard accuracy failure: `_gate_axis_level` compares
    ...
    regressions_state = _gate_axis_level(
        ...
        float(thresholds.warn_regressions) if thresholds.warn_regressions is not None else None,
        higher_is_better=False,
    )
    ...
    if nested_adequacy is not None:
        _append_bool_axis(
            axes,
            "model_selection",
            passed=nested_adequacy.recovered_true_order_bic,
        )
    ...
    if calibration is not None and not calibration.skipped:
        _append_bool_axis(axes, "sigma_calibration", passed=calibration.passed)
    ...
    if speed_inference is not None and not speed_inference.skipped:
        _append_bool_axis(axes, "speed_inference", passed=speed_inference.passed)
...
def _gate_resolve_run(
    category: str,
    *,
    as_json: bool,
) -> tuple[dict[str, Any], Path, str, str]:
    """Locate the latest run + manifest for the gate, or exit loudly.

    Returns ``(manifest, path, run_id, baseline_id)``. Raises ``typer.Exit(2)``
    (after echoing an error in the JSON or human format matching ``as_json``)
    when the latest run under ``category`` is missing ``results.json`` or when
    no run exists at all — refusing to silently gate on stale/absent data.
    """
    ...
    return manifest, path, run_id, baseline_id


def _gate_load_inference_evidence(
    path: Path,
) -> tuple[CalibrationResult | None, SpeedInferenceResult | None]:
    """Extract the evidence-conditional inference records from results.json.

    Mirrors the extraction pattern in ``oracles.audit.runner`` — camelCase
    keys, try/except guarded so a missing or malformed inference block never
    causes a false gate failure (returns ``(None, None)`` on any error).
    """
    calibration: CalibrationResult | None = None
    speed_inference: SpeedInferenceResult | None = None
    try:
        results_raw = json.loads(path.read_text(encoding="utf-8"))
        inf = results_raw.get("inference") or {}
        if inf.get("calibration"):
            calibration = CalibrationResult.model_validate(inf["calibration"])
        if inf.get("speedInference"):
            speed_inference = SpeedInferenceResult.model_validate(inf["speedInference"])
    except Exception:  # noqa: BLE001 - defensive: parse error / missing build
        calibration = None
        speed_inference = None
    return calibration, speed_inference


def _gate_axis_summary(
    report: GateReport,
    *,
    baseline_id: str,
    min_geomean: float,
    max_dr2: float,
    max_regressions: int,
    warn_geomean: float | None,
    warn_dr2: float | None,
    warn_regressions: int | None,
    say: Callable[..., None],
) -> tuple[
    float,
    list[str],
    ...
    """Build the per-axis JSON-shape dict + failure/warning messages.

    Echoes the headline summary line via ``say`` (suppressed in --json mode by
    the caller's closure), then walks the speed/accuracy/regressions verdicts
    from ``report`` into the legacy ``axes`` dict shape the --json consumers
    read, plus the human-readable failure/warning message lists.

    Returns ``(geomean, regression_ids, axes, failures, warnings)``.
    """
    ...
    axes: dict[str, dict[str, float | int | str | bool | None]] = {
        "speed": {
            "value": geomean,
    ...
    match accuracy_level:
        case "fail":
            failures.append(
                f"accuracy parity broken (max |Δr²| {dr2:.2e} > {max_dr2:.0e})",
            )
        case "warn":
            warnings.append(
                f"accuracy drift: max |Δr²| {dr2:.2e} above warn floor {warn_dr2:.0e}",
            )
    match regressions_level:
        case "fail":
            preview = ", ".join(reg_ids[:6]) + ("…" if len(reg_ids) > 6 else "")
            failures.append(
    ...
    return geomean, reg_ids, axes, failures, warnings


def _gate_self_perf_check(
    *,
    category: str,
    baseline_id: str,
    geomean: float,
    perf_tolerance: float,
    warn_self_perf: float | None,
    axes: dict[str, dict[str, float | int | str | bool | None]],
    failures: list[str],
    warnings: list[str],
    say: Callable[..., None],
) -> dict[str, str | bool | float | None] | None:
    """Self-vs-self perf gate — only fires when a baseline has been pinned.

    Refuses to compare across mismatched contexts (different baseline solver /
    category): the pin was honest about its assumptions; do not silently grade
    against them. Mutates ``axes``/``failures``/``warnings`` in place (adding
    the ``self_perf`` axis + any failure/warning message) and returns the
    ``pin_summary`` dict for the --json payload (``None`` when no baseline is
    pinned).
    """
    pin = read_perf_baseline()
    if pin is None:
        return None
    if pin.get("category") != category or pin.get("baseline_solver_id") != baseline_id:
        say(
...
def _gate_emit_verdict(
    *,
    ...
    axes: dict[str, dict[str, float | int | str | bool | None]],
    reg_ids: list[str],
    pin_summary: dict[str, str | bool | float | None] | None,
    failures: list[str],
    warnings: list[str],
) -> None:
    """Compute the overall status and print/exit exactly like the original inline tail.

    Overall status is the worst per-axis level via the shared
    ``_worst_gate_state`` helper (same ``fail > warn > pass`` rank as
    ``_compute_default_gate_state``). WARN exits 0 (CI doesn't fail) but the
    JSON status is ``"warn"`` so the step-summary block renders amber.

    Note:
        The ``--json`` payload is a single object on stdout so a CI script can
        ``jq -r '.status'``. All four gate axes plus the regression case IDs are
        surfaced so the consumer can post per-file annotations (e.g. GitHub Actions
        ``::error file=...::``) without having to re-read ``manifest.json``.
    """
    # `failures`/`warnings` lists drive the message emission below; the
    ...
    level_list: list[GateState] = ["fail"] * len(failures) + ["warn"] * len(warnings)
    all_levels: Sequence[GateState] = level_list or ["pass"]
    status: GateState = _worst_gate_state(all_levels)
    if as_json:
        result = {
            "status": status,
            "run_id": run_id,
...
def gate(
    min_geomean: float = typer.Option(
        1.0,
        help="Fail if geomean speedup vs the baseline solver is below this.",
    ),
    max_dr2: float = typer.Option(
        1e-3,
        help="Fail if max |Δr²| vs the baseline solver exceeds this.",
    ),
    ...
    *,
    as_json: bool = typer.Option(
        False,  # noqa: FBT003 — typer.Option's own constructor, default is conventionally positional
        "--json",
        help=(
    ...
    Note:
        Each axis has an optional WARN threshold sitting between today's hard
        floor and the headroom. WARN exits 0 (so CI doesn't fail) but stamps
        ``status="warn"`` in the JSON and the matching axis's ``level="warn"`` for
        amber annotation. Omitting all ``--warn-*`` flags keeps the binary
        PASS/FAIL behaviour bit-for-bit.
    """
    ...
    manifest, path, run_id, baseline_id = _gate_resolve_run(category, as_json=as_json)

    # Extract inference evidence from results.json for the evidence-conditional gate
    # axes (W10/W11: sigma_calibration + speed_inference).
    _gate_calibration, _gate_speed_inference = _gate_load_inference_evidence(path)
    ...
    geomean, reg_ids, axes, failures, warnings = _gate_axis_summary(
        report,
    ...
    pin_summary = _gate_self_perf_check(
        category=category,
        baseline_id=baseline_id,
        geomean=geomean,
        perf_tolerance=perf_tolerance,
        warn_self_perf=warn_self_perf,
        axes=axes,
        failures=failures,
        warnings=warnings,
        say=_say,
...
def pin_baseline(
    ...
    if path is None:
        typer.echo(
            f"pin-baseline: no results under {REPORTS_ROOT / category}",
            err=True,
        )
        raise typer.Exit(2)
    manifest = json.loads((path.parent / "manifest.json").read_text(encoding="utf-8"))
...
def _fmt_dr2(value: float) -> str:
    r"""Format max $|\Delta r^2|$ in scientific notation (``1.3e-04``)."""
    return f"{value:.1e}"
...
def _narrate_from_manifest(manifest: dict[str, Any]) -> str:
    ...
    Note:
        The regressions clause is worth spelling out even for zero ("No
        regressions.") and, deliberately unlike the truncated regression-id
        previews used elsewhere in this module, never truncates the id list —
        the paragraph is written for release notes, not logs.
    """
    ...
    return (
        ...
        f"categories (optfn and robust excluded by design). spectrafit wins "
...
# ---------------------------------------------------------------------------
# spc-bench trend — per-axis history viewer
# ---------------------------------------------------------------------------
...
def trend(
    ...
    Note:
        The tracked field set (:data:`_TREND_FIELDS`) matches ``manifest.json``
        keys; its four gate-axis fields match ``spc-bench gate``, so a
        green-then-red trend in any of those columns reads the same as the gate
        failing — just with history attached.
    """
    ...
    for key in fields_to_show:
        columns[key] = [
            int(r.get(key, 0) or 0) if key == "regressions" else float(r.get(key, 0.0) or 0.0)
...
def sweep(
    ...
    category: str = typer.Option(
        "sweep",
        help="Subcategory under .spectrafit_reports/ (default: sweep).",
    ),
) -> None:
    ...
    Note:
        Each tier reuses ``build_report`` + ``write_run`` so it lands as a full,
        gate-able run in the same shape any other ``spc-bench run`` produces —
        sweep adds no new contract, it's just an orchestrator. To read the
        summary table: scan the `regressions` column for swings (1→0→1 with
        budget suggests timing noise, not real signal) and the `geomean` column
        for convergence (the high-tier geomean is the honest number).
    """
    try:
        tier_list = [int(t.strip()) for t in tiers.split(",") if t.strip()]
    except ValueError as exc:
        typer.echo(
            f"sweep: invalid --tiers ({exc}); expected comma-separated ints",
            err=True,
        )
...
@app.command()
def stability(
    ladder_dir: Path = typer.Argument(  # noqa: B008 - typer's own idiom
        ...,
        help=(
    ...
    out: Path = typer.Option(  # noqa: B008 - typer's own idiom
        ...,
        "--out",
        help="Output directory for stability.json + stability.md.",
    ),
) -> None:
    r"""Aggregate a reps-ladder of runs into variance-vs-N stability artifacts.
    ...
    - ``stability.md`` — a reps $\times$ backend table of geomean speedups with signed
      $\Delta$% vs the highest-reps reference, plus a converged-at-N verdict.
    ...
    typer.echo(
        f"stability study over reps ladder {study.reps_ladder} → {json_path} + {md_path}",
    )
