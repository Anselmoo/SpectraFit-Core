...
def _resolve_run_backends(
    backends_arg: str | None, baseline_arg: str | None
    ...
    if backends_arg is None:
        ...
        if not requested:
            raise typer.BadParameter(
                "no backend names given (empty --backends value)",
                param_hint="--backends",
            )
...
def _append_bool_axis(
    axes: list[GateAxisResult], axis: GateAxisName, passed: bool
) -> None:
    ...
    state = _gate_axis_level(
        value, fail_threshold=1.0, warn_threshold=None, higher_is_better=True
    )
...
def _gate_evaluate(
    ...
    *,
    ...
    if nested_adequacy is not None:
        _append_bool_axis(
            axes, "model_selection", nested_adequacy.recovered_true_order_bic
        )
    ...
    if calibration is not None and not calibration.skipped:
        _append_bool_axis(axes, "sigma_calibration", calibration.passed)
    ...
    if speed_inference is not None and not speed_inference.skipped:
        _append_bool_axis(axes, "speed_inference", speed_inference.passed)
...
# ---------------------------------------------------------------------------
# Structural extraction of the Typer ``gate`` command below: the command was
# mixing run resolution, evidence loading, axis-summary formatting, the
# self-vs-self perf check, and final JSON/human emission in one function.
# The five helpers below split those cohesive sub-steps out; ``gate`` itself
# is now a thin sequencing wrapper. Pure structural move — no behavior change.
# ---------------------------------------------------------------------------


def _gate_resolve_run(category: str, as_json: bool) -> tuple[dict, Path, str, str]:
...
def _gate_axis_summary(
    ...
    *,
    ...
    say: Callable[..., None],
) -> tuple[float, list[str], dict[str, dict], list[str], list[str]]:
...
def gate(
    ...
    manifest, path, run_id, baseline_id = _gate_resolve_run(category, as_json)
