...
def _build_fit_order(
    ...
    r"""Build a ``fit_order(order) -> (rss, k)`` closure for the featured case.

    Args:
        case (BenchCase): The featured materialized case — its ``x``/``y`` are the fixed
            data.
        spectrafit_backend (Backend): The subject backend (spectrafit); all order fits
            use this backend.

    Returns:
        ``(true_order, fit_order)`` where ``true_order`` is the generative
        peak count (m*), and ``fit_order(order)`` fits the case at the
        requested order and returns ``(rss, k)`` where $\mathrm{rss} = \sum \mathrm{resid}^2$ and
        ``k`` = fitted free-parameter count. A failed fit returns
        ``(rss_null, k)`` where ``rss_null`` is the null-model RSS
        (fit = mean(y)) — never a fabricated zero — with a loud-skip log.
    """
    ...
    # Null RSS: $\sum(y - \bar{y})^2$ — used as a fallback when a fit fails so downstream
    ...
    def fit_order(order: int) -> tuple[float, int]:
        ...
        match order - true_order:
            ...
            case _:
                ...
                except Exception as exc:  # noqa: BLE001 - fallback to raw residuals
                    _LOG.warning(
                        "over-order seed fit failed for case %s: %r; "
                        "falling back to raw y as the residual proxy for seeding",
                        case.id,
                        exc,
                    )
                    residuals = y
...
def _order_bench_case(
    ...
    guess = [
        c.model_copy(
            update={k: v * (1.0 + rng.uniform(-0.05, 0.05)) for k, v in c.to_params().items()},
        )
        for c in comps
    ]
...
def _run_nested_adequacy(
    ...
    except Exception as exc:  # noqa: BLE001 - log and skip nested adequacy
