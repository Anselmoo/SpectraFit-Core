...
def _wheel_eval(model_type: str, x: Array, params: dict[str, float]) -> Array:
    ...
    graph_json = json.dumps(
        {"schema_version": "0.1", "nodes": nodes, "expr_edges": []}
    )
    params_json = json.dumps(
        {f"{node_id}.{k}": float(v) for k, v in params.items()}
    )
...
_BUILTIN_MODELS: tuple[PeakModel, ...] = (
    ...
    PeakModel(
        ...
        formula_latex=(
            r"A \cdot \dfrac{\cos\!\left(\tfrac{\pi\gamma}{2}+\arctan\!\left(\tfrac{x-c}{\sigma}\right)\right)}"
            r"{\left((x-c)^2+\sigma^2\right)^{(1-\gamma)/2}}"
        ),
    ),
    PeakModel(
        key="log_normal",
    ...
    PeakModel(
        ...
        evaluate=pearson7,
        jax_supported=False,
        formula_latex=r"A \cdot \left(1 + \left(\dfrac{x-c}{\sigma}\right)^{\!2}\right)^{\!-m}",
    ),
    PeakModel(
        key="split_gaussian",
    ...
    PeakModel(
        ...
        jax_supported=False,
        formula_latex=(
            r"A \cdot \exp\!\left(-\dfrac{(x-c)^2}{2\sigma^2(1+k(x-c))^2}\right)"
        ),
    ),
    PeakModel(
        key="harmonic_ir",
        ...
        jax_supported=False,
        formula_latex=r"A \cdot \left(1 - \left(\dfrac{x-c}{\sigma}\right)^{\!2}\right)^{\!2} \cdot \Theta(1-|x-c|/\sigma)",
    ),
    PeakModel(
        key="tauc",
