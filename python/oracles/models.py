...
from scipy.special import (
    erfcx,  # scaled erfc, exp(x²)·erfc(x) — overflow-free EMG tail
...
# Finite bounds for the long-tail shape parameters an oracle's LM-family search
# can otherwise drive into a model function's overflow/NaN region (e.g. pearson7
# ``m → 0⁺`` makes ``2^{1/m}`` overflow → NaN → the solver aborts). The truth-side
# generators in ``cases.py`` draw from much narrower ranges; these envelopes are
# ~10x wider so an oracle stays independent (it can still disagree with spectrafit)
# but cannot reach a numerically degenerate corner. Keyed by shape-parameter name
# (pearson7 ``m``, split_pearson7 ``m_l``/``m_r``, moffat ``beta``, students_t
# ``nu``, fano``/``breit_wigner ``q``, asym_ir ``k``). Registered once here so the
# lmfit and scipy-ls oracle backends read the same table instead of hand-syncing
# two private copies (the CX-033 NaN cascade that originally motivated this table
# is the precedent for keeping it registry-backed, not duplicated).
