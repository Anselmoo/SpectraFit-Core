...
def pseudo_voigt(
    ...
    r"""Pseudo-Voigt: $\mathrm{fraction}\cdot\text{Lorentzian} + (1-\mathrm{fraction})\cdot\text{Gaussian}$ (peak ``amplitude``).
...
def fano(x: Array, amplitude: float, center: float, gamma: float, q: float) -> Array:
    r"""Fano resonance: $A\cdot(q+\varepsilon)^2/(1+\varepsilon^2)$, $\varepsilon=(x-\mathrm{center})/\gamma$."""
    eps = (x - center) / gamma
...
def true_voigt(
    ...
    r"""True Voigt (Gaussian $\otimes$ Lorentzian) via the Faddeeva fn, peak height ``amplitude``.
...
def skewed_gaussian(
    ...
    r"""Skewed Gaussian $A\exp(-\tfrac12((x-c)/\sigma)^2)\cdot(1+\mathrm{erf}(\gamma(x-c)/(\sigma\sqrt2)))$ ($\gamma$ = skew)."""
    dx = x - center
...
def doniach_sunjic(
    ...
    r"""Doniach–Šunjić $A\cos[\pi\gamma/2+(1-\gamma)\arctan(u)]/(1+u^2)^{(1-\gamma)/2}$, $u=(x-c)/\sigma$ ($\gamma$ = asym)."""
    u = (x - center) / sigma
...
def log_normal(x: Array, amplitude: float, center: float, sigma: float) -> Array:
    r"""Log-normal peak $A\exp(-(\ln(x/\mathrm{center}))^2/(2\sigma^2))$ for ``x>0`` (else 0).
...
def pearson7(
    ...
    r"""Pearson VII $A/[1+((x-c)/\sigma)^2\cdot(2^{1/m}-1)]^m$; $\sigma$ = HWHM, m→1 Lorentzian, m→$\infty$ Gaussian.
...
def split_gaussian(
    ...
    r"""Split (asymmetric) Gaussian — width ``sigma_l`` for x<center, ``sigma_r`` for $x \ge$ center.

    Covers catalog #6 (asymmetric split-$\sigma$ Gaussian) and #10 (bi-Gaussian) — the same
...
def moffat(
    ...
    r"""Moffat $A/(((x-c)/\sigma)^2+1)^\beta$; parity oracle for the Rust ``moffat`` kernel."""
...
def students_t(
    ...
    r"""Student's-t $A/(1+((x-c)/\sigma)^2/\nu)^{(\nu+1)/2}$; oracle for the Rust ``students_t`` kernel."""
...
def breit_wigner(
    ...
    r"""Breit-Wigner-Fano $A\cdot(qg+(x-c))^2/(g^2+(x-c)^2)$, $g=\sigma/2$; oracle for the Rust kernel."""
    g = sigma / 2.0
    return amplitude * (q * g + (x - center)) ** 2 / (g * g + (x - center) ** 2)


def asym_ir(x: Array, amplitude: float, center: float, sigma: float, k: float) -> Array:
    r"""Asymmetric IR band $A\cdot G\cdot\text{sigmoid}$; sigmoid exponent clamped to match the Rust kernel."""
...
def harmonic_ir(x: Array, amplitude: float, center: float, sigma: float) -> Array:
    r"""Harmonic-oscillator IR $A/((c^2-x^2)^2+(\sigma x)^2)$; oracle for the Rust ``harmonic_ir`` kernel."""
    return amplitude / ((center**2 - x**2) ** 2 + (sigma * x) ** 2)


def tauc(x: Array, amplitude: float, e_gap: float, exponent: float) -> Array:
    r"""Tauc band-gap edge $A\cdot(x-e_{\mathrm{gap}})^p$ for ``x>e_gap`` (else 0); oracle for the Rust kernel.
...
def cauchy_dispersion(x: Array, a: float, b: float, c: float) -> Array:
    r"""Cauchy dispersion $n(x)=a+b/x^2+c/x^4$ for ``x>0`` (else 0); oracle for the Rust kernel."""
    with np.errstate(divide="ignore", invalid="ignore"):
        val = a + b / x**2 + c / x**4
    return np.where(x > 0.0, val, 0.0)
...
def kww(x: Array, amplitude: float, tau: float, beta: float) -> Array:
    r"""KWW stretched exponential $A\exp(-(x/\tau)^\beta)$ for $x \ge 0$ (else 0); oracle for the Rust kernel.

    The base $x/\tau$ is masked to a safe ``1.0`` where ``x<0`` so the fractional power
...
def saturating_exponential(x: Array, amplitude: float, rate: float) -> Array:
    r"""Saturating exponential $\mathrm{amplitude}\cdot(1-\exp(-\mathrm{rate}\cdot x))$ (BoxBOD model).
...
def power_saturation(x: Array, amplitude: float, rate: float) -> Array:
    r"""Power-law saturation $\mathrm{amplitude}\cdot(1-(1+\mathrm{rate}\cdot x/2)^{-2})$ (Misra1b model).
...
def power_law_offset(x: Array, amplitude: float, offset: float, shape: float) -> Array:
    r"""Power-law with offset $\mathrm{amplitude}\cdot(\mathrm{offset}+x)^{-1/\mathrm{shape}}$ (Bennett5 model).
...
_BUILTIN_MODELS: tuple[PeakModel, ...] = (
    ...
    PeakModel(
        ...
        formula_latex=(r"A \cdot \exp\!\left(-\dfrac{(\ln x - c)^2}{2\sigma^2}\right),\quad x>0"),
    ),
    PeakModel(
        key="pearson7",
