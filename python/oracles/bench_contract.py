...
class BackendProfile(_Base):
    ...
    jacobian_condition_number: float | None = Field(
        default=None,
        description=(
            "κ(J) at convergence. Structural property of the case, not the "
            "solver. κ ≥ 1e6 → ill-conditioned (explains backend disagreement); "
            "κ ≤ 1e4 → well-posed. Carried through BackendOutcome; backends "
            "that don't expose a Jacobian leave this None (scipy-ls backends "
            "populate it from raw.jac)."
        ),
    )
...
class BenchReport(_Base):
    ...
    manifest: ManifestSignals | None = Field(
        ...
        description=(
            ...
            "max |Δr²| / win-rate / pinned ratio without a sidecar fetch. "
