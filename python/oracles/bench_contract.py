...
fails if the two drift (the guard that closes the SP-3-class latent-red
...
from oracles.contract import SolverMeta  # re-export; class lives in oracles.contract
...
class AccuracyDist(_Base):
    r"""Per-solver reduced-$\chi^2$ distribution over repetitions."""
...
class Uncertainty(_Base):
    r"""Parameter-pull diagnostics: pulls = $(\mathrm{est}-\mathrm{truth})/\sigma$, coverage, reported $\sigma$.

    ``coverage`` is ``None`` when no valid $\sigma$ was available for any MC sample
...
class SpreadPt(_Base):
    r"""A mean $\pm$ sd point versus run count (param-spread / stability series)."""
...
class StabilityEntry(_Base):
    r"""One backend's metric stability versus run count (mean $\pm$ sd per metric)."""
...
class MultiDim(_Base):
    r"""Genuinely N-dimensional ($\ge$3-D) fit showcase (SP-2).
...
class BackendProfile(_Base):
    r"""All of ONE backend's featured-case metrics, grouped (no parallel dicts).
    ...
    jacobian_condition_number: float | None = Field(
        default=None,
        description=(
            r"$\kappa(J)$ at convergence. Structural property of the case, not the "
            r"solver. $\kappa \ge$ 1e6 → ill-conditioned (explains backend "
            r"disagreement); $\kappa \le$ 1e4 → well-posed. Carried through "
            r"BackendOutcome; backends that don't expose a Jacobian leave this None "
            r"(scipy-ls backends populate it from raw.jac)."
        ),
    )
    theta_distance: list[float] | None = Field(
        default=None,
        description=(
            r"Scale-normalized per-iteration distance of the parameter vector to the "
            r"synthetic ground truth, "
            r"$d_k = \lVert(\theta_k - \theta_{\mathrm{true}})/s\rVert_2$ "
            r"($s$ = per-parameter magnitude scale). Same length as `conv`. The REAL "
            r"convergence-to-truth metric — distinct from the $\chi^2$ descent in "
            r"`conv`. None for non-synthetic cases (no $\theta_{\mathrm{true}}$) or "
            r"backends without a parameter trajectory (only spectrafit's faer LM "
            r"records it). Additive field — old payloads validate with None."
        ),
    )
...
class SelectionStats(_Base):
    r"""Frozen contract view of ``oracles.nested.SelectionStats``.
...
class SuiteMetric(_Base):
    ...
    convergence_efficiency: float | None = Field(
        default=None,
        description=(
            r"Mean cost reduction per iteration: "
            r"$(\mathrm{cost}_0 - \mathrm{cost}_n)$ / n_iter. "
    ...
    ill_conditioned: bool | None = Field(
        default=None,
        description=(
            r"True when $\kappa(J) \ge$ 1e6 (ill-conditioned). "
            r"None when $\kappa(J)$ was not computed. Additive field."
        ),
    )
    red_chi2_weighted: float | None = Field(
        default=None,
        description=(
            r"$\sigma$-weighted reduced-$\chi^2$: $\sum((y-\hat{y})/\sigma)^2$ / dof. "
            r"None when $\sigma = 0$ (noiseless case); see metric_undefined_reason. "
            "Additive field — old payloads validate with None."
        ),
    )
    ...
    def _reject_non_finite_optional(cls, v: float | None) -> float | None:
        ...
        if v is not None and not math.isfinite(v):
            msg = f"optional metric field must be finite or None (got {v!r})"
            raise ValueError(msg)
        return v
...
class SuiteCase(_Base):
    ...
    winner_reason: str | None = Field(
        ...
        description=(
            ...
            r"key signals ($\kappa(J)$, n_iter, convergence_efficiency) for the winner "
...
class ManifestSignals(_Base):
    ...
    harmonic_mean_speedup_vs_baseline: float | None = Field(
        ...
        description=(
            ...
            r"is the correct aggregate for equal-time comparisons and is always $\le$ "
            r"the "
    ...
    nonfinite_dr2_case_ids: list[str] = Field(
        default_factory=list,
        description=(
            r"Case ids whose $|\Delta r^2|$ vs the baseline was non-finite (NaN/Inf). "
            r"The "
    ...
    saturated_categories: list[str] = Field(
        default_factory=list,
        description=(
            r"Categories where every supported backend hits $r^2 \ge$ 0.999 on every "
            r"case. "
    ...
    def _harmonic_le_geomean(self) -> ManifestSignals:
        r"""Invariant V (V3): HM $\le$ GM for positive speedups (Eeckhout 2024).
...
# --------------------------------------------------------------------------- #
# Panel registry — consumed by the web PanelRenderer
# --------------------------------------------------------------------------- #
ChartKind = Literal[
    "line",
    "scatter",
...
class InferenceConfig(_Base):
    r"""Pre-registered inference parameters (written to the manifest, not tuned).
...
class CalibrationResult(_Base):
    r"""Frozen contract view of ``oracles.inference.CalibrationStat`` (Task 5.3).
...
class BenchReport(_Base):
    ...
    baseline_solver_id: str = Field(
        default="lmfit",
        description=(
            "Solver id whose median runtime defines the speedup baseline "
            '(x1.0). Was implicitly "lmfit" through hardcoded engine lookups '
            '(`by_name["lmfit"]`) and a hardcoded web label; now an explicit '
            "contract field so adding a fourth backend or retiring lmfit "
            "doesn't ripple across every consumer. Additive 1.0 -> 1.1 bump: "
            "old payloads without this field default here."
        ),
    )
    manifest: ManifestSignals | None = Field(
        default=None,
        description=(
            "Manifest headline numbers (mirrors `reports._headline`) — "
            "exposed so the web gate-state UI can render geomean / "
            r"max $|\Delta r^2|$ / win-rate / pinned ratio without a sidecar fetch. "
            "Additive 1.1 -> 1.2 bump; old payloads on disk validate against "
            "the bumped schema because Pydantic fills the default."
        ),
    )
    ...
    panels: list[PanelSpec] = Field(
        default_factory=list,
        description=(
            "Declarative panel registry consumed by the web PanelRenderer. "
            "Additive 1.2 -> 1.3 bump: old payloads validate with [] and the "
            'web app renders its explicit "report predates panel specs" '
            "empty state (no silent fallback to a hardcoded layout)."
        ),
    )
    ...
    git_commit: str | None = Field(
        default=None,
        description=(
            r"Git commit hash (short, $\ge$7 chars) at benchmark run time. "
