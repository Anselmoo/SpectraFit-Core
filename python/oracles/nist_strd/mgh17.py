r"""NIST StRD MGH17 (Osborne 1) dataset — verbatim from itl.nist.gov.

Model (5 parameters, 33 observations, DOF = 28):

    $$
    y = b1 + b2 \cdot \exp(-b4 \cdot x) + b3 \cdot \exp(-b5 \cdot x)
    $$

NIST classifies MGH17 as **"Average"** difficulty. (This docstring previously
recorded it as "Higher"; NIST's own nls_main listing is the authority and says
Average. The tier string here is parsed by
``manuscript/examples/figures/extract_bench_summary.py`` as the single source
of truth for the manuscript's Figure 4, so an error here propagates silently
into a published figure axis.) The problem is nonetheless start-sensitive:
Start 1 is far from the solution and is a known non-convergence case, which is
why ``tests/scenario/nist_strd/test_mgh17.py`` xfails Start-1 convergence.
...
Total free parameters: 1 + 4 = 5, matching NIST's $\mathrm{DOF} = N - p = 33 - 5 = 28$.
...
Attributes:
    START1: NIST-published starting guess (5 free params).
    START2: The alternative NIST-published starting guess.
    CERTIFIED: Certified parameter values and 1$\sigma$ standard errors,
        copied verbatim from NIST's published .dat file. Do not round when
        comparing against a fitted result.
    RSS: Certified residual sum of squares; consistent with
        RESIDUAL_STD_DEV via $\sqrt{\mathrm{RSS} / \mathrm{DOF}}$.
    RESIDUAL_STD_DEV: Certified residual standard deviation.
    DOF: Degrees of freedom.
    N_OBS: Number of observations.
    X: Independent variable, one entry per observation. NIST's data file
        has columns (y, x); reordered here to (x, y).
