r"""NIST StRD Lanczos1 dataset — verbatim from itl.nist.gov.

Model (6 parameters, 24 observations, DOF = 18):

    $$
    y = b1 \cdot \exp(-b2 \cdot x) + b3 \cdot \exp(-b4 \cdot x) + b5 \cdot \exp(-b6 \cdot x)
    $$
...
that reaches the global minimum will achieve RSS $\approx$ 1e-22 to 1e-25
...
Total free parameters: 4 + 2 = 6, matching NIST's $\mathrm{DOF} = N - p = 24 - 6 = 18$.

Source: https://www.itl.nist.gov/div898/strd/nls/data/lanczos1.shtml

Attributes:
    START1: NIST-published starting guess (6 free params).
    ...
    X: Independent variable, one entry per observation.
