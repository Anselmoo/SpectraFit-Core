...
def _compute_rung(wires: list[WireResult]) -> CredibilityRung:
    ...
    - **Soft-cap wires** (W2a/W2b/W2c): ``skipped`` means the audit sidecar was
      ...
      (e.g. W2b's σ-coverage falling outside [0.5, 0.85]) — a real, disclosed
    ...
    # from rung capping — in BOTH directions — so structural drift can't conflate
    # with "the science is unverified" (RUNG_2's meaning) AND structural passes can't
    # inflate a rung the numerical evidence doesn't support. Promote them once the
    # structural backlog clears.
    ...
    # V4 — no silent skip, no silent warn: a *skipped* audited value wire (a wire
    # that verifies a numerical value/claim) means that value was never checked,
    # and a *warn* means it was checked and found outside its target band — either
    # way it must not pass silently into the upper rungs. A disclosed "gap"
    # (subject does not expose the metric) is honest and stays non-capping; a skip
    # on the visible/render lane (W5) is conservative-by-design, not a value hole.
    # W8 (external replication) is excluded: its skip is already handled by the
    ...
    # Structure (S-prefixed) wires are excluded here too, for the same reason
    # they're excluded from `numerical` above: a passing structure wire verifies
    # repo self-description, not this run's numbers, so it must not count toward
    # pass_count either — otherwise structural checks alone could lift the rung
    # (SL-11: previously only excluded from the fail-floor check, not from here).
    w8_passed = any(w.wire_id == "W8" and w.status == "pass" for w in wires)
    _TOP_RUNG_GATES = {"W8", "W10", "W11"}
    core = [
        w
        for w in wires
        if w.wire_id not in _TOP_RUNG_GATES and not w.wire_id.startswith("S")
    ]
...
def _load_audit_records(run_dir: Path) -> list | None:
...
def _inline_trust_block(
    results_path: Path, block: TrustBlock, wire_status: Mapping[str, str]
) -> None:
    """Inline the trust block into results.json.
...
def _update_run_index(
    run_dir: Path, block: TrustBlock, wires: list[WireResult]
) -> None:
