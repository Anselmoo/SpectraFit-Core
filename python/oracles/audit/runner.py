...
from oracles.audit.wires import ALL_WIRES, _AuditRecord
from oracles.exceptions import AuditError
from oracles.trust_ledger import CredibilityRung, TrustBlock, TrustLedger, WireResult

if TYPE_CHECKING:
    from oracles.audit.nist import NistValidation
...
def _nullify_non_finite(obj: object) -> object:
    r"""Recursively replace non-finite floats (NaN / $\pm$Inf) with ``None``.

    Mirrors ``oracles.reports._sanitize``'s traversal shape (same dict/list
    recursion), renamed to avoid colliding with that unrelated same-named
    helper, and replaces with ``None`` (JSON null) rather than 0.0 so that
    diagnostic detail fields (e.g.
    ...
    Note:
        ``bool`` is a subclass of ``float`` in Python, so a true/false value
        matches both class patterns. ``case bool()`` MUST precede
        ``case float()`` in the match below, and the G5 annotation check
        explicitly excludes ``bool`` (``not isinstance(v, bool)``) so a
        boolean value is never mistaken for a suppressed non-finite float.
    """
    match obj:
        case bool():
            return obj
        ...
        case dict():
            ...
            for k, v in obj.items():
                key: str = str(k)
                out[key] = _nullify_non_finite(v)
                if isinstance(v, float) and not isinstance(v, bool) and not math.isfinite(v):
                    label = "nan" if math.isnan(v) else "inf"
                    out[f"{key}_suppressed"] = f"non-finite ({label}) suppressed"
            return out
        case list() as xs:
            return [_nullify_non_finite(v) for v in xs]
        case _:
            return obj


def _compute_rung(wires: list[WireResult]) -> CredibilityRung:
    r"""Map wire statuses to a credibility rung.
    ...
    - **Soft-cap wires** (W2a/W2b/W2c): ``skipped`` means the audit sidecar was
      absent so the recompute could not run — a graceful degradation. ``warn``
      means the recompute DID run and found a value outside its target band
      (e.g. W2b's $\sigma$-coverage falling outside [0.5, 0.85]) — a real, disclosed
      numerical concern, not a hard failure. Both ``skipped`` and ``warn`` on
      any of these caps the rung at RUNG_3 (V4 guard) — a "warn" must not be
      silently invisible to the rung the way an unrecognised status would be.
    ...
    # Hard-cap: pytest-cache-backed value-oracle wires whose UNKNOWN state
    # (absent cache) maps to "skipped". An absent cache on W2d is
    # indistinguishable from "this check never ran"; the claim must not pass
    # silently. Treat "skipped" on these wires exactly like "fail" — cap at
    # RUNG_2.
    hard_cap_value_oracle_wires = {"W2d"}
    if any(w.wire_id in hard_cap_value_oracle_wires and w.status == "skipped" for w in wires):
        return CredibilityRung.RUNG_2

    soft_cap_value_wires = {"W2a", "W2b", "W2c"}
    soft_cap_triggering_statuses = {"skipped", "warn"}
    value_wire_unresolved = any(
        w.wire_id in soft_cap_value_wires and w.status in soft_cap_triggering_statuses
        for w in wires
    )

    w8_passed = any(w.wire_id == "W8" and w.status == "pass" for w in wires)
    _TOP_RUNG_GATES = {"W8", "W10", "W11"}
    core = [w for w in wires if w.wire_id not in _TOP_RUNG_GATES and not w.wire_id.startswith("S")]
    ...
    # V4: an unresolved (skipped or warn) soft-cap value wire caps the ladder below
    # the coverage rung — RUNG_4/5 require every value wire actually checked and
    # in-band, never skipped- or warn-but-silently-passed.
    if value_wire_unresolved and base > CredibilityRung.RUNG_3:
        base = CredibilityRung.RUNG_3

    # Inferential wires: $\sigma$-calibration (W10) and speed inference (W11) must both
    ...
    # ladder is fully climbed.  All three gates (W8 $\land$ W10 $\land$ W11) are required.
...
def _load_audit_records(run_dir: Path) -> list[_AuditRecord] | None:
    """Load the audit.json sidecar if present (written by cli.run)."""
    audit_path = run_dir / "audit.json"
    return json.loads(audit_path.read_text()) if audit_path.exists() else None


def _load_nist_validation() -> NistValidation | None:
    """Compute the NIST StRD validation ONCE before the wires loop.

    Both the W8 WireResult and TrustBlock.nist_validation derive from the SAME
    ...
    try:
        ...
        return run_nist_validation()
    except Exception:  # noqa: BLE001 - defensive: missing build/fixtures  # pragma: no cover
        return None


def _load_nested_adequacy(results_path: Path) -> NestedAdequacy | None:
    """Extract the nested-model adequacy V&V result from results.json.

    Uses the first analyzed case that carries a nestedAdequacy block. This is
    pre-computed once so W9 and any future consumer share the same object —
    the same pattern as the nist_validation block above.
    """
    ...
    try:
        ...
        for _entry in _analyzed:
            ...
            if _na_raw is not None:
                return _NestedAdequacy.model_validate(_na_raw)
        return None
    except Exception:  # noqa: BLE001 - defensive: parse error / missing build  # pragma: no cover
        return None


def _load_inference_blocks(
    results_path: Path,
) -> tuple[CalibrationResult | None, SpeedInferenceResult | None, bool]:
    """Extract the inference block (W7/W10/W11) from results.json.

    The JSON key is camelCase ``inference`` → ``calibration`` / ``speedInference`` /
    ``cases``. Pre-computed once so W7/W10/W11 share the same read — same pattern as
    W8/W9. The third return value, ``has_inference_cases``, is True iff
    ``inference.cases`` is a non-empty list in the payload — used by W7 to report
    "skipped" instead of a vacuous "pass" when a run has no non-baseline backend.
    """
    if not results_path.exists():
    ...
    try:
        ...
        calibration = (
            _CalibrationResult.model_validate(_inf["calibration"])
            if _inf.get("calibration")
            else None
        )
        speed_inference = (
            _SpeedInferenceResult.model_validate(_inf["speedInference"])
            if _inf.get("speedInference")
            else None
        )
        has_inference_cases = bool(_inf.get("cases"))
        return calibration, speed_inference, has_inference_cases
    except Exception:  # noqa: BLE001 - defensive: parse error / missing build  # pragma: no cover
        return None, None, False
...
def _collect_wires(
    run_dir: Path,
    *,
    recs: list[_AuditRecord] | None,
    nist_validation: NistValidation | None,
    nested_adequacy: NestedAdequacy | None,
    calibration: CalibrationResult | None,
    speed_inference: SpeedInferenceResult | None,
    has_inference_cases: bool,
) -> list[WireResult]:
    """Walk every registered wire (numerical + structural).

    Each wire fn is dispatched with whichever pre-computed objects its
    signature declares.
    ...
    Note:
        Structure wires (S1..S5) verify the repo's *self-description* — that
        the claims the tree makes about itself (hook source-of-truth paths,
        INDEX.yaml anchors, doc owners, the FFI stub, model-list parity)
        have not drifted from the structure. They emit the same
        ``WireResult`` records as the numerical wires, so they ride in the
        same ``TrustBlock`` and surface on the same audit. They are
        NON-CAPPING for now (see ``_compute_rung``): structural drift is
        tracked and visible but does not yet pin the numerical credibility
        rung, because some confirmed structural items (e.g. F11 phantom
        pre-merge anchors) are pending triage decisions. Promote to capping
    ...
    for wire_fn in ALL_WIRES:
        ...
        match tuple(name for name in _injectable if name in sig):
            ...
            case ("has_inference_cases", *_):
                wires.extend(wire_fn(has_inference_cases=has_inference_cases))  # ty: ignore[unknown-argument]  # guarded by runtime signature check
...
def _inline_trust_block(
    results_path: Path,
    block: TrustBlock,
    wire_status: Mapping[str, str],
) -> None:
    r"""Inline the trust block into results.json.

    Written under the contract's camelCase alias `trustBlock` (BenchReport
    uses a to_camel _Base with extra="forbid"; a snake_case `trust_block`
    sibling would be rejected as an extra key and 500 the API).

    Note:
        The trust block is sanitized before inlining: ``WireResult.details``
        may carry non-finite floats (e.g. ``max_abs_delta=Inf`` when
        ``worst=inf``) and ``NistParam.sig_figs_agreed`` is $\infty$-capped
        for exact agreement. Raw ``model_dump()`` hands these through as
        Python ``float("inf")``, which ``json.dumps()`` renders as the
        non-RFC-8259 token ``Infinity`` — valid in Python but rejected by
        ``JSON.parse()`` in every browser/bundler.
    """
    if not results_path.exists():
        return
    results = json.loads(results_path.read_text())
    results.pop("trust_block", None)  # drop any stale snake_case sibling
    results["trustBlock"] = _nullify_non_finite(block.model_dump(mode="json"))
...
def _update_run_index(
    ...
    """Track rung + wire pass/total on the run's index entry.

    Lets `spc-bench trend` chart verification health over time (the
    maintainer self-healing log). Best-effort: a tracking-write failure must
    never red the audit.
    """
...
def run_audit(run_dir: Path) -> TrustLedger:
    ...
    Note:
        A claim is audited iff its backing wire passed. The WireResults
        already collected for the rung are reused here — no extra
        recompute. A failing, skipped, or gap wire leaves its claims
        un-audited, so the count honestly falls below the registered total
        instead of being vacuous.
    """
    recs = _load_audit_records(run_dir)
    nist_validation = _load_nist_validation()

    results_path = run_dir / "results.json"
    nested_adequacy = _load_nested_adequacy(results_path)
    calibration, speed_inference, has_inference_cases = _load_inference_blocks(
        results_path,
    )

    wires = _collect_wires(
        run_dir,
        recs=recs,
        ...
        has_inference_cases=has_inference_cases,
    )
    ...
    _inline_trust_block(results_path, block, wire_status)
    _update_run_index(run_dir, block, wires)
