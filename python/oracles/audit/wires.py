...
def _w2c_non_finite_kappas(audit_records: list[_AuditRecord]) -> list[str]:
    """(case,backend) pairs where κ was computed but is non-finite (inf/nan)."""
    return [
        f"{r['case']}/{r['backend']}"
...
def _w2c_subject_capability(
    audit_records: list[_AuditRecord], n_absent_all: int
) -> WireResult:
    """W2c step 2: subject-centric pass/gap — does spectrafit expose κ(J)?

    Oracle backends (lmfit/jax) that do not expose κ at all are a disclosed
...
def _w8_result_from_validation(
    nist_validation: NistValidation, threshold: float
) -> WireResult:
...
def wire_w8_nist_certified_validation(
    ...
    if skip_result is not None:
        return [skip_result]
    assert (
        nist_validation is not None
    )  # guaranteed by _w8_resolve_validation's contract
