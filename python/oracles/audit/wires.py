r"""One function per wire. Each returns a list of WireResult records.
...
def wire_w2a_metric_identity(
    ...
    r"""W2a: recompute $r^2$/rmse/reduced-$\chi^2$ from the FULL audit arrays; compare to stored.
    ...
    Note:
        This is a metric-IDENTITY check: each metric is recomputed under the
        SAME definition the engine stored it with. The engine stores
        reduced-$\chi^2$ UNWEIGHTED (SSR/dof), so the identity recompute is
        unweighted too — passing the sidecar's $\sigma$ here re-weighted it
        by $1/\sigma^2$, which diverged ~$1/\sigma^2$ on every noisy case
        and went to $\infty$ for the noiseless ($\sigma=0$) optfn landscapes
        (the Track-0 root cause). The $\sigma$-weighted "true" reduced-$\chi^2$
        is a separate, additive metric — not this identity oracle.
    """
...
def wire_w2b_coverage(
    ...
    r"""W2b: recompute $1\sigma$ pull coverage from the MC ensemble in the sidecar."""
    if not audit_records:
        return [
            WireResult(
                wire_id="W2b",
    ...
    for rec in audit_records:
        ...
        # Skip $\sigma$-absent records (coverage=None): they carry no calibration
    ...
    return [
        WireResult(
            ...
            evidence=(f"mean 1σ coverage over MC ensemble = {mean_cov:.2f} (target ≈0.68)"),
...
def _w2c_non_finite_kappas(audit_records: list[_AuditRecord]) -> list[str]:
    r"""(case,backend) pairs where $\kappa$ was computed but is non-finite (inf/nan)."""
    return [
        f"{r['case']}/{r['backend']}"
...
def _w2c_subject_capability(
    audit_records: list[_AuditRecord],
    n_absent_all: int,
) -> WireResult:
    r"""W2c step 2: subject-centric pass/gap — does spectrafit expose $\kappa(J)$?

    Oracle backends (lmfit/jax) that do not expose $\kappa$ at all are a disclosed
    per-backend limitation, not a capability gap — their absence is reported
    in ``details`` but does NOT change the pass.
    """
    ...
    oracle_absent = [
        r for r in audit_records if r.get("backend") != _SUBJECT_BACKEND and r.get("kappa") is None
    ]
    ...
    # No subject finite $\kappa$ and no non-finite $\kappa$ anywhere → subject does not expose $\kappa$.
    n_total = len(audit_records)
    return WireResult(
        wire_id="W2c",
        name="jacobian_kappa",
...
def wire_w2c_jacobian_kappa(
    ...
    r"""W2c: $\kappa(J)$ per (case,backend) — verify the subject's Jacobian conditioning.

    Four outcomes (in priority order):

    - ``skipped`` — no audit sidecar present; the wire cannot recompute.
    - ``fail``    — at least one $\kappa$ WAS computed (by any backend) but is non-finite
      (inf/nan): a real numerical failure that caps the rung.
    - ``pass``    — the subject (spectrafit) exposes a finite $\kappa(J)$ for every one of
      its entries AND no backend has a non-finite computed $\kappa$.  Oracle backends
      (lmfit/jax) that do not expose $\kappa$ at all are a disclosed per-backend limitation,
      not a capability gap — their absence is reported in ``details`` but does NOT
      change the pass.
    - ``gap``     — no non-finite $\kappa$, AND the subject has no entries with a finite $\kappa$
      (subject does not expose $\kappa$ either).  This is a genuine subject capability gap.
    """
    if not audit_records:
        return [
            WireResult(
    ...
    # --- (1) non-finite check: any computed but non-finite $\kappa$ is a hard failure ---
    non_finite = _w2c_non_finite_kappas(audit_records)
    ...
    # --- (2) subject-centric pass/gap: does the subject (spectrafit) expose $\kappa$? ---
    return [_w2c_subject_capability(audit_records, n_absent_all)]


def wire_w2d_solver_output_oracle() -> list[WireResult]:
    r"""Wire W2d: spectrafit's solver output matches an independent oracle.
...
def wire_w7_inference_validity(
    *,
    has_inference_cases: bool | None = None,
) -> list[WireResult]:
    ...
    """
    if has_inference_cases is False:
        return [
            WireResult(
                wire_id="W7",
                ...
                status="skipped",
                evidence=(
                    "no inference cases available for this run (empty, missing, "
                    "or unreadable) -- nothing to validate"
                ),
            ),
        ]
...
def _w8_resolve_validation(
    nist_validation: NistValidation | None,
) -> tuple[NistValidation | None, WireResult | None]:
    """Resolve the NIST validation block for W8.

    Returns ``(validation, None)`` on success, or ``(None, skip_result)`` if
    the caller passed nothing and the harness could not run.
    """
    if nist_validation is not None:
        return nist_validation, None
    # Fallback path: called directly (tests, CLI) without a pre-computed block.
    try:
        from oracles.audit.nist import run_nist_validation

        return run_nist_validation(), None
    except Exception as exc:  # noqa: BLE001 pragma: no cover - defensive: missing build/fixtures
...
def _w8_result_from_validation(
    nist_validation: NistValidation,
    threshold: float,
) -> WireResult:
    """Build the W8 pass/fail WireResult from an already-resolved validation block."""
...
def wire_w8_nist_certified_validation(
    ...
    r"""W8: independent external replication against NIST StRD certified values.
    ...
    - ``pass``     — every dataset reproduces the certified values to $\geq$ the
    ...
    nist_validation, skip_result = _w8_resolve_validation(nist_validation)
    if skip_result is not None:
        return [skip_result]
    assert nist_validation is not None  # guaranteed by _w8_resolve_validation's contract
    ...
    return [_w8_result_from_validation(nist_validation, threshold)]


def _w9_result_from_adequacy(nested_adequacy: NestedAdequacy) -> WireResult:
    """Build the W9 pass/fail WireResult from an already-computed adequacy block.

    Note:
        After fix I1, ``recovered_true_order_bic=True`` guarantees
        ``sel_bic == true_order``, so the "==" in the pass branch is always
        correct. Both sides are rendered from the actual values (rather
        than assumed equal) so any future drift becomes immediately
        visible.
    """
...
def wire_w9_nested_adequacy(
    ...
    return [_w9_result_from_adequacy(nested_adequacy)]


def _w10_result_from_calibration(calibration: CalibrationResult) -> WireResult:
    """Build the W10 pass/fail WireResult from an already-computed calibration block."""
...
def wire_w10_sigma_calibration(
    ...
    r"""W10: $\sigma$-calibration quality — practical-equivalence gate on pull coverage.

    Verifies that the subject's reported $\sigma$ values are calibrated: the fraction
    of $(\theta_{\mathrm{est}} - \theta_{\mathrm{true}}) / \sigma_{\mathrm{est}}$ pulls that fall within $\pm 1\sigma$ should lie close
    ...
    Gate criterion (A2 fix — CI-inclusion TOST for a proportion):
      ...
      within $[\mathrm{nominal} - \mathrm{margin}, \mathrm{nominal} + \mathrm{margin}]$ (default margin = 0.03 = $\pm 3$ pp).
    ...
    Honest strict diagnostic retained:
      ...
      optimistic but within $\pm 0.03$ equivalence band."  Only the binary gate
    ...
    return [_w10_result_from_calibration(calibration)]


def _w11_result_from_inference(speed_inference: SpeedInferenceResult) -> WireResult:
    """Build the W11 pass/fail WireResult from an already-computed inference block."""
...
def wire_w11_speed_inference(
    ...
    return [_w11_result_from_inference(speed_inference)]
