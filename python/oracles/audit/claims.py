r"""Claim registry. Claims register themselves; the runner discovers them.
...
def resolve_source_field(payload: Any, path: str) -> bool:
    r"""Return True iff ``path`` resolves to $\geq 1$ non-null value in ``payload``.
...
class Claim:
    ...
    source_field: ClassVar[
        str
    ]  # JSON path into results.json, e.g. "suite[].backends.spectrafit.r2"
...
def register_claim(cls: type[Claim]) -> type[Claim]:
    ...
    cid = cls.claim_id
    if "." not in cid:
        msg = f"claim_id must be a dotted namespace (e.g. 'metric.r2'); got {cid!r}"
        raise RegistryError(msg)
    if cid in CLAIM_REGISTRY:
        msg = f"claim {cid!r} already registered"
        raise RegistryError(msg)
...
def audited_count(wire_status: Mapping[str, str]) -> int:
    r"""Count claims whose backing verification wire passed.
...
class _MetricR2(Claim):
    ...
    description = (
        "R² metric formula recomputed from stored best-fit and consistent with stored value"
    )
...
class _MetricRmse(Claim):
    ...
    description = (
        "RMSE metric formula recomputed from stored best-fit and consistent with stored value"
    )
