        ...
        if m_bracket:
            ...
            if isinstance(node, list):
                return any(
                    ...
                    and item.get("name") == key
                    and _resolve(item, rest)
                    for item in node
                )
...
def register_claim(cls: type[Claim]) -> type[Claim]:
    ...
    if "." not in cid:
        raise RegistryError(
            f"claim_id must be a dotted namespace (e.g. 'metric.r2'); got {cid!r}"
        )
    if cid in CLAIM_REGISTRY:
        raise RegistryError(f"claim {cid!r} already registered")
...
def audited_count(wire_status: Mapping[str, str]) -> int:
    ...
    """
    return sum(
        1 for c in CLAIM_REGISTRY.values() if wire_status.get(c.wire_id) == "pass"
    )
