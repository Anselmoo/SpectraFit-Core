...
_PATH_TOKEN = re.compile(r"(?<![\w/.-])(\.?(?:[\w.-]+/)+[\w.-]+\.(?:tsx?|py|json|yaml|rs|sh))\b")
...
def s2_anchor_trigger_liveness(root: Path | None = None) -> WireResult:
    ...
    if not anchors:
        return _skip("S2", "anchor-trigger-liveness", "no deployed *.sh anchors in INDEX.yaml")
...
def s4_ffi_stub_completeness(root: Path | None = None) -> WireResult:
    ...
    runtime = set(
        re.findall(
            r"wrap_pyfunction!\(\s*([A-Za-z_]\w*)", lib_rs.read_text("utf-8")
        )
    )
    if not runtime:
...
def _rust_modeltype_wire_strings(root: Path) -> set[str]:
    ...
    return {
        m.group(1) for m in re.finditer(r'\b[A-Z]\w*\s*=>\s*"([\w]+)"', block)
    }
...
def s5_model_list_parity(root: Path | None = None) -> WireResult:
    ...
    if only_rust:
        problems.append(f"in Rust ModelTypeStr but not Python ModelType: {sorted(only_rust)}")
    if only_py:
        problems.append(f"in Python ModelType but not Rust ModelTypeStr: {sorted(only_py)}")
