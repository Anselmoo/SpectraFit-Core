r"""Structure wires — the *self-description* trust ledger (S-wires).
...
def s1_hook_reference_liveness(root: Path | None = None) -> WireResult:
    r"""S1: every source-of-truth path a hook cites must exist on the tree.

    A hook whose REQUIRED_* comment / guard names a file that does not exist
    is dead code lying about its own scope.
...
def s2_anchor_trigger_liveness(root: Path | None = None) -> WireResult:
    ...
    if not anchors:
        return _skip(
            ...
            "no deployed *.sh anchors in INDEX.yaml",
        )
    ...
    def is_live(anchor: str) -> bool:
        """F11 contract: an anchor is honest iff it cannot imply automation it lacks.
        ...
        statement that it is invoked by hand, not silently un-wired.
        """
...
def s3_doc_owner_truth(
    ...
    *,
    ...
    r"""S3: a doc that names the *symbol* owner must point where it is defined.
...
def s4_ffi_stub_completeness(root: Path | None = None) -> WireResult:
    ...
    if missing or extra:
        return _bad(
            ...
            f"_core.pyi drifted from the runtime PyO3 surface — missing {missing}, phantom {extra}",
            runtime=len(runtime),
...
def _rust_modeltype_wire_strings(root: Path) -> set[str]:
    """Extract the Rust ``ModelTypeStr::ALL`` wire-string set from ``types.rs``.

    The canonical wire list is the `Variant => "wire"` arms inside the
    ``model_manifest! { ... }`` invocation (which generates
    ``ModelTypeStr::ALL`` + ``as_str()``). Extraction is scoped to that
    brace-block so it doesn't also capture ``TerminationReason`` / other
    ``=>`` arms elsewhere in the file.
    """
