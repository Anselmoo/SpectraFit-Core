r"""Value-provenance spine — Invariant V, the single declarative source of truth.
...
r"""What a *missing* oracle check means (V4). ``caps_rung`` — a skip is a real
...
class ValueProvenance(BaseModel):
    ...
    def _structural_invariants(self) -> ValueProvenance:
        if "." not in self.id:
            msg = f"provenance id must be a dotted namespace (e.g. 'metric.r2'); got {self.id!r}"
...
def _build_registry(records: list[ValueProvenance]) -> dict[str, ValueProvenance]:
    ...
    for rec in records:
        if rec.id in registry:
            msg = f"duplicate provenance id {rec.id!r}"
