...
def _upgrade_1_0_to_1_1(payload: dict[str, Any]) -> dict[str, Any]:
...
def migrate_report(
    ...
    *,
    ...
    if key not in MIGRATIONS:
        msg = f"No migration path from {from_v} to {to_v}"
...
def _upgrade_1_1_to_1_2(payload: dict[str, Any]) -> dict[str, Any]:
...
def _upgrade_1_2_to_1_3(payload: dict[str, Any]) -> dict[str, Any]:
...
def _upgrade_1_3_to_1_4(payload: dict[str, Any]) -> dict[str, Any]:
...
def _upgrade_1_4_to_1_5(payload: dict[str, Any]) -> dict[str, Any]:
...
def _upgrade_1_5_to_1_6(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    time-specific feature. Field rename → breaking, so this migrator
...
def _upgrade_1_6_to_1_7(payload: dict[str, Any]) -> dict[str, Any]:
    r"""1.6 -> 1.7: ``multidim`` reshaped from a 2-D map to a genuine N-D fit.

    The field was a 2-D-only ``gaussian2d`` map (``nx``/``ny`` + 2-D
    ``obs``/``model``/``resid`` grids + ``MultiDimPeak`` cx/cy/sx/sy); it is now a
    real $\geq 3$-D ``gaussian_nd`` showcase (``nDims``/``shape``/``rSquared`` +
...
# --------------------------------------------------------------------------- #
def migrate_payload_to_current(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    *proves* that assumption per payload, so a stamp-only migration that yields
    ...
    if current is None:
        msg = "payload has no schemaVersion field; cannot migrate"
        raise ContractError(msg)
    migrated = False
    while current != SCHEMA_VERSION:
        ...
        if step is None:
            msg = f"No migration path from {current} to {SCHEMA_VERSION}"
            raise ContractError(msg)
        payload = step(payload)
        migrated = True
        bumped = payload.get("schemaVersion")
        if bumped == current:
            msg = f"migration from {current} did not advance schemaVersion"
            raise ContractError(msg)
        current = bumped
    if migrated:
