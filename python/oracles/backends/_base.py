...
# Synthetic proxy gradient-norm trace only (display purposes, no chi2/r2/gating
# impact): shrinks the final point so the reconstructed curve visually tapers
# toward convergence instead of ending flat.
...
def reconstruct_history(
    ...
    grad = [
        float(
            np.sqrt(max(c, 0.0))
            * (_DISPLAY_TRACE_SHRINK_FACTOR if i == n_iter - 1 else 1.0)
        )
