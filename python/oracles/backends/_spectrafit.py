...
def _jacobian_kappa(condition_number: float | None) -> float | None:
    r"""Convert Rust's $\kappa(J^T J)$ to $\kappa(J)$ for the benchmark axis.

    The Rust kernel records the *Gram* condition number $\kappa(J^T J) = \kappa(J)^2$.
    scipy's ``np.linalg.cond(jac)`` returns $\kappa(J)$ directly, so to put both
    backends on the same axis we take the square root here.

    Returns ``None`` for ``None``, negative, non-finite, or imaginary inputs.
    The result is capped at ``_KAPPA_CAP`` (1e16, mirroring
    ``oracles.backends._scipy_ls._KAPPA_CAP`` so both backends share the same
    axis ceiling) to protect downstream aggregations from ``inf`` contamination.
    """
...
class SpectraFitBackend(Backend):
    ...
    def extract(self, raw: Any, case: BenchCase) -> BackendOutcome:
        ...
        result, best_fit = raw
        # result.dof = n_data - n_free_params (from Rust); clamp to $\ge$ 1.
        ...
        return BackendOutcome(
            ...
            # Rust emits $\kappa(J^T J) = \kappa(J)^2$; take sqrt to match scipy's $\kappa(J)$ axis.
