    ...
    for i, comp in enumerate(case.comp_guess):
        ...
        for pname in pm.param_names:
            ...
            if flat_name in fixed_flat:
                ...
                lo_list.append(lo_x)
                hi_list.append(hi_x)
            elif pname in ("sigma", "gamma"):
                lo_list.append(1e-6)
                hi_list.append(math.inf)
            elif pname == "fraction":
                lo_list.append(0.0)
                hi_list.append(1.0)
            elif pname in _SHAPE_BOUNDS:
    ...
    match method:
        ...
        case "trf" | "dogbox":
            ...
            return least_squares(
                residual, box.free_x0, method=method,
...
def _stderr_from_jac(
    ...
    try:
        ...
        threshold = (
            np.finfo(float).eps * max(jac.shape) * sv[0] if sv.size else 0.0
        )
        sv_inv = np.where(
            sv > threshold, 1.0 / np.where(sv > 0, sv, 1.0), 0.0
        )
...
def _upgrade_success(raw: Any, r2_quality: float) -> bool:
    ...
    if (
        not scipy_success
        and r2_quality >= 0.9
        and int(getattr(raw, "status", 1)) >= 0
    ):
        return True
