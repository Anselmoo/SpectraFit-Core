...
Bounds policy uses `oracles.models.SHAPE_BOUNDS` (the long-tail shape
parameters that an LM-family search can otherwise drive into the model
function's overflow/NaN region) — the same registry-backed table `_lmfit.py`
reads, so the two oracles always see an identical parameter box. For `trf`
and `dogbox` the bounds are
...
from oracles.models import SHAPE_BOUNDS, get_model

_Method = Literal["lm", "trf", "dogbox"]

_KAPPA_CAP = 1e16
r"""$\kappa(J)$ cap — anything beyond this is reported as ``1e16`` to keep downstream
aggregations (geomean, ratios, plots) from contaminating with ``inf``.
"""

_LM_BARRIER_WEIGHT = 10.0
r"""Soft-barrier weight for the ``lm`` path (the MINPACK kernel cannot accept
bounds). $10\times$ a unit residual so a 10 % excursion past a bound contributes
the same magnitude as a 10 % residual error — see `_make_lm_residual` docstring.
"""
...
class _ParamBox(BaseModel):
    ...
    Note:
        This and this module's other internal records are Pydantic-first
        (not dataclasses) so a language server can check every consumer, and
        ``extra="forbid"`` keeps the structural contract explicit.
    """

    model_config = ConfigDict(extra="forbid", arbitrary_types_allowed=True)

    names: list[str]
    """Full parameter inventory (all = free union fixed, in component order).

    - `x0`: full x0 (all params, clipped)
    - `lo`: full lo (all params)
    - `hi`: full hi (all params)
    - `init_fit`: the initial-guess fit trace
    """
    x0: np.ndarray
    lo: np.ndarray
    hi: np.ndarray
    ...
    - `free_x0`, `free_lo`, `free_hi`: the free-only bound/guess vectors
    - `free_indices`: positions in the full vector
    """
    free_x0: np.ndarray
    free_lo: np.ndarray
    free_hi: np.ndarray
    ...
    - `fixed_vals`: the held values
    - `fixed_indices`: positions in the full vector
    """
...
def _bounds_for(
    ...
    fraction in `[0, 1]`, plus `SHAPE_BOUNDS` for the long-tail shape params.
            ...
            if flat_name in fixed_flat:
                ...
                match pname:
                    ...
                    case "fraction":
                        lo_list.append(0.0)
                        hi_list.append(1.0)
                    case _ if pname in SHAPE_BOUNDS:
                        a, b = SHAPE_BOUNDS[pname]
                        lo_list.append(a)
                        hi_list.append(b)
                    case _:
...
def _predict(theta: np.ndarray, case: BenchCase) -> np.ndarray:
    ...
    for comp in case.comp_guess:
        ...
        kwargs = dict(zip(pm.param_names, theta[j : j + n], strict=True))
...
def _build_initial_guess(case: BenchCase) -> _ParamBox:
    ...
    """
    names, x0, lo, hi, free_indices, fixed_indices, _fixed_vals = _bounds_for(case)
    ...
    fixed_vals_clipped = x0[fixed_indices] if fixed_indices else np.empty(0, dtype=float)
...
def _solve(method: _Method, box: _ParamBox, case: BenchCase) -> Any:
    ...
    match method:
        ...
        case _:  # pragma: no cover - Literal types pin this at the type level
            msg = f"_solve: unsupported method {method!r}"
...
def _kappa_of(jac: np.ndarray | None) -> float | None:
    r"""Condition number $\kappa(J)$, capped at ``_KAPPA_CAP``; ``None`` if undefined.

    Structural property of the case (Wire W2c): $\kappa \geq 10^6$ → ill-conditioned.
...
def _stderr_from_jac(
    ...
    r"""Curve_fit-style stderr via SVD pseudo-inverse of the Jacobian.

    $\mathrm{cov} = \mathrm{inv}(J^T J) \cdot 2 \cdot \mathrm{cost}/(m-n)$ via SVD
    pseudo-inverse handles rank deficiency the way `scipy.optimize.curve_fit`
    does. When the Jacobian is missing, under-determined, or numerically
    degenerate, all stderrs collapse to ``None`` — better than reporting a
    bogus number.
    """
    ...
    for flat_name, v in zip(names, var, strict=True):
        i_str, p_str = flat_name.split("_", 1)
        out[f"{i_str}.{p_str}"] = float(math.sqrt(v)) if v > 0 and math.isfinite(v) else None
    return out
...
def _extract_uncertainty(
    ...
    r"""Return ($\kappa(J)$, stderr map) from the raw ``OptimizeResult``.

    The Jacobian returned by ``least_squares`` has shape
    ``(n_data, n_free)`` — one column per FREE parameter.  $\kappa(J)$ is computed
    ...
    """
    jac = np.asarray(raw.jac, dtype=float) if hasattr(raw, "jac") and raw.jac is not None else None
    kappa = _kappa_of(jac)
...
def _extract_params(raw: Any, box: _ParamBox) -> dict[str, float]:
    ...
    for flat_name, val in zip(box.names, full, strict=True):
        i_str, p_str = flat_name.split("_", 1)
...
def _upgrade_success(raw: Any, r2_quality: float) -> bool:
    r"""Promote soft failures to success via an $r^2$-quality upgrade.

    (status $\geq 0$, $r^2 \geq 0.9$).

    Mirrors the post-fit policy in ``apply_postfit_guards``
    (`crates/spectrafit-solver/src/postfit.rs`, and the Cycle 5.5 entry in
    ``DECISIONS-archive.md`` on $r^2$-quality upgrades promoting soft-failure
    terminations).
    ...
    for. A fit that reached $r^2 \geq 0.9$ is materially converged regardless of
...
class ScipyLeastSquaresBackend(Backend):
    ...
    def extract(self, raw: Any, case: BenchCase) -> BackendOutcome:
        r"""Map `OptimizeResult` to the normalised `BackendOutcome`.

        AIC/BIC use the Gaussian-error formula $-2 \log L = n \cdot \log(\chi^2/n)$
        (the $n \cdot (\ln(2\pi)+1)$ constant is dropped so cross-backend
        $\Delta\mathrm{AIC}/\Delta\mathrm{BIC}$ are comparable — the parity test in
        ...
        freedom for reduced-$\chi^2$ likewise uses ``n_total_params``.

        Note:
            ``raw.fun`` for the ``lm`` path is ``[main_residuals; soft_barrier]``;
            it is trimmed to the data residuals here so $\chi^2$ and $r^2$ stay
            honest. The residual functions (:func:`_make_lm_residual` /
            :func:`_make_bounded_residual`) return ``pred - y``, so the model
            prediction is ``y + residuals`` — NOT ``y - residuals``. The latter
            would reflect the fit across the data (``2y - pred``), producing a
            mirror-image ``best_fit`` that, via the engine's central
            ``resid = case.y - best_fit``, flips the stored residual sign
            relative to spectrafit/lmfit/jax (they emit obs - fit). Adding
            restores both the true curve and the canonical obs - fit residual
            convention; $\chi^2$ is unaffected since it squares the residual.
        """
