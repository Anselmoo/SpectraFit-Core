...
class LmfitBackend(Backend):
    ...
    def build(self, case: BenchCase) -> Any:
        ...
        Note:
            Each parameter falls back to ``PeakModel.extra_defaults`` when
            absent from the case-spec guess (e.g. ``fraction`` on pseudo_voigt
            when only amplitude/center/sigma are supplied); explicit case-spec
            values always win, so extra_defaults are the last resort and lmfit
            never sees an uninitialised parameter.

            ``case.spec.expr_edges`` entries (e.g. ``{"target_node": "p1",
            "target_param": "sigma", "expression": "p0.sigma"}``) are applied as
            lmfit parameter expressions. lmfit expects the ``"p0_sigma"`` form
            (underscore prefix), so dotted node references in the expression are
            translated to underscores before assignment.
        """
        import lmfit
        ...
        for i, comp in enumerate(case.comp_guess):
            ...
            if de:
                ...
                for w, (lo_b, hi_b) in SHAPE_BOUNDS.items():
                    if w in names:
        ...
        if params is not None:
            for edge in case.spec.expr_edges:
                ...
                lmfit_expr = re.sub(r"\b(p\d+)\.(\w+)\b", r"\1_\2", lmfit_expr)
    ...
    def extract(self, raw: Any, case: BenchCase) -> BackendOutcome:
        ...
        for i, comp in enumerate(case.comp_true):
            for p in get_model(comp.model).param_names:
                ...
                stderr[f"p{i}.{p}"] = float(par.stderr) if par.stderr is not None else None
