"""N-dimensional and shared-model multi-spectrum global-fit showcases.

Split out of ``oracles.engine``: these four functions are a self-contained
...
def _gauss(xv: np.ndarray, a: float, c: float, s: float) -> np.ndarray:
    r"""Gaussian kernel $a\exp(-\tfrac{1}{2}((x-c)/s)^2)$."""
    return a * np.exp(-0.5 * ((xv - c) / s) ** 2)


def _multidim() -> MultiDim:
    r"""A *genuinely fitted* N-dimensional (3-D) example.
    ...
    spectrafit-core is the **subject**: a real spectrafit $\geq 3$-D solve, not the
...
) -> tuple[list[MeasurementData], list[np.ndarray]]:
    """Synthesize multi-dataset slices for global fit demo."""
    from spectrafit_core.data import MeasurementData

    datasets, obs_slices = [], []
    n_pts = len(x)
    for i in range(len(a0_t)):
        y = _gauss(x, a0_t[i], c0, s0) + _gauss(x, a1_t[i], c1, s1) + rng.normal(0, noise, n_pts)
        obs_slices.append(y)
...
def _assemble_global_fit(
    ...
    """Assemble GlobalFit from recovered shared-model fit parameters."""
    n_slices = len(dataset_axis)
    slices = [
        GlobalFitSlice(
            coord=float(dataset_axis[i]),
            obs=obs_slices[i].tolist(),
            model=(_gauss(x, amp0[i], cen0, sig0) + _gauss(x, amp1[i], cen1, sig1)).tolist(),
        )
...
def _global_fit() -> GlobalFit:
    ...
    def _gauss_node(nid: str, a: float, c: float, s: float) -> ModelNodeSpec:
    ...
    gfg = GlobalFitGraph(
        global_nodes=[],
        local_nodes=[_gauss_node("p0", 4.0, c0, s0), _gauss_node("p1", 1.0, c1, s1)],
        n_slices=n_slices,
