...
def _cold_ms(backend: Backend, case: BenchCase) -> float:
    ...
    except Exception as exc:  # noqa: BLE001 - cold timing failed
        _LOG.warning(
            "cold-timing solve failed for backend %s on case %s: %r; "
            "reporting NaN for this cold-start measurement",
            backend.name,
            case.id,
            exc,
        )
...
def _theta_distance_to_truth(o: BackendOutcome, case: BenchCase) -> list[float] | None:
    r"""Per-iteration convergence distance to synthetic ground truth.

    $d_k = \|(\theta_k - \theta_{\text{true}})/s\|_2$ with $s_i = \max(|\theta_{\text{true},i}|, 1.0)$ — the REAL
    convergence-to-truth metric (DECISIONS.md 2026-06-13), distinct from the $\chi^2$
...
def _build_profile(
    ...
    *,
    ...
    r"""Assemble one backend's full per-case profile (the single grouping).

    Note:
        ``jacobian_condition_number`` — $\kappa(J)$, the condition number of the
        Jacobian at convergence (Wire W2c) — is carried through from
        :class:`BackendOutcome`; backends that don't expose a Jacobian leave
        it ``None`` (the scipy-ls backends populate it from ``raw.jac``).

        ``conv_eff`` is a provenance-gated field: it is derived from ``ch``
        (the cost history), which is a *proxy* series when
        ``history_source != "real"``. A measured efficiency series is only
        emitted for the subject's real cost history; otherwise ``conv_eff``
        is ``None`` so a consumer can never read a reconstructed-derived
        series as measured.
    """
    ...
    if raw_sink is not None:
        ...
        if audit:
            ...
            entry["audit"] = {
                ...
                "dof": (
                    o.fit_dof if o.fit_dof is not None else max(int(y.size) - len(o.params), 1)
                ),
                ...
                "mc_ses": [
                    {k: (None if v is None else float(v)) for k, v in s.items()} for s in ses
                ],
    ...
    resid_scaled = [
        v for v in (np.abs(resid) / max(case.spec.noise, 1e-9)).tolist() if math.isfinite(v)
    ]
...
def _warmup(cold_ms: float, hot_ms: float) -> Warmup:
    ...
    return Warmup(
        ...
        pts=[WarmupPt(n=k, per_run=(cold + hot_ms * (k - 1)) / k) for k in _WARMUP_SCHED],
...
def _summary(
    ...
    r"""Build the headline KPI block with $\Delta$AIC/$\Delta$BIC vs best solver."""
