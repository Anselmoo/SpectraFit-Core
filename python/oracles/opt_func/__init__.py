...
from oracles.exceptions import RegistryError, UnknownKeyError
...
def register_landscape(
    ...
    def deco(fn: Callable[[Array], Array]) -> Callable[[Array], Array]:
        if key in LANDSCAPE_REGISTRY:
            msg = f"landscape {key!r} already registered"
...
def get_landscape(name: str) -> Callable[[Array], Array]:
    ...
    try:
        ...
        msg = f"unknown landscape {name!r}; registered: {sorted(LANDSCAPE_REGISTRY)}"
