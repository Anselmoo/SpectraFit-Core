...
def discover_ladder(root: Path) -> LadderLayout:
    ...
    Args:
        root (Path): Directory expected to contain ``reps-N/results.json`` subtrees.
    ...
    if root.is_dir():
        entries.extend(
            LadderRunFile(reps=int(m.group(1)), results_path=child / "results.json")
...
def backend_headline(report: BenchReport, solver_id: str) -> BackendHeadline | None:
    ...
    Args:
        report (BenchReport): A contract-valid benchmark report.
        solver_id (str): The backend to summarize.
...
def build_stability_study(
    ...
    *,
    ...
    Args:
        root (Path): Directory containing ``reps-N/results.json`` subtrees.
        tolerance (float): Relative band for the converged-at-N verdict.
...
def render_markdown(study: StabilityStudy) -> str:
    r"""Render the study as a human-readable Markdown table + one-line verdict.

    Rows are reps budgets, columns are backends; each cell shows the geomean
    speedup with its signed $\Delta\%$ vs the reference (highest-reps) value.
    """
