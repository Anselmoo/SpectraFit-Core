...
from oracles.reports import _RUN_RE, REPORTS_ROOT, latest_results
...
__all__ = ["app", "router"]
"""``app`` is the ASGI entrypoint (``uvicorn oracles.api:app``, ``TestClient(api.app)``);
``router`` is exported for callers that want to mount the same handlers on their
own app without duplicating the route definitions.
"""
...
class _DeprecatedAliasMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(
        ...
        Args:
            request: The incoming request; both its ``url.path`` and the
                presence of ``request.scope["route"]`` are inspected (see
                Note below for why ``route.path`` itself is not used) — an
                untagged 404 under ``/api/`` (no route matched) is never
                stamped.
            call_next: The next handler in the middleware chain; awaited once
                to obtain the response before it is (maybe) stamped.
        ...
        if not request_path.startswith(_LEGACY_PREFIX + "/"):
            return response
        if request_path.startswith(_CANONICAL_PREFIX + "/") or request_path == _CANONICAL_PREFIX:
            return response
        response.headers["Deprecation"] = "true"
        response.headers["Sunset"] = _SUNSET_DATE
        # ``Link`` to the canonical successor per RFC 8594 § 3.
        successor = _CANONICAL_PREFIX + request_path[len(_LEGACY_PREFIX) :]
...
router = APIRouter()
"""All endpoints live on a single router; the FastAPI app mounts it twice below
so there is one source of truth for handlers:
...
def get_trust() -> dict[str, Any]:
    r"""Latest run's verification ledger slice (trust block + inference).

    The downloadable 'verification ledger' the UI links to for reviewers. The full
    V&V detail (wires, NIST datasets, nested-adequacy, $\sigma$-calibration, speed-significance)
