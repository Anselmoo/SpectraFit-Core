...
class _DeprecatedAliasMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(
        ...
        if route is None:
            return response
        # Discriminate on the REQUEST path, not ``route.path``: as of FastAPI
        # 0.140.0, ``app.include_router(router, prefix=...)`` no longer produces
        # prefix-combined ``APIRoute`` objects reachable via
        # ``request.scope['route']`` — that key resolves to the router-relative
        # route (``path == "/runs"``) for BOTH the ``/api/*`` and ``/api/v1/*``
        # mounts, so a ``route.path``-based check silently never matches either
        # prefix. ``request.url.path`` is the actual incoming path and is
        # unaffected by that internal representation.
        request_path = request.url.path
        if not request_path.startswith(_LEGACY_PREFIX + "/"):
            return response
        if (
            request_path.startswith(_CANONICAL_PREFIX + "/")
            or request_path == _CANONICAL_PREFIX
        ):
            return response
