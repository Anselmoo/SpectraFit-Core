...
from oracles.contract import _Contract
...
class CredibilityRung(IntEnum):
    ...
    RUNG_5 = 5  # reserved: independent differential validation + UQ (external replication)


class WireResult(_Contract):
    r"""One verification wire's outcome.
    ...
    status: WireStatus = Field(
        ...,
        description=(
            "Wire outcome: `pass`, `warn`, or `fail` for a computed verdict; "
            "`skipped` when the underlying test never ran; `gap` for a "
            "disclosed capability gap (see the class Note)."
        ),
    )
    ...
    details: dict[str, float | int | str | bool | None] = Field(
        ...
        description=(
            "Optional structured detail (counts, thresholds, sample sizes, "
            "...) supporting `evidence`; empty when the one-line statement is "
            "self-sufficient."
        ),
    )


class NistParam(_Contract):
    ...
    sig_figs_agreed: float = Field(
        ...,
        description=(
            r"$-\log_{10}\left(|\mathrm{fitted}-\mathrm{certified}|/"
            r"|\mathrm{certified}|\right)$; $\infty$-capped for exact agreement."
        ),
    )


class NistDataset(_Contract):
    ...
    passed: bool = Field(
        ...,
        description=r"True iff min_sig_figs $\ge$ the validation threshold.",
    )


class NistValidation(_Contract):
    ...
    passed: bool = Field(
        ...,
        description=r"True iff every dataset agrees to $\ge$ threshold sig figs.",
    )
...
class TrustBlock(_Contract):
    """Aggregate trust evidence attached to a BenchReport."""

    rung: CredibilityRung = Field(
        ...,
        description=(
            "Aggregate V&V credibility rung (see `CredibilityRung`), capped by "
            "the worst genuine `fail` among `wires` — a `gap` or `skipped` wire "
            "never lowers it."
        ),
    )
    wires: list[WireResult] = Field(
        ...,
        description="Every verification wire's outcome making up this ledger.",
    )
    n_claims_audited: int = Field(
        ...,
        description="Number of report claims this audit run actually checked.",
    )
    n_claims_total: int = Field(
        ...,
        description=(
    ...
    def _rung5_requires_nist_and_inference(self) -> TrustBlock:
        ...
        # Inferential wires W10 ($\sigma$-calibration) and W11 (speed inference) must
        # both be present with status "pass" (no pass-by-absence at the ceiling).
        w10_pass = any(w.wire_id == "W10" and w.status == "pass" for w in self.wires)
        if not w10_pass:
            msg = (
            ...
            raise ValueError(msg)
        w11_pass = any(w.wire_id == "W11" and w.status == "pass" for w in self.wires)
        if not w11_pass:
            msg = (
...
class TrustLedger(BaseModel):
    ...
    run_id: str = Field(..., description="The benchmark run id this ledger belongs to.")
    block: TrustBlock = Field(..., description="The aggregate trust evidence itself.")
