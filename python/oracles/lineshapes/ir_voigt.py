...
def build(rng: random.Random, variant: str) -> tuple[list, str]:
    ...
    """
    from oracles.cases import _asym, _binned_centers, _linear_bg  # lazy — avoids circular import at module load

    n = int(variant)
    peaks = [
        _asym(
            rng, "true_voigt", rng.uniform(3.0, 6.0), c, rng.uniform(0.4, 0.8)
        )
