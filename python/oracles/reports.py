...
def _compute_headline_numbers(
    ...
    Note:
        ...
        LM-family cases; the ``optfn`` category is excluded because it is
        multimodal global optimization where spectrafit's global solver and
        lmfit's ``differential_evolution`` legitimately reach different
        optima — a delta there would not be a real regression signal. A
