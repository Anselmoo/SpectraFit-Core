...
def _compute_default_gate_state(
    ...
    r"""Compute the aggregate gate state using the default ``spc-bench gate`` thresholds.
...
def _harmonic_mean(values: list[float]) -> float | None:
    r"""Return the harmonic mean of *values* ($N / \sum(1/x_i)$), or None on empty input.
...
def _headline(
    ...
    Note:
        ...
        alias ``geomean_speedup_vs_lmfit`` is retained for one deprecation
        cycle so old gates, dashboards, and on-disk manifests keep parsing.
        The contract field ``baseline_solver_id`` names which solver is
