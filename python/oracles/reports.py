...
from oracles.cases import NON_COMPARABLE_CATEGORIES
...
def _sanitize_tracked(obj: object, path: str = "$") -> tuple[object, list[str]]:
    r"""Recursively replace non-finite floats (NaN / $\pm$Inf) with 0.0, tracking paths.

    A single degenerate metric (e.g. ``reduced_chi2`` with dof $\le$ 0, an Inf cost-history
    ...
    ``*_suppressed`` keys (the ``oracles.audit.runner._nullify_non_finite`` pattern)
    are NOT an option here: every contract model is ``extra="forbid"`` and would
    reject them on round-trip.
    """
    match obj:
        case bool():
            return obj, []
...
def allocate_run_dir(category: str = "benchmark", root: Path = REPORTS_ROOT) -> Path:
    ...
    nums = [int(m.group(2)) for d in base.iterdir() if d.is_dir() and (m := _RUN_RE.match(d.name))]
...
def _saturated_categories(
    suite: Sequence[SuiteCase | dict[str, Any]],
    interbackend_tol: float = _SATURATION_INTERBACKEND_TOL,
    r2_floor: float = _SATURATION_R2_FLOOR,
) -> list[str]:
    r"""Return categories saturated by inter-backend $r^2$ agreement.

    A category is saturated when every case in it shows $\max(r^2) - \min(r^2) \le$
    ``interbackend_tol`` across all reporting backends, AND $\min(r^2) \ge$ ``r2_floor``
    ...
    captures "indistinguishable"; the floor (``r2_floor`` $\ge$ 0.99) captures
    "indistinguishable *and good*". Empirically the easy/edge/lineshapes/etc.
    cases hit max-min $\le$ 1e-10 across all 6 backends on the same noise draw at
    $r^2$ near 1.0 — exactly what "solved, below differentiation noise" means.
    Per memory: triage/benchmark-saturation-real-life-too-easy.md.

    Args:
        suite (list): List of :class:`oracles.bench_contract.SuiteCase` instances (or
            dicts with the same shape). Each has ``category: str`` and
            ``m: dict[solver_id, SuiteMetric]`` where ``SuiteMetric`` has
            ``r2: float``.
        interbackend_tol (float): Maximum allowed $\max(r^2) - \min(r^2)$ per case for
            "agreement". Default 1e-3.
        r2_floor (float): Minimum allowed $\min(r^2)$ per case. Guards against vacuous
            saturation when every backend produced garbage *or merely
            mediocre* fits. Default 0.99 — a real fit-quality ceiling, so
            "agreement" must be agreement at a good fit.
    ...
    for case in suite:
        case_cat = getattr(case, "category", None) or (
            case.get("category") if isinstance(case, dict) else None
        )
        if not case_cat:
            continue
        metrics_map = getattr(case, "m", None) or (case.get("m") if isinstance(case, dict) else {})
        r2s: list[float] = []
        for metric in (metrics_map or {}).values():
            ...
            if r2 is not None:
                r2s.append(float(r2))
        # Need $\ge 2$ backends to talk about inter-backend agreement.
        agrees = (
...
def _compute_default_gate_state(
    ...
    r"""Compute aggregate gate state from threshold defaults.

    This is the single computation that populates ``manifest.json`` and
    ``ManifestSignals.gate_state`` (Wire W6) so the web GateBadge can read
    one field rather than recomputing from regression flags. Thresholds
    match the defaults in ``oracles.cli.gate`` (``min_geomean=1.0``,
    ``max_dr2=1e-3``, ``max_regressions=0``).

    Note:
        The accuracy axis fails when ``max_dr2`` exceeds the threshold OR
        any compared case had a non-finite $|\Delta r^2|$ — a non-finite metric
        must never pass silently (``NaN > threshold`` is ``False``, which
        would otherwise let it through as a pass).
    ...
    # accuracy axis: lower-is-better (non-finite handling: see docstring)
    levels.append(
...
def _harmonic_mean(values: list[float]) -> float | None:
    r"""Return harmonic mean of *values*; None if empty.

    Per Eeckhout (2024): the harmonic mean is the correct aggregate for
    equal-time comparisons and is always $\le$ the geometric mean for positively-
...
def _compute_headline_numbers(
    ...
    r"""Shared math for `_headline` (manifest.json) and `compute_manifest_signals`.
    ...
    whose $|\Delta r^2|$ was non-finite so the accuracy gate fails on them instead of
    ...
    Note:
        Accuracy parity (``dr2``) is only accumulated for the deterministic
        LM-family cases; the non-comparable categories
        (``CategoryDef.baseline_comparable = False``) are excluded: ``optfn`` is
        multimodal global optimization where spectrafit's global solver and
        lmfit's ``differential_evolution`` legitimately reach different optima,
        and ``robust`` pits an IRLS M-estimator against least squares on
        contaminated data — a delta in either is not a regression signal. A
        non-finite delta is treated as a defect, not a tiny number: it is
        recorded in ``nonfinite_dr2_case_ids`` rather than let into ``dr2``
        (which would corrupt the max) or dropped silently.
    """
    ...
    for case in report.suite:
        ...
        if sf and baseline and case.category not in NON_COMPARABLE_CATEGORIES:
        ...
        if case.winner == "spectrafit" and "spectrafit" in case.m:
            sf_wins += 1
    geomean = math.exp(sum(map(math.log, speedups)) / len(speedups)) if speedups else 1.0
...
def compute_manifest_signals(
    ...
    Note:
        A corrupt pin (fields missing or malformed) is treated as absent
        rather than fatal, mirroring :func:`read_perf_baseline`'s tolerance
        for malformed sidecars; the CLI gate already handles this branch by
        skipping the self-vs-self check.
    """
    geomean, max_dr2, sf_win_rate, reg_ids, harmonic, nonfinite_dr2_ids = _compute_headline_numbers(
        report,
    )
...
def _backend_facts(report: BenchReport) -> dict[str, dict[str, float | int | None]]:
    ...
    Note:
        Deliberately NOT mirrored onto :class:`ManifestSignals` / the OpenAPI
        contract: ``manifest.json`` is a plain file artifact off the OpenAPI
        surface, so this field stays additive here and needs no schema bump,
        no golden regen, and no migrator (same precedent as
        ``nonfinite_dr2_case_ids``). It is nested one level (~6 backends x 5
        scalars) and rides into every ``index.json`` row too, so keep it
        small.
    """
    ids: set[str] = set()
    for case in report.suite:
...
def _headline(
    ...
    Note:
        The canonical key is ``geomean_speedup_vs_baseline``; the legacy
        alias ``geomean_speedup_vs_lmfit`` is retained through 0.1.0 (drop
        after 0.1.x) so old gates, dashboards, and on-disk manifests keep
        parsing. The contract field ``baseline_solver_id`` names which solver
        is actually the baseline.
    """
    geomean, max_dr2, sf_win_rate, reg_ids, harmonic, nonfinite_dr2_ids = _compute_headline_numbers(
        report,
    )
    return {
        ...
        "geomean_speedup_vs_baseline": geomean,
        "geomean_speedup_vs_lmfit": geomean,  # DEPRECATED — kept through 0.1.0; drop after 0.1.x
        "baseline_solver_id": report.baseline_solver_id,
        "max_abs_delta_r2": max_dr2,
        "spectrafit_win_rate": sf_win_rate,
        "regressions": len(reg_ids),
        "regression_case_ids": reg_ids,
        # Cases whose $|\Delta r^2|$ was non-finite — surfaced so the accuracy gate fails on
        # them. A list survives `_sanitize` (floats→0.0 would erase the signal).
        "nonfinite_dr2_case_ids": nonfinite_dr2_ids,
        # Harmonic mean complements geomean per Eeckhout (2024): always $\le$ geomean
...
def write_run(
    ...
    """Write results/manifest JSONs to run dir and update index.

    Note:
        Both artifacts are sanitized (non-finite floats coerced to 0.0): a
        single degenerate metric (NaN ``reduced_chi2``, an Inf headline
        geomean) must not raise mid-write and strand an empty run dir.
        ``allow_nan=False`` stays as a backstop so anything that slipped
        through still fails loudly rather than emitting RFC-invalid JSON.

        G5 disclosure: each artifact reports its OWN sanitizer suppressions
        — ``results.json`` via the contract field
        ``manifest.sanitizedValuePaths``, ``manifest.json`` via its
        ``sanitized_value_paths`` key — injected post-dump (the paths are
        only known after sanitizing); the contract field's default ``[]``
        makes the injected value round-trip-valid.

        The index update happens INSIDE the ``try`` block, after both writes
        succeed, so a failed run never leaks a half-written entry into
        ``index.json``; it uses the lean ``manifest`` dict, not
        ``run_manifest`` (see :func:`_per_case_points`).
    """
    ...
    if isinstance(manifest_block, dict):
        cast("dict[str, object]", manifest_block)["sanitizedValuePaths"] = payload_suppressed
    ...
    try:
        ...
        if not (run_dir / "results.json").exists() or not (run_dir / "manifest.json").exists():
            shutil.rmtree(run_dir, ignore_errors=True)
        raise
    return run_dir


def _update_index(manifest: dict[str, Any], root: Path = REPORTS_ROOT) -> None:
    """Prepend *manifest* to ``index.json`` (newest first; deduplicate).
    ...
    runs = [
        r
        for r in runs
        if not (r.get("run_id") == manifest["run_id"] and r.get("category") == manifest["category"])
    ]
...
def read_perf_baseline(root: Path = REPORTS_ROOT) -> dict[str, Any] | None:
...
def write_perf_baseline(manifest: dict[str, Any], root: Path = REPORTS_ROOT) -> Path:
    ...
    Note:
        This is the Vista-trap fix: a single pinned JSON sidecar at
        ``.spectrafit_reports/perf_baseline.json`` captures the geomean
        speedup of a chosen run so the gate can answer "did *we* get
        slower?", not just "are we still faster than the oracle?". The pin
        is intentionally cross-category (one file) — ``category`` is
        recorded inside the payload so a pin from ``benchmark`` is not used
        to gate a future ``quick`` run; ``baseline_solver_id`` is stored for
        the same reason, so a baseline pinned against lmfit cannot silently
        grade a later run that switched baselines.
    """
    geomean = float(
        manifest.get(
