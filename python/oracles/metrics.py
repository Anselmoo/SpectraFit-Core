r"""Statistics that turn fit outcomes into the frozen report contract sub-objects.

Everything here is computed from real measurements (timing repetitions, Monte-Carlo
noise realizations, covariance at the solution); nothing is fabricated. The 1/$\sqrt{n}$
...
def pcts(xs: list[float], qs: tuple[float, ...]) -> list[float]:
    ...
    Args:
        xs: Sample values.
        qs: Quantiles to compute, each in ``[0, 1]``.

    Returns:
        One interpolated value per entry in ``qs``, in the same order.
    """
    s = sorted(xs)
    out = []
    for q in qs:
...
def accuracy_dist(red_chi2_samples: list[float]) -> AccuracyDist:
    r"""Build an :class:`AccuracyDist` from reduced-$\chi^2$ Monte-Carlo samples."""
...
def cov_to_corr(cov: list[list[float | None]] | None) -> list[list[float]]:
    ...
    if not cov:
        return []
    mat = np.array(
        [[0.0 if v is None else float(v) for v in row] for row in cov],
        dtype=float,
    )
...
def spread_vs_runs(samples: list[float], runs_sched: list[int]) -> list[SpreadPt]:
    r"""Mean and sd of *samples* subsampled per run count.
    ...
    Args:
        samples: The full pool of measurements to subsample from.
        runs_sched: Cumulative-run counts to evaluate; each is clamped to
            ``[1, len(samples)]`` before taking the leading slice.

    Returns:
        One :class:`SpreadPt` per entry of ``runs_sched``, in that order.
    """
    out = []
...
def amortization_curve(
    ...
    Args:
        cold_ms: One-off cost paid on the first run (e.g. JIT warm-up).
        hot_ms: Steady-state per-run cost paid on every subsequent run.
        schedule: Cumulative run counts to evaluate.

    Returns:
        One :class:`Point2D` per entry of ``schedule``, with ``x`` the run
        count ``k`` and ``y`` the amortized per-run time
        ``(cold_ms + hot_ms * (k - 1)) / k``.
    """
    return [Point2D(x=k, y=(cold_ms + hot_ms * (k - 1)) / k) for k in schedule]


class _PullSample(NamedTuple):
    r"""One valid MC sample: parameter name, its $\sigma$, and the resulting pull value.
...
def _iter_valid_pulls(
    ...
    r"""Yield a :class:`_PullSample` for every valid (est, se, true) sample.

    A sample is valid when $\sigma$ is a positive number AND the estimate dict carries the
    parameter key. Invalid samples (None / non-positive $\sigma$ / missing key) are skipped
    silently — they contribute neither to pulls nor to the $\sigma$ vector.

    Args:
        estimates: Per-MC-iteration dicts of fitted parameter values.
        stderrs: Per-MC-iteration dicts of fitted parameter standard errors
            (paired with ``estimates`` by index; ``None`` where unavailable).
        true_params: The ground-truth value for each parameter of interest.

    Yields:
        A :class:`_PullSample` for each ``(estimate, stderr, true)`` triple
        that passes the validity check above.
    """
    for est, se in zip(estimates, stderrs, strict=True):
...
def pulls_from_mc(
    ...
    r"""Pull statistic and coverage across MC fits.
    ...
    Returns:
        An :class:`Uncertainty` with the pull list (``[0.0]`` if empty),
        the coverage fraction (``None`` if no valid pulls), and the
    ...
    coverage: float | None = sum(1 for p in pulls if abs(p) < 1.0) / len(pulls) if pulls else None
    return Uncertainty(
        pulls=pulls or [0.0],
        coverage=coverage,
...
def r2_of(y: Array, fit: Array) -> float:
    ...
    Args:
        y: Observed data.
        fit: Model evaluated at the fitted parameters.
...
def rmse_of(y: Array, fit: Array) -> float:
    ...
    Returns:
        The root-mean-square residual between ``y`` and ``fit``.
    """
...
def chi2_red_of(y: Array, fit: Array, sigma: Array | None, dof: int) -> float:
    r"""Reduced $\chi^2$; unweighted if ``sigma`` is None.

    Args:
        y: Observed data.
        fit: Model evaluated at the fitted parameters.
        sigma: Per-point uncertainty, or ``None`` for unweighted (all-ones
            weights).
        dof: Degrees of freedom to divide by (floored at 1).

    Returns:
        $\chi^2_{\nu} = \sum w_i (y_i - \mathrm{fit}_i)^2 / \max(\mathrm{dof}, 1)$,
        with $w_i = 1/\sigma_i^2$ (or $1$ when ``sigma`` is ``None``).
    """
    y_arr = np.asarray(y)
    fit_arr = np.asarray(fit)
    weights = np.ones_like(y_arr) if sigma is None else 1.0 / np.asarray(sigma) ** 2
