...
def spread_vs_runs(samples: list[float], runs_sched: list[int]) -> list[SpreadPt]:
    r"""Mean $\pm$ sd of *samples* subsampled to each run count (real 1/$\sqrt{n}$ behaviour).

    The sample sd needs $\ge$2 points; for a single-point subsample sd is 0.0 (never
...
def _iter_valid_pulls(
    ...
    r"""Yield a :class:`_PullSample` for every valid (est, se) $\times$ true_params sample.
...
def pulls_from_mc(
    ...
    r"""Pull = $(\mathrm{estimate}-\mathrm{truth})/\sigma$ across MC fits; coverage = frac |pull|<1.
...
def chi2_red_of(y: Array, fit: Array, sigma: Array | None, dof: int) -> float:
    r"""Reduced $\chi^2$. If ``sigma`` is None, falls back to $\sigma$=1 (unweighted)."""
    y_arr = np.asarray(y)
