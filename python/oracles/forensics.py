...
def render_case(
    ...
    try:
        ...
        msg = "matplotlib is required for forensics; install via `uv sync --extra benchmark`"
    ...
    # strict=False: colors is a fixed 6-entry palette deliberately truncated to
    # len(fits_by_backend); if more than 6 backends are ever registered, the extras
    # simply go unplotted here rather than crashing forensics rendering.
    for (backend_name, (fit_curve, _)), color in zip(
        fits_by_backend.items(),
        colors[: len(fits_by_backend)],
        strict=False,
    ):
    ...
    ax_fit.grid(True, alpha=0.3)  # noqa: FBT003 — matplotlib Axes.grid's own signature
    ...
    # strict=False: same fixed 6-entry color palette as the fit-curve loop above;
    # a backend count beyond len(colors) degrades gracefully by skipping the extras.
    ...
    ax_resid.grid(True, alpha=0.3)  # noqa: FBT003 — matplotlib Axes.grid's own signature
...
def render_regressions(
    ...
    Args:
        run_dir (Path): Directory containing ``manifest.json`` for the benchmark run.
        backends (Sequence[Backend] | None): Backends to use for fitting. When *None* (the default), the
            full set of available backends is resolved at call time via
            ``oracles.backends.get_backends()``.  Callers that already have a
            backends list (e.g. ``spc-bench forensics`` in the CLI) should
            pass it explicitly so that this module carries **no runtime
    ...
    for case_id in regression_case_ids:
        ...
        except Exception as exc:  # noqa: BLE001 - skip a case whose PNG render raised
