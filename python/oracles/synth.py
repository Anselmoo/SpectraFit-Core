...
A small, seeded, contract-valid report used to prove the :mod:`contract` is
complete, to provide the ``web/`` UI with a fixture, and to give the
round-trip schema test a target. It mirrors the essential structure of the
``BenchReport`` contract at reduced size.
...
# ---------------------------------------------------------------------------
# Phase helpers (see build_featured's docstring for the RNG-order invariant)
# ---------------------------------------------------------------------------
...
def _build_featured_per_solver_timings() -> _FeaturedTimings:
    ...
    param_names: list[str] = [
        f"{lbl}.{p}" for lbl in ("g1", "g2", "g3") for p in ("amplitude", "center", "sigma")
    ]
...
def _stab_entry(idx: int, runs_sched: list[int]) -> StabilityEntry:
    ...
    return StabilityEntry(
        ...
        red_chi2=[SpreadPt(n=k, mean=1.0, sd=0.3 / math.sqrt(k)) for k in runs_sched],
        iters=[SpreadPt(n=k, mean=7.0 + idx, sd=1.0 / math.sqrt(k)) for k in runs_sched],
    )
...
def _assemble_backend_profile(
    ...
    *,
    ...
    c0 = amed * (n - 9) * 80
    conv = [amed * (n - 9) + (c0 - amed * (n - 9)) * math.exp(-0.7 * i) for i in range(n_iter)]
    ...
    return BackendProfile(
        fit=SolverFit(
            params=fit_block.pf,
            curve=fit_block.curve,
            resid=fit_block.resid,
        ),
        conv=conv,
        grad=[math.sqrt(max(c, 1e-9)) * math.exp(-0.6 * i) for i, c in enumerate(conv)],
        # history_source is "reconstructed" here, so conv_eff stays None: a
        # proxy-derived efficiency must not be presented as a measured series.
        conv_eff=None,
        ...
        summary=summary,
        param_err=[abs(rng.gauss(0, timings.perturb[sid])) * 100 for _ in timings.param_names],
        ...
        warmup=Warmup(
            curve=[Point2D(x=k, y=(cold + ms * (k - 1)) / k) for k in range(1, 101)],
            pts=[WarmupPt(n=k, per_run=(cold + ms * (k - 1)) / k) for k in timings.schedule],
            hot_throughput=1000.0 / ms,
            cold_ms=cold,
            hot_ms=ms,
        ),
        scaling=[Point2D(x=nn, y=a * nn + b + rng.gauss(0, 0.04 * b)) for nn in timings.n_grid],
...
def _build_featured_extras(
    ...
    *,
    ...
    peaks = [
        Peak(label=f"g{k + 1}", y=[_g(xi, sf[k].a, sf[k].c, sf[k].s) for xi in x]) for k in range(3)
    ]
    corr = [
...
def build_featured(rng: random.Random, n: int = 48) -> Featured:
    ...
    Note:
        The ``_build_featured_*`` helpers below are pydantic-first wrappers
        for the four logical groups of fields this builder fills. Each helper
        preserves the original RNG draw order; reordering any call here will
        silently shift the deterministic JSON output and break the
        byte-identical regression guard in
        ``tests/unit/oracles/test_build_featured_phases.py``.
    """
...
def build_suite(rng: random.Random) -> list[SuiteCase]:
    ...
    Note:
        Mirrors real data: jax is unsupported on multimodal/global (``optfn``)
        cases, so it is absent from a case's metric map there. Keeping the
        fixture realistic means the render smoke exercises the
        missing-backend code paths.
    """
    out: list[SuiteCase] = []
...
def build_report(seed: int = 20260602) -> BenchReport:
    ...
    Note:
        ``manifest`` is filled with deterministic, plausible defaults for the
        GateBadge; this synthetic fixture is used by the FastAPI smoke (no
        real fits), so the numbers are pinned rather than derived from
        ``_compute_headline_numbers`` over the random suite. Real runs go
        through ``engine.build_report``, which calls
        ``compute_manifest_signals(report)`` for the live numbers.
    """
