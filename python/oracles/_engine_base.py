...
def _finite(x: float, default: float = 0.0) -> float:
    r"""Return *x* if finite, else *default* — keeps NaN/Inf out of the contract.
    ...
    - ``r2`` uses ``-1.0`` (impossible for a fit result; distinguishable from 0.0
      which is a legitimate $R^2$ value on a constant model).
    - ``red_chi2`` uses ``0.0`` ($\chi^2$ is non-negative; 0.0 is a safe sentinel
      for a failed/missing $\chi^2$ because ``success=False`` is the authoritative flag).
    """
