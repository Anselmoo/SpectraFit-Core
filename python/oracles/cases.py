...
r"""Info:
    EASY: one peak shape under a qualitative condition (tag, noise, $\sigma$-scale,
    amplitude).
"""
...
r"""Info:
    COMPLEX: each (model $\times$ peak-count) blend exactly once.
"""
...
r"""Info:
    SCALING: peak count $\times$ noise level (the repetition axis is explicit, not
    padding).
"""
...
def _lineshapes(rng: random.Random, k: int) -> tuple[list[Component], dict[str, Any]]:
    ...
    Note:
        Recipes in ``_LINESHAPE_X_RANGE`` (Cauchy dispersion, KWW, Tauc band-gap, and
        the NIST regression/saturation curves — BoxBOD, Misra1b, Bennett5, MGH09) get
        a positive-x range because their kernels are only defined for ``x > 0`` (or
        their physical edge lives at positive energies): on a grid spanning negative
