...
class Pearson7Spec(_Component):
    r"""Pearson VII peak (tunable tail weight ``m``: m→1 Lorentzian, m→$\infty$ Gaussian)."""
...
class SaturatingExponentialSpec(_Component):
    r"""Saturating exponential $\mathrm{amplitude}\cdot(1-\exp(-\mathrm{rate}\cdot x))$ (NIST BoxBOD, positive-x)."""
...
class PowerSaturationSpec(_Component):
    r"""Power-law saturation $\mathrm{amplitude}\cdot(1-(1+\mathrm{rate}\cdot x/2)^{-2})$ (NIST Misra1b, positive-x)."""
...
class PowerLawOffsetSpec(_Component):
    r"""Power-law with offset $\mathrm{amplitude}\cdot(\mathrm{offset}+x)^{-1/\mathrm{shape}}$ (Bennett5-like, positive-x).
...
class RationalCubicSpec(_Component):
    r"""Rational cubic over cubic, denominator constant pinned at 1 (NIST StRD Kirby2/Hahn1/Thurber).
    ...
    so the denominator stays positive across the grid.
    """
...
def _edge_case(rng: random.Random, k: int) -> tuple[list[Component], dict[str, Any]]:
    r"""Dispatch one ill-conditioned regime per case over the unique (regime $\times$ shape) grid.
