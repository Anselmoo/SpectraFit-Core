...
!!! info "Edge-case design philosophy"
...
!!! note "Diversity-driven generation"
...
from oracles.exceptions import CaseSpecError
...
class CategoryDef(BaseModel):
    ...
    r"""Whether the category counts toward the baseline-parity axes.
...
CATEGORY_REGISTRY: dict[str, CategoryDef] = {
    c.id: c
    for c in (
        # Counts are diversity-driven (one case per distinct model $\times$ condition), NOT
        ...
        # Tied/shared-parameter cases: two peaks with shared $\sigma$, tied centers, or
        ...
        # Robust fitting: IRLS (Huber / bisquare / Cauchy) on spectra with a
        # declared fraction of gross outliers. The baselines run plain least
        # squares on the same contaminated data, so this category is scored on
        # recovery-vs-truth, not on parity with the baseline.
        CategoryDef(
            id="robust",
            label="Robust / contaminated",
            prefix="RB",
            hue="var(--warn)",
            count=6,  # == len(_ROBUST_GRID), asserted in tests
            baseline_comparable=False,
        ),
    )
}
...
NON_COMPARABLE_CATEGORIES: frozenset[str] = frozenset(
    cid for cid, c in CATEGORY_REGISTRY.items() if not c.baseline_comparable
)
...
class _Component(BaseModel):
    ...
    def to_params(self) -> dict[str, float]:
        """Parameters dict for the model kernel (excludes discriminator)."""
...
class Pearson7Spec(_Component):
    r"""Pearson VII peak with tunable tail weight ``m``."""
...
class MoffatSpec(_Component):
    r"""Moffat peak ($\beta$ tail weight)."""
...
class AsymIrSpec(_Component):
    r"""Asymmetric IR band (Gaussian $\times$ logistic sigmoid)."""
...
class CauchyDispersionSpec(_Component):
    r"""Cauchy refractive-index dispersion $a + b/x^2 + c/x^4$ (positive-x)."""
...
class SaturatingExponentialSpec(_Component):
    r"""Saturating exponential (NIST BoxBOD, positive-x).
...
class PowerSaturationSpec(_Component):
    r"""Power-law saturation (NIST Misra1b, positive-x).
...
class PowerLawOffsetSpec(_Component):
    r"""Power-law with offset (Bennett5-like, positive-x).
...
class Mgh09RationalSpec(_Component):
    r"""Kowalik–Osborne rational function (NIST StRD MGH09, positive-x).
...
class RationalCubicSpec(_Component):
    r"""Rational cubic with unit denominator constant (NIST StRD benchmarks).
    ...
    model: Literal["rational_cubic"] = "rational_cubic"
    a0: float
    a1: float
    a2: float
    a3: float
...
class GeneralisedLogisticSpec(_Component):
    r"""Generalised logistic (Richards) curve (NIST StRD Rat42/Rat43).

    $\mathrm{amplitude}/(1+\exp(\mathrm{shift}-\mathrm{rate}\cdot x))^{1/\mathrm{shape}}$;
    unrestricted domain, no positive-x requirement.
    """

    model: Literal["generalised_logistic"] = "generalised_logistic"
    amplitude: float
    shift: float
    rate: float
    shape: float
...
class ExpOverLinearSpec(_Component):
    r"""Exponential decay over a line (NIST StRD Chwirut1/Chwirut2).
    ...
    model: Literal["exp_over_linear"] = "exp_over_linear"
    rate: float
    lin_const: float
    lin_slope: float
...
class OutlierSpec(BaseModel):
    r"""Gross-outlier contamination applied by :func:`materialize` after the noise.
    ...
    fraction: float = Field(gt=0.0, le=0.5)
...
class BenchCase(BaseModel):
    ...
    def true_params(self) -> dict[str, float]:
        ...
        return {
            f"p{i}.{k}": v for i, c in enumerate(self.comp_true) for k, v in c.to_params().items()
        }
...
def _binned_centers(rng: random.Random, n: int, lo: float, hi: float) -> list[float]:
    ...
    step = (hi - lo) / n
    return [round(lo + step * (i + 0.5) + rng.uniform(-0.25, 0.25) * step, 3) for i in range(n)]
...
def _peak(rng: random.Random, model: str, center: float, sigma_cap: float) -> Component:
    ...
    match model:
        ...
        case _:  # pragma: no cover - generator passes known keys
            msg = f"not a peak model: {model!r}"
            raise CaseSpecError(msg)
...
class CaseFamily(BaseModel):
    ...
    category: str
    count: int
    compose: Callable[[random.Random, int], tuple[list[Component], dict[str, Any]]]
    ...
    def expand(self, rng: random.Random) -> list[CaseSpec]:
        ...
        for k in range(self.count):
            ...
            n_points = (
                self.n_points * (1 + k // 6) if self.points_scale_per_block else self.n_points
            )
...
""""Complex" = genuinely hard shapes (asymmetric / true-Voigt / Fano), NOT the
trivial single-Gaussian family — those belong in `easy`. Blends of these stress
the solver.
"""
_REALITY_RECIPES = [
    "xps",
...
"""All registered optimization landscapes (derived — adding one to
`LANDSCAPE_REGISTRY` grows `optfn` by exactly one unique case, no padding).
"""
...
r"""EASY: one peak shape under a qualitative condition (tag, noise, $\sigma$-scale,
amplitude).
"""
_EASY_GRID: list[tuple[str, tuple[str, float, float, float]]] = [
    (m, cond) for m in _EASY_MODELS for cond in _EASY_CONDITIONS
]
_COMPLEX_GRID: list[tuple[str, int]] = [(m, n) for m in _COMPLEX_MODELS for n in (2, 3, 4, 5, 6)]
r"""COMPLEX: each (model $\times$ peak-count) blend exactly once.
"""
_REALITY_GRID: list[tuple[str, int]] = [(r, v) for r in _REALITY_RECIPES for v in (0, 1)]
"""REALITY: each recipe under a low / high noise variant.
"""
_SCALING_GRID: list[tuple[int, str]] = [(n, lvl) for n in (5, 8, 11, 14) for lvl in ("lo", "hi")]
r"""SCALING: peak count $\times$ noise level (the repetition axis is explicit, not
padding).
"""


def _easy(rng: random.Random, k: int) -> tuple[list[Component], dict[str, Any]]:
...
def _complex(rng: random.Random, k: int) -> tuple[list[Component], dict[str, str]]:
...
def _reality_recipe(
    ...
    recipe: str,
    v: int,
) -> tuple[list[Component], str, dict[str, tuple[float, float]]]:
    ...
    match recipe:
        ...
        case "xas":
            edge = _edge(
                rng,
                "arctan_step" if v else "erfc_step",
                rng.uniform(-1.0, 1.0),
            )
            return (
...
def _reality(rng: random.Random, k: int) -> tuple[list[Component], dict[str, Any]]:
...
def _optfn(_rng: random.Random, k: int) -> tuple[list[Component], dict[str, Any]]:
...
def _scaling(rng: random.Random, k: int) -> tuple[list[Component], dict[str, Any]]:
...
# --------------------------------------------------------------------------- #
# Edge / ill-conditioned recipes — design rationale in the module docstring.
# --------------------------------------------------------------------------- #
...
def _typed_peak(
    ...
    """Create one peak from explicit geometry.

    No rng — the edge knobs set everything.
    """
    match model:
        case "gaussian":
            return GaussianSpec(
        ...
        case _:  # pragma: no cover - generator passes known keys
            msg = f"not an edge peak model: {model!r}"
            raise CaseSpecError(msg)


def _edge_doublet(rng: random.Random, k: int) -> tuple[list[Component], dict[str, Any]]:
...
def _edge_case(rng: random.Random, k: int) -> tuple[list[Component], dict[str, Any]]:
    r"""Dispatch ill-conditioned regimes over regime $\times$ shape grid.

    Note:
        ``_EDGE_GRID_LEN`` relies on 5 regimes and 4 peak shapes being coprime: over
        ``k=0..19``, ``(k % 5, k % 4)`` enumerates all 20 (regime, shape) combinations
        exactly once, so ``count=20`` gives a unique grid with no cycling
        (``mixed_blend`` ignores shape; its 4 slots vary only the noise draw).
    """
...
_LINESHAPE_X_RANGE: dict[str, tuple[float, float]] = {
    ...
    # Keeps the rational_cubic denominator (1+b1x+b2x²+b3x³) away from its
    # root — same grid the parity-test values were derived against.
    "rational_cubic": (-4.0, 8.0),
    # exp_over_linear denominator (lin_const+lin_slope·x) is monotone
    # increasing for x>=0 with both coefficients positive, so a positive-x
    # grid keeps it >= lin_const with no sign-flip risk (mirrors mgh09_rational).
    "exp_over_linear": (0.0, 8.0),
}
...
"""Per-model `gamma` ranges.

- `true_voigt`: Lorentzian HWHM
- `skewed_gaussian`: skew
- `exp_gaussian`: EMG rate
- `doniach_sunjic`: DS asymmetry
"""
...
def _lineshapes(rng: random.Random, k: int) -> tuple[list[Component], dict[str, Any]]:
    """One asymmetric-lineshape scenario per case, indexed over the explicit grid.

    Note:
        Recipes in ``_LINESHAPE_X_RANGE`` (Cauchy dispersion, KWW, Tauc band-gap, the
        NIST regression/saturation curves — BoxBOD, Misra1b, Bennett5, MGH09, Chwirut —
        and rational_cubic's narrower symmetric grid) get a positive-x range because
        their kernels are only defined for ``x > 0`` (or their physical edge lives at
        positive energies), or a narrowed range to keep a denominator away from its
        root: on a grid spanning negative
        x they hit a negative-base fractional power / domain-guard NaN, so each is
        kept to a positive-x range (``offset + x > 0``, ``1 + rate * x / 2 > 0`` stay
        satisfied).
    """
...
def _lineshape_recipe(
    ...
    """Dispatch to the registered lineshape recipe.
    ...
    if fn is None:
        msg = (
            f"unknown lineshape recipe {recipe!r}; registered: {sorted(LINESHAPE_RECIPE_REGISTRY)}"
        )
        raise CaseSpecError(msg)
...
def _fixed_case(rng: random.Random, k: int) -> tuple[list[Component], dict[str, Any]]:
    """Single-peak case with the center held fixed at its truth value.

    Note:
        Four cases (see ``_FIXED_GRID``): each fixes the center of the first
        (and only) peak at truth so the solver must recover amplitude +
        sigma while the center is held constant. Models covered: gaussian,
        lorentzian, pseudo_voigt, and gaussian again under noise.
    """
...
def _tied_case(rng: random.Random, k: int) -> tuple[list[Component], dict[str, Any]]:
    """Two-peak case with one parameter tied between peaks via expr_edges.

    Note:
        Four cases (see ``_TIED_GRID``): two-peak fits with a parameter tied
        across peaks — tie semantics e.g. ``p1.sigma = p0.sigma`` (shared
        width), or similar. Backend support: spectrafit (``FitGraph``
        ``expr_edges``) and lmfit (``expr=`` param). Disclosed exclusions:
        jax and scipy-ls cannot express parameter ties.
    """
...
_ROBUST_WEIGHTS = ("huber", "bisquare", "cauchy")
_ROBUST_FRACTIONS = (0.05, 0.15)
_ROBUST_GRID = [(w, f) for f in _ROBUST_FRACTIONS for w in _ROBUST_WEIGHTS]
"""IRLS weight function $\\times$ contamination fraction — one case each."""
...
def _robust_case(
    ...
    r"""One Gaussian under gross-outlier contamination, fitted with ``irls:<weight>``.

    The spike magnitude (4 $\times$ the peak height) sits far outside the
    noise floor so the case isolates the loss function: Huber bounds the
    influence, bisquare rejects, Cauchy down-weights smoothly. ``condition``
    keeps the per-category uniqueness invariant (same model, distinct
    weight/fraction pairs).
    """
    weight, fraction = _ROBUST_GRID[k]
    comps = [
        _typed_peak(
            "gaussian",
            round(rng.uniform(3.0, 6.0), 3),
    ...
    return comps, {
        "name": f"{weight} IRLS · {pct}% outliers",
        "solver_hint": f"irls:{weight}",
        "outliers": OutlierSpec(fraction=fraction, magnitude=4.0),
        "condition": f"{weight}-{pct}pct",
        "x_range": (-5.0, 5.0),
    }
...
FAMILIES: list[CaseFamily] = [
    ...
    # Doniach–Šunjić; XPS spin-orbit doublets, XAS L-edge L3/L2, XANES K-edge; plus
    # the NIST StRD rational/logistic/decay kernels (rational_cubic, generalised_logistic,
    # exp_over_linear).
    CaseFamily(
        category="lineshapes",
    ...
    # Robust fitting: one Gaussian, Gaussian noise, plus 5 % or 15 % gross
    # outliers; spectrafit runs IRLS with the named weight function while the
    # baselines run least squares on the same contaminated data.
    CaseFamily(
        category="robust",
        count=len(_ROBUST_GRID),
        compose=_robust_case,
        x_range=(-5.0, 5.0),
        n_points=160,
        noise=0.03,
        difficulty=(0.3, 0.6),
...
def materialize(spec: CaseSpec) -> BenchCase:
    ...
    if spec.landscape is not None:
        ...
        if spec.outliers is not None:
            # Same id-seeded RNG, drawn AFTER the noise so the clean draw is
            # unchanged; distinct indices, random sign, amplitude-scaled spike.
            n_out = round(spec.outliers.fraction * spec.n_points)
            spike = spec.outliers.magnitude * float(np.abs(curve(x, truth)).max())
            for idx in rng.sample(range(spec.n_points), n_out):
                y[idx] += spike if rng.random() < 0.5 else -spike
