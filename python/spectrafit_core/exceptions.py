"""Domain exception types raised by ``spectrafit_core``.

Every error this package raises deliberately derives from **both**
``SpectraFitError`` — so a caller can catch everything originating here
...
Caveat: this guarantee covers only errors raised directly by this package's
own validators and API functions. Errors that originate in the Rust engine
and cross the PyO3 boundary (``crates/spectrafit-core/src/lib.rs``, which
maps ``CoreError`` and JSON-parse failures to ``PyValueError``) arrive as a
plain ``ValueError``, not a ``SpectraFitError`` subclass.
"""
...
class SpectraFitError(Exception):
    """Base class for every error raised by ``spectrafit_core``.

    Deliberately *not* a ``ValueError`` subclass, so that
