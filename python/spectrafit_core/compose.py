"""``compose()`` DSL — pure-Python sugar for building [`FitGraph`][spectrafit_core.FitGraph].

The [`FitGraph`][spectrafit_core.FitGraph] Pydantic contract is unchanged. This module adds two layers
on top of it:

* **Factory functions** per canonical [`ModelType`][spectrafit_core.ModelType] member
  ([`gaussian`][spectrafit_core.compose.gaussian], [`lorentzian`][spectrafit_core.compose.lorentzian], [`voigt`][spectrafit_core.compose.voigt], …) that take the model's
  parameters as keyword arguments and return a ready-to-use
  [`ModelNodeSpec`][spectrafit_core.ModelNodeSpec]. Bounds and other [`Parameter`][spectrafit_core.Parameter] fields propagate
...
* [`compose`][spectrafit_core.compose] + [`ComposeBuilder`][spectrafit_core.ComposeBuilder] — a chainable helper that gathers
...
def generalised_logistic(
    ...
    **params: ParamKwarg,
    ...
    r"""Build a generalised-logistic node (``amplitude``, ``shift``, ``rate``, ``shape``).
...
class ComposeBuilder(BaseModel):
    ...
    Iterating over a [`ComposeBuilder`][spectrafit_core.ComposeBuilder] yields its nodes, so the builder
    ...
    [`FitGraph`][spectrafit_core.FitGraph], including any [`ExprEdge`][spectrafit_core.ExprEdge] constraints added via
    ...
    def build(self) -> FitGraph:
        """Materialise an immutable [`FitGraph`][spectrafit_core.FitGraph] from the accumulator."""
        return FitGraph(
...
def compose(nodes: Sequence[ModelNodeSpec]) -> ComposeBuilder:
    """Start a [`ComposeBuilder`][spectrafit_core.ComposeBuilder] from a sequence of model nodes.

    Args:
        nodes (Sequence[ModelNodeSpec]): Nodes built via the factory
            functions above (or any other [`ModelNodeSpec`][spectrafit_core.ModelNodeSpec] instances).

    Returns:
        A fresh [`ComposeBuilder`][spectrafit_core.ComposeBuilder] ready for [`ComposeBuilder.bind`][spectrafit_core.ComposeBuilder.bind]
