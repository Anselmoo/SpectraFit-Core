...
def fit(graph_json: str, data_json: str, options_json: str) -> str:
    """Run a fit with the solver named in `options_json` and return JSON.

    Runs the solver named by the `solver` field of `options_json` (default
    `"lm"`, Levenberg-Marquardt on the faer-native trust-region core).
...
def fit_arrays(
    ...
    Raises:
        ValueError: If the JSON is malformed, the graph is invalid, or the array
            shapes are inconsistent (``n_total * n_dims != len(x)``).  A Rust
            panic raised inside the solver is caught at the PyO3 boundary and
            re-raised as a ``ValueError``, never an uncatchable
