...
def evaluate(
    ...
    Validates ``graph`` into a [`FitGraph`][spectrafit_core.FitGraph], then
    ...
    Args:
        graph (FitGraph): Model topology as a
            [`FitGraph`][spectrafit_core.FitGraph] (or anything
            `FitGraph.model_validate` accepts). ``nodes`` must have
            unique ids; ``expr_edges`` and per-parameter ``expr``
            constraints, if present, must form a DAG.
        params (Mapping[str, object]): Parameter values keyed by
            ``"node_id.param_name"`` (dotted notation), e.g.
            ``{"peak1.amplitude": 1.0, "peak1.center": 0.0}``. Values are
            JSON-encoded as-is (`BaseModel`,
            `numpy.ndarray`/`numpy.generic`, and nested
            list/dict/mapping values are all converted to plain JSON via
            `_dump_params_json`).
        data (MeasurementInput): One or more datasets as a
            [`MeasurementData`][spectrafit_core.MeasurementData] or a sequence
            thereof (anything accepted by
            `dump_measurement_json`). Only the
    ...
    Raises:
        pydantic.ValidationError: If ``graph`` fails
            [`FitGraph`][spectrafit_core.FitGraph] validation (duplicate
...
def evaluate_components(
    ...
    Identical in spirit to [`evaluate`][spectrafit_core.evaluate], but calls the
    ...
    summing them, so the returned components sum to what [`evaluate`][spectrafit_core.evaluate]
    (or [`best_fit`][spectrafit_core.FitResult]) would return.
    ...
    Args:
        graph (FitGraph): Model topology as a
            ...
            constraints, if present, must form a DAG.
        params (Mapping[str, object]): Parameter values keyed by
            ``"node_id.param_name"`` (dotted notation). Values are
            JSON-encoded via `_dump_params_json`.
        data (MeasurementInput): One or more datasets as a
            [`MeasurementData`][spectrafit_core.MeasurementData] or a sequence
