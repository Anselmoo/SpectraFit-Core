...
def evaluate(
    ...
    Args:
        graph (FitGraph): Model topology as a
            :class:`~spectrafit_core.graph.FitGraph` (or anything
            :meth:`FitGraph.model_validate` accepts). ``nodes`` must have
        ...
        params (Mapping[str, object]): Parameter values keyed by
            ...
            JSON-encoded as-is (:class:`~pydantic.BaseModel`,
...
def evaluate_components(
    ...
    Args:
        ...
        params (Mapping[str, object]): Parameter values keyed by
            ...
            JSON-encoded via :func:`~spectrafit_core.graph._dump_params_json`.
        data (MeasurementInput): One or more datasets as a
            :class:`~spectrafit_core.data.MeasurementData` or a sequence
