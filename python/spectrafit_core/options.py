...
class FitOptions(BaseModel):
    r"""Solver configuration for :func:`~spectrafit_core.fit`.

    Attributes:
        schema_version (str): IR schema version (do not change).
        solver (str): Which solver to use.  Supported values:
        ...
        max_iterations (int): Maximum solver iterations (default 200). Must be
            ``>= 1`` — ``max_iterations=0`` would silently produce a no-op fit.
