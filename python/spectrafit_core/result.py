...
class FitResult(BaseModel):
    ...
    Attributes:
        ...
        dof (int): Degrees of freedom, clamped to
            ``max(n_data - n_free_params, 1)``. When the fit is exactly
            determined or over-parameterised the true value is $\le 0$ and
            ``reduced_chi2`` is not meaningful; the clamp keeps it finite
    ...
    def params(self) -> dict[str, ParameterResult]:
        """Alias for [`parameters`][spectrafit_core.FitResult] (dotted-name → fitted result)."""
