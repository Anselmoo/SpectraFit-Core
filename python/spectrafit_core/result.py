...
class FitResult(BaseModel):
    r"""Result of a single :func:`~spectrafit_core.fit` call.
    ...
    Attributes:
        ...
        chi2 (float): Total sum of squared (weighted) residuals.
        reduced_chi2 (float): ``chi2 / dof`` — values near 1.0 indicate a
            good fit.
        r_squared (float): Coefficient of determination $R^2$.
        dof (int): Degrees of freedom (``n_data - n_free_params``).
        aic (float): Akaike Information Criterion.
        bic (float): Bayesian Information Criterion.
        n_iter (int): Number of solver iterations taken.
        ...
        success (bool): ``True`` if the solver converged within
            ``max_iterations``.
        message (str): Human-readable solver status message.
        ...
        gradient_norm_history (list[float]): Per-iteration gradient
            ...
            :attr:`cost_history` entry. Empty when not tracked.
        params_history (list[list[float]]): Per-iteration free-parameter
            vector $\theta$ recorded alongside each :attr:`cost_history`
        ...
        covariance_param_order (list[str] | None): Ordered list of
            ...
            :attr:`covariance`. ``covariance[i][j]`` is the covariance
            between ``covariance_param_order[i]`` and
            ``covariance_param_order[j]``.  Use this to look up cross-terms
            by name rather than relying on :attr:`parameters` iteration
