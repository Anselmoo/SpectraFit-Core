...
export function TaxonomyPanel() {
  ...
  const renderRow = (m: FailureMode) => (
    ...
    >
      <div>
        <div style={{ fontFamily: "var(--font-mono)", fontSize: "0.84rem", color: "var(--text-primary)" }}>{m.bug}</div>
        <div style={{ fontSize: "0.8rem", color: "var(--danger)", marginTop: "var(--space-1)" }}>before · {m.before}</div>
      </div>
      <div>
        <div style={{ fontSize: "0.8rem", color: "var(--success)" }}>fix · {m.fix}</div>
        <div
          style={{
            fontSize: "0.76rem",
  ...
  return (
    <div className="glass" style={{ padding: "var(--space-6)" }}>
      ...
      <p
        data-taxonomy-lead
        style={{ margin: "0 0 var(--space-4)", fontSize: "0.8rem", color: "var(--text-secondary)", lineHeight: 1.6 }}
      >
