...
export function fitBody(report: BenchReport, ctx: PanelCtx): ReactNode {
  ...
  const metrics = metricRows(f, solverIds);
  return (
    <>
            ...
            >
              <span
                style={{
                  ...
                  width: 14,
                  height: 2,
                  background: colors[id] ?? "var(--system-blue)",
                }}
              />
              {solverLabel[id] ?? id}
            </span>
      ...
      {/* Spectrum overlay */}
      <PlotMount
        make={(w) => spectrumPlot(spectrumSeries(f, solverIds), { colors, width: w })}
        deps={[report, ctx.selectedId, ctx.view]}
        title={PLOT_SPECS["spectrum"].question}
      />
      {/* Residuals strip */}
      <PlotMount
        make={(w) => residualPlot(residualRows(f, solverIds), { colors, width: w })}
        deps={[report, ctx.selectedId, ctx.view]}
        title={PLOT_SPECS["residual"].question}
      />
              ...
              {metrics.map((row) => (
                <tr key={row.backend}>
                  <td style={{ padding: "4px 8px", borderBottom: "1px solid var(--border-subtle)" }}>
                    <BackendTag
                      backend={row.backend}
                      label={solverLabel[row.backend] ?? row.backend}
                      color={colors[row.backend] ?? "var(--system-blue)"}
                    />
                  </td>
                  <td style={{ textAlign: "right", padding: "4px 8px", borderBottom: "1px solid var(--border-subtle)" }}>
                    {row.r2.toFixed(4)}
                  </td>
                  <td style={{ textAlign: "right", padding: "4px 8px", borderBottom: "1px solid var(--border-subtle)" }}>
...
export function peaksBody(report: BenchReport, ctx: PanelCtx): ReactNode {
  ...
  return (
    <PlotMount
      make={(w) => peaksPlot(peakRows(f), w)}
      deps={[report, ctx.selectedId, ctx.view]}
      title={PLOT_SPECS["peaks"].question}
    />
  );
}
...
export function recoveryBody(report: BenchReport, ctx: PanelCtx): ReactNode {
  ...
  return (
    <>
      <PlotMount
        make={(w) => recoveryPlot(recoveryRows(f, ctx.solverIds), { colors: ctx.colors, width: w })}
        deps={[report, ctx.selectedId, ctx.view]}
        title={PLOT_SPECS["recovery"].question}
      />
      <p
        style={{
...
export function pullsBody(report: BenchReport, ctx: PanelCtx): ReactNode {
  ...
  if (f == null) return null;
  const solverLabel = solverLabelMap(report);
  const pullEntries = pullSeries(f, ctx.solverIds);
  return (
    <>
      <PlotMount
        make={(w) => pullsPlot(pullSeries(f, ctx.solverIds), { colors: ctx.colors, width: w })}
        deps={[report, ctx.selectedId, ctx.view]}
        title={PLOT_SPECS["pulls"].question}
      />
      <div
        style={{
...
export function convergenceBody(report: BenchReport, ctx: PanelCtx): ReactNode {
  ...
  const series = convSeries(f, ctx.solverIds);
  ...
  return (
    <>
      <PlotMount
        ...
        deps={[report, ctx.selectedId, ctx.view]}
        title={PLOT_SPECS["convergence"].question}
      />
      {reconstructedBackends.length > 0 && (
        <div className="absent-note" style={{ marginTop: "var(--space-3)" }}>
...
export function timingBody(report: BenchReport, ctx: PanelCtx): ReactNode {
  ...
  return (
    <PlotMount
      make={(w) => timingBoxPlot(timingBoxes(f, ctx.solverIds), { colors: ctx.colors, width: w })}
      deps={[report, ctx.selectedId, ctx.view]}
      title={PLOT_SPECS["timing"].question}
    />
  );
}
...
export function warmupBody(report: BenchReport, ctx: PanelCtx): ReactNode {
  ...
  return (
    <PlotMount
      make={(w) => warmupPlot(warmupLines(f, ctx.solverIds), { colors: ctx.colors, width: w })}
      deps={[report, ctx.selectedId, ctx.view]}
      title={PLOT_SPECS["warmup"].question}
    />
  );
}
...
export function scalingBody(report: BenchReport, ctx: PanelCtx): ReactNode {
  ...
  return (
    <PlotMount
      make={(w) =>
        scalingPlot(scalingLines(f, ctx.solverIds), {
          colors: ctx.colors,
          crossN: f.crossN,
          width: w,
        })
      ...
      deps={[report, ctx.selectedId, ctx.view]}
      title={PLOT_SPECS["scaling"].question}
    />
  );
}
...
export function reproducibilityBody(report: BenchReport, ctx: PanelCtx): ReactNode {
  ...
  return (
    <PlotMount
      make={(w) => stabilityPlot(stabilityBand(f, ctx.solverIds, "iters"), { colors: ctx.colors, width: w, metric: "iters" })}
      deps={[report, ctx.selectedId, ctx.view]}
      title={PLOT_SPECS["reproducibility"].question}
    />
  );
}
...
export function residualQQBody(report: BenchReport, ctx: PanelCtx): ReactNode {
  ...
  return (
    <PlotMount
      make={(w) =>
        residualQQPlot(residualQQSeries(report, f.id, ctx.solverIds), { colors: ctx.colors, width: w })
      }
      deps={[report, ctx.selectedId, ctx.view]}
      title={PLOT_SPECS["residual-qq"].question}
    />
  );
}
...
export function iterationsBody(report: BenchReport, ctx: PanelCtx): ReactNode {
  ...
  return (
    <PlotMount
      make={(w) =>
        iterationsPlot(iterationsSeries(report, f.id, ctx.solverIds), { colors: ctx.colors, width: w })
      }
      deps={[report, ctx.selectedId, ctx.view]}
      title={PLOT_SPECS["iterations"].question}
    />
  );
}
...
export function infoCriteriaBody(report: BenchReport, ctx: PanelCtx): ReactNode {
  const f = selectedCase(report, ctx);
  if (f == null) return null;
  const rows = infoCriteriaRows(f, ctx.solverIds);
  if (rows.length === 0) {
    return (
      <p style={{ margin: 0, fontSize: "0.85rem", color: "var(--text-secondary)", fontFamily: "var(--font-mono)" }}>
            ...
            <tr key={row.backend}>
              <td style={{ padding: "4px 8px", borderBottom: "1px solid var(--border-subtle)" }}>
                <span style={{ display: "inline-flex", alignItems: "center", gap: 6 }}>
                  ...
                  />
                  {row.best && (
                    <span style={{ color: "var(--success)", fontSize: "0.72rem" }} title="preferred model (lowest ΔAIC)">
                      ★ preferred
                    </span>
                  )}
                </span>
              </td>
              <td style={{ ...cell, color: row.best ? "var(--success)" : "var(--text-secondary)" }}>
...
export function accuracyBody(report: BenchReport, ctx: PanelCtx): ReactNode {
  const f = selectedCase(report, ctx);
  if (f == null) return null;
  const boxes = accuracyBoxes(f, ctx.solverIds);
  if (boxes.length === 0) {
    return (
      <p style={{ margin: 0, fontSize: "0.85rem", color: "var(--text-secondary)", fontFamily: "var(--font-mono)" }}>
  ...
  return (
    <>
      <PlotMount
        ...
        deps={[report, ctx.selectedId, ctx.view]}
        title={PLOT_SPECS["accuracy-dist"].question}
      />
      <p
        style={{
          margin: "var(--space-3) 0 0",
...
export function conditioningBody(report: BenchReport, ctx: PanelCtx): ReactNode {
  const f = selectedCase(report, ctx);
  if (f == null) return null;
  const kappaRowsData = kappaRows(f, ctx.solverIds);
  const topCouplingsData = topCouplings(f, 3);
  const absentKappaBackends = kappaRowsData.filter((r) => r.absent).map((r) => r.backend);
  return (
    <>
      <PlotMount
        make={(w) => {
          const rows = kappaRows(f, ctx.solverIds);
        ...
        deps={[report, ctx.selectedId, ctx.view]}
        title={PLOT_SPECS["conditioning"].question}
      />
