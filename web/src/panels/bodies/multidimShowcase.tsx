...
function StatsGrid({ md }: { md: MultiDim }): ReactNode {
  return (
    ...
    >
      ...
      <span style={{ color: "var(--text-primary)", fontWeight: 600 }}>{md.nDims}-D</span>
      <span style={{ color: "var(--text-secondary)" }}>Grid</span>
      <span>
        {md.shape.join(" × ")} ({md.nPoints.toLocaleString()} points)
      </span>
      <span style={{ color: "var(--text-secondary)" }}>r²</span>
      <span style={{ color: "var(--text-primary)", fontWeight: 600 }}>{md.rSquared.toFixed(4)}</span>
      {md.peaks.map((p, i) => (
        <span key={`peak-${i}`} style={{ display: "contents" }}>
          <span style={{ color: "var(--text-secondary)" }}>Peak {i} (A={fmt(p.amplitude)})</span>
          <span>
    ...
    <>
      ...
      {md.projections.map((proj, i) => (
        <PlotMount
          ...
          deps={[report, i]}
          title={`${PLOT_SPECS["multidim-projection"].question} (projection ${i + 1} of ${md.projections.length})`}
        />
      ))}
      <p style={{ margin: "var(--space-3) 0 0", fontSize: "0.72rem", color: "var(--text-secondary)", ...MONO }}>
