...
export function saturationBody(report: BenchReport, ctx: PanelCtx): ReactNode {
  ...
  const saturatedCategories = report.manifest?.saturatedCategories ?? [];
        ...
        {equivalence.map((eq) => (
          <span
            ...
            style={{
              ...
              fontSize: "0.76rem",
              fontFamily: "var(--font-mono)",
              border: `1px solid ${eq.equivalent ? "var(--success)" : "var(--border-subtle)"}`,
              color: eq.equivalent ? "var(--success)" : "var(--text-secondary)",
  ...
  return (
    <>
      <p style={{ margin: "0 0 var(--space-4)", fontSize: "0.82rem", color: "var(--text-secondary)", fontFamily: "var(--font-mono)", lineHeight: 1.5 }}>
        ...
        <strong style={{ color: "var(--text-primary)" }}>{baseline}</strong> —{" "}
        {inGateNonEquiv.length === 0 ? (
          ...
          <>
            ...
            {inGateNonEquiv.map((e, i) => (
              <span key={e.category}>
                {i > 0 ? ", " : ""}
                <strong style={{ color: "var(--warning)" }}>{labelOf(e.category)}</strong> (Δ{" "}
        ...
        {outOfGate.length > 0 && (
          <>
            ...
            {outOfGate.map((e, i) => (
              <span key={e.category}>
                {i > 0 ? ", " : ""}
                <strong style={{ color: "var(--text-secondary)" }}>{labelOf(e.category)}</strong>
              </span>
            ))}{" "}
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: "2px", fontFamily: "var(--font-mono)", fontSize: "0.8rem" }}>
        {equiv.map((e) => {
          ...
          return (
            ...
            >
              <span style={{ minWidth: "11rem", color: "var(--text-secondary)" }}>{labelOf(e.category)}</span>
              <span
                style={{
                  color: e.equivalent ? "var(--success)" : outOfScope ? "var(--text-secondary)" : "var(--warning)",
                  fontWeight: 600,
                  minWidth: "5.5rem",
                }}
              >
              ...
              <span style={{ color: "var(--text-secondary)" }}>Δ {e.diff.toExponential(2)}</span>
            </div>
          );
        })}
      </div>
...
export function suiteTableBody(report: BenchReport, ctx: PanelCtx): ReactNode {
  ...
  return (
    <>
      {/* S6/WCAG 2.1.1: 360px scroll window over up-to-2812px-tall content —
          without tabIndex + an accessible name, a keyboard user has no way
          to reach the clipped rows at all. */}
      <section
        aria-label="All cases (suite) — scrollable table"
        tabIndex={0}
        style={{ maxHeight: 360, overflow: "auto" }}
      >
        <SuiteTable suite={report.suite} solverIds={ctx.solverIds} onSelect={onSelect} />
      </section>
      <div style={{ marginTop: "var(--space-3)" }}>
        <button
          ...
          onClick={() => {
            const blob = new Blob([toCsv(report.suite, ctx.solverIds)], { type: "text/csv" });
...
export function deltaR2CiBody(report: BenchReport, ctx: PanelCtx): ReactNode {
  ...
  return (
    <>
      {/* S6/WCAG 2.1.1: 420px scroll window — same reachability fix as the
          suite table above. */}
      <section
        aria-label="Per-case Δr² 95% confidence intervals — scrollable chart"
        tabIndex={0}
      ...
      >
        <PlotMount
          make={(w) =>
            report.inference
              ? ciIntervalPlot(
                  ciRows(report.inference.cases, "deltaR2Ci").sort(
          ...
          deps={[report, ctx.view]}
          title={PLOT_SPECS["delta-r2-ci"].question}
        />
      </section>
      <p
        style={{
  ...
  return (
    <>
      {/* S6/WCAG 2.1.1: 420px scroll window — same reachability fix as the
          suite table above. */}
      <section
        aria-label="Per-case speedup 95% confidence intervals — scrollable chart"
        tabIndex={0}
        style={{ maxHeight: 420, overflowY: "auto" }}
      >
        <PlotMount
          make={(w) =>
            report.inference
              ? ciIntervalPlot(
                  ciRows(report.inference.cases, "speedupCi").sort(
          ...
          deps={[report, ctx.view]}
          title={PLOT_SPECS["speedup-ci"].question}
        />
      </section>
      <p
        style={{
  ...
  return (
    <>
      <PlotMount
        ...
        deps={[report, ctx.view]}
        title={PLOT_SPECS["winner-stability"].question}
      />
...
export function paretoBody(report: BenchReport, ctx: PanelCtx): ReactNode {
  ...
  return (
    <PlotMount
      ...
      deps={[report, ctx.view]}
      title={PLOT_SPECS["pareto"].question}
    />
  );
}
...
export function perfProfileBody(report: BenchReport, ctx: PanelCtx): ReactNode {
  ...
  return (
    <PlotMount
      ...
      deps={[report, ctx.view]}
      title={PLOT_SPECS["perf-profile"].question}
    />
  );
}
...
export function successRateBody(report: BenchReport, ctx: PanelCtx): ReactNode {
  ...
  return (
    <PlotMount
      ...
      deps={[report, ctx.view]}
      title={PLOT_SPECS["success-rate"].question}
    />
  );
}
...
export function recoveryErrorBody(report: BenchReport, ctx: PanelCtx): ReactNode {
  ...
  return (
    <PlotMount
      ...
      deps={[report, ctx.view]}
      title={PLOT_SPECS["recovery-error-suite"].question}
    />
  );
}
...
export function speedupDistBody(report: BenchReport, ctx: PanelCtx): ReactNode {
  ...
  return (
    <PlotMount
      ...
      deps={[report, ctx.view]}
      title={PLOT_SPECS["speedup-dist"].question}
    />
  );
}
