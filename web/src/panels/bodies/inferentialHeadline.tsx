...
const ROW_STYLE: CSSProperties = { borderBottom: "1px solid var(--border-subtle)" };
      ...
      <tbody>
        {rows.map((row, i) => (
          <tr key={i} style={ROW_STYLE}>
            <td style={{ ...TD, color: "var(--text-primary)", maxWidth: "20rem" }}>{row.label}</td>
            <td style={{ ...TD, textAlign: "right" as const }}>{row.value}</td>
            <td style={{ ...TD, textAlign: "center" as const }}>
              {row.pass != null ? <VerdictBadge pass={row.pass} /> : <span style={{ color: "var(--text-tertiary)" }}>—</span>}
            </td>
          </tr>
        ))}
      </tbody>
