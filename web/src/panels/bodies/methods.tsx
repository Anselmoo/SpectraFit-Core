  ...
  return (
    <div className="glass" style={{ padding: "var(--space-6)" }}>
      ...
      >
        ...
        <a href="#standing" style={{ color: "var(--system-blue)", textDecoration: "none" }}>
          Standing ↑
        </a>
        .
      </p>
      ...
      {tb != null ? (
        <>
          ...
          {groupWires(wiresOf(tb)).map(({ group, wires: groupWires }, gi) => (
            <div
              ...
              style={{
                marginBottom: gi < CLAIM_GROUPS.length - 1 ? "var(--space-4)" : 0,
              }}
            >
              <div
                style={{
          ...
          {tb.nClaimsTotal > 0 && (
            ...
            >
              {tb.nClaimsAudited} of {tb.nClaimsTotal} claims audited
              {tb.nClaimsAudited < tb.nClaimsTotal
                ? ` — ${tb.nClaimsTotal - tb.nClaimsAudited} ${
                    tb.nClaimsTotal - tb.nClaimsAudited === 1 ? "awaits" : "await"
                  } a passing wire`
                : ""}
            </p>
          )}
        ...
        <p style={{ margin: 0, fontSize: "0.85rem", color: "var(--text-secondary)", fontFamily: "var(--font-mono)" }}>
 ...
 * results stored in `report.trustBlock.nistValidation` (the W8 evidence block).
 *
...
export function nistValidationBody(report: BenchReport): ReactNode {
  const nv = report.trustBlock?.nistValidation;
  if (nv == null) {
    return (
      <p style={{ margin: 0, fontSize: "0.85rem", color: "var(--text-secondary)", fontFamily: "var(--font-mono)" }}>
...
// ---------------------------------------------------------------------------
type NistValidation = NonNullable<NonNullable<BenchReport["trustBlock"]>["nistValidation"]>;
type NistDataset = NistValidation["datasets"][number];

function PassIcon({ passed }: { passed: boolean }): ReactNode {
  return (
    <span
      style={{
...
/** One dataset's summary row plus its per-parameter sub-rows.
 *  Returns a Fragment (not a single <tr>) since it renders the summary row
 *  plus one sub-row per parameter; the caller's .map() supplies the key. */
function NistDatasetRow({ ds, thresholdSigFigs }: { ds: NistDataset; thresholdSigFigs: number }): ReactNode {
  return (
    <Fragment>
      <tr style={{ borderBottom: "1px solid var(--border-subtle)" }}>
        ...
        <td style={{ padding: "var(--space-2) var(--space-3)", textAlign: "center" }}>{ds.nParams}</td>
        <td
          style={{
            padding: "var(--space-2) var(--space-3)",
            textAlign: "center",
            color: ds.minSigFigs >= thresholdSigFigs ? "var(--success)" : "var(--danger)",
            fontWeight: 600,
          }}
        >
          {ds.minSigFigs.toFixed(2)}
        </td>
        <td style={{ padding: "var(--space-2) var(--space-3)", textAlign: "center" }}><PassIcon passed={ds.passed} /></td>
      </tr>
      {ds.params.map((p) => (
        <tr
          key={`${ds.name}-${p.name}`}
          style={{ borderBottom: "1px solid color-mix(in srgb, var(--border-subtle) 50%, transparent)" }}
        >
          <td
            style={{
              padding: "var(--space-1) var(--space-3) var(--space-1) calc(var(--space-3) * 2)",
              color: "var(--text-secondary)",
              fontSize: "0.72rem",
            }}
          >
          ...
          <td
            style={{
              ...
              textAlign: "center",
              fontSize: "0.72rem",
              color: p.sigFigsAgreed >= thresholdSigFigs ? "var(--success)" : "var(--warning)",
            }}
          >
            {p.sigFigsAgreed.toFixed(2)}
          </td>
          <td />
        </tr>
      ))}
...
/** The full per-dataset detail table (shown when expanded). */
function NistDatasetTable({ datasets, thresholdSigFigs }: { datasets: NistDataset[]; thresholdSigFigs: number }): ReactNode {
  return (
    <table
      style={{
    ...
    >
      ...
      <tbody>
        {datasets.map((ds) => (
          <NistDatasetRow key={ds.name} ds={ds} thresholdSigFigs={thresholdSigFigs} />
        ))}
      </tbody>
    </table>
  );
...
function NistValidationCard({ nv }: { nv: NistValidation }): ReactNode {
  ...
  return (
    <div style={{ overflowX: "auto" }}>
      ...
      >
        ...
        threshold ≥ {nv.thresholdSigFigs} sig figs ·{" "}
        worst case {nv.minSigFigs.toFixed(2)} sig figs ·{" "}
        overall <PassIcon passed={nv.passed} />
      </p>
      ...
      {expanded && <NistDatasetTable datasets={nv.datasets} thresholdSigFigs={nv.thresholdSigFigs} />}
      ...
      >
        ...
        {nv.totalAvailable != null ? `${nv.totalAvailable} ` : ""}the NIST StRD
...
export function scopeBoundariesCard(report: BenchReport): ReactNode {
  ...
  const nistN = tb.nistValidation?.datasets?.length ?? 0;
  // Data-derived denominator — read from the contract, never hardcoded.
  const nistTotal: number | null = tb.nistValidation?.totalAvailable ?? null;
  ...
  const item = (label: string, body: ReactNode) => (
    <li style={{ marginBottom: "var(--space-3)", lineHeight: 1.55 }}>
  ...
  return (
    <div className="glass" style={{ padding: "var(--space-6)" }}>
      ...
      <p style={{ margin: "0 0 var(--space-4)", fontSize: "0.82rem", color: "var(--text-secondary)", fontFamily: "var(--font-mono)", lineHeight: 1.5 }}>
      ...
      <ul style={{ margin: 0, paddingLeft: "1.1rem", fontFamily: "var(--font-mono)", fontSize: "0.84rem" }}>
        ...
        {w10Pass && w11Pass
          ? item(
              ...
              <>
                ...
                1σ band (binomial p = {fmtP(cal!.binomialP)});
        ...
        {nistN > 0 &&
          item(
            "NIST StRD breadth",
            <>
              certified-value reproduction covers <strong style={{ color: "var(--text-primary)" }}>{nistN}{nistTotal != null ? ` of ${nistTotal}` : ""}</strong>{" "}
      ...
      <p style={{ margin: "var(--space-4) 0 0", fontSize: "0.74rem", color: "var(--text-secondary)", fontFamily: "var(--font-mono)", lineHeight: 1.5 }}>
        ...
        {cfg != null &&
          practice(
            ...
            <>
              ...
              <strong style={{ color: "var(--text-primary)" }}>{cfg.bootstrapB.toLocaleString()}</strong> resamples
        ...
        {cfg != null &&
          practice(
            ...
            <>
              ...
              <strong style={{ color: "var(--text-primary)" }}>{cfg.equivalenceMargin}</strong> across {nEquiv}{" "}
              categor{nEquiv === 1 ? "y" : "ies"}, with the false-discovery rate held at q ={" "}
              <strong style={{ color: "var(--text-primary)" }}>{cfg.fdrQ}</strong> — not a single hand-picked case.
            </>,
          )}
        {practice(
          "Calibration",
          ...
          practice(
            ...
            <>
              <strong style={{ color: "var(--text-primary)" }}>
                {tb.nClaimsAudited} of {tb.nClaimsTotal}
              </strong>{" "}
              ...
              <a href="#audit" style={{ color: "var(--system-blue)", textDecoration: "none" }}>
                see the wire matrix ↑
              </a>
            </>,
          )}
...
export function pinnedBaselineCard(report: BenchReport): ReactNode {
  ...
  const row = (k: string, v: ReactNode) => (
    <div style={{ display: "flex", gap: "var(--space-3)", padding: "2px 0" }}>
      <span style={{ minWidth: "8.5rem", color: "var(--text-secondary)" }}>{k}</span>
