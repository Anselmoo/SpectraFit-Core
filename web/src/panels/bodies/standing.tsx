...
import type { BackendFact } from "../../series/backendFacts";
...
function resolveHarmonicMean(report: BenchReport): number | null {
  ...
  const suite: Array<{ m: Record<string, { speedup: number }> }> =
    Array.isArray(report.suite) ? report.suite : [];
  ...
  return (
    <div className="glass" style={{ padding: "var(--space-6)" }}>
      ...
      {m != null ? (
        ...
        >
          <dt style={{ color: "var(--text-secondary)", fontFamily: "var(--font-mono)", fontSize: "0.8rem" }}>
          ...
          <dd style={{ margin: 0, fontFamily: "var(--font-mono)", color: "var(--text-primary)" }}>
            ...
            {hmSpeedup != null && (
              <span
                style={{ color: "var(--text-secondary)", marginLeft: "var(--space-2)", fontSize: "0.82em" }}
          ...
          {m.regressions > 0 && (
            <>
              ...
              <dd style={{ margin: 0, fontFamily: "var(--font-mono)", color: "var(--danger)" }}>
                {m.regressions}
              </dd>
            </>
          )}
            ...
            <>
              <dt style={{ color: "var(--text-secondary)", fontFamily: "var(--font-mono)", fontSize: "0.8rem" }}>
                ...
                >
                  ...
                  <a href="/api/v1/trust" style={{ color: "var(--text-secondary)" }}>verification ledger ↓</a>
                </small>
              </dt>
              <dd style={{ margin: 0, fontFamily: "var(--font-mono)", color: "var(--text-primary)" }}>
                {tb.rung}/5
...
// trustBlock.nistValidation.totalAvailable (data-derived, never hardcoded).
...
export function renderTruthCard(report: BenchReport): ReactNode {
  ...
  return (
    <div className="glass" style={{ padding: "var(--space-6)" }}>
      ...
      <ul style={SCOPE_UL}>
        {nist != null && nistN > 0 && (
          <li>
            ...
            datasets reproduced to ≥{nist.thresholdSigFigs} significant figures (worst case{" "}
            {nist.minSigFigs}).
          </li>
        )}
        <li>
          <strong>{tb.nClaimsAudited} of {tb.nClaimsTotal}</strong> dashboard claims are backed by
          a verification wire.
        </li>
        {passed.length > 0 && (
          <li>
      ...
      <h3 style={{ ...SCOPE_H3, color: "var(--warning)" }}>Not tested / open</h3>
      <ul style={SCOPE_UL}>
        {!w9Pass && (
          <li>
        ...
        {w10Pass && w11Pass ? (
          <li>
            ...
            {fmtP(cal!.binomialP)}); the geomean speedup
      ...
      >
        ...
        >
          ...
          <strong style={{ color: "var(--text-secondary)" }}>not a trust guarantee</strong>. It counts how many
...
function computeRunDate(report: BenchReport): string | null {
  const ts: number | null = report.runTimestampUnix ?? null;
  if (ts == null) return null;
  const d = new Date(ts * 1000);
  return `${d.getFullYear()}-${String(d.getMonth() + 1).padStart(2, "0")}-${String(d.getDate()).padStart(2, "0")}`;
}

/** Two-column masthead: title/description on the left, key facts on the right. */
function FactsMasthead({
  manifest,
  nCases,
  nBackends,
  baseline,
  ...
  return (
    ...
    >
      ...
      >
        {manifest?.gateState != null && (
          <div style={FACT_ROW}>
            <span style={FACT_KEY}>gate</span>
            <span style={FACT_VAL}>
              {/^pass$/i.test(manifest.gateState) ? (
                <span style={PASS_STYLE}>{manifest.gateState}</span>
              ) : (
                <span style={{ ...PASS_STYLE, borderColor: "var(--danger)", color: "var(--danger)" }}>
                  {manifest.gateState}
                </span>
              )}
            </span>
        ...
        {manifest?.maxAbsDeltaR2 != null && (
          <div style={FACT_ROW}>
            <span style={FACT_KEY}>max |Δr²|</span>
            <span style={FACT_VAL}>{manifest.maxAbsDeltaR2.toExponential(2)}</span>
          </div>
        )}
        {runDate != null && (
          <div style={FACT_ROW}>
...
/** Per-backend results table — one row per backend, sorted alphabetically. */
function ResultsTable({ facts, baseline }: { facts: BackendFact[]; baseline: string }): ReactNode {
  return (
    <div className="glass" style={{ padding: "var(--space-6)", overflowX: "auto" }}>
      <h2
        style={{
          fontFamily: "var(--font-display)",
          fontWeight: 400,
          ...
          {facts.map((f, idx) => {
            ...
            return (
              <tr key={f.id} style={{ cursor: "default" }}>
                <td style={{ ...TD_FIRST, borderBottom: isLast ? "none" : "1px solid var(--border-subtle)" }}>
                  {f.id}
                </td>
...
/** Flow link from Standing to the Evidence destination. */
function EvidenceFlowLink(): ReactNode {
  return (
    <a
      className="glass"
      href="#evidence"
    ...
    >
      ...
      <span style={{ fontSize: "1.6rem", color: "var(--system-blue)" }}>→</span>
    </a>
  );
}

/** Footer note disclosing a roster backend that ran 0 cases this run (e.g. jax). */
function AbsentBackendNote({ optionalAbsent }: { optionalAbsent: string[] }): ReactNode {
  if (optionalAbsent.length === 0) return null;
  return (
    <p
      style={{
...
export function factsLandingCard(report: BenchReport): ReactNode {
  ...
  const runDate = computeRunDate(report);
  ...
  return (
    <>
      <FactsMasthead
        ...
        runDate={runDate}
      />
      <ResultsTable facts={facts} baseline={baseline} />
      <EvidenceFlowLink />
      <AbsentBackendNote optionalAbsent={optionalAbsent} />
    </>
  );
}
