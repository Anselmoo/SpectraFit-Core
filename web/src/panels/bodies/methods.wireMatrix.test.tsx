...
describe("wireMatrixCard — Wave B2 tree structure (Dye)", () => {
  it("renders [data-claim-group] containers when wires are present", () => {
  ...
  it("W1 and W2a appear in the Accuracy group", () => {
    const node = wireMatrixCard(makeReport(SAMPLE_WIRES));
    const { container } = render(node as ReactElement);
  ...
  it("W8 appears in the External validation group", () => {
    const node = wireMatrixCard(makeReport(SAMPLE_WIRES));
    const { container } = render(node as ReactElement);
  ...
  it("W5 appears in the Render fidelity group", () => {
    const node = wireMatrixCard(makeReport(SAMPLE_WIRES));
    const { container } = render(node as ReactElement);
...
describe("wireMatrixCard — Tog orientation header", () => {
  it("renders [data-audit-orientation] with a link to Standing", () => {
    const node = wireMatrixCard(makeReport(SAMPLE_WIRES));
    const { container } = render(node as ReactElement);
  ...
  it("orientation is also rendered when trustBlock is null", () => {
...
describe("wireMatrixCard — W5 honest CI disclosure", () => {
  it("skipped W5 wire shows CI-related text (not just a bare dash)", () => {
    const node = wireMatrixCard(makeReport(SAMPLE_WIRES));
    const { container } = render(node as ReactElement);
  ...
  it("skipped W5 dot has aria-label 'verified in CI' (not bare 'skipped')", () => {
    const node = wireMatrixCard(makeReport(SAMPLE_WIRES));
    const { container } = render(node as ReactElement);
    const w5Row = Array.from(container.querySelectorAll(".wire-row")).find(
      (el) => el.textContent?.includes("W5"),
...
describe("wireMatrixCard — null trustBlock", () => {
  it("returns a card (not null) when trustBlock is absent", () => {
    const report = { trustBlock: null } as unknown as BenchReport;
    const node = wireMatrixCard(report);
