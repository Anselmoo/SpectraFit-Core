...
describe("nist-validation panel (body function — audit destination removed)", () => {
  it("is importable and callable directly (not through PANELS registry)", () => {
  ...
  it("renders dataset rows for each NistDataset (after expanding)", () => {
    const report = makeNistReport(SAMPLE_NIST);
    const node = nistValidationBody(report);
  ...
  it("shows min sig figs for each dataset (after expanding)", () => {
    const report = makeNistReport(SAMPLE_NIST);
    const node = nistValidationBody(report);
  ...
  it("shows aggregate threshold and overall min sig figs in summary line", () => {
    const report = makeNistReport(SAMPLE_NIST);
    const node = nistValidationBody(report);
    const { container } = render(<>{node}</>);
  ...
  it("renders gracefully when nistValidation is null", () => {
  ...
  it("renders certified and fitted values in param sub-rows (after expanding)", () => {
    const report = makeNistReport(SAMPLE_NIST);
    const node = nistValidationBody(report);
    const { container } = render(<>{node}</>);
  ...
  it("expand button starts collapsed (aria-expanded=false) and toggles to true", () => {
    const report = makeNistReport(SAMPLE_NIST);
    const node = nistValidationBody(report);
    const { container } = render(<>{node}</>);
