      ...
      <tbody>
        {suite.map((c) => (
          <tr key={c.id} data-case-id={c.id} data-regression={c.regression ? "1" : "0"}
              ...
              style={{ color: c.regression ? "var(--warning)" : "var(--text-secondary)", textAlign: "right", ...(onSelect ? { cursor: "pointer" } : {}) }}
            ...
            <td style={{ textAlign: "left", ...(onSelect ? { color: "var(--system-blue)" } : {}) }}>{c.id}</td>
