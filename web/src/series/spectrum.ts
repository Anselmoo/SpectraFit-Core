 ...
 */
export function spectrumSeries(f: Featured, solverIds: string[]): SpectrumSeries {
  return {
    ref: zip(f.x, f.ref),
 ...
 */
export function metricRows(f: Featured, solverIds: string[]): MetricRow[] {
  return solverIds
 ...
 */
export function residualRows(f: Featured, solverIds: string[]): Array<XY & { backend: string }> {
  return solverIds.flatMap((id) => {
    const fit = f.profiles?.[id]?.fit;
    if (!fit) return [];
    const curve = fit.curve;
    const stored = fit.resid;
    if (curve && f.ref) {
