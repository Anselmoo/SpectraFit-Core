...
import type { BenchReport, SuiteCase } from "../contract";
 ...
 */
export function performanceProfileSeries(report: BenchReport, solverIds: string[]): ProfileSeries[] {
  const suite = Array.isArray(report.suite) ? report.suite : [];

  const cost = (c: SuiteCase, b: string): number | null => {
    const v = c.m[b]?.medMs;
