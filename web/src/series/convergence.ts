...
import type { Featured, BackendProfile } from "../contract";
export interface ConvLine { backend:string; mode:"line"|"endpoints"; rows:{iter:number;cost:number;backend:string}[]; }
export function convSeries(f:Featured, solverIds:string[]):ConvLine[] {
 ...
 */
export function thetaDistanceSeries(f: Featured): ThetaDistance | null {
  const profiles: Record<string, BackendProfile> = f.profiles ?? {};
  const hit = Object.entries(profiles).find(
    ([, p]) => Array.isArray(p?.thetaDistance) && (p.thetaDistance?.length ?? 0) > 0,
  );
  if (!hit) return null;
  ...
  return {
    backend,
    recoveryTol: 1e-2,
    rows: (prof.thetaDistance ?? []).map((d, i) => ({
      iter: i,
