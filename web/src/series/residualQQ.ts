...
import type { BenchReport, Featured } from "../contract";
 ...
 */
function residualsFor(f: Featured, backend: string): number[] {
  const resid = f.profiles?.[backend]?.fit?.resid;
  if (Array.isArray(resid) && resid.length > 0) {
    return resid;
  }
  // Fallback: ecdfResid is a list of {x,y} points; use the x values (residuals)
  const ecdf = f.profiles?.[backend]?.ecdfResid;
  if (Array.isArray(ecdf) && ecdf.length > 0) {
    return ecdf.map((pt) => pt.x);
  }
  return [];
}
 ...
 */
export function residualQQSeries(report: BenchReport, caseId: string, solverIds: string[]): QQBackend[] {
  // Locate the analyzed case
  const analyzed = Array.isArray(report.analyzed) ? report.analyzed : [];
