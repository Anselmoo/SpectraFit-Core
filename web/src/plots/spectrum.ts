...
export function spectrumPlot(
  ...
  return toSvg(
    Plot.plot({
      ...
      color: {
        domain: ids,
        range: ids.map((id) => o.colors[id] ?? "var(--system-blue)"),
      },
      marks: [
        // reference DATA = markers (spectrum_marker_convention: data has markers, fits are lines)
        Plot.dot(s.ref, { x: "x", y: "y", r: 1.6, fill: "var(--text-secondary)", fillOpacity: 0.55 }),
...
export function residualPlot(
  ...
  return toSvg(
    Plot.plot({
      ...
      marks: [
        Plot.ruleY([0], { stroke: "var(--border-subtle)" }),
