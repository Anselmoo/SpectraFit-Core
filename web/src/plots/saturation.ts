...
export function saturationHeatmap(
  ...
  // moderately-good one. Failed cells (r²<0.9) are rendered in var(--danger) to
  ...
  const plot = Plot.plot({
    ...
    // scale entirely and receive a fixed --danger fill via a separate mark.
    ...
    marks: [
      ...
      // Failed cells (r²<0.9): fixed --danger fill so a catastrophic backend
      ...
      // S6/WCAG 1.4.1: red-vs-green is a color-only encoding — a deuteranopic
      // or protanopic reader can't distinguish a failed cell from the 0.9
      // agree-band floor by hue alone. A "✗" glyph gives fail cells the same
      // shape channel the SuiteTable's accuracy strip already uses for
      // pass/fail (✓/✗/—), so failure reads without relying on color.
      Plot.text(failRows, {
        x: "backend",
        y: "category",
        text: () => "✗",
