...
// Real types for the series/* transforms (replaces the AnyCase escape hatch).
export type SuiteCase = components["schemas"]["SuiteCase"];
export type SuiteMetric = components["schemas"]["SuiteMetric"];
export type CaseInference = components["schemas"]["CaseInference"];
export type Point2D = components["schemas"]["Point2D"];
export type SpreadPt = components["schemas"]["SpreadPt"];
export type PeakACS = components["schemas"]["PeakACS"];
 ...
 */
export function assertSupported(report: BenchReport): void {
  if (!SUPPORTED_SCHEMA.has(report.schemaVersion)) {
    throw new Error(`unsupported schema: ${report.schemaVersion}`);
  }
}

/**
 * Return the ordered list of solver ids from the data.
 * NO hardcoded ids — enumerated from report.solvers.
 */
export function solversOf(report: BenchReport): string[] {
  return report.solvers.map((s) => s.id);
}
 ...
 */
export function analyzedById(report: BenchReport, id: string): Featured | undefined {
  return report.analyzed.find((f) => f.id === id);
}
 ...
 */
export function defaultCaseId(report: BenchReport): string {
  const fallback = report.analyzed?.[0]?.id ?? "";
  const analyzedIds = new Set(report.analyzed?.map((f) => f.id) ?? []);
  let best = fallback;
  let bestSpread = -1;
...
export function runCompleteness(report: BenchReport): RunCompleteness {
  const nCases = report.suite?.length ?? 0;
  const hasSuiteTiming = (report.suite ?? []).some((c) =>
    Object.values(c.m).some((m) => typeof m.medMs === "number" && Number.isFinite(m.medMs)),
  );
  const hasConvergenceTruth = (report.analyzed ?? []).some((f) =>
