...
describe("CaseVerdict", () => {
  it("shows convergence count, fastest backend (non-spectrafit), and its ms", () => {
  ...
  it("shows 'saturated' note when r² spread < 1e-6", () => {
  ...
  it("renders nothing when the case is absent from suite", () => {
    const { container } = render(<CaseVerdict report={missingCaseReport} caseId="NO-SUCH" />);
    expect(container.textContent).toBe("");
  });

  it("shows partial convergence (1/2 backends) and does not crown a subject", () => {
  ...
  it("subject-blindness: component text never contains literal 'spectrafit' when another backend is fastest", () => {
  ...
  it("saturated case carries a labelled 'saturated case' badge", () => {
    const { container } = render(<CaseVerdict report={saturatedReport} caseId="EZ-001" />);
  ...
  it("discloses a roster backend that did not run this case (no silent gap)", () => {
  ...
  it("no n/a disclosure when every roster backend ran the case", () => {
  ...
  it("saturated case offers a jump to a discriminating case", () => {
    const { container } = render(<CaseVerdict report={saturatedWithSiblingReport} caseId="EZ-001" />);
  ...
  it("no jump affordance when the case is already discriminating", () => {
  ...
  it("does not crown a fast non-converged backend as fastest", () => {
  ...
  it("EF-PANELS-10: fastest backend shows the human label, not the raw id", () => {
