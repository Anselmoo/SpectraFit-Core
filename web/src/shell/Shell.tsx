...
export function Shell({ report }: { report: BenchReport }): ReactElement {
  ...
  return (
    ...
    >
      {/* S6: skip link — the suite table alone is 154 rows of tab stops before
          a keyboard user would otherwise reach any chart; this jumps straight
          to <main>. Visually hidden until focused (see .skip-link in
          tokens.css), first in the DOM so it's the first Tab stop. */}
      ...
      {/* S6: real <header> landmark (was a bare <nav>, so there was no header
          landmark at all — measured header=0). Width/max-width moved up from
          nav so the visual result is unchanged. */}
      <header
        style={{
          width: "100%",
      ...
      {/* Active destination panel — per-destination max width. S6: real
          <main> landmark (was a bare <div>, so there was no main landmark at
          all — measured main=0, [role=main]=0) — also the skip link's target. */}
      <main id="main-content" style={{ width: "100%", flex: 1, display: "flex", flexDirection: "column" }}>
        <TabPanel
          id={dest}
          idBase="destination"
          style={{
