...
export function EvidenceCaseView({
  ...
  return (
    <Fragment key="ev-case">
      ...
      {/* S6: was <h2> with no <h1> anywhere in this view. Promoted to <h1>;
          the section headings below move from <h3> to <h2> in the same fix
          so the hierarchy doesn't skip a level (.ev-section styling is
          class-based, not tag-based, so this is a pure semantics change). */}
      {f != null && (
        <h1
          style={{
            margin: 0,
      ...
      <section id="sec-fit">
        <h2 className="ev-section">The fit</h2>
      ...
      {f != null && (
        <section id="sec-perf">
          <h2 className="ev-section">Per-backend performance</h2>
      ...
      {f != null && (
        <section id="sec-repro">
          <h2 className="ev-section">Reproducibility &amp; conditioning</h2>
