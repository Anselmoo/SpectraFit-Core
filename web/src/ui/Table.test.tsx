...
test("Table renders semantic table/thead/tbody with header labels and cell values", () => {
...
test("Table right-aligns a column when align is set", () => {
  const { container } = render(<Table columns={columns} rows={rows} />);
  const headers = container.querySelectorAll("th");
  expect(headers[1].style.textAlign).toBe("right");
});

test("Table renders one row per data entry", () => {
  const { container } = render(<Table columns={columns} rows={rows} />);
  expect(container.querySelectorAll("tbody tr").length).toBe(2);
});

test("Table dense tightens cell padding", () => {
