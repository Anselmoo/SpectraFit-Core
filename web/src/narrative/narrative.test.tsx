...
describe("wiresOf adapter", () => {
  it("maps contract wire_id (snake_case) to id", async () => {
    const { wiresOf } = await import("./index");
    const trustBlock = {
      rung: 2,
      wires: [
        { wire_id: "W1", name: "timing_isolation", status: "pass", evidence: "bench harness ok" },
        { wire_id: "W2c", name: "jacobian_kappa", status: "warn", evidence: "partial coverage" },
      ],
    };
  ...
  it("returns empty array for missing wires", async () => {
    ...
    expect(wiresOf({})).toEqual([]);
    expect(wiresOf({ wires: [] })).toEqual([]);
  });

  it("preserves a `gap` status (not coerced to skipped)", async () => {
    const { wiresOf } = await import("./index");
    const rows = wiresOf({
      wires: [{ wire_id: "W2c", name: "jacobian_kappa", status: "gap", evidence: "κ(J) not exposed" }],
    });
