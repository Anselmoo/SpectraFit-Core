 ...
 * contract's trustBlock (exact snake_case field names from openapi.gen.ts)
...
//
// EXACT field names from openapi.gen.ts WireResult schema:
//   wire_id  (snake_case, NOT wireId)
//   name     string
...
export function wiresOf(trustBlock: any): WireRow[] {
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  return (trustBlock?.wires ?? []).map((w: any) => {
    ...
    const status: WireStatus = VALID_STATUSES.has(w.status)
      ...
      : "skipped";
    return {
      id: w.wire_id,        // exact contract field name from openapi.gen.ts
      name: w.name,
      status,
      evidence: w.evidence,
    };
