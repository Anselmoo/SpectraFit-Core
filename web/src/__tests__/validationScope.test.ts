...
function makeRung5Report(): BenchReport {
  return {
    trustBlock: {
      ...
      nClaimsTotal: 16,
      wires: [
        { wireId: "W1", name: "Residual sanity", status: "pass", evidence: "residuals finite" },
        { wireId: "W2c", name: "Jacobian conditioning", status: "gap", evidence: "κ(J) not exposed by lmfit/jax" },
        { wireId: "W8", name: "NIST StRD external validation", status: "pass", evidence: "certified values reproduced" },
      ],
      nistValidation: {
        thresholdSigFigs: 4,
        totalAvailable: 27,
        datasets: [
          ...
          { name: "BoxBOD", model: "Saturating exponential", nParams: 2, params: [], minSigFigs: 5, passed: true },
        ],
        minSigFigs: 5,
        passed: true,
      },
