 ...
 * in the nist-validation panel.  A populated trustBlock.nistValidation must
 ...
 * evidence root (`trustBlock.nistValidation`) is referenced in registry.tsx —
...
function makeRung5Report() {
  // Minimal BenchReport-shaped object with a rung-5 trustBlock + nistValidation.
        ...
        datasets: [
          {
            ...
            nParams: 8,
            params: [
              { name: "b1", certified: 98.0, fitted: 98.01, sigFigsAgreed: 6.0 },
            ],
            minSigFigs: 6.0,
            passed: true,
          },
...
describe("L3 render-level claim-evidence integrity (I5)", () => {
  it("rung-5 report with nistValidation renders dataset name, not the empty fallback", async () => {
  ...
  it("report without nistValidation renders the empty fallback (guard for the negative case)", async () => {
    const { nistValidationBody } = await import("../panels/registry");
...
// ---------------------------------------------------------------------------
// Source-scan: trustBlock.nistValidation must appear in the panel
...
// access (`report.trustBlock?.nistValidation`) into bodies/methods.tsx; scanning
...
describe("L3 source-scan: claim evidence path referenced in the panel render code", () => {
  it("the panels access trustBlock.nistValidation (camelCase, not the old nist_validation)", () => {
    // The NIST claim source_field root — must be present as a real ACCESS, not just
    // a comment. nistValidationBody reads `report.trustBlock?.nistValidation`.
    expect(PANEL_SOURCE).toMatch(/trustBlock\??\.\s*nistValidation/);
    // The old (pre-fix) snake_case must NOT be the access key anywhere in the panels —
    // trust_ledger.py now aliases to camelCase like every other contract field.
    expect(PANEL_SOURCE).not.toMatch(/trustBlock\??\.\s*nist_validation/);
  });
});
